---
title: glm4_moe_tool_parser - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/tool_parsers/glm4_moe_tool_parser/
source: sitemap
fetched_at: 2026-05-07T21:36:01.90314988-03:00
rendered_js: false
word_count: 92
summary: This class implements a parser for the GLM-4 model to extract tool calls from XML-formatted output and handle incremental streaming by diffing generated text.
tags:
    - glm-4
    - tool-calling
    - xml-parsing
...
```
classGlm4MoeModelToolParser(ToolParser):
...
"""Adjust request parameters for tool call token handling.
    ...
    def_extract_content(self, current_text: str) -> str | None:
"""Return unsent non-tool-call text, or None.
    ...
    def_extract_tool_call_regions(self, text: str) -> list[tuple[str, bool]]:
"""Extract ``(inner_text, is_complete)`` for each ``<tool_call>`` region."""
    ...
    def_extract_tool_name_from_region(self, inner_text: str) -> str | None:
...
"""Build the JSON arguments string from the XML pairs seen so far.
    ...
    def_compute_args_diff(self, index: int, args_so_far: str) -> str | None:
"""Return new argument text not yet sent for tool *index*, or None."""
    ...
    def_ensure_tool_state_for(self, index: int) -> None:
