...
source: crawler
fetched_at: 2026-04-24T17:00:12.171328009-03:00
rendered_js: false
word_count: 916
summary: This document serves as a guide to running Hermes Agent specifically within a Docker container, detailing how to perform quick setup, run in various modes like gateway and dashboard, manage persistent data volumes, and configure resource limits or environment variables.
tags:
    - docker-container
    - hermes-agent
    - api-server
...
2. **Docker as a terminal backend** — the agent runs on your host but executes commands inside a Docker sandbox (see [Configuration → terminal.backend](https://hermes-agent.nousresearch.com/docs/user-guide/configuration))
...
Port 8642 exposes the gateway's [OpenAI-compatible API server](https://hermes-agent.nousresearch.com/docs/user-guide/api-server.md) and health endpoint. It's optional if you only use chat platforms (Telegram, Discord, etc.), but required if you want the dashboard or external tools to reach the gateway.
...
-v ~/.hermes:/opt/data \
-p9119:9119 \
-eGATEWAY_HEALTH_URL=http://$HOST_IP:8642 \
...
Environment variableDescriptionDefault`GATEWAY_HEALTH_URL`Base URL of the gateway's API server, e.g. `http://gateway:8642`*(unset — local PID check only)*`GATEWAY_HEALTH_TIMEOUT`Health probe timeout in seconds`3`
...
PathContents`.env`API keys and secrets`config.yaml`All Hermes configuration`SOUL.md`Agent personality/identity`sessions/`Conversation history`memories/`Persistent memory store`skills/`Installed skills`cron/`Scheduled job definitions`hooks/`Event hooks`logs/`Runtime logs`skins/`Custom CLI skins
...
ResourceMinimumRecommendedMemory1 GB2–4 GBCPU1 core2 coresDisk (data volume)500 MB2+ GB (grows with sessions/skills)
