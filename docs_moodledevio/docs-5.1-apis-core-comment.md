---
title: Comment API | Moodle Developer Resources
url: https://moodledev.io/docs/5.1/apis/core/comment
source: sitemap
fetched_at: 2026-02-17T15:34:03.45684-03:00
rendered_js: false
word_count: 532
summary: This document explains how to use the Moodle Comment API to integrate and manage user comments within Moodle entities and plugins. It covers class instantiation, rendering comment areas, and implementing required validation callbacks.
tags:
    - moodle-api
    - comments
    - core-comment
...
$count=$comment->count();
echoget_string('totalcomments','mod_myplugin',$count);
...
## Add plugin control[​](#add-plugin-control "Direct link to Add plugin control")
 ...
 * @return boolean
 */
functionmod_myplugin_comment_display(stdClass$comments,stdClass$args):stdClass{
if($args->commentarea!='entry_comments'){
thrownewcomment_exception('invalidcommentarea');
}
if($args->itemid!=0){
thrownewcomment_exception('invalidcommentitemid');
}
return$comments;
}
```
