...
source: sitemap
fetched_at: 2026-01-22T22:25:17.483057994-03:00
rendered_js: false
word_count: 2231
summary: This document explains how to integrate Pydantic AI agents with Temporal to achieve durable execution, allowing long-running agentic workflows to recover from failures and resume state. It describes the architecture of workflows and activities and how to wrap agents for fault-tolerant execution.
tags:
    - temporal
    - durable-execution
    - pydantic-ai
...
```
brewinstalltemporal
temporalserverstart-dev
```

With Pydantic AI GatewayDirectly to Provider API

[Learn about Gateway](https://ai.pydantic.dev/gateway) temporal\_agent.py
...
@workflow.defn
classGeographyWorkflow(PydanticAIWorkflow):  # (2)!
...
01. The original `Agent` cannot be used inside a deterministic Temporal workflow, but the `TemporalAgent` can.
02. As explained above, the workflow represents a deterministic piece of code that can use non-deterministic activities for operations that require I/O. Subclassing [`PydanticAIWorkflow`](https://ai.pydantic.dev/api/durable_exec/#pydantic_ai.durable_exec.temporal.PydanticAIWorkflow "PydanticAIWorkflow") is optional but provides proper typing for the `__pydantic_ai_agents__` class variable.
03. List the `TemporalAgent`s used by this workflow. The [`PydanticAIPlugin`](https://ai.pydantic.dev/api/durable_exec/#pydantic_ai.durable_exec.temporal.PydanticAIPlugin "PydanticAIPlugin") will automatically register their activities with the worker. Alternatively, if modifying the worker initialization is easier than the workflow class, you can use [`AgentPlugin`](https://ai.pydantic.dev/api/durable_exec/#pydantic_ai.durable_exec.temporal.AgentPlugin "AgentPlugin") to register agents directly on the worker.
04. [`TemporalAgent.run()`](https://ai.pydantic.dev/api/durable_exec/#pydantic_ai.durable_exec.temporal.TemporalAgent.run "run            async   ") works just like [`Agent.run()`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AbstractAgent.run "run            async   "), but it will automatically offload model requests, tool calls, and MCP server communication to Temporal activities.
05. We connect to the Temporal server which keeps track of workflow and activity execution.
06. This assumes the Temporal server is [running locally](https://github.com/temporalio/temporal#download-and-start-temporal-server-locally).
07. The [`PydanticAIPlugin`](https://ai.pydantic.dev/api/durable_exec/#pydantic_ai.durable_exec.temporal.PydanticAIPlugin "PydanticAIPlugin") tells Temporal to use Pydantic for serialization and deserialization, treats [`UserError`](https://ai.pydantic.dev/api/exceptions/#pydantic_ai.exceptions.UserError "UserError") exceptions as non-retryable, and automatically registers activities for agents listed in `__pydantic_ai_agents__`.
08. We start the worker that will listen on the specified task queue and run workflows and activities. In a real world application, this might be run in a separate service.
09. The agent's `name` is used to uniquely identify its activities.
...
To account for these limitations, tool functions and the [event stream handler](#streaming) running inside activities receive a limited version of the agent's [`RunContext`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   "), and it's your responsibility to make sure that the [dependencies](https://ai.pydantic.dev/dependencies/) object provided to [`TemporalAgent.run()`](https://ai.pydantic.dev/api/durable_exec/#pydantic_ai.durable_exec.temporal.TemporalAgent.run "run            async   ") can be serialized using Pydantic.
...
Because Temporal activities cannot stream output directly to the activity call site, [`Agent.run_stream()`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AbstractAgent.run_stream "run_stream            async   "), [`Agent.run_stream_events()`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AbstractAgent.run_stream_events "run_stream_events"), and [`Agent.iter()`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.Agent.iter "iter            async   ") are not supported.

Instead, you can implement streaming by setting an [`event_stream_handler`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.EventStreamHandler "EventStreamHandler            module-attribute   ") on the `Agent` or `TemporalAgent` instance and using [`TemporalAgent.run()`](https://ai.pydantic.dev/api/durable_exec/#pydantic_ai.durable_exec.temporal.TemporalAgent.run "run            async   ") inside the workflow. The event stream handler function will receive the agent [run context](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") and an async iterable of events from the model's streaming response and the agent's execution of tools. For examples, see the [streaming docs](https://ai.pydantic.dev/agents/#streaming-all-events).
...
[`Agent.run(model=...)`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AbstractAgent.run "run            async   ") normally supports both model strings (like `'openai:gpt-5.2'`) and model instances. However, `TemporalAgent` does not support arbitrary model instances because they cannot be serialized for Temporal's replay mechanism.
...
frompydantic_aiimport Agent
frompydantic_ai.durable_exec.temporalimport TemporalAgent
...
frompydantic_ai.providersimport Provider
...
defmy_provider_factory(run_context: RunContext[Deps], provider_name: str) -> Provider[Any]:
...
@workflow.defn
classMultiModelWorkflow:
    @workflow.run
    async defrun(self, prompt: str, use_reasoning: bool, use_fast: bool) -> str:
        if use_reasoning:
