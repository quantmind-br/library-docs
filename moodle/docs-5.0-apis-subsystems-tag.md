---
title: Tag API | Moodle Developer Resources
url: https://moodledev.io/docs/5.0/apis/subsystems/tag
source: sitemap
fetched_at: 2026-02-17T15:28:14.455976-03:00
rendered_js: false
word_count: 1190
summary: This document provides a technical overview of the Moodle Tag API, explaining how to define tag areas, implement UI elements for tagging, and manage tag data within plugins. It details the core classes and functions required to create, display, and delete tag instances associated with system records.
tags:
    - moodle-development
    - tag-api
    - php
...
## Tag API usage[​](#tag-api-usage "Direct link to Tag API usage")
...
### Defining a tag area[​](#defining-a-tag-area "Direct link to Defining a tag area")
...
There are more options such as specifying the default value for "Standard tags", having a fixed collection or excluding from search. They can be found in comments in [Core library tag.php](https://moodledev.io/docs/5.0/apis/commonfiles/tag.php)

important

After making changes to `db/tag.php`, you must bump the plugin version in **version.php** and perform a Moodle upgrade.

### Adding tags element to the editing form[​](#adding-tags-element-to-the-editing-form "Direct link to Adding tags element to the editing form")
...
1. Populate the form with existing tags before calling $form-&gt;set\_data($data):
...
### Displaying tags next to the item[​](#displaying-tags-next-to-the-item "Direct link to Displaying tags next to the item")
...
### Deleting and clearing tags[​](#deleting-and-clearing-tags "Direct link to Deleting and clearing tags")
...
What does this example do, and why is it in the
 ...
 */
functionbook_reset_course_form_definition(&$mform){
 ...
 */
functionbook_reset_userdata($data){
global$DB;
...
You can choose to backup and restore tags for each item individually (as it is done in mod\_wiki) OR backup all tags in the context at once (as it is done in mod\_glossary or mod\_forum). Second option is preferable for performance reasons. Make sure to take into account `$userinfo` (whether user information is backed up / restored), for example wiki pages is not user information but glossary entries are, tags on them follow the same rule.

mod/glossary/backup\_glossary\_stepslib.php

```
protectedfunctiondefine_structure(){
// ...
$tags=newbackup_nested_element('entriestags');
$tag=newbackup_nested_element('tag',['id'],['itemid','rawname']);
// ...
...
mod/glossary/restore\_glossary\_stepslib.php

```
protectedfunctiondefine_structure(){
// ...
if($userinfo){
$paths[]=newrestore_path_element('glossary_entry_tag','/activity/glossary/entriestags/tag');
}
// ...
}

protectedfunctionprocess_glossary_entry_tag($data){
$data=(object)$data;
...
### Search callback[​](#search-callback "Direct link to Search callback")
...
```
functionmod_wiki_get_tagged_pages($tag,$exclusivemode=false,$fromctx=0,$ctx=0,$rec=1,$page=0){
// Find items.
...
$builder=newcore_tag_index_builder('mod_wiki','wiki_pages',$query,$params,$page*$perpage,$perpage+1);
...
if($items){
$tagfeed=newcore_tag\output\tagfeed();
...
returnnewcore_tag\output\tagindex($tag,'mod_wiki','wiki_pages',$content,
$exclusivemode,$fromctx,$ctx,$rec,$page,$totalpages);
}
}
```

### User-specific tags[​](#user-specific-tags "Direct link to User-specific tags")
...
### Advanced usages[​](#advanced-usages "Direct link to Advanced usages")

Custom plugins may go beyond the standard tags handling and use them without mixing with regular course/user/wiki/blogs tags, hide them from the "Tag search" page and "Tags" block but instead have their own interface to search/categorise using tags. This can be achieved by defining tag collection, make it not searchable and specify a custom URL to link to when the tag is actually displayed. This all can be defined in db/tag.php, see the comments in [Core library tag.php](https://moodledev.io/docs/5.0/apis/commonfiles/tag.php)

- **tag\_area::get\_collection($component, $itemtype)** - will return you the collection that your tag area is in
- **tag\_collection::get\_tag\_cloud()** - will return all tags in the collection. There are no API methods to get the tags used in the tag area - this is actually the purpose of tag collections.

## See also[​](#see-also "Direct link to See also")

- [Core APIs](https://moodledev.io/docs/5.0/apis)
- [Core library tag.php](https://moodledev.io/docs/5.0/apis/commonfiles/tag.php)
