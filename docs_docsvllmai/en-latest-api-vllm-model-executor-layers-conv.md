---
title: conv - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/conv/
source: sitemap
fetched_at: 2026-05-07T21:24:08.266769264-03:00
rendered_js: false
word_count: 112
summary: This document defines custom convolution layer implementations for 2D and 3D operations, including a causal 2D variant, within the vLLM model executor framework.
tags:
    - convolution-layers
...
## CausalConv2dLayer [¶](#vllm.model_executor.layers.conv.CausalConv2dLayer "Permanent link")
...
```
classCausalConv2dLayer(Conv2dLayer):
"""
...
## Conv2dLayer [¶](#vllm.model_executor.layers.conv.Conv2dLayer "Permanent link")
...
"""Conv layer with Conv2d."""
    ...
    def_forward_mulmat(self, x: torch.Tensor) -> torch.Tensor:
        assert x.dim() == 4
        B, C, H, W = x.shape
    ...
    def_forward_conv(self, x: torch.Tensor) -> torch.Tensor:
        assert x.dim() == 4
        x = F.conv2d(
            x,
            self.weight,
    ...
    defforward_native(self, x: torch.Tensor) -> torch.Tensor:
"""Expected input shape: (batch_size, in_channels, height, width)"""
    ...
    defforward_cuda(self, x: torch.Tensor) -> torch.Tensor:
        # By default, we use CUDNN's convolution ops with optimization.
        return self._forward_conv(x)
```

### forward\_native [¶](#vllm.model_executor.layers.conv.Conv2dLayer.forward_native "Permanent link")

Expected input shape: (batch\_size, in\_channels, height, width)
...
## Conv3dLayer [¶](#vllm.model_executor.layers.conv.Conv3dLayer "Permanent link")
...
### forward\_native [¶](#vllm.model_executor.layers.conv.Conv3dLayer.forward_native "Permanent link")

Expected input shape: (batch\_size, in\_channels, time, height, width)
...
## ConvLayerBase [¶](#vllm.model_executor.layers.conv.ConvLayerBase "Permanent link")
