---
title: Component Visibility - FastMCP
url: https://gofastmcp.com/servers/visibility
source: crawler
fetched_at: 2026-01-22T22:21:48.504097629-03:00
rendered_js: false
word_count: 885
summary: This document explains how to dynamically manage the visibility and availability of FastMCP components at runtime using enable and disable methods. It covers global server-level configurations, provider-level overrides, and per-session access control for tools and resources.
tags:
    - fastmcp
    - dynamic-visibility
    - feature-flags
...
New in version `3.0.0` Components can be dynamically enabled or disabled at runtime. A disabled tool disappears from listings and cannot be called. This enables runtime access control, feature flags, and context-aware component exposure.
...
ComponentKey FormatExampleTool`tool:{name}``tool:delete_everything`Resource`resource:{uri}``resource:data://config`Template`template:{uri}``template:file://{path}`Prompt`prompt:{name}``prompt:analyze`
...
ParameterDescription`names`Component names or URIs to match`keys`Component keys (e.g., `{"tool:my_tool"}`)`tags`Tags to match (component must have at least one)`version`Version specification to match`components`Component types (`{"tool"}`, `{"resource"}`, `{"prompt"}`, `{"template"}`)`match_all`If `True`, matches all components regardless of other criteria
