---
title: mxfp4 - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/quantization/mxfp4/
source: sitemap
fetched_at: 2026-05-07T21:27:24.778735152-03:00
rendered_js: false
word_count: 129
summary: This document defines the configuration and implementation classes for MXFP4 quantization specifically tailored for GPT-OSS models and Mixture-of-Experts (MoE) layers within the vLLM framework.
tags:
    - vllm
    - mxfp4
    - quantization
...
## GptOssMxfp4Config [¶](#vllm.model_executor.layers.quantization.mxfp4.GptOssMxfp4Config "Permanent link")

Bases: `Mxfp4Config`

MXFP4 config for GPT-OSS checkpoints.

Checkpoints carry `"quant_method": "mxfp4"` in their JSON config. override\_quantization\_method() maps that to the canonical internal name so that the rest of the loading path uses "gpt\_oss\_mxfp4" consistently.

Source code in `vllm/model_executor/layers/quantization/mxfp4.py`

```
classGptOssMxfp4Config(Mxfp4Config):
"""MXFP4 config for GPT-OSS checkpoints.
...
## GptOssMxfp4MoEMethod [¶](#vllm.model_executor.layers.quantization.mxfp4.GptOssMxfp4MoEMethod "Permanent link")
...
```
classGptOssMxfp4MoEMethod(FusedMoEMethodBase):
"""MXFP4 MoE quantization method."""
...
## Mxfp4Config [¶](#vllm.model_executor.layers.quantization.mxfp4.Mxfp4Config "Permanent link")

Bases: `QuantizationConfig`

Canonical base config for MXFP4 quantization.

Subclasses override get\_name() and override\_quantization\_method() to register themselves as the handler for a specific checkpoint format.

Source code in `vllm/model_executor/layers/quantization/mxfp4.py`

```
classMxfp4Config(QuantizationConfig):
"""Canonical base config for MXFP4 quantization.
    ...
    def__init__(self, ignored_layers: list[str] | None = None):
        super().__init__()
...
### is\_mxfp4\_quant [¶](#vllm.model_executor.layers.quantization.mxfp4.Mxfp4Config.is_mxfp4_quant "Permanent link")
...
## Mxfp4MoEMethod [¶](#vllm.model_executor.layers.quantization.mxfp4.Mxfp4MoEMethod "Permanent link")
...
```
classMxfp4MoEMethod(FusedMoEMethodBase):
"""MXFP4 MoE quantization method."""
