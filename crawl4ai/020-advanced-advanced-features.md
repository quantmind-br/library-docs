---
title: Overview - Crawl4AI Documentation (v0.8.x)
url: https://docs.crawl4ai.com/advanced/advanced-features/
source: sitemap
fetched_at: 2026-04-26T07:45:11.289171802-03:00
rendered_js: false
word_count: 806
summary: This document outlines advanced configuration features for Crawl4AI, including proxy usage, capturing media, SSL certificate retrieval, custom header management, and session state persistence.
tags:
    - proxy-configuration
    - pdf-export
    - ssl-certificate
...
```
importos,asyncio
frombase64import b64decode
fromcrawl4aiimport AsyncWebCrawler, CacheMode, CrawlerRunConfig
...
async defmain():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        if result.success:
            print(f"Screenshot data present: {result.screenshotisnotNone}")
            print(f"PDF data present: {result.pdfisnotNone}")
...
```
fromcrawl4aiimport UndetectedAdapter
fromcrawl4ai.async_crawler_strategyimport AsyncPlaywrightCrawlerStrategy
...
Detection Level Recommended Approach No protection Regular browser Basic checks Regular + Stealth mode Advanced protection Undetected browser Maximum evasion Undetected + Stealth mode
