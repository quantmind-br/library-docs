---
title: Navigation API | Moodle Developer Resources
url: https://moodledev.io/docs/5.2/apis/core/navigation
source: sitemap
fetched_at: 2026-02-17T15:43:38.456415-03:00
rendered_js: false
word_count: 1881
summary: This document explains the Moodle Navigation API and how to use the $PAGE object to manage navigation, settings structures, and breadcrumbs. It details how the navigation system is structured and the methods for extending it within custom plugins or page code.
tags:
    - moodle-development
    - navigation-api
    - php-development
...
## What the navigation is[​](#what-the-navigation-is "Direct link to What the navigation is")
...
![Information Architecture](https://moodledev.io/assets/images/Moodle-IA-bf78c45daa4be208b4594d483811a4a7.png)
...
## What the navigation is not[​](#what-the-navigation-is-not "Direct link to What the navigation is not")
...
## How the navigation works[​](#how-the-navigation-works "Direct link to How the navigation works")
...
- `$PAGE->course` is the course the user is viewing. This is essential if the context is CONTEXT\_COURSE or anything within it. However, it is also useful in other contexts such as CONTEXT\_USER.
- `$PAGE->cm` is the course module instance. This is essential if the context is CONTEXT\_MODULE.
- `$PAGE->url` is used to match the active navigation item.

Nearly every page sets `$PAGE->url` through a call to `$PAGE->set_url()` however not many explicitly set the context, course, or cm. When you call `require_login()` with a course or context\_module, it automatically calls the following:

Set up the global course
...
2. The page is using CONTEXT\_SYSTEM, CONTEXT\_COURSECAT, or CONTEXT\_USER (call `$PAGE->set_context()`).
...
## Extending the navigation[​](#extending-the-navigation "Direct link to Extending the navigation")

### Code extension[​](#code-extension "Direct link to Code extension")
...
##### Extending the main navigation structure[​](#extending-the-main-navigation-structure "Direct link to Extending the main navigation structure")

```
$previewnode=$PAGE->navigation->add(
get_string('preview'),
newmoodle_url('/a/link/if/you/want/one.php'),
...
The above lines of code adds a preview node to the bottom of the navigation and then adds a thing node to the preview node (adding a leaf to our tree). The final line of code makes the thing node active so that the navbar finds it however if the URL you give it is the same as the url you set for the page it will automatically be marked active and you won't need this call.

##### Extending the navigation for the course[​](#extending-the-navigation-for-the-course "Direct link to Extending the navigation for the course")
...
Moodle loads plugins in alphabetical order. This means that plugin\_b can find a node added by plugin\_a but not the other way around. However, plugins must abide by the [Component communication principles](https://moodledev.io/general/development/policies/component-communication).

#### Settings navigation[​](#settings-navigation "Direct link to Settings navigation")
...
##### Add Settings folder to navigation[​](#add-settings-folder-to-navigation "Direct link to Add Settings folder to navigation")

This example adds a settings folder to the navigation API at Site administration / Plugins / Activity modules / Assignment.

![The structure of the Assignment plugin in the Settings navigation](https://moodledev.io/assets/images/assignmentmenu-38eb54e97b8b87d7e2553ee485cd8875.png)
...
$settings=newadmin_settingpage($section,get_string('settings','block_sample'));
if($ADMIN->fulltree){
$settings->add(
...
#### Navbar[​](#navbar "Direct link to Navbar")
...
The above code tells the navbar to ignore the automatically detected *active page* and to instead use what is manually added, at which point we add two items as shown.

### Plugin Callbacks[​](#plugin-callbacks "Direct link to Plugin Callbacks")
...
```
functionmy_plugin_extend_settings_navigation($settingsnav,$context){
...
$url=newmoodle_url('/grade/report/grader/index.php',[
'id'=>$id,
]);
...
#### Course Formats[​](#course-formats "Direct link to Course Formats")
...
#### Course Reports[​](#course-reports "Direct link to Course Reports")
...
#### Course settings[​](#course-settings "Direct link to Course settings")
...
#### Category settings[​](#category-settings "Direct link to Category settings")
...
#### Frontpage settings[​](#frontpage-settings "Direct link to Frontpage settings")
...
### Boost theme[​](#boost-theme "Direct link to Boost theme")
...
It is possible, but *not recommended*, for plugins to add nodes to the flat navigation (see [FAQ's and troubleshooting](#faqs-and-troubleshooting) for more information).

## FAQ's and troubleshooting[​](#faqs-and-troubleshooting "Direct link to FAQ's and troubleshooting")

### **Q.** My page is on the navigation but it doesn't find it?[​](#q-my-page-is-on-the-navigation-but-it-doesnt-find-it "Direct link to q-my-page-is-on-the-navigation-but-it-doesnt-find-it")
...
newmoodle_url('/your/url/here.php',[
'param'=>'value',
])
);
```

### **Q.** How do I add a node to the "flat" navigation in the Boost theme?[​](#q-how-do-i-add-a-node-to-the-flat-navigation-in-the-boost-theme "Direct link to q-how-do-i-add-a-node-to-the-flat-navigation-in-the-boost-theme")
