...
source: crawler
fetched_at: 2026-04-24T17:00:22.677727213-03:00
rendered_js: false
word_count: 1447
summary: This document explains how to create and structure capabilities in Hermes Agent, detailing the differences between creating a 'Skill' versus a 'Tool.' It also provides comprehensive documentation on metadata fields like required tools, platforms, configuration settings, and environment variable handling for skills.
tags:
    - hermes-agent
    - skill-vs-tool
    - metadata-structure
...
FieldBehavior`requires_toolsets`Skill is **hidden** when ANY listed toolset is **not** available`requires_tools`Skill is **hidden** when ANY listed tool is **not** available`fallback_for_toolsets`Skill is **hidden** when ANY listed toolset **is** available`fallback_for_tools`Skill is **hidden** when ANY listed tool **is** available
...
TokenReplaced with`${HERMES_SKILL_DIR}`Absolute path to the skill's directory`${HERMES_SESSION_ID}`The active session id (left in place if there is no session)
