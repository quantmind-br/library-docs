---
title: Transactions | Moodle Developer Resources
url: https://moodledev.io/docs/4.4/apis/core/dml/delegated-transactions
source: sitemap
fetched_at: 2026-02-17T15:02:20.248435-03:00
rendered_js: false
word_count: 437
summary: This document explains the implementation and usage of delegated database transactions in Moodle for managing CRUD operations and ensuring data integrity. It details the API methods for starting, committing, and rolling back transactions, as well as rules for handling nested transactions and exceptions.
tags:
    - moodle-development
...
Moodle allows data manipulation to take place within a database transaction, known as a *Delegated transaction*. This allows you to perform CRUD[1](#user-content-fn-1) operations, and roll them back if a failure takes place.
...
2. Code should **not** rely on a rollback happening. It is only a measure to reduce (not to eliminate) DB[2](#user-content-fn-2) garbled information
...
1. All the handling must go, exclusively, to a `moodle_database` object, leaving real drivers only implementing (protected) the old begin/commit/rollback\_sql() functions
...
6. Normal usage of the moodle\_transaction will be:
...
![The flow of transactions in diagram format](https://moodledev.io/assets/images/TransactionsAndExceptionsFlow-8f8e565adf6b9664f444f701a8067bdf.png)

1. Any default exception handler will:
   
   1. Catch uncaught transaction\_exception exceptions
...
1. Create Read Update Delete [↩](#user-content-fnref-1)
2. The Moodle database [↩](#user-content-fnref-2)