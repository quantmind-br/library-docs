...
title: Text sizing protocol
word_count: 3733
summary: This document defines a terminal escape code protocol that allows applications to render text at varying sizes and precisely control character cell dimensions for improved typography and layout.
category: api
optimized: true
...
terminal. Public discussion of this protocol is here <8226>.
...
text which is simply plain text encoded as safe_utf8, the text must be
...
> See the section on detect_text_sizing on how client applications can
...
1. If the text is a combining character it is added to the existing multicell
   character
1. If the text will overwrite the top-left cell of the multicell character, the
   entire multicell character must be erased
1. If the text will overwrite any cell in the topmost row of the multicell
   ...
   wide characters)
1. If the text will overwrite cells from a row after the first row, then cursor should be moved past the
...
> [!WARNING]
> This algorithm is under public discussion in 8533. If serious issues
...
1. Next, check if the code point is *invalid*, and if it is, discard it
...
1. Next, check if there is a previous cell before the
...
1. Next, calculate the width in cells of the received code point,
   which can be 0, 1, or 2 depending on the code point properties in
   the Unicode standard.

1. If there is no previous cell and the code point's width is zero,
   the code point is discarded and its processing is finished.

1. If there is a previous cell, the
...
1. If there is no boundary, the current code point is added to the previous
   cell and processing of the code point is finished. See the var_select
   section below for handling of Unicode Variation selectors.

1. If there is a boundary, but the width of the current code point is zero,
   it is added to the previous cell and processing is finished.

1. The code point is added to the current cell and the cursor is moved forward
...
1. *Doublewidth*: Parse `EastAsianWidth.txt
   ...
   they are marked as A in EastAsianWidth.txt: :code:`[0x3400,
   0x4DBF], [0x4E00, 0x9FFF], [0xF900, 0xFAFF], [0x20000, 0x2FFFD], [0x30000, 0x3FFFD]`

1. *Wide emoji*: Parse `emoji-sequences.txt
   ...
   followed by FE0F in the file. The leading codepoints in all
...
1. *Marks*: These are all zero width code points. They are code points with Unicode
...
1. All remaining code points have a width of one cell.

### Unicode variation selectors

There are two code points (U+FE0E and U+FE0F) that can actually
[alter the width of the previous code point](https://www.unicode.org/reports/tr11/#ED4)
