...
url: https://moodledev.io/docs/5.0/apis/plugintypes/format
source: sitemap
fetched_at: 2026-02-17T15:25:36.314621-03:00
rendered_js: false
word_count: 5368
summary: This document explains the architecture and implementation of course format plugins in Moodle, detailing the required file structure and the roles of key files like format.php and lib.php.
tags:
    - moodle-development
    - course-formats
    - plugin-structure
...
In case your plugin is a Moodle 3.11 compatible plugin, see the [migration guide](https://moodledev.io/docs/5.0/apis/plugintypes/format/migration) for more information.
...
## Implementing format specific actions[​](#implementing-format-specific-actions "Direct link to Implementing format specific actions")
...
### Execute format specific action from the course page via Ajax[​](#execute-format-specific-action-from-the-course-page-via-ajax "Direct link to Execute format specific action from the course page via Ajax")
...
### Execute state actions via non-AJAX requests[​](#execute-state-actions-via-non-ajax-requests "Direct link to Execute state actions via non-AJAX requests")
...
namespaceformat_yourplugin;

usecore_courseformat\output\local\courseupdate;

classcourseupdateextendscourseupdate{

publicfunctionsection_dosomething_confirmation_dialog(
...
continue:newurl($this->actionurl,['confirm'=>1]),
displayoptions:[
...
## Activity badge[​](#activity-badge "Direct link to Activity badge")
...
```
namespacecore_courseformat\output;

abstractclassactivitybadgeimplementsnamed_templatable, \renderable {
...
abstractprotectedfunctionupdate_content():void;
...
- **Forum**, to display the unread messages.
  ...
  ```
  namespacemod_forum\output\courseformat;
  
  classactivitybadgeextends\core_courseformat\output\activitybadge{
  
  protectedfunctionupdate_content():void{
  global$CFG;
...
- **Resource**, to show the file type (extension). The rest of the resource information (size and creation date) has been kept in the `afterlink` section.
  ...
  ```
  namespacemod_resource\output\courseformat;
