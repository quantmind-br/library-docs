---
title: metrics - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/spec_decode/metrics/
source: sitemap
fetched_at: 2026-05-07T21:41:53.355352368-03:00
rendered_js: false
word_count: 123
summary: This document describes the classes and infrastructure for tracking, aggregating, and exporting speculative decoding performance metrics in vLLM.
tags:
...
## SpecDecodingLogging [¶](#vllm.v1.spec_decode.metrics.SpecDecodingLogging "Permanent link")

Aggregate and log spec decoding metrics.

LoggingStatLogger aggregates per-iteration metrics over a set time interval using observe() and then logs them using log() before resetting to zero.

Source code in `vllm/v1/spec_decode/metrics.py`

```
classSpecDecodingLogging:
"""Aggregate and log spec decoding metrics.
    ...
    defobserve(self, spec_decoding_stats: SpecDecodingStats):
...
## SpecDecodingProm [¶](#vllm.v1.spec_decode.metrics.SpecDecodingProm "Permanent link")

Record spec decoding metrics in Prometheus.

The acceptance rate can be calculated using a PromQL query:

rate(vllm:spec\_decode\_num\_accepted\_tokens\_total\[\\(interval]) / rate(vllm:spec\_decode\_num\_draft\_tokens\_total\[\\)interval])

The mean acceptance length (conventionally including bonus tokens) can be calculated using:

1 + ( rate(vllm:spec\_decode\_num\_accepted\_tokens\_total\[\\(interval]) / rate(vllm:spec\_decode\_num\_drafts\[\\)interval]))

A per-position acceptance rate vector can be computed using

vllm:spec\_decode\_num\_accepted\_tokens\_per\_pos\[\\(interval] / vllm:spec\_decode\_num\_drafts\[\\)interval]

Source code in `vllm/v1/spec_decode/metrics.py`

```
classSpecDecodingProm:
"""Record spec decoding metrics in Prometheus.
    ...
    defobserve(self, spec_decoding_stats: SpecDecodingStats, engine_idx: int = 0):
        if not self.spec_decoding_enabled:
            return
...
## SpecDecodingStats `dataclass` [¶](#vllm.v1.spec_decode.metrics.SpecDecodingStats "Permanent link")

Per-step iteration decoding stats from scheduler.

Each scheduler step, statistics on spec decoding performance are aggregated across requests by the scheduler and returned to the frontend in EngineCoreOutputs-&gt;SchedulerStats.

Source code in `vllm/v1/spec_decode/metrics.py`

```
@dataclass
classSpecDecodingStats:
"""Per-step iteration decoding stats from scheduler.
    ...
    @classmethod
    defnew(cls, num_spec_tokens: int) -> "SpecDecodingStats":
        return cls(
            num_spec_tokens=num_spec_tokens,
            num_accepted_tokens_per_pos=[0] * num_spec_tokens,
        )

    defobserve_draft(self, num_draft_tokens: int, num_accepted_tokens: int):
        self.num_drafts += 1
