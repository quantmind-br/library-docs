...
source: sitemap
fetched_at: 2026-05-07T21:17:35.272784933-03:00
rendered_js: false
word_count: 4
summary: This class provides a Python interface for NCCL collective communication operations, enabling efficient inter-process data movement and reduction for distributed training workflows.
tags:
    - nccl
...
"""
    ...
    defsend(self, tensor: torch.Tensor, dst: int, stream=None):
        if self.disabled:
            return
    ...
    defrecv(self, tensor: torch.Tensor, src: int, stream=None):
        if self.disabled:
            return
        assert tensor.device == self.device, (
            f"this nccl communicator is created to work on {self.device}, "
    ...
    defbroadcast(self, tensor: torch.Tensor, src: int, stream=None):
        if self.disabled:
            return
        assert tensor.device == self.device, (
            f"this nccl communicator is created to work on {self.device}, "
    ...
    defregister_comm_window(self, tensor: torch.Tensor):
    ...
    defregister_comm_window_raw(self, ptr: int, size: int):
        return self.nccl.ncclCommWindowRegister(self.comm, buffer_type(ptr), size, 1)

    defderegister_comm_window(self, window):
        return self.nccl.ncclCommWindowDeregister(self.comm, window)

    defbatch_isend_irecv(self, p2p_ops: list, stream=None):
        if self.disabled:
            return
        if stream is None:
            stream = current_stream()
