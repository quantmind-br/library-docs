---
title: ProgramOfThought - DSPy
url: https://dspy.ai/api/modules/ProgramOfThought/
source: sitemap
fetched_at: 2026-01-23T08:02:00.779638403-03:00
rendered_js: false
word_count: 853
summary: This document defines the ProgramOfThought module in DSPy, which enables language models to solve problems by iteratively generating and executing Python code.
tags:
    - dspy
    - program-of-thought
    - python-interpreter
...
A DSPy module that runs Python programs to solve a problem. This module requires deno to be installed. Please install deno following https://docs.deno.com/runtime/getting\_started/installation/
...
The maximum number of iterations to retry code generation and execution.

`3` `interpreter` `PythonInterpreter | None`
...
```
def__init__(self, signature: str | type[Signature], max_iters: int = 3, interpreter: PythonInterpreter | None = None):
"""
    Args:
...
### Functions[¶](#dspy.ProgramOfThought-functions "Permanent link")

#### `__call__(*args, **kwargs) -> Prediction` [¶](#dspy.ProgramOfThought.__call__ "Permanent link")
...
#### `acall(*args, **kwargs) -> Prediction` `async` [¶](#dspy.ProgramOfThought.acall "Permanent link")
...
#### `deepcopy()` [¶](#dspy.ProgramOfThought.deepcopy "Permanent link")
...
#### `dump_state(json_mode=True)` [¶](#dspy.ProgramOfThought.dump_state "Permanent link")
...
#### `forward(**kwargs)` [¶](#dspy.ProgramOfThought.forward "Permanent link")
...
#### `get_lm()` [¶](#dspy.ProgramOfThought.get_lm "Permanent link")
...
#### `inspect_history(n: int = 1)` [¶](#dspy.ProgramOfThought.inspect_history "Permanent link")
...
#### `load(path, allow_pickle=False)` [¶](#dspy.ProgramOfThought.load "Permanent link")
...
#### `load_state(state)` [¶](#dspy.ProgramOfThought.load_state "Permanent link")
...
#### `map_named_predictors(func)` [¶](#dspy.ProgramOfThought.map_named_predictors "Permanent link")
...
#### `named_parameters()` [¶](#dspy.ProgramOfThought.named_parameters "Permanent link")
...
#### `named_predictors()` [¶](#dspy.ProgramOfThought.named_predictors "Permanent link")
...
#### `named_sub_modules(type_=None, skip_compiled=False) -> Generator[tuple[str, BaseModule], None, None]` [¶](#dspy.ProgramOfThought.named_sub_modules "Permanent link")
...
#### `parameters()` [¶](#dspy.ProgramOfThought.parameters "Permanent link")
...
#### `predictors()` [¶](#dspy.ProgramOfThought.predictors "Permanent link")
...
#### `reset_copy()` [¶](#dspy.ProgramOfThought.reset_copy "Permanent link")
...
#### `save(path, save_program=False, modules_to_serialize=None)` [¶](#dspy.ProgramOfThought.save "Permanent link")
...
#### `set_lm(lm)` [¶](#dspy.ProgramOfThought.set_lm "Permanent link")
