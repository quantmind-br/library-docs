...
This is the **exact prompt** the dispatcher injects into each parallel `Agent` call during Phase 2. Substitute `<FILES>` and `<SIBLINGS>` before sending. Every agent runs in isolation — the prompt is fully self-contained.
...
The `title` field comes from each file's H1 (or filename slug if H1 absent) — NOT from `metadata.json`'s top-level `title` field, which is often the sitemap-fetched page title and may be duplicated across docs in a corpus, making title-matching useless.
...
```
You optimize markdown documentation files for AI/LLM agent consumption. Read each file listed below, rewrite it in place using the Edit/Write tool, then return a one-line confirmation per file.

## Files to optimize (absolute paths)
<FILES>

## Sibling docs in same folder (for link resolution)
JSON list of every surviving doc in the target folder. Use ONLY this list to detect which links in the file point to siblings (see Workflow per file → step 4). Do NOT read `metadata.json` yourself — the dispatcher already filtered it.
<SIBLINGS>
