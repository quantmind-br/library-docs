---
title: core_client - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/engine/core_client/
source: sitemap
fetched_at: 2026-05-07T21:40:34.435674076-03:00
rendered_js: false
word_count: 462
summary: This document defines the AsyncMPClient class, which provides an asyncio-compatible interface for communicating with a multi-process EngineCore within the vLLM framework.
tags:
    - asyncio
    - multi-processing
    - engine-core
...
## AsyncMPClient [¶](#vllm.v1.engine.core_client.AsyncMPClient "Permanent link")
...
"""Asyncio-compatible client for multi-proc EngineCore."""
    ...
    def_ensure_output_queue_task(self):
        ...
        async defprocess_outputs_socket():
            try:
                while True:
    ...
    async defget_output_async(self) -> EngineCoreOutputs:
        self._ensure_output_queue_task()
        ...
        if isinstance(outputs, Exception):
            raise self._format_exception(outputs) fromNone
        return outputs
...
"""
        ...
        defadd_pending(f: asyncio.Future[zmq.MessageTracker]):
    ...
    async defcall_utility_async(self, method: str, *args) -> Any:
        return await self._call_utility_async(method, *args, engine=self.core_engine)

    async def_call_utility_async(
    ...
    async defget_supported_tasks_async(self) -> tuple[SupportedTask, ...]:
        return await self.call_utility_async("get_supported_tasks")

    async defadd_request_async(self, request: EngineCoreRequest) -> None:
        request.client_index = self.client_index
        await self._send_input(EngineCoreRequestType.ADD, request)
        self._ensure_output_queue_task()

    async defabort_requests_async(self, request_ids: list[str]) -> None:
        if request_ids and not self.resources.engine_dead:
            await self._send_input(EngineCoreRequestType.ABORT, request_ids)

    async defpause_scheduler_async(
        self, mode: PauseMode = "abort", clear_cache: bool = True
    ) -> None:
        await self.call_utility_async("pause_scheduler", mode, clear_cache)

    async defresume_scheduler_async(self) -> None:
        await self.call_utility_async("resume_scheduler")

    async defis_scheduler_paused_async(self) -> bool:
        return await self.call_utility_async("is_scheduler_paused")

    async defprofile_async(
        self, is_start: bool = True, profile_prefix: str | None = None
    ) -> None:
        await self.call_utility_async("profile", is_start, profile_prefix)

    async defreset_mm_cache_async(self) -> None:
        await self.call_utility_async("reset_mm_cache")

    async defreset_prefix_cache_async(
    ...
    async defreset_encoder_cache_async(self) -> None:
        await self.call_utility_async("reset_encoder_cache")

    async defsleep_async(self, level: int = 1, mode: PauseMode = "abort") -> None:
        await self.call_utility_async("sleep", level, mode)

    async defwake_up_async(self, tags: list[str] | None = None) -> None:
        await self.call_utility_async("wake_up", tags)

    async defis_sleeping_async(self) -> bool:
        return await self.call_utility_async("is_sleeping")

    async defexecute_dummy_batch_async(self) -> None:
        await self.call_utility_async("execute_dummy_batch")

    async defadd_lora_async(self, lora_request: LoRARequest) -> bool:
        return await self.call_utility_async("add_lora", lora_request)

    async defremove_lora_async(self, lora_id: int) -> bool:
        return await self.call_utility_async("remove_lora", lora_id)

    async deflist_loras_async(self) -> set[int]:
        return await self.call_utility_async("list_loras")

    async defpin_lora_async(self, lora_id: int) -> bool:
        return await self.call_utility_async("pin_lora", lora_id)

    async defsave_sharded_state_async(
        self, path: str, pattern: str | None = None, max_size: int | None = None
    ) -> None:
        await self.call_utility_async("save_sharded_state", path, pattern, max_size)

    async defcollective_rpc_async(
        self,
...
### \_send\_input\_message [¶](#vllm.v1.engine.core_client.AsyncMPClient._send_input_message "Permanent link")
...
## BackgroundResources `dataclass` [¶](#vllm.v1.engine.core_client.BackgroundResources "Permanent link")
...
"""Clean up background resources."""
    ...
    defvalidate_alive(self, frames: Sequence[zmq.Frame]):
        if len(frames) == 1 and (frames[0].buffer == EngineCoreProc.ENGINE_CORE_DEAD):
            self.engine_dead = True
            raise EngineDeadError()
```

### \_\_call\__ [¶](#vllm.v1.engine.core_client.BackgroundResources.__call__ "Permanent link")
...
## DPAsyncMPClient [¶](#vllm.v1.engine.core_client.DPAsyncMPClient "Permanent link")
...
```
classDPAsyncMPClient(AsyncMPClient):
"""Asyncio-compatible client for multi-proc, multi-engine (data parallel)
    ...
    def_ensure_stats_update_task(self):
        resources = self.resources
        ...
        async defrun_engine_stats_update_task():
            with (
    ...
    defget_core_engine_for_request(self, request: EngineCoreRequest):
        return self.core_engine
```

## DPLBAsyncMPClient [¶](#vllm.v1.engine.core_client.DPLBAsyncMPClient "Permanent link")
...
```
classDPLBAsyncMPClient(DPAsyncMPClient):
"""Asyncio-compatible client for multi-proc, multi-engine (data parallel)
    ...
    defget_core_engine_for_request(self, request: EngineCoreRequest) -> EngineIdentity:
    ...
    @staticmethod
    async defprocess_engine_outputs(
    ...
    @staticmethod
    async defeep_process_engine_core_notification(
        ...
        if notification_type == EEPNotificationType.RECONFIGURE_FINISHED:
            fromvllm.v1.engineimport UtilityResult
...
"""Scale elastic EP data parallel size"""
    ...
    async def_eep_wait_for_setup_switch_complete(self) -> None:
"""
    ...
    def_setup_elastic_ep_reconfig_bootstrap(self) -> tuple[str, int]:
        fromvllm.distributed.utilsimport create_tcp_store
        fromvllm.utils.network_utilsimport get_open_ports_list
...
"""Scale up the data parallel size by creating new engine cores
    ...
    async def_scale_down_elastic_ep(
        self, cur_data_parallel_size: int, new_data_parallel_size: int
    ) -> None:
...
### \_eep\_wait\_for\_setup\_switch\_complete `async` [¶](#vllm.v1.engine.core_client.DPLBAsyncMPClient._eep_wait_for_setup_switch_complete "Permanent link")

```
_eep_wait_for_setup_switch_complete() -> None
```

Wait for core engines to switch to the new setup.

In eep\_process\_engine\_core\_notification(), a dummy UtilityOutput with EEP\_NOTIFICATION\_CALL\_ID will be set when RECONFIGURE\_FINISHED notification is received from engine 0. We create a future with that call\_id and wait for it to be resolved.
...
### \_scale\_down\_elastic\_ep `async` [¶](#vllm.v1.engine.core_client.DPLBAsyncMPClient._scale_down_elastic_ep "Permanent link")
...
### \_scale\_up\_elastic\_ep `async` [¶](#vllm.v1.engine.core_client.DPLBAsyncMPClient._scale_up_elastic_ep "Permanent link")
...
### scale\_elastic\_ep `async` [¶](#vllm.v1.engine.core_client.DPLBAsyncMPClient.scale_elastic_ep "Permanent link")
...
## EngineCoreClient [¶](#vllm.v1.engine.core_client.EngineCoreClient "Permanent link")
...
"""
    ...
    * AsyncMPClient: ZMQ + background proc EngineCore w/ asyncio (for AsyncLLM)
    ...
    defget_output(self) -> EngineCoreOutputs:
        raise NotImplementedError

    defget_supported_tasks(self) -> tuple[SupportedTask, ...]:
        raise NotImplementedError

    defadd_request(self, request: EngineCoreRequest) -> None:
        raise NotImplementedError

    defprofile(self, is_start: bool = True, profile_prefix: str | None = None) -> None:
        raise NotImplementedError
    ...
    defsleep(self, level: int = 1, mode: PauseMode = "abort") -> None:
        raise NotImplementedError

    defwake_up(self, tags: list[str] | None = None) -> None:
        raise NotImplementedError

    defis_sleeping(self) -> bool:
        raise NotImplementedError
    ...
    defabort_requests(self, request_ids: list[str]) -> None:
        raise NotImplementedError
    ...
    defdp_engines_running(self) -> bool:
...
### dp\_engines\_running [¶](#vllm.v1.engine.core_client.EngineCoreClient.dp_engines_running "Permanent link")

```
dp_engines_running() -> bool
```

Returns True if data parallel engines are collectively in a running state.
...
## InprocClient [¶](#vllm.v1.engine.core_client.InprocClient "Permanent link")

Bases: `EngineCoreClient`

InprocClient: client for in-process EngineCore. Intended for use in LLMEngine for V0-style add\_request() and step() EngineCore setup in this process (no busy loop).
...
```
classInprocClient(EngineCoreClient):
"""
...
## MPClient [¶](#vllm.v1.engine.core_client.MPClient "Permanent link")
...
- pushes EngineCoreRequests via input\_socket
- pulls EngineCoreOutputs via output\_socket
...
```
classMPClient(EngineCoreClient):
"""
...
"""Shutdown engine manager under timeout and clean up resources."""
    ...
    def_format_exception(self, e: Exception) -> Exception:
"""If errored, use EngineDeadError so root cause is clear."""
        return (
            EngineDeadError(suppress_context=True) if self.resources.engine_dead else e
        )

    defensure_alive(self):
        if self.resources.engine_dead:
            raise EngineDeadError()

    defadd_pending_message(self, tracker: zmq.MessageTracker, msg: Any):
        if not tracker.done:
            self.pending_messages.appendleft((tracker, msg))

    deffree_pending_messages(self):
    ...
    defstart_engine_core_monitor(self):
"""Start a monitor thread for engine core processes."""
    ...
    def_apply_ready_response(self, payload: bytes) -> None:
...
### \_apply\_ready\_response [¶](#vllm.v1.engine.core_client.MPClient._apply_ready_response "Permanent link")

```
_apply_ready_response(payload: bytes) -> None
```

Decode an EngineCoreReadyResponse and sync any post-initialization config changes (e.g. auto-fitted max\_model\_len) back to the frontend.
...
### \_format\_exception [¶](#vllm.v1.engine.core_client.MPClient._format_exception "Permanent link")
...
### shutdown [¶](#vllm.v1.engine.core_client.MPClient.shutdown "Permanent link")
...
### start\_engine\_core\_monitor [¶](#vllm.v1.engine.core_client.MPClient.start_engine_core_monitor "Permanent link")
...
## SyncMPClient [¶](#vllm.v1.engine.core_client.SyncMPClient "Permanent link")
...
"""Synchronous client for multi-proc EngineCore."""
    ...
    def_send_input(self, request_type: EngineCoreRequestType, request: Any):
        self.ensure_alive()
        self.free_pending_messages()
    ...
    defcall_utility(self, method: str, *args) -> Any:
        call_id = uuid.uuid1().int >> 64
...
## \_process\_utility\_output [¶](#vllm.v1.engine.core_client._process_utility_output "Permanent link")
