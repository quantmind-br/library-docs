---
title: SGLang Native APIs - SGLang Documentation
url: https://docs.sglang.io/docs/basic_usage/native_api
source: sitemap
fetched_at: 2026-05-11T05:49:09.825858465-03:00
rendered_js: false
word_count: 688
summary: This document provides an overview and usage instructions for the native SGLang Runtime server APIs, covering endpoints for text generation, embedding, reranking, and server management.
tags:
    - sglang
    - runtime-api
    - llm-inference
...
Generate completions. This is similar to the `/v1/completions` in OpenAI API. Detailed parameters can be found in the [sampling parameters](https://docs.sglang.io/docs/basic_usage/sampling_params).
...
Flush the radix cache. It will be automatically triggered when the model weights are updated by the `/update_weights` API. Parameters:
...
Encode text into embeddings. Note that this API is only available for [embedding models](https://docs.sglang.io/docs/basic_usage/openai_api_embeddings) and will raise an error for generation models. Therefore, we launch a new server to server an embedding model.
