...
source: llms
fetched_at: 2026-01-24T14:16:09.50024139-03:00
rendered_js: false
word_count: 1403
summary: This document explains how BuildKit garbage collection manages the build cache and provides instructions for customizing prune policies and storage limits across different Docker build drivers.
tags:
    - docker-build
    - buildkit
    - garbage-collection
...
> 
> `daemon.json``buildkitd.toml``type=source.local``type==source.local``private=true``private==true``shared=true``shared==true`
> 
> See [prune filters](https://docs.docker.com/reference/cli/docker/buildx/prune/#filter) for information about available GC filters. GC configuration in `daemon.json` supports all filters except `mutable` and `immutable`.
...
See [buildx prune filters](https://docs.docker.com/reference/cli/docker/buildx/prune/#filter) for information about available GC filters.