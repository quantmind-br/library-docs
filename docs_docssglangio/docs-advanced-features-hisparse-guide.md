---
title: 'HiSparse: Hierarchical Sparse Attention - SGLang Documentation'
url: https://docs.sglang.io/docs/advanced_features/hisparse_guide
source: sitemap
fetched_at: 2026-05-11T05:49:35.358163551-03:00
rendered_js: false
word_count: 541
summary: This document explains the HiSparse optimization technique, which reduces GPU memory usage during LLM decoding by offloading KV cache data to host memory and using on-demand swapping.
tags:
    - llm-inference
    - gpu-optimization
    - kv-cache
...
ArgumentType / DefaultDescription`—enable-hisparse`flag; default: disabledEnable HiSparse on the decode instance`—hisparse-config`JSON stringConfiguration for HiSparse (see below)

### HiSparse Config Parameters

Pass as a JSON string via `--hisparse-config`:

ParameterType / DefaultDescription`top_k`intNumber of topk entries`device_buffer_size`intNumber of token slots in the per-request GPU device buffer`host_to_device_ratio`intRatio of logical pool size to device pool size, determining host memory capacity
...
- On the decode instance, the following flags are **required** for HiSparse:
  ...
  - `--hisparse-config` — HiSparse configuration (top\_k, device\_buffer\_size, host\_to\_device\_ratio).
