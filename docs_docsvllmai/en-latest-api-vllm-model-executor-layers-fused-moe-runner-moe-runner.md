---
title: moe_runner - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/fused_moe/runner/moe_runner/
source: sitemap
fetched_at: 2026-05-07T21:25:38.140074684-03:00
rendered_js: false
word_count: 108
summary: The MoERunner class orchestrates the execution of Mixture of Experts (MoE) layers, managing token routing, expert computation, output transformation, and distributed synchronization across various parallel hardware configurations.
tags:
    - moe
    - mixture-of-experts
...
```
classMoERunner(MoERunnerInterface):
"""
    ...
    def_select_forward(self) -> Callable:
    ...
    defapply_routed_input_transform(
...
"""Apply transform for routed experts (e.g., latent projection).
    ...
    defapply_routed_output_transform(
        self,
...
"""Apply transform to routed expert output (e.g., latent to full dim).
    ...
    def_maybe_apply_routed_scale_to_output(
        self,
...
"""Truncate padded dimensions and all-reduce the combined output.
    ...
    def_encode_layer_name(self) -> str | LayerName:
        if _USE_LAYERNAME:
    ...
    def_trtllm_mxfp4_unpadded_dim(self) -> int:
"""Return ``hidden_dim_unpadded`` when the active backend is TRT-LLM
        ...
        """
        fromvllm.model_executor.layers.fused_moe.experts.trtllm_mxfp4_moeimport (
...
"""Run expert routing and the fused MoE kernel via the quant method.
    ...
    def_sequence_parallel_context(self):
"""Return a context manager for sequence-parallel token
    ...
    def_maybe_sync_shared_experts_stream(
        self,
        shared_experts_input: torch.Tensor | None,
    ):
    ...
    def_maybe_add_zero_expert_output(
        self,
        result: torch.Tensor,
    ) -> torch.Tensor:
...
"""Invoke the fused moe layer.
    ...
    @property
    defdo_naive_dispatch_combine(self) -> bool:
        return (
