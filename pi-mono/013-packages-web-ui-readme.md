---
title: README
url: https://github.com/badlogic/pi-mono/blob/main/packages/web-ui/README.md
source: git
fetched_at: 2026-05-03T09:32:24.553903838-03:00
rendered_js: false
word_count: 469
summary: Reusable web UI components for building AI chat interfaces powered by @mariozechner/pi-ai and @mariozechner/pi-agent-core, built with mini-lit and Tailwind CSS v4.
tags:
    - web-components
    - ai-chat
    - ui-library
...
Reusable web UI components for building AI chat interfaces powered by [[002-packages-ai-readme|@mariozechner/pi-ai]] and [[009-packages-coding-agent-readme|@mariozechner/pi-agent-core]].
...
| Feature | Description |
|---------|-------------|
| Chat UI | Message history, streaming, tool execution |
| Tools | JavaScript REPL, document extraction, artifacts (HTML, SVG, Markdown) |
...
| Custom Providers | Ollama, LM Studio, vLLM, OpenAI-compatible APIs |
...
> [!example]
> See [[012-packages-web-ui-example-readme|the example directory]] for a complete working application.
...
| `enableAttachments` | `boolean` | `true` | Show attachment button |
| `enableModelSelector` | `boolean` | `true` | Show model selector |
| `enableThinkingSelector` | `boolean` | `true` | Show thinking level selector |
...
Custom LLM providers (see [[022-packages-coding-agent-docs-custom-provider|Custom Providers]] for full guide):
...
> [!info]
> Providers requiring proxy: `zai` always; `anthropic` only OAuth tokens (`sk-ant-oat-*`).

## Dialogs

| Dialog | Purpose |
|--------|---------|
| `SettingsDialog` | Settings with ProvidersModelsTab, ProxyTab, ApiKeysTab |
| `SessionListDialog` | Browse and load saved sessions |
| `ApiKeyPromptDialog` | Prompt for API key when needed |
| `ModelSelector` | Select model from available options |
...
- [[012-packages-web-ui-example-readme|example/]] - Complete web app with sessions, artifacts, custom messages
...
#web-components #ai-chat #ui-library #typescript #indexeddb #tailwind-css
