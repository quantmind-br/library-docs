---
title: fp8_kernel_utils - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/lora/ops/triton_ops/fp8_kernel_utils/
source: sitemap
fetched_at: 2026-05-07T21:22:48.171537289-03:00
rendered_js: false
word_count: 566
summary: This document describes Triton-based utility functions for Punica kernel construction, specifically focusing on matrix multiplication and LoRA expansion kernels with support for various FP8 quantization techniques.
tags:
    - triton
    - lora
    - fp8
...
## \_accumulate\_mm [¶](#vllm.lora.ops.triton_ops.fp8_kernel_utils._accumulate_mm "Permanent link")
...
Name Type Description Default `tiled_a` `tensor`

Loaded tile from A matrix

*required* `tiled_b` `tensor`

Loaded tile from B matrix

*required* `accumulator` `tensor`

Current accumulator value

*required* `a_scale_ptr` `tensor`

Scale pointer for A matrix

*required* `b_scale_ptr` `tensor`

Scale pointer for B matrix

*required* `a_scale_k_stride` `int`

K dimension stride for A's block-wise scales

*required* `b_scale_k_stride` `int`

K dimension stride for B's block-wise scales

*required* `iter_k` `int`

Current iteration's global K offset

*required* `group_k` `constexpr`

Block size for K dimension in block-wise quantization

*required* `group_n` `constexpr`

Block size for N dimension in block-wise quantization

*required* `use_fp8_w8a8` `constexpr`
...
## do\_expand\_kernel\_fp8 [¶](#vllm.lora.ops.triton_ops.fp8_kernel_utils.do_expand_kernel_fp8 "Permanent link")
...
FP8-compatible expand kernel for LoRA. Given an array of integers that identifies the rows of A, ram, a lora index that identifies which LoRA to use from lora\_ptr, lora\_index, a slice\_id that identifies the input/output slice, compute the matrix product with FP8 quantization support and store in the appropriate output location.

For expand kernel, the input (shrink output) may be in FP32/FP16/BF16, while the LoRA B weights can be in FP8.

Supports: - FP8 W8A8 quantization for LoRA B weights - Block-wise quantization with configurable group\_k and group\_n - Per-channel quantization - Tensor-wise quantization

Source code in `vllm/lora/ops/triton_ops/fp8_kernel_utils.py`

```
@triton.jit
defdo_expand_kernel_fp8(
    pid_n,
    lora_index,
    slice_id,
    input_ptr,
...
## do\_shrink\_kernel\_fp8 [¶](#vllm.lora.ops.triton_ops.fp8_kernel_utils.do_shrink_kernel_fp8 "Permanent link")
...
do_shrink_kernel_fp8(
    ...
    use_fp8_w8a8: constexpr,
    per_channel_quant: constexpr,
    launch_pdl: constexpr,
)
```

Given an array of integers that identifies the rows of A, ram, a lora index that identifies which LoRA to use from lora\_ptr, lora\_index, a slice\_id that identifies the input/output slice, compute the matrix product and store in the appropriate output location.

Source code in `vllm/lora/ops/triton_ops/fp8_kernel_utils.py`

```
@triton.jit
defdo_shrink_kernel_fp8(
    pid_n,
    pid_sk,
    slice_id,
    lora_index,
...
## fp8\_mm\_k [¶](#vllm.lora.ops.triton_ops.fp8_kernel_utils.fp8_mm_k "Permanent link")
...
FP8-compatible matrix multiplication kernel with quantization support. Given a\_ptr and b\_ptr, that identify the rows of A (m x k) and columns of B (k x n), iterate through the K dimension to compute the partial/complete matrix block product with proper dequantization.

Parameters:

Name Type Description Default `a_ptr` `tensor`

Array of pointers, identifying rows of A (FP8 or other dtype)

*required* `b_ptr` `tensor`
...
*required* `ak_stride` `int`

K dimension stride of the A matrix

*required* `bk_stride` `int`

K dimension stride of the B matrix
...
*required* `offset_k` `int`

Base offset along K dimension

*required* `K` `constexpr`

Length of the K dimension

*required* `BLOCK_M` `constexpr`

M dimension of the output block m x n

*required* `BLOCK_N` `constexpr`

N dimension of the output block m x n

*required* `BLOCK_K` `constexpr`

K dimension atom

*required* `EVEN_K` `constexpr`

True if the blocks of A and B can be loaded without masking

*required* `SPLIT_K` `constexpr`
...
*required* `per_channel_quant` `constexpr`

Whether using per-channel quantization

*required* `CAST_TYPE` `constexpr`

if True, cast the values from the A matrix to the B matrix dtype.

*required* `b_dtype` `constexpr`

datatype of the B matrix

*required* `USE_GDC` `constexpr`

Whether to use PDL. True indicates use.

*required* `base_k` `int`

Base offset along K dimension for current SPLIT\_K group
