---
title: Templates | Moodle Developer Resources
url: https://moodledev.io/docs/4.4/guides/templates
source: sitemap
fetched_at: 2026-02-17T15:05:38.128036-03:00
rendered_js: false
word_count: 4325
summary: This document explains how Moodle utilizes the Mustache template system for rendering HTML output, covering core syntax features and Moodle-specific extensions like helpers and blocks.
tags:
    - moodle-development
    - mustache-templates
    - frontend-rendering
...
And the values for the placeholder tags `name` and `grade` are provided as a JSON object, referred to as *rendering context* (note the *context* here has nothing to do with Moodle [permission contexts](https://moodledev.io/docs/4.4/apis/subsystems/roles#context). In Mustache, the term basically represents the data passed to the template).
...
### Raw unescaped variable[​](#raw-unescaped-variable "Direct link to Raw unescaped variable")
...
### Partials[​](#partials "Direct link to Partials")
...
### userdate[​](#userdate "Direct link to userdate")
...
### shortentext[​](#shortentext "Direct link to shortentext")
...
## Template files[​](#template-files "Direct link to Template files")
...
### Mustache file boilerplate[​](#mustache-file-boilerplate "Direct link to Mustache file boilerplate")
...
## Rendering in PHP[​](#rendering-in-php "Direct link to Rendering in PHP")
...
$data->things=[];
foreach($this->thingsas$thing){
$data->things[]=$thing->to_record();
}
$data->navigation=[];
foreach($this->navigationas$button){
 ...
 * @return string HTML
 */
protectedfunctionrender_mywidget(mywidget$widget){
...
## Rendering in JavaScript[​](#rendering-in-javascript "Direct link to Rendering in JavaScript")

Rendering a template from JavaScript is fairly easy. There is a [JavaScript module](https://moodledev.io/docs/4.4/guides/javascript/modules) that can load (and cache) a template and then use it for rendering the given data.
...
## Templates requiring JavaScript[​](#templates-requiring-javascript "Direct link to Templates requiring JavaScript")
...
## Overriding templates in a theme[​](#overriding-templates-in-a-theme "Direct link to Overriding templates in a theme")
...
Path for original templateComponentTheme override path`blocks/myoverview/templates/view-summary.mustache``block_myoverview``theme/mytheme/templates/block_myoverview/view-summary.mustache``group/templates/group_details.mustache``core_group``theme/mytheme/templates/core_group/group_details.mustache``lib/templates/modal.mustache``core``theme/mytheme/templates/core/modal.mustache``theme/boost/templates/navbar.mustache``theme_boost``theme/mytheme/templates/theme_boost/navbar.mustache`

## Documenting the templates[​](#documenting-the-templates "Direct link to Documenting the templates")
...
### Include GPL at the top of each template[​](#include-gpl-at-the-top-of-each-template "Direct link to Include GPL at the top of each template")
...
### Use data-attributes for JS hooks[​](#use-data-attributes-for-js-hooks "Direct link to Use data-attributes for JS hooks")
...
### Avoid custom CSS for templates[​](#avoid-custom-css-for-templates "Direct link to Avoid custom CSS for templates")
...
### Re-use core templates as much as possible[​](#re-use-core-templates-as-much-as-possible "Direct link to Re-use core templates as much as possible")
...
### Do use the CSS framework classes directly in the templates[​](#do-use-the-css-framework-classes-directly-in-the-templates "Direct link to Do use the CSS framework classes directly in the templates")
...
### Avoid IDs for styling or JavaScript[​](#avoid-ids-for-styling-or-javascript "Direct link to Avoid IDs for styling or JavaScript")
...
### Follow CSS coding style[​](#follow-css-coding-style "Direct link to Follow CSS coding style")
...
### Wrap each template in one node with a classname that matches the template name[​](#wrap-each-template-in-one-node-with-a-classname-that-matches-the-template-name "Direct link to Wrap each template in one node with a classname that matches the template name")
...
### Iterating over PHP arrays in Mustache templates[​](#iterating-over-php-arrays-in-mustache-templates "Direct link to Iterating over PHP arrays in Mustache templates")
...
### Test for non-empty array in Mustache templates[​](#test-for-non-empty-array-in-mustache-templates "Direct link to Test for non-empty array in Mustache templates")
...
### Squash whitespace in a template[​](#squash-whitespace-in-a-template "Direct link to Squash whitespace in a template")
...
### Access to globals[​](#access-to-globals "Direct link to Access to globals")
...
## Core templates[​](#core-templates "Direct link to Core templates")
...
- Added a `$tabname` block for in the tab\_header\_item template to make the name flexible on import.
- Added a `$tabpanelcontant` block in the tab\_content\_item template to make the content flexible on import. Cool, so let's see what that looks like in our example now:
