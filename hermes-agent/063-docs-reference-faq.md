...
source: crawler
fetched_at: 2026-04-24T17:00:05.269252039-03:00
rendered_js: false
word_count: 3316
summary: This document serves as a comprehensive FAQ and troubleshooting guide for Hermes Agent, explaining supported LLM providers, platform compatibility (Windows/Android), data handling, local model usage, core feature differences (memory vs. skills), and installation fixes.
tags:
    - faq
    - llm-provider
    - local-model
...
sudoaptinstall python3.12   # Ubuntu/Debian
...
Want to...UseAdd a new provider`hermes model` (from terminal)Enter/change API keys`hermes model` (from terminal)Switch model mid-session`/model <name>` (inside session)Switch to different configured provider`/model provider:model` (inside session)
...
ModeHow it works**Allowlist**Only user IDs listed in config can interact**DM pairing**First user to message in DM claims exclusive access**Open**Anyone can interact (not recommended for production)
...
hermes config show |grep-A12 mcp_servers
...
### Can I move a profile to a different machine?[​](#can-i-move-a-profile-to-a-different-machine "Direct link to Can I move a profile to a different machine?")
