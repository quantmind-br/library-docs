...
source: crawler
fetched_at: 2026-05-06T21:37:48.89995764-03:00
rendered_js: false
word_count: 2275
summary: This document explains how to use Rust lint check attributes, such as allow, warn, deny, and expect, to manage compilation diagnostic messages and enforce coding patterns.
tags:
    - rust
    - linting
    - compiler-attributes
...
The following [attributes](https://doc.rust-lang.org/stable/reference/attributes.html) are used for controlling or generating diagnostic messages during compilation.
...
The lint attributes `allow`, `expect`, `warn`, `deny`, and `forbid` use the [MetaListPaths](https://doc.rust-lang.org/stable/reference/attributes.html#grammar-MetaListPaths) syntax to specify a list of lint names to change the lint level for the entity to which the attribute applies.
...
> Note
> 
> The lint checks supported by `rustc` can be found via `rustc -W help`, along with their default settings and are documented in the [rustc book](https://doc.rust-lang.org/stable/rustc/lints/index.html).
...
> Note
> 
> `rustc` allows setting lint levels on the [command-line](https://doc.rust-lang.org/stable/rustc/lints/levels.html#via-compiler-flag), and also supports [setting caps](https://doc.rust-lang.org/stable/rustc/lints/levels.html#capping-lints) on the lints that are reported.

### [Lint reasons](#lint-reasons)
...
### [The `#[expect]` attribute](#the-expect-attribute)
...
> Note
> 
> `rustc` currently recognizes the tool lints for “[clippy](https://github.com/rust-lang/rust-clippy)” and “[rustdoc](https://doc.rust-lang.org/stable/rustdoc/lints.html)”.
...
- [MetaListNameValueStr](https://doc.rust-lang.org/stable/reference/attributes.html#grammar-MetaListNameValueStr) syntax with two optional fields:
...
The `deprecated` attribute may be applied to any [item](https://doc.rust-lang.org/stable/reference/items.html), [trait item](https://doc.rust-lang.org/stable/reference/items/traits.html), [enum variant](https://doc.rust-lang.org/stable/reference/items/enumerations.html), [struct field](https://doc.rust-lang.org/stable/reference/items/structs.html), [external block item](https://doc.rust-lang.org/stable/reference/items/external-blocks.html), or [macro definition](https://doc.rust-lang.org/stable/reference/macros-by-example.html). It cannot be applied to [trait implementation items](https://doc.rust-lang.org/stable/reference/items/implementations.html#trait-implementations). When applied to an item containing other items, such as a [module](https://doc.rust-lang.org/stable/reference/items/modules.html) or [implementation](https://doc.rust-lang.org/stable/reference/items/implementations.html), all child items inherit the deprecation attribute.
...
The `must_use` attribute can be applied to user-defined composite types ([`struct`s](https://doc.rust-lang.org/stable/reference/items/structs.html), [`enum`s](https://doc.rust-lang.org/stable/reference/items/enumerations.html), and [`union`s](https://doc.rust-lang.org/stable/reference/items/unions.html)), [functions](https://doc.rust-lang.org/stable/reference/items/functions.html), and [traits](https://doc.rust-lang.org/stable/reference/items/traits.html).

The `must_use` attribute may include a message by using the [MetaNameValueStr](https://doc.rust-lang.org/stable/reference/attributes.html#grammar-MetaNameValueStr) syntax such as `#[must_use = "example message"]`. The message will be given alongside the warning.

When used on user-defined composite types, if the [expression](https://doc.rust-lang.org/stable/reference/expressions.html) of an [expression statement](https://doc.rust-lang.org/stable/reference/statements.html#expression-statements) has that type, then the `unused_must_use` lint is violated.
...
When used on a function, if the [expression](https://doc.rust-lang.org/stable/reference/expressions.html) of an [expression statement](https://doc.rust-lang.org/stable/reference/statements.html#expression-statements) is a [call expression](https://doc.rust-lang.org/stable/reference/expressions/call-expr.html) to that function, then the `unused_must_use` lint is violated.
...
When used on a [trait declaration](https://doc.rust-lang.org/stable/reference/items/traits.html), a [call expression](https://doc.rust-lang.org/stable/reference/expressions/call-expr.html) of an [expression statement](https://doc.rust-lang.org/stable/reference/statements.html#expression-statements) to a function that returns an [impl trait](https://doc.rust-lang.org/stable/reference/types/impl-trait.html) or a [dyn trait](https://doc.rust-lang.org/stable/reference/types/trait-object.html) of that trait violates the `unused_must_use` lint.
...
> Note
> 
> Trivial no-op expressions containing the value will not violate the lint. Examples include wrapping the value in a type that does not implement [`Drop`](https://doc.rust-lang.org/stable/reference/special-types-and-traits.html#drop) and then not using that type and being the final expression of a [block expression](https://doc.rust-lang.org/stable/reference/expressions/block-expr.html) that is not used.
> 
> ```rust
> #![allow(unused)]
fn main() {
...
> Note
> 
> It is idiomatic to use a [let statement](https://doc.rust-lang.org/stable/reference/statements.html#let-statements) with a pattern of `_` when a must-used value is purposely discarded.
> 
> ```rust
> #![allow(unused)]
fn main() {
...
The attribute should be placed on a [trait declaration](https://doc.rust-lang.org/stable/reference/items/traits.html), though it is not an error to be located in other positions.

The attribute uses the [MetaListNameValueStr](https://doc.rust-lang.org/stable/reference/attributes.html#grammar-MetaListNameValueStr) syntax to specify its inputs, though any malformed input to the attribute is not considered as an error to provide both forwards and backwards compatibility.
...
All three options accept a string as an argument, interpreted using the same formatting as a [`std::fmt`](https://doc.rust-lang.org/stable/alloc/fmt/index.html) string.
...
### [The `diagnostic::do_not_recommend` attribute](#the-diagnosticdo_not_recommend-attribute)
...
The attribute should be placed on a [trait implementation item](https://doc.rust-lang.org/stable/reference/items/implementations.html#trait-implementations), though it is not an error to be located in other positions.
