---
title: Testing of integrated issues | Moodle Developer Resources
url: https://moodledev.io/general/development/process/testing/integrated-issues
source: sitemap
fetched_at: 2026-02-17T16:00:40.367802-03:00
rendered_js: false
word_count: 993
summary: This document outlines the procedures, roles, and technical requirements for testing Moodle code, covering the workflow from environment setup to issue resolution in the tracker.
tags:
    - moodle-testing
    - development-process
...
Testing is an important part of the [Moodle development process](https://moodledev.io/general/development/process). Depending on the integration period (normal or continuous) testing can take place at different times. Your first priority as a tester should be to finish testing on the day they have been assigned. You should update the testing status so that the testing status is clear.

## The testing process[​](#the-testing-process "Direct link to The testing process")
...
7. It's the responsibility of the tester to test the issue again, unless the patch is from tester. If the tester provides fix patch then test will be re-assigned. changed.
8. The tester who passes the issue will be set as the tester for the issue.

Failing a test session
...
## Expectation from developer and peer-reviewer[​](#expectation-from-developer-and-peer-reviewer "Direct link to Expectation from developer and peer-reviewer")

Testing instructions are clear, concise, complete, and accurate. Where possible they should be easy to perform. Please follow the [Testing instructions guide](https://moodledev.io/general/development/process/testing/guide) recommendations.

## Expectations of the Integration team[​](#expectations-of-the-integration-team "Direct link to Expectations of the Integration team")
...
## Expectation from tester[​](#expectation-from-tester "Direct link to Expectation from tester")
...
### Checking tests assigned to you[​](#checking-tests-assigned-to-you "Direct link to Checking tests assigned to you")

1. Log in to [Tracker](https://moodle.atlassian.net/).
2. Visit [Issues waiting to be tested](https://moodle.atlassian.net/issues/?filter=11801&jql=project%20%3D%20MDL%20AND%20Tester%20%3D%20currentUser%28%29%20AND%20status%20%3D%20%22Waiting%20for%20testing%22) page.

## Differences in test process during continuous integration periods[​](#differences-in-test-process-during-continuous-integration-periods "Direct link to Differences in test process during continuous integration periods")

During [continuous integration](https://moodledev.io/general/development/process/integration#during-continuous-integrationfreezeqa-period) the schedule is changed to allow faster iteration and for bug fixes to be applied more rapidly than the usual weekly cycle. The goal during this period is ... goal during this period is to release a new version of main multiple times per week. We try to keep the process more flexible during this time in order that developers who have less pressing issues than others can take the load off those concentrating on big fixes. It works best if we work together to help each other out.

warning

Priority is given to testing issues to ensure we can release regularly

## Installing a local test site from the integration repository[​](#installing-a-local-test-site-from-the-integration-repository "Direct link to Installing a local test site from the integration repository")

Moodle uses two Git repositories for its source code. Their names are moodle.git and integration.git and they live at [http://git.moodle.org](http://git.moodle.org). All submitted patches that were accepted during the integration review go to the integration.git first. Testers use integration.git as the source of the code to test. Patches that survive testing are then promoted to moodle.git and become the part of the official Moodle weekly build.
...
Alternatively, you can also use [Moodle Development Kit (MDK)](https://moodledev.io/general/development/tools/mdk), and add the option `--integration` to install a Moodle instance based on integration.git.

### Changing theme to another one[​](#changing-theme-to-another-one "Direct link to Changing theme to another one")
