---
title: Deep Crawling - Crawl4AI Documentation (v0.8.x)
url: https://docs.crawl4ai.com/core/deep-crawling/
source: sitemap
fetched_at: 2026-04-26T07:47:04.866680943-03:00
rendered_js: false
word_count: 1551
summary: This tutorial explains how to implement deep crawling in Crawl4AI, covering traversal strategies like BFS, DFS, and BestFirst, as well as methods for filtering content and streaming results.
tags:
    - web-scraping
    - deep-crawling
    - automation
...
```
fromcrawl4ai.deep_crawlingimport DFSDeepCrawlStrategy
...
```
fromcrawl4ai.deep_crawlingimport BestFirstCrawlingStrategy
fromcrawl4ai.deep_crawling.scorersimport KeywordRelevanceScorer
...
```
fromcrawl4ai.deep_crawling.filtersimport FilterChain, URLPatternFilter
...
```
fromcrawl4ai.deep_crawling.filtersimport (
    FilterChain,
    URLPatternFilter,
    DomainFilter,
    ContentTypeFilter
...
```
fromcrawl4ai.deep_crawling.filtersimport FilterChain, SEOFilter
...
```
fromcrawl4ai.deep_crawling.filtersimport FilterChain, ContentRelevanceFilter
...
async defrun_advanced_crawler():
    ...
    print(f"Average score: {sum(r.metadata.get('score',0)forrinresults)/len(results):.2f}")
...
async defsave_state_to_redis(state: dict):
...
async defcapture_state(state: dict):
...
async defmain():
    ...
    async defpersist_state(state: dict):
...
```
async defhandle_state(state: dict):
...
Scenario Behavior Cancel before first URL Returns empty results, `cancelled=True` Cancel during crawl Completes current URL, then stops Callback raises exception Logged as warning, crawl continues (fail-open) Strategy reuse after cancel Works normally (cancel flag auto-resets) Sync callback function Supported (auto-detected and handled)
...
Processing Step Normal Mode Prefetch Mode Fetch HTML ✅ ✅ Extract links ✅ ✅ (fast `quick_extract_links()`) Generate markdown ✅ ❌ Skipped Content scraping ✅ ❌ Skipped Media extraction ✅ ❌ Skipped LLM extraction ✅ ❌ Skipped
...
async deftwo_phase_crawl(start_url: str):
    async with AsyncWebCrawler() as crawler:
        # ═══════════════════════════════════════════════
