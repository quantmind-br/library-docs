...
url: https://hermes-agent.nousresearch.com/docs/developer-guide/adding-platform-adapters
source: crawler
fetched_at: 2026-04-24T17:00:36.750604072-03:00
rendered_js: false
word_count: 635
summary: This guide provides a comprehensive walkthrough detailing the necessary steps and architectural considerations for integrating a new messaging platform (like Telegram or Discord) into the Hermes gateway system.
tags:
    - messaging-platform
    - gateway-integration
    - adapter-pattern
...
## Step-by-Step Checklist[​](#step-by-step-checklist "Direct link to Step-by-Step Checklist")
...
defcheck_newplat_requirements()->bool:
"""Return True if dependencies are available."""
return SOME_SDK_AVAILABLE

classNewPlatAdapter(BasePlatformAdapter):
def__init__(self, config: PlatformConfig):
...
asyncdefsend(self, chat_id, content, reply_to=None, metadata=None):
# Send message via platform API
return SendResult(success=True, message_id="...")

asyncdefget_chat_info(self, chat_id):
...
FileWhat to add`website/docs/user-guide/messaging/newplat.md`Full platform setup page`website/docs/user-guide/messaging/index.md`Platform comparison table, architecture diagram, toolsets table, security section, next-steps link`website/docs/reference/environment-variables.md`All NEWPLAT\_* env vars`website/docs/reference/toolsets-reference.md`hermes-newplat toolset`website/docs/integrations/index.md`Platform link`website/sidebars.ts`Sidebar entry for the docs page`website/docs/developer-guide/architecture.md`Adapter count + listing`website/docs/developer-guide/gateway-internals.md`Adapter file listing
...
asyncdef_handle_callback(self, request):
...
asyncdefconnect(self):
ifnot acquire_scoped_lock("newplat", self._token):
...
AdapterPatternComplexityGood reference for`bluebubbles.py`REST + webhookMediumSimple REST API integration`weixin.py`Long-poll + CDNHighMedia handling, encryption`wecom_callback.py`Callback/webhookMediumHTTP server, AES crypto, multi-app`telegram.py`Long-poll + Bot APIHighFull-featured adapter with groups, threads