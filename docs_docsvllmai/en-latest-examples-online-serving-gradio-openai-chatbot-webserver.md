...
source: sitemap
fetched_at: 2026-05-07T21:13:17.013497779-03:00
rendered_js: false
word_count: 101
summary: This document provides a Python script demonstrating how to deploy a Gradio-based web interface that interacts with a vLLM-hosted model using the OpenAI-compatible API.
tags:
    - vllm
    - gradio
    - openai-api
...
defpredict(message, history, client, model_name, temp, stop_token_ids):
    messages = [
...
defbuild_gradio_interface(client, model_name, temp, stop_token_ids):
    defchat_predict(message, history):
