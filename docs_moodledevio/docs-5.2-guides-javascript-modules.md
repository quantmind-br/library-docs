---
title: Modules | Moodle Developer Resources
url: https://moodledev.io/docs/5.2/guides/javascript/modules
source: sitemap
fetched_at: 2026-02-17T15:47:00.135675-03:00
rendered_js: false
word_count: 1314
summary: This document explains how to create and manage JavaScript modules in Moodle, detailing the transition from AMD to ESM and providing instructions on development workflows, transpilation with Grunt, and integration with PHP.
tags:
    - moodle
    - javascript-modules
    - esm
...
Since Moodle 3.10, any new JavaScript code written for Moodle core **must** be written in the [ESM](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules) format (commonly referred to as ES6). See [MDLSITE-6130](https://moodle.atlassian.net/browse/MDLSITE-6130) for further information.
...
For information on the development toolchain, see our [documentation on how to install and use NodeJS and Grunt](https://moodledev.io/general/development/tools/nodejs#grunt).
...
### Transpiling Modules[​](#transpiling-modules "Direct link to Transpiling Modules")

Since all JavaScript must now be transpiled you must use the [Grunt](https://moodledev.io/general/development/tools/nodejs#grunt) in order for you changes to appear in the browser.
...
## ES Modules[​](#es-modules "Direct link to ES Modules")
...
Call the 'init' function on myplugin/myfile
...
mod/myplugin/amd/src/myfile.js
...
### Export default[​](#export-default "Direct link to Export default")
...
If both are used, then the *default* export will override all *named* exports.

### Inline JavaScript[​](#inline-javascript "Direct link to Inline JavaScript")
...
## First module for your plugin[​](#first-module-for-your-plugin "Direct link to First module for your plugin")

This shows the absolute minimum module you need to get started adding modules to your plugins.

path/to/plugin/amd/src/somefile.js
...
You must use the complete '[Frankenstyle](https://moodledev.io/general/development/policies/codingstyle/frankenstyle)' plugin name, but you do not need to supply the `.js` extension.
...
Initialising a Module from PHP with argument
...
Consuming these variables in the init function

```
exportconst init =(first, last){
window.console.log(`The first name was '${first}' and the last name was '${last}'`);
};
```

## "Hello World" I am a JavaScript Module[​](#hello-world-i-am-a-javascript-module 'Direct link to "Hello World" I am a JavaScript Module')
 ...
 */
consthideAllNotes=()=>document.querySelectorAll('.note').map(note=> note.addClass('hidden'));
 ...
 * @returns{Promise}
 */
exportconstformal=(name)=>Str.get_string('formallygreet','block_overview', name);
 ...
 * @returns{Promise}
 */
exportconstinformal=(name)=>{
returnStr.get_string('informallygreet','block_overview', name);
};
```
...
### Loading modules dynamically[​](#loading-modules-dynamically "Direct link to Loading modules dynamically")
...
```
exportconstshowTheThing=(thingToShow)=>{
...
## Calling modules from a page[​](#calling-modules-from-a-page "Direct link to Calling modules from a page")

After you have created your JavaScript modules, then next area to consider is how you should call them.

Any JavaScript code that calls a JavaScript module must execute *after* the requirejs module loader has finished loading. Moodle provides a function `js_call_amd` that will call a single function from the module with the specified parameters.
...
- if the size of the params array is too large (&gt; 1Kb), this will produce a developer warning. Do not attempt to pass large amounts of data through this function, it will pollute the page size. A preferred approach is to pass css selectors for DOM elements that contain data-attributes for any required data, or fetch data via ajax in the background. AMD / JS code can also be embedded on a page via mustache templates see here: [https://docs.moodle.org/dev/Templates#What\_if\_a\_template\_contains\_JavaScript.3F](https://docs.moodle.org/dev/Templates#What_if_a_template_contains_JavaScript.3F)

## Troubleshooting[​](#troubleshooting "Direct link to Troubleshooting")

### npm-shrinkwrap.json sha1 / sha512 changes[​](#npm-shrinkwrapjson-sha1--sha512-changes "Direct link to npm-shrinkwrap.json sha1 / sha512 changes")
...
## But I have a mega JS file I don't want loaded on every page?[​](#but-i-have-a-mega-js-file-i-dont-want-loaded-on-every-page "Direct link to But I have a mega JS file I don't want loaded on every page?")
