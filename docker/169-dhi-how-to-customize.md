---
title: Customize an image or chart
url: https://docs.docker.com/dhi/how-to/customize/
source: llms
fetched_at: 2026-01-24T14:20:31.873571948-03:00
rendered_js: false
word_count: 1149
summary: This document provides instructions for customizing Docker Hardened Images (DHI) and charts using the Docker Hub interface, covering how to add packages, OCI artifacts, and configure build settings for automated security compliance.
tags:
    - dhi
    - docker-hub
    - image-customization
...
## Customize a Docker Hardened Image or chart DHI Enterprise

Subscription: Docker Hardened Images Enterprise

When you have a Docker Hardened Images subscription, you can customize Docker Hardened Images (DHI) and charts to suit your specific needs using the Docker Hub web interface. For images, this lets you select a base image, add packages, add OCI artifacts (such as custom certificates or additional tools), and configure settings. For charts, this lets you customize the image references.
...
### [Create an image customization](#create-an-image-customization)

To customize a Docker Hardened Image, follow these steps:

01. Sign in to [Docker Hub](https://hub.docker.com).
02. Select **My Hub**.
03. In the namespace drop-down, select your organization that has a mirrored DHI repository.
04. Select **Hardened Images** &gt; **Manage** &gt; **Mirrored Images**.
05. For the mirrored DHI repository you want to customize, select the menu icon in the far right column.
06. Select **Customize**.
...
07. Select the image version you want to customize.
08. Optional. Add packages.
...
09. Select **Next: Configure** to configure the following image settings:
    
    01. Specify the [environment variables](https://docs.docker.com/reference/dockerfile/#env) and their values that the image will contain.
    02. Add [labels](https://docs.docker.com/reference/dockerfile/#label) to the image.
    03. Add [annotations](https://docs.docker.com/build/metadata/annotations/) to the image.
    04. Specify the users to add to the image.
    05. Specify the user groups to add to the image.
    06. Select which [user](https://docs.docker.com/reference/dockerfile/#user) to run the images as.
    07. Add [`ENTRYPOINT`](https://docs.docker.com/reference/dockerfile/#entrypoint) arguments to the image. These arguments are appended to the base image's entrypoint.
    08. Add [`CMD`](https://docs.docker.com/reference/dockerfile/#cmd) arguments to the image. These arguments are appended to the base image's command.
    09. Override the default (`/`) [working directory](https://docs.docker.com/reference/dockerfile/#workdir) for the image.
