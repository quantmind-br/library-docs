...
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/models/mimo_v2_omni/
source: sitemap
fetched_at: 2026-05-07T21:31:43.954312941-03:00
rendered_js: false
word_count: 384
summary: This document defines the MiMoV2OmniForCausalLM class within the vLLM framework, providing the implementation for a multi-modal model that supports image, video, and audio inputs for causal language modeling.
tags:
    - vllm
    - multimodal
    - causal-lm
...
## MiMoV2OmniForCausalLM [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoV2OmniForCausalLM "Permanent link")
...
classMiMoV2OmniForCausalLM(nn.Module, SupportsMultiModal, SupportsPP, SupportsQuant):
    ...
    def_parse_and_validate_audio_input(self, **kwargs: object) -> dict | None:
    ...
    def_process_audio_input(self, audio_input: dict) -> tuple[torch.Tensor, ...]:
...
### forward [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoV2OmniForCausalLM.forward "Permanent link")
...
Flattened (concatenated) position ids corresponding to a batch. **NOTE**: If mrope is enabled (default setting for Qwen2.5-VL opensource models), the shape will be `(3, seq_len)`, otherwise it will be \`(seq\_len,).
...
## MiMoV2OmniMultiModalProcessor [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoV2OmniMultiModalProcessor "Permanent link")

Bases: `BaseMultiModalProcessor[MiMoV2OmniProcessingInfo]`

vLLM multimodal processor for MiMo-Omni (image + video).

Key differences from Qwen2.5-VL: - Videos use timestamp tokens between temporal grid positions. - The HF processor expects `(TCHW_tensor, timestamps_T_tensor)` video tuples rather than plain numpy arrays. - `video_start_times` is tracked so prompt-update reconstruction can regenerate the exact same timestamp token IDs.

Source code in `vllm/model_executor/models/mimo_v2_omni.py`

```
classMiMoV2OmniMultiModalProcessor(BaseMultiModalProcessor[MiMoV2OmniProcessingInfo]):
"""vLLM multimodal processor for MiMo-Omni (image + video).
...
"""Convert numpy video arrays to (TCHW, timestamps) tuples for MiMo.
    ...
    def_get_prompt_updates(
        ...
        defget_image_replacement(item_idx: int) -> PromptUpdateDetails:
            out_item = out_mm_kwargs["image"][item_idx]
            grid_thw = out_item["image_grid_thw"].data
            n_tokens = int(grid_thw.prod()) // merge_size**2
            return [image_pad_id] * n_tokens

        defget_video_replacement(item_idx: int) -> PromptUpdateDetails:
        ...
        defget_audio_replacement(item_idx: int) -> PromptUpdateDetails:
...
### \_call\_hf\_processor [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoV2OmniMultiModalProcessor._call_hf_processor "Permanent link")

Convert numpy video arrays to (TCHW, timestamps) tuples for MiMo. Also remap 'audios' → 'audio' since MiMoOmniProcessor.**call** uses the singular form.
...
## MiMoVisionAttention [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoVisionAttention "Permanent link")
...
```
classMiMoVisionAttention(nn.Module):
    def__init__(
        self,
        embed_dim: int,
        num_heads: int,
...
"""Window attention via flash_attn_varlen_func with window_size."""
        fromvllm.vllm_flash_attnimport flash_attn_varlen_func
...
### \_forward\_window\_attn [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoVisionAttention._forward_window_attn "Permanent link")

Window attention via flash\_attn\_varlen\_func with window\_size.
...
### forward [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoVisionAttention.forward "Permanent link")

Parameters:

Name Type Description Default `x` `Tensor`

\[seq\_len, batch=1, embed\_dim] (seq-first convention)

*required* `cu_seqlens` `Tensor`

cumulative sequence lengths \[num\_seqs+1], int32

*required* `rotary_pos_emb_cos` `Tensor`

\[seq\_len, qk\_channels // 2]

*required* `rotary_pos_emb_sin` `Tensor`

\[seq\_len, qk\_channels // 2]

*required* `max_seqlen` `Tensor`

maximum sequence length

*required* `full_attn` `bool`

if True, full attention; if False, window attention
...
## MiMoVisionTransformer [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoVisionTransformer "Permanent link")
...
```
classMiMoVisionTransformer(nn.Module):
    def__init__(
        self,
    ...
    defapply_index(self, tensor: torch.Tensor, index: torch.Tensor) -> torch.Tensor:
...
"""Compute 1D window indices for col-based or row-based SWA reordering."""
    ...
    defrot_pos_emb(self, grid_thw: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
"""Compute 2D rotary position embedding cos/sin for given grid sizes.
    ...
    defforward(self, x: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
"""
        Args:
...
### apply\_index [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoVisionTransformer.apply_index "Permanent link")

Reindex tensor at the spatial\_merge\_unit granularity.
...
### forward [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoVisionTransformer.forward "Permanent link")

Parameters:

Name Type Description Default `x` `Tensor`

\[total\_tokens, C] pre-flattened patches

*required* `grid_thw` `Tensor`

\[num\_images, 3] tensor of (t, h, w) for each image/video

*required*

Returns: \[merged\_tokens, out\_hidden\_size]
...
### get\_window\_index\_1d [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoVisionTransformer.get_window_index_1d "Permanent link")
...
### rot\_pos\_emb [¶](#vllm.model_executor.models.mimo_v2_omni.MiMoVisionTransformer.rot_pos_emb "Permanent link")

Compute 2D rotary position embedding cos/sin for given grid sizes.

Returns:

Name Type Description `cos` `Tensor`

\[total\_tokens, qk\_channels // 2]
