---
title: CodeAct - DSPy
url: https://dspy.ai/api/modules/CodeAct/
source: sitemap
fetched_at: 2026-01-23T08:01:47.82408454-03:00
rendered_js: false
word_count: 1080
summary: This document provides the technical specification for the CodeAct module, a component that enables iterative task-solving by combining a code interpreter with external function calls.
tags:
    - dspy
    - codeact
    - code-interpreter
...
[](https://github.com/stanfordnlp/dspy/blob/main/docs/docs/api/modules/CodeAct.md "Edit this page")

## `dspy.CodeAct(signature: str | type[Signature], tools: list[Callable], max_iters: int = 5, interpreter: PythonInterpreter | None = None)` [¶](#dspy.CodeAct "Permanent link")
...
Name Type Description Default `signature` `Union[str, Type[Signature]]`

The signature of the module.

*required* `tools` `list[Callable]`

The tool callables to be used. CodeAct only accepts functions and not callable objects.

*required* `max_iters` `int`

The maximum number of iterations to generate the answer.

`5` `interpreter` `PythonInterpreter | None`

PythonInterpreter instance to use. If None, a new one is instantiated.
...
```
fromdspy.predictimport CodeAct
deffactorial(n):
    if n == 1:
        return 1
...
````
def__init__(self, signature: str | type[Signature], tools: list[Callable], max_iters: int = 5, interpreter: PythonInterpreter | None = None):
"""
    Initializes the CodeAct class with the specified model, temperature, and max tokens.
...
### Functions[¶](#dspy.CodeAct-functions "Permanent link")

#### `__call__(*args, **kwargs) -> Prediction` [¶](#dspy.CodeAct.__call__ "Permanent link")
...
#### `deepcopy()` [¶](#dspy.CodeAct.deepcopy "Permanent link")
...
#### `dump_state(json_mode=True)` [¶](#dspy.CodeAct.dump_state "Permanent link")
...
#### `get_lm()` [¶](#dspy.CodeAct.get_lm "Permanent link")
...
#### `inspect_history(n: int = 1)` [¶](#dspy.CodeAct.inspect_history "Permanent link")
...
#### `load(path, allow_pickle=False)` [¶](#dspy.CodeAct.load "Permanent link")
...
#### `load_state(state)` [¶](#dspy.CodeAct.load_state "Permanent link")
...
#### `map_named_predictors(func)` [¶](#dspy.CodeAct.map_named_predictors "Permanent link")
...
#### `named_parameters()` [¶](#dspy.CodeAct.named_parameters "Permanent link")
...
#### `named_predictors()` [¶](#dspy.CodeAct.named_predictors "Permanent link")
...
#### `named_sub_modules(type_=None, skip_compiled=False) -> Generator[tuple[str, BaseModule], None, None]` [¶](#dspy.CodeAct.named_sub_modules "Permanent link")
...
#### `parameters()` [¶](#dspy.CodeAct.parameters "Permanent link")
...
#### `predictors()` [¶](#dspy.CodeAct.predictors "Permanent link")
...
#### `reset_copy()` [¶](#dspy.CodeAct.reset_copy "Permanent link")
...
#### `save(path, save_program=False, modules_to_serialize=None)` [¶](#dspy.CodeAct.save "Permanent link")
...
#### `set_lm(lm)` [¶](#dspy.CodeAct.set_lm "Permanent link")
...
## CodeAct[¶](#codeact "Permanent link")
...
## ⚠️ Limitations[¶](#limitations "Permanent link")

### Only accepts pure functions as tools (no callable objects)[¶](#only-accepts-pure-functions-as-tools-no-callable-objects "Permanent link")
...
# ❌ NG
classAdd():
    def__call__(self, a: int, b: int):
        return a + b

dspy.CodeAct("question -> answer", tools=[Add()])
```

### External libraries cannot be used[¶](#external-libraries-cannot-be-used "Permanent link")
...
### All dependent functions need to be passed to `CodeAct`[¶](#all-dependent-functions-need-to-be-passed-to-codeact "Permanent link")
