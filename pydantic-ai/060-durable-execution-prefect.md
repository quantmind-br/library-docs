...
source: sitemap
fetched_at: 2026-01-22T22:25:13.673734073-03:00
rendered_js: false
word_count: 1272
summary: This document explains how to integrate Prefect with Pydantic AI to enable durable execution, providing transactional semantics, automatic retries, and task caching for agentic workflows.
tags:
    - prefect
    - durable-execution
...
- Wraps [`Agent.run`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AbstractAgent.run "run            async   ") and [`Agent.run_sync`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AbstractAgent.run_sync "run_sync") as Prefect flows.
...
```
pipinstallpydantic-ai[prefect]
```

```
uvaddpydantic-ai[prefect]
```
...
```
pipinstallpydantic-ai-slim[prefect]
```

```
uvaddpydantic-ai-slim[prefect]
```
...
3. [`PrefectAgent.run()`](https://ai.pydantic.dev/api/durable_exec/#pydantic_ai.durable_exec.prefect.PrefectAgent.run "run            async   ") works like [`Agent.run()`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AbstractAgent.run "run            async   "), but runs as a Prefect flow and executes model requests, decorated tool calls, and MCP communication as Prefect tasks.
...
The event stream handler function will receive the agent [run context](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") and an async iterable of events from the model's streaming response and the agent's execution of tools. For examples, see the [streaming docs](https://ai.pydantic.dev/agents/#streaming-all-events).
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) serve\_agent.py
