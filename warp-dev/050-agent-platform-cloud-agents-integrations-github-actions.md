---
title: GitHub Actions | Agents | Warp
url: https://docs.warp.dev/agent-platform/cloud-agents/integrations/github-actions
source: sitemap
fetched_at: 2026-04-29T15:04:29.297524968-03:00
rendered_js: false
optimized: true
optimized_at: 2026-04-29T00:00:00Z
word_count: 1165
summary: This document provides a guide for integrating Oz agents into GitHub Actions to automate CI tasks like code reviews, bug fixes, and issue triaging using specialized skills and repository context.
tags:
    - github-actions
    - ci-cd
    - automation
...
category: guide
---
Run Oz agents directly in your GitHub Actions workflows using `oz-agent-action`. The agent integrates seamlessly into your CI pipeline, automating tasks like code review, issue triage, bug fixing, and maintenance using your repository context and GitHub permissions.
...
The `oz-agent-action` wraps the Oz CLI inside a GitHub Action and:
...
- Passes workflow context, event data, and previous step outputs into the agent prompt
...
To use Oz agents in GitHub Actions:

- Workflow permissions that match your intended actions (e.g., `pull-requests: write` if the agent should commit or comment on PRs)
- The `oz-agent-action` step added to your workflow

## Using Skills

Skills provide reusable instructions for Oz agents. Use pre-built skills from the [oz-skills repository](https://github.com/warpdotdev/oz-skills) or create custom [[160-reference-cli-skills|skills]] for your specific workflows. Skills can also be deployed as [[061-agent-platform-cloud-agents-skills-as-agents|standalone agents]] to run on a schedule or in response to events.

### How to use skills

Specify a skill using the `skill` input parameter, either instead of or in combination with prompts.
...
Combine skills with prompts to provide specialized context while customizing the specific task. In this example, the `code-review` skill provides the base context and approach for code review, while the prompt narrows the focus to security concerns in authentication code.
...
**When to use:** Interactive coding assistance during review or issue triage.
...
>
> This is a GitHub platform limitation for private organizations — any user must be an org member to be mentioned in comments on private repos. Public repositories are not affected by this requirement.
...
**When to use:** Fast initial review before human reviewers step in.
...
**When to use:** Automating bug fixes, small features, or maintenance tasks.
...
**When to use:** Daily visibility into new work across your repositories.
...
**When to use:** Reducing downtime from failing builds or flaky tests.
...
**When to use:** Quickly addressing straightforward review feedback such as typos, naming tweaks, style nits, and small refactors.
...
> [!note]
> Even if `@oz-agent` doesn't autocomplete, you can still type the mention manually — but the workflow will only trigger if the user is an org member (for private repos).