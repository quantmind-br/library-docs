---
title: flashinfer_mxint4_moe - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/quantization/utils/flashinfer_mxint4_moe/
source: sitemap
fetched_at: 2026-05-07T21:27:51.726615171-03:00
rendered_js: false
word_count: 279
summary: This document provides utility functions for integrating FlashInfer MxInt4 kernels into MoE layers, including execution of the fused MoE kernel, weight preparation, and feature availability checks.
tags:
    - moe
    - flashinfer
    - quantization
...
## flashinfer\_trtllm\_mxint4\_moe [¶](#vllm.model_executor.layers.quantization.utils.flashinfer_mxint4_moe.flashinfer_trtllm_mxint4_moe "Permanent link")
...
Input hidden states. dtype: bfloat16

*required* `router_logits` `Tensor`

Router logits for expert selection. dtype: bfloat16/float32

*required* `w13_weight_packed` `Tensor`

Packed gate+up weights. dtype: uint8

*required* `w13_weight_scale` `Tensor`

Scales for gate+up weights. dtype: bfloat16

*required* `w2_weight_packed` `Tensor`

Packed down weights. dtype: uint8

*required* `w2_weight_scale` `Tensor`

Scales for down weights. dtype: bfloat16

*required* `global_num_experts` `int`
...
*required* `intermediate_size_per_partition` `int`

Intermediate size per partition

*required* `local_num_experts` `int`

Number of experts on this rank

*required* `ep_rank` `int`

Expert parallelism rank (default: 0)

`0` `num_expert_group` `int | None`

Number of expert groups (default: None -&gt; 0)

`None` `topk_group` `int | None`

Top-k within groups (default: None -&gt; 0)

`None` `e_score_correction_bias` `Tensor | None`

Optional routing bias. dtype: bfloat16

`None` `routing_method_type` `int | None`
...
```
defflashinfer_trtllm_mxint4_moe(
    x: torch.Tensor,
...
"""
    ...
    Returns:
        Output tensor from MoE layer. dtype: same as x (bfloat16)
    """
    fromflashinferimport RoutingMethodType
    fromflashinfer.fused_moeimport trtllm_mxint4_block_scale_moe
...
## is\_flashinfer\_mxint4\_moe\_available `cached` [¶](#vllm.model_executor.layers.quantization.utils.flashinfer_mxint4_moe.is_flashinfer_mxint4_moe_available "Permanent link")
...
```
@functools.cache
defis_flashinfer_mxint4_moe_available() -> bool:
"""Return `True` when FlashInfer MxInt4 kernels can be used."""
    return (
...
## prepare\_static\_weights\_for\_trtllm\_mxint4\_moe [¶](#vllm.model_executor.layers.quantization.utils.flashinfer_mxint4_moe.prepare_static_weights_for_trtllm_mxint4_moe "Permanent link")

Prepare MxInt4 weights for TRT-LLM kernel.

Input

gemm1\_weights: \[num\_experts, 2*intermediate\_size, hidden\_size//8] int32 (checkpoint uint4b8 packed) or uint8 (already packed signed int4) gemm1\_scales: \[num\_experts, 2*intermediate\_size, hidden\_size//32] bf16 gemm2\_weights: \[num\_experts, hidden\_size, intermediate\_size//8] int32 (checkpoint uint4b8 packed) or uint8 (already packed signed int4) gemm2\_scales: \[num\_experts, hidden\_size, intermediate\_size//32] bf16

Returns:

Type Description `dict[str, Tensor]`

Dict with keys 'gemm1\_weights', 'gemm1\_scales', 'gemm2\_weights', 'gemm2\_scales' containing shuffled/packed tensors ready for kernel

Source code in `vllm/model_executor/layers/quantization/utils/flashinfer_mxint4_moe.py`

```
defprepare_static_weights_for_trtllm_mxint4_moe(
...
"""
    ...
    """
    fromflashinferimport block_scale_interleave
    fromflashinfer.fused_moeimport (
    ...
    fromvllm.model_executor.layers.quantization.utils.flashinfer_fp4_moeimport (
        reorder_w1w3_to_w3w1,
    )
    fromvllm.model_executor.layers.quantization.utils.quant_utilsimport (
