...
source: sitemap
fetched_at: 2026-01-22T22:25:11.755661095-03:00
rendered_js: false
word_count: 937
summary: This guide explains how to integrate Pydantic AI agents with DBOS to provide durable execution and state checkpointing for long-running workflows. It covers wrapping agents with DBOSAgent to ensure automatic recovery from failures using database-backed persistence.
tags:
    - dbos
    - pydantic-ai
    - durable-execution
...
```
pipinstallpydantic-ai-slim[dbos]
```

```
uvaddpydantic-ai-slim[dbos]
```
...
2. [`DBOSAgent.run()`](https://ai.pydantic.dev/api/durable_exec/#pydantic_ai.durable_exec.dbos.DBOSAgent.run "run            async   ") works like [`Agent.run()`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AbstractAgent.run "run            async   "), but runs as a DBOS workflow and executes model requests, decorated tool calls, and MCP communication as DBOS steps.
