---
title: offload - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/config/offload/
source: sitemap
fetched_at: 2026-05-07T21:17:07.219985857-03:00
rendered_js: false
word_count: 585
summary: Describes the OffloadConfig class in vLLM, which manages model weight offloading configurations to optimize GPU memory usage via UVA or prefetch backends.
tags:
    - vllm
    - memory-optimization
    - gpu-offloading
...
## OffloadConfig [¶](#vllm.config.offload.OffloadConfig "Permanent link")
...
"""Parameters for prefetch offloading backend."""
    ...
    defvalidate_offload_config(self) -> "OffloadConfig":
...
### offload\_backend `class-attribute` `instance-attribute` [¶](#vllm.config.offload.OffloadConfig.offload_backend "Permanent link")
...
### prefetch `class-attribute` `instance-attribute` [¶](#vllm.config.offload.OffloadConfig.prefetch "Permanent link")
...
### uva `class-attribute` `instance-attribute` [¶](#vllm.config.offload.OffloadConfig.uva "Permanent link")
...
### compute\_hash [¶](#vllm.config.offload.OffloadConfig.compute_hash "Permanent link")

Provide a hash that uniquely identifies all the offload configs.

All fields are included because PrefetchOffloader patches module forwards and inserts custom ops (wait\_prefetch, start\_prefetch) into the computation graph. Changing any offload setting can alter which layers are hooked and how prefetch indices are computed, so the compilation cache must distinguish them.
...
### validate\_offload\_config [¶](#vllm.config.offload.OffloadConfig.validate_offload_config "Permanent link")
...
## PrefetchOffloadConfig [¶](#vllm.config.offload.PrefetchOffloadConfig "Permanent link")
...
### offload\_group\_size `class-attribute` `instance-attribute` [¶](#vllm.config.offload.PrefetchOffloadConfig.offload_group_size "Permanent link")
...
### offload\_num\_in\_group `class-attribute` `instance-attribute` [¶](#vllm.config.offload.PrefetchOffloadConfig.offload_num_in_group "Permanent link")
...
### offload\_params `class-attribute` `instance-attribute` [¶](#vllm.config.offload.PrefetchOffloadConfig.offload_params "Permanent link")
...
### offload\_prefetch\_step `class-attribute` `instance-attribute` [¶](#vllm.config.offload.PrefetchOffloadConfig.offload_prefetch_step "Permanent link")
...
## UVAOffloadConfig [¶](#vllm.config.offload.UVAOffloadConfig "Permanent link")
...
### cpu\_offload\_gb `class-attribute` `instance-attribute` [¶](#vllm.config.offload.UVAOffloadConfig.cpu_offload_gb "Permanent link")
...
### cpu\_offload\_params `class-attribute` `instance-attribute` [¶](#vllm.config.offload.UVAOffloadConfig.cpu_offload_params "Permanent link")
