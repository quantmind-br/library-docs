---
title: Crawl4AI v0.5.0 Release Notes - Crawl4AI Documentation (v0.8.x)
url: https://docs.crawl4ai.com/blog/releases/0.5.0/
source: sitemap
fetched_at: 2026-04-26T07:46:20.660987515-03:00
rendered_js: false
word_count: 1337
summary: This document outlines the v0.5.0 release of Crawl4AI, introducing significant updates including deep crawling strategies, a memory-adaptive dispatcher, multiple crawling modes, Docker support, and a new command-line interface.
tags:
    - web-scraping
    - crawling
    - automation
...
```
importtime
fromcrawl4aiimport AsyncWebCrawler, CrawlerRunConfig, BFSDeepCrawlStrategy
fromcrawl4ai.content_scraping_strategyimport LXMLWebScrapingStrategy
fromcrawl4ai.deep_crawlingimport DomainFilter, ContentTypeFilter, FilterChain, URLPatternFilter, KeywordRelevanceScorer, BestFirstCrawlingStrategy
importasyncio
...
```
fromcrawl4aiimport AsyncWebCrawler, CrawlerRunConfig, MemoryAdaptiveDispatcher
importasyncio
...
```
fromcrawl4aiimport AsyncWebCrawler, CrawlerRunConfig, HTTPCrawlerConfig
fromcrawl4ai.async_crawler_strategyimport AsyncHTTPCrawlerStrategy
importasyncio
...
dockerbuild-tcrawl4ai.

# Run the container
dockerrun-d-p8000:8000--namecrawl4aicrawl4ai
```
...
See the [Docker deployment documentation](https://docs.crawl4ai.com/blog/core/docker-deployment.md) for detailed instructions.
...
crwlhttps://example.com-Bbrowser.yml-Ccrawler.yml

# Use LLM-based extraction
crwlhttps://example.com-eextract.yml-sschema.json

# Ask a question about the crawled content
crwlhttps://example.com-q"What is the main topic?"
...
importasyncio
fromcrawl4aiimport ProxyConfig
async defmain():
...
```
fromcrawl4aiimport AsyncWebCrawler, CrawlerRunConfig, DefaultMarkdownGenerator
...
fromcrawl4ai.processors.pdfimport PDFCrawlerStrategy, PDFContentScrapingStrategy
importasyncio
...
01. **Update Imports:** Adjust imports for `DeepCrawlStrategy`, `BreadthFirstSearchStrategy`, and related classes due to the new `deep_crawling` module structure.
02. **`CrawlerRunConfig`:** Move `max_depth` to `CrawlerRunConfig`. If using `content_filter`, migrate to an extraction strategy or a markdown generator with a filter.
03. **`arun_many()`:** Adapt code to the new `MemoryAdaptiveDispatcher` behavior and the return type.
04. **`BrowserContext`:** Update code using the `BrowserContext` API.
05. **Models:** Handle potential `None` values for optional fields in data models.
06. **Scraping:** Replace `ScrapingMode` enum with `WebScrapingStrategy` or `LXMLWebScrapingStrategy`.
07. **Docker:** Review the updated Docker documentation and adjust your deployment accordingly.
08. **CLI:** Migrate to the new `crwl` command and update any scripts using the old CLI.
09. **Proxy:**: Removed synchronous WebCrawler support and related rate limiting configurations.
