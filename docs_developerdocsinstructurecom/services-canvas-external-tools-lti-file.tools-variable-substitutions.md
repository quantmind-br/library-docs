---
title: Variable Substitutions | Instructure Developer Documentation Portal
url: https://developerdocs.instructure.com/services/canvas/external-tools/lti/file.tools_variable_substitutions
source: sitemap
fetched_at: 2026-02-15T08:57:01.483803-03:00
rendered_js: false
word_count: 5237
summary: This document explains how to use and configure LTI variable substitutions to pass contextual data from Canvas to external tools during launch requests.
tags:
    - lti
    - canvas-lms
...
Some substitutions may be used as 'enabled\_capabilities' for LTI2 tools. These substitutions have a 'Launch Parameter' label indicating the parameter name that will be sent in the tool launch if enabled.

For more information on variable substitution, see the [IMS LTI specificationarrow-up-right](https://www.imsglobal.org/specs/ltiv1p1p1/implementation-guide#toc-9).

Variable substitutions can be configured for a tool in 3 ways:

Custom fields can be [configured via UIarrow-up-right](https://community.canvaslms.com/t5/Admin-Guide/How-do-I-configure-a-manual-entry-external-app-for-an-account/ta-p/219) by editing the tool configuration and adding the desired variable to the Custom Fields text box.

The following would add the domain as a launch parameter called custom\_arbitrary\_name:

```
arbitrary_name=$Canvas.api.domain
```

Custom fields can also be [configured via API](https://developerdocs.instructure.com/services/canvas/resources/external_tools#method.external_tools.create).
...
JSON can be used to [configure an LTI 1.3 Developer Keyarrow-up-right](https://community.canvaslms.com/t5/Admin-Guide/How-do-I-configure-an-LTI-key-for-an-account/ta-p/140).
...
Custom fields can also be [configured via XML](https://developerdocs.instructure.com/services/canvas/external-tools/lti/file.tools_xml).
...
LTI - Custom parameter substitution: ResourceLink.id Returns the LTI value for the resource\_link.id property Returns "$ResourceLink.id" otherwise.

**Availability**: *always* **Launch Parameter**: *resourcelink\_id*

LTI - Custom parameter substitution: ResourceLink.description Returns the LTI value for the resource\_link.description property Returns "$ResourceLink.description" otherwise.

**Availability**: *always* **Launch Parameter**: *resourcelink\_description*

LTI - Custom parameter substitution: ResourceLink.title Returns the LTI value for the resource\_link.title property Returns "$ResourceLink.title" otherwise.

**Availability**: *always* **Launch Parameter**: *resourcelink\_title*
...
**Availability**: *always* **Launch Parameter**: *resourcelink\_available\_startdatetime*
...
**Availability**: *always* **Launch Parameter**: *resourcelink\_available\_enddatetime*
...
**Availability**: *always* **Launch Parameter**: *resourcelink\_submission\_enddatetime*
...
**Availability**: *when launched in a course (or a Group within a course)* **Launch Parameter**: *com\_instructure\_user\_observees*
...
Returns an array of the section names in a JSON-escaped format that the user is enrolled in, if the context of the tool launch is within a course. The names are sorted by the course\_section\_id, so that they are useful in conjunction with the Canvas.course.sectionIds substitution.

**Availability**: *when launched from a course (or a Group within a course)* **Launch Parameter**: *com\_instructure\_user\_section\_names*

```
[\"Section 1, M-T\", \"Section 2, W-Th\", \"TA Section\"]
```

### com.instructure.RCS.app\_host

Returns the host of the rich content service for the current region.

**Availability**: *always* **Launch Parameter**: *com\_instructure\_rcs\_app\_host*

```
"rich-content-iad.inscloudgate.net"
```

### com.instructure.User.student\_view

Returns true if the user is launching from student view.

**Availability**: *when launched by a logged in user* **Launch Parameter**: *com\_instructure\_user\_student\_view*

### com.instructure.Observee.sisIds

returns all observee ids linked to this observer as an String separated by `,`.

**Availability**: *when launched in a course (or a Group within a course)* **Launch Parameter**: *com\_instructure\_observee\_sis\_ids*

The title of the context.

**Availability**: *always* **Launch Parameter**: *context\_title*

### com.instructure.Editor.contents

The contents of the text editor associated with the content item launch.

**Availability**: *when the tool is launched from the editor\_button placement* **Launch Parameter**: *com\_instructure\_editor\_contents*
...
**Availability**: *when the tool is launched from the editor\_button placement* **Launch Parameter**: *com\_instructure\_editor\_selection*
...
**Availability**: \** **Launch Parameter**: *com\_instructure\_post\_message\_token*
...
**Availability**: *always* **Launch Parameter**: *com\_instructure\_assignment\_lti\_id*
...
**Availability**: *always* **Launch Parameter**: *com\_instructure\_assignment\_description*
...
**Availability**: *when the tool is used to upload a file as an assignment submission* **Launch Parameter**: *com\_instructure\_assignment\_allowed\_file\_extensions*

### com.instructure.OriginalityReport.id

The Canvas id of the Originality Report associated with the launch.

**Availability**: \** **Launch Parameter**: *com\_instructure\_originality\_report\_id*

### com.instructure.Submission.id

The Canvas id of the submission associated with the launch.

**Availability**: \** **Launch Parameter**: *com\_instructure\_submission\_id*

The Canvas id of the file associated with the submission in the launch.

**Availability**: \** **Launch Parameter**: *com\_instructure\_file\_id*

the LIS identifier for the course offering.

**Availability**: *when launched in a course (or a Group within a course)* **Launch Parameter**: *lis\_course\_offering\_sourcedid*

an opaque identifier that uniquely identifies the context of the tool launch.

**Availability**: *always* **Launch Parameter**: *context\_id*
...
**Availability**: *always* **Launch Parameter**: *launch\_presentation\_document\_target*

returns the current locale.

**Availability**: *always* **Launch Parameter**: *launch\_presentation\_locale*

### ToolConsumerInstance.guid

returns a unique identifier for the Tool Consumer (Canvas).

**Availability**: *always* **Launch Parameter**: *tool\_consumer\_instance\_guid*
...
### Canvas.externalTool.global\_id
...
returns the URL to retrieve the brand config javascript for the launching context. This URL should be used as the src attribute for a script tag on the external tool provider's web page. It is configured to be used with the [instructure-ui node modulearrow-up-right](https://github.com/instructure/instructure-ui). More information on on how to use instructure ui react components can be found [herearrow-up-right](http://instructure.github.io/instructure-ui/).
...
### Canvas.root\_account.global\_id

returns the root account's global id for the current context.

**Availability**: *always*

### Canvas.root\_account.id *\[deprecated]*

returns the root account id for the current context.

**Availability**: *always*

### vnd.Canvas.root\_account.uuid

returns the account uuid for the current context.

**Availability**: *always* **Launch Parameter**: *vnd\_canvas\_root\_account\_uuid*

```
"Ioe3sJPt0KZp9Pw6xAvcHuLCl0z4TvPKP0iIOLbo"
```

### Canvas.root\_account.sisSourceId *\[deprecated]*
...
**Availability**: *when launched in a course (or a Group within a course) that has a term with a name* **Launch Parameter**: *canvas\_term\_name*

returns the current course's term numerical id.

**Availability**: \*when launched in a course (or a Group within a course) that has a term * **Launch Parameter**: *canvas\_term\_id*

returns the current course sis source id to return the section source id use Canvas.course.sectionIds.

**Availability**: *when launched in a course (or a Group within a course)* **Launch Parameter**: *lis\_course\_section\_sourcedid*
...
### com.instructure.Assignment.anonymous\_grading

returns true if the assignment has anonymous grading enabled.

**Availability**: *when launched as an assignment* **Launch Parameter**: *com\_instructure\_assignment\_anonymous\_grading*

### com.instructure.Assignment.restrict\_quantitative\_data

returns true if the assignment restricts quantitative data. Assignment types: points, percentage, gpa\_scale are all considered quantitative.

**Availability**: *when launched as an assignment* **Launch Parameter**: *com\_instructure\_assignment\_restrict\_quantitative\_data*
...
**Availability**: *when launched in a course (or a Group within a course)* **Launch Parameter**: *com\_instructure\_course\_grading\_scheme*
...
**Availability**: *when launched from a course or an account (or a Group within a course or account)* **Launch Parameter**: *com\_instructure\_membership\_roles*
...
**Availability**: *when launched from a course or an account (or a Group within a course or account)* **Launch Parameter**: *canvas\_membership\_roles*
...
### com.instructure.Course.rce\_studio\_embed\_improvements
...
**Availability**: *when launched by a logged in user* **Launch Parameter**: *lis\_person\_name\_full*

Returns the display name of the launching user.

**Availability**: *when launched by a logged in user* **Launch Parameter**: *person\_name\_display*

Returns the last name of the launching user.

**Availability**: *when launched by a logged in user* **Launch Parameter**: *lis\_person\_name\_family*

Returns the first name of the launching user.

**Availability**: *when launched by a logged in user* **Launch Parameter**: *lis\_person\_name\_given*

### com.instructure.Person.name\_sortable

Returns the sortable name of the launching user.

**Availability**: *when launched by a logged in user* **Launch Parameter**: *com\_instructure\_person\_name\_sortable*

Returns the primary email of the launching user.

**Availability**: *when launched by a logged in user* **Launch Parameter**: *lis\_person\_contact\_email\_primary*

### com.instructure.Person.pronouns

Returns pronouns for the current user.

**Availability**: *when launched by a logged in user* **Launch Parameter**: *com\_instructure\_person\_pronouns*
...
**Availability**: *when launched by a logged in user* **Launch Parameter**: *user\_image*
...
Returns the Canvas user\_id of the launching user.

**Availability**: *when launched by a logged in user* **Launch Parameter**: *user\_id*
...
Returns the Canvas user\_uuid of the launching user for the context.
...
### vnd.instructure.User.current\_uuid \[duplicates User.uuid]

Returns the current Canvas user\_uuid of the launching user.
...
**Availability**: *when launched in a course (or a Group within a course)* **Launch Parameter**: *com\_instructure\_course\_groupids*
...
Returns the [IMS LTI membership servicearrow-up-right](https://www.imsglobal.org/specs/ltimemv1p0/specification-3) roles for filtering via query parameters. Or, for LTI 1.3 tools, returns the [IMS LTI Names and Role Provisioning Servicearrow-up-right](https://www.imsglobal.org/spec/lti-nrps/v2p0) roles for filtering via query parameters.
...
Returns the Canvas global user\_id of the launching user.
...
**Availability**: *when pseudonym is in use* **Launch Parameter**: *lis\_person\_sourcedid*
...
Returns the Canvas user\_id for the masquerading user. This is the pseudonym the user is actually logged in as. It may not hold all the sis info needed in other launch substitutions.

**Availability**: *when the user is being masqueraded*

### Canvas.masqueradingUser.userId

Returns the opaque user\_id for the masquerading user. This is the pseudonym the user is actually logged in as. It may not hold all the sis info needed in other launch substitutions.
...
Returns a comma separated list of section\_id's that the user is enrolled in.
...
Returns a comma separated list of section sis\_id's that the user is enrolled in.
...
**Availability**: *when launched in a course (or a Group within a course)* **Launch Parameter**: *context\_label*

Returns the module\_id that the module item was launched from.

**Availability**: *when content tag is present*

Returns the module\_item\_id of the module item that was launched.

**Availability**: *when content tag is present*

Returns the assignment\_id of the assignment that was launched.

**Availability**: *when launched as an assignment*

### Canvas.assignment.new\_quizzes\_type

Returns the new\_quizzes type of the assignment that was launched.

**Availability**: *when launched as an assignment*

### Canvas.assignment.anonymous\_participants
...
Returns the assignment\_description of the assignment that was launched.
...
**Availability**: *when launched by a logged in user and when launched as an assignment* **Launch Parameter**: *vnd\_canvas\_group\_id*

### com.instructure.Group.name

Returns the name of the group the current user is in if launching from a group assignment.

**Availability**: *when launched by a logged in user and when launched as an assignment* **Launch Parameter**: *vnd\_canvas\_group\_name*

Returns the Canvas id of the differentiation tag the current user is assigned to for the current assignment context.

**Availability**: *when launched by a logged in user and when launched as an assignment* **Launch Parameter**: *com\_instructure\_tag\_id*

Returns the name of the differentiation tag the current user is assigned to for the current assignment context.

**Availability**: *when launched by a logged in user and when launched as an assignment* **Launch Parameter**: *com\_instructure\_tag\_name*
...
Returns the [Tool Consumer Profilearrow-up-right](https://www.imsglobal.org/specs/ltiv2p0/implementation-guide#toc-46) url for the tool. Only available for LTI 2.0.
...
**Availability**: *always* **Launch Parameter**: *vnd\_canvas\_originality\_report\_url*
...
**Availability**: *always* **Launch Parameter**: *vnd\_canvas\_submission\_url*
...
**Availability**: *always* **Launch Parameter**: *vnd\_canvas\_submission\_history\_url*
...
### com.instructure.Course.accept\_canvas\_resource\_types

Returns the types of resources that can be imported to the current page, forwarded from the request. Value is a comma-separated array of one or more values of: \["assignment", "assignment\_group", "audio", "discussion\_topic", "document", "image", "module", "quiz", "page", "video"]
...
### com.instructure.Course.canvas\_resource\_type

Returns the target resource type for the current page, forwarded from the request. Value is the largest logical unit of the page. Possible values are: \["assignment", "assignment\_group", "audio", "discussion\_topic", "document", "image", "module", "quiz", "page", "video"] on Pages Index -&gt; 'page' on Modules -&gt; 'module' and so on.
...
### com.instructure.Course.canvas\_resource\_id
...
### com.instructure.Course.allow\_canvas\_resource\_selection
...
### com.instructure.Course.available\_canvas\_resources

Returns a JSON-encoded list of content groups which can be selected, providing ID and name of each group, forwarded from the request. Empty value if com.instructure.Course.allow\_canvas\_resource\_selection is false.
...
### com.instructure.Account.usage\_metrics\_enabled

**Availability**: *always*

### com.instructure.user.lti\_1\_1\_id.history
...
Returns the points possible of the assignment that was launched. For other LineItem properties, use the LTI 1.3 [Assignments and Grade Services](https://developerdocs.instructure.com/services/canvas/external-tools/lti/file.assignment_tools)

This is an alias of `Canvas.assignment.pointsPossible`.

**Availability**: *when launched as an assignment*

### Canvas.account.decimal\_separator

Returns the decimal separator for the current context account. This is used to have custom formatting on numbers, independent from the account's locale. If the account does not have a decimal separator set, it will return "$Canvas.account.decimal\_separator".

**Availability**: *when launched in a course (or a Group within a course)*

### Canvas.account.thousand\_separator

Returns the thousand separator for the current context account. This is used to have custom formatting on numbers, independent from the account's locale. If the account does not have a thousand separator set, it will return "$Canvas.account.thousand\_separator".
