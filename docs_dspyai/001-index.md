...
url: https://dspy.ai/
source: sitemap
fetched_at: 2026-01-23T08:01:20.255047755-03:00
rendered_js: false
word_count: 1650
summary: This document introduces DSPy, a declarative framework that enables developers to build modular AI systems by programming with structured modules rather than manually engineering prompts.
tags:
    - dspy-framework
    - ai-programming
    - language-models
...
## *Programming*—not prompting—*LMs*[¶](#programmingnot-promptinglms "Permanent link")
...
OpenAIAnthropicDatabricksGeminiLocal LMs on your laptopLocal LMs on a GPU serverOther providers
...
>pipinstallflashinfer-ihttps://flashinfer.ai/whl/cu121/torch2.4/

>CUDA_VISIBLE_DEVICES=0python-msglang.launch_server--port7501--model-pathmeta-llama/Llama-3.1-8B-Instruct
```
...
## 1) **Modules** help you describe AI behavior as *code*, not strings.[¶](#1-modules-help-you-describe-ai-behavior-as-code-not-strings "Permanent link")
...
```
defsearch_wikipedia(query: str) -> list[str]:
...
```
classExtractInfo(dspy.Signature):
...
```
defevaluate_math(expression: str):
    return dspy.PythonInterpreter({}).execute(expression)

defsearch_wikipedia(query: str):
    results = dspy.ColBERTv2(url="http://20.102.90.50:2017/wiki17_abstracts")(query, k=3)
    return [x["text"] for x in results]
...
classDraftSection(dspy.Signature):
...
classDraftArticle(dspy.Module):
    def__init__(self):
        self.build_outline = dspy.ChainOfThought(Outline)
        self.draft_section = dspy.ChainOfThought(DraftSection)

    defforward(self, topic):
...
## 2) **Optimizers** tune the prompts and weights of your AI modules.[¶](#2-optimizers-tune-the-prompts-and-weights-of-your-ai-modules "Permanent link")
...
## 3) **DSPy's Ecosystem** advances open-source AI research.[¶](#3-dspys-ecosystem-advances-open-source-ai-research "Permanent link")
