...
source: crawler
fetched_at: 2026-01-22T22:21:42.405076515-03:00
rendered_js: false
word_count: 557
summary: This document explains the component provider abstraction in FastMCP, detailing how they manage and aggregate tools, resources, and prompts from multiple sources.
tags:
    - fastmcp
    - component-providers
    - server-composition
...
ProviderWhat it doesHow you use it`LocalProvider`Stores components you define in code`@mcp.tool`, `mcp.add_tool()``FastMCPProvider`Wraps another FastMCP server`mcp.mount(server)``ProxyProvider`Connects to remote MCP servers`create_proxy(client)`
...
TransformPurpose`Namespace`Prefixes names to avoid conflicts`ToolTransform`Modifies tool schemas (rename, description, arguments)
...
- [Mount another server](https://gofastmcp.com/servers/providers/mounting) into yours
...
- [Mounting](https://gofastmcp.com/servers/providers/mounting) - Compose servers together
