---
title: Moodle Development Kit | Moodle Developer Resources
url: https://moodledev.io/general/development/tools/mdk
source: sitemap
fetched_at: 2026-02-17T16:01:32.886947-03:00
rendered_js: false
word_count: 1138
summary: This document introduces MDK, a command-line toolkit for Moodle developers designed to automate instance management, installation, and common development workflows.
tags:
    - moodle
    - development-tools
    - mdk
...
## Typical workflows using MDK[​](#typical-workflows-using-mdk "Direct link to Typical workflows using MDK")
...
### Installing a new instance[​](#installing-a-new-instance "Direct link to Installing a new instance")
...
### Fixing an issue[​](#fixing-an-issue "Direct link to Fixing an issue")
...
### Peer reviewing a patch[​](#peer-reviewing-a-patch "Direct link to Peer reviewing a patch")
...
### Backporting a fix[​](#backporting-a-fix "Direct link to Backporting a fix")

MDK could handle for you most of the cherry-picking work when you backport a fix. To do so, you need to have installed locally instances called "stable\_XX" for each version you want to backport your fix. To do so you can use commands like:
...
Once you have the stable\_XX versions, go to the instance where you have the branch you want to backport and execute:
...
After the execution, your stable\_XX instances will be on the fix branch, commits are pushed to you github project and backport links will be updated in the tracker.

If for some reason one cherry pick could not finish successfully, mdk will ask you if you want to keep this backport or no. If you choose to keep the partial backport, your stable\_XX instance will be in cherry-picking state. In that case you need to solve any conflict and use mdk to push your changes:

1. go to your stable\_XX with cherry pick conflict
2. solve conflicts with `git mergetool` or any other tool
3. `git cherry-pick --continue`
4. once all conflicts are solved, push to your repository and tracker using

### Upgrading the instances[​](#upgrading-the-instances "Direct link to Upgrading the instances")
...
### Executing behat tests[​](#executing-behat-tests "Direct link to Executing behat tests")
...
## Upgrading MDK[​](#upgrading-mdk "Direct link to Upgrading MDK")
...
2. Check out the `main` branch for all of your `main` instances, e.g. in your stable\_main branch: `git checkout main`
3. Upgrade MDK
   
   1. Via pip: `sudo pip install moodle-sdk --upgrade`
   2. Via Homebrew: `brew upgrade moodle-sdk`
4. Run MDK doctor to fix its `masterBranch` configuration. `mdk doctor --fix --masterbranch`
5. Run MDK doctor to check the `main` instances. `mdk doctor --all`
   
   1. If you don't see an error saying something like "stable\_main is on branch main instead of MOODLE\_XX\_STABLE", then you're all good. Otherwise, do a hard reset your main instances:
...
## The MDK Suite[​](#the-mdk-suite "Direct link to The MDK Suite")
...
### MDK Browser Extension[​](#mdk-browser-extension "Direct link to MDK Browser Extension")
...
### MDK Authentication[​](#mdk-authentication "Direct link to MDK Authentication")
