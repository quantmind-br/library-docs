---
title: Quantization — SGLang
url: https://docs.sglang.io/advanced_features/quantization.html
source: crawler
fetched_at: 2026-02-04T08:46:49.776493509-03:00
rendered_js: false
word_count: 940
summary: This document explains how to implement and use offline and online quantization methods in SGLang to optimize model inference performance. It provides detailed instructions and examples for integrating various quantization tools like Unsloth, auto-round, GPTQModel, and LLM Compressor.
tags:
    - quantization
    - sglang
...
## Offline Quantization[#](#offline-quantization "Link to this heading")
...
--model-pathhugging-quants/Meta-Llama-3.1-8B-Instruct-AWQ-INT4\
--port30000--host0.0.0.0
```
...
```
python3-msglang.launch_server\
--model-pathneuralmagic/Meta-Llama-3.1-8B-Instruct-FP8-dynamic\
--quantizationw8a8_fp8\
--port30000--host0.0.0.0
```

### Examples of Offline Model Quantization[#](#examples-of-offline-model-quantization "Link to this heading")

#### Using [Unsloth](https://docs.unsloth.ai/basics/inference-and-deployment/sglang-guide)[#](#using-unsloth "Link to this heading")
...
#### Using [auto-round](https://github.com/intel/auto-round)[#](#using-auto-round "Link to this heading")

```
# Install
pipinstallauto-round
```
...
```
# for VLMs
fromauto_roundimport AutoRoundMLLM
...
3. Limited Support for Quantized VLMs
   ...
   auto\_round:auto\_gptq format: Accuracy is close to zero.
...
#### Using [GPTQModel](https://github.com/ModelCloud/GPTQModel)[#](#using-gptqmodel "Link to this heading")

```
# install
pipinstallgptqmodel--no-build-isolation-v
```

```
fromdatasetsimport load_dataset
fromgptqmodelimport GPTQModel, QuantizeConfig
...
#### Using [LLM Compressor](https://github.com/vllm-project/llm-compressor/)[#](#using-llm-compressor "Link to this heading")
...
fromllmcompressor.transformersimport SparseAutoModelForCausalLM
fromllmcompressor.transformersimport oneshot
...
#### Using [NVIDIA ModelOpt](https://github.com/NVIDIA/Model-Optimizer)[#](#using-nvidia-modelopt "Link to this heading")
...
pipinstallsglang[modelopt]
```

##### Quantization and Export Workflow[#](#quantization-and-export-workflow "Link to this heading")
...
pythonexamples/usage/modelopt_quantize_and_export.pyquantize\
--model-pathTinyLlama/TinyLlama-1.1B-Chat-v1.0\
--export-dir./quantized_tinyllama_fp8\
--quantization-methodmodelopt_fp8

# For FP4 quantization
pythonexamples/usage/modelopt_quantize_and_export.pyquantize\
--model-pathTinyLlama/TinyLlama-1.1B-Chat-v1.0\
--export-dir./quantized_tinyllama_fp4\
--quantization-methodmodelopt_fp4
```

##### Available Quantization Methods[#](#available-quantization-methods "Link to this heading")
...
##### Python API Usage[#](#python-api-usage "Link to this heading")

You can also use ModelOpt quantization programmatically:

```
importsglangassgl
fromsglang.srt.configs.device_configimport DeviceConfig
fromsglang.srt.configs.load_configimport LoadConfig
fromsglang.srt.configs.model_configimport ModelConfig
fromsglang.srt.model_loader.loaderimport get_model_loader
...
##### Deploying Quantized Models[#](#deploying-quantized-models "Link to this heading")
...
##### Benefits of ModelOpt[#](#benefits-of-modelopt "Link to this heading")
...
## Online Quantization[#](#online-quantization "Link to this heading")
...
### torchao online quantization method[#](#torchao-online-quantization-method "Link to this heading")
...
python3-msglang.launch_server\
--model-pathmeta-llama/Meta-Llama-3.1-8B-Instruct\
--torchao-configint8dq\
--disable-cuda-graph\
--port30000--host0.0.0.0
```

### `quark_int4fp8_moe` online quantization method[#](#quark-int4fp8-moe-online-quantization-method "Link to this heading")
