---
title: Thinking Budgets via Model Name Parentheses
url: https://help.router-for.me/configuration/thinking
source: crawler
fetched_at: 2026-01-14T22:10:00.060701568-03:00
rendered_js: false
word_count: 213
summary: This document explains how to append values in parentheses to a model name to control its thinking budget or reasoning effort, detailing accepted values and how they are applied by different providers.
tags:
    - model-configuration
    - thinking-budget
    - reasoning-effort
...
LevelApprox. BudgetPurpose`minimal`512Lowest cost reasoning`low`1024Fast reasoning`medium`8192Default reasoning depth`high`24576Deep reasoning`xhigh`32768Extra-deep reasoning`auto`Dynamic (-1 when allowed, else mid/min)Let the provider choose`none`0 (clamped to min when zero is disallowed)Disable thinking

- Empty `()` is ignored. For `provider://model` forms, place the parentheses after the model (e.g., `openrouter://gemini-3-pro-preview(high)`).

## How It’s Applied [​](#how-it-s-applied)
