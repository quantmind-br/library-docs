...
source: git
fetched_at: 2026-04-26T05:48:12.4048853-03:00
rendered_js: false
word_count: 2252
optimized: true
optimized_at: 2026-04-26T09:00:00Z
summary: This document explains how to register, override, and configure custom model providers within the pi-coding-agent framework using the extension API.
tags:
    - extension-api
    - model-provider
    - custom-api
...
Extensions register custom model providers via `pi.registerProvider()` enabling proxies, custom endpoints, OAuth/SSO, and non-standard LLM APIs.

- **Proxies** — route requests through corporate proxies or API gateways
- **Custom endpoints** — self-hosted or private model deployments
- **OAuth/SSO** — authentication flows for enterprise providers
- **Custom APIs** — streaming for non-standard LLM APIs
...
> [!info] The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. pi waits for the factory before startup continues, so the provider is available during interactive startup and to `pi --list-models`.

## Override Existing Provider

Redirect an existing provider through a proxy — the simplest use case.
...
> [!note] When only `baseUrl` and/or `headers` are provided (no `models`), all existing models for that provider are preserved with the new endpoint.
...
> [!warning] When `models` is provided, it **replaces** all existing models for that provider.

## Unregister Provider

Use `pi.unregisterProvider(name)` to remove a provider previously registered via `pi.registerProvider(name, ...)`:
...
Unregistering removes the provider's dynamic models, API key fallback, OAuth provider registration, and custom stream handler registrations. Any overridden built-in models or provider behavior are restored.

> [!note] Calls made after the initial extension load phase are applied immediately — no `/reload` required.
...
> [!info] Use `qwen-chat-template` for local Qwen-compatible servers that read `chat_template_kwargs.enable_thinking`.
> Use `cacheControlFormat: "anthropic"` for OpenAI-compatible providers that expose Anthropic-style prompt caching via `cache_control` on the system prompt, last tool definition, and last user/assistant text content.

> [!note] **Migration note:** Mistral moved from `openai-completions` to `mistral-conversations`. Use `mistral-conversations` for native Mistral models. If you intentionally route Mistral-compatible/custom endpoints through `openai-completions`, set `compat` flags explicitly as needed.
...
The `callbacks` object provides three authentication methods:
...
> [!note] The `partial` field in each event contains the current `AssistantMessage` state. Update `output.content` as you receive data, then include `output` as the `partial`.
...
Test against the same test suites used by built-in providers. Copy and adapt from [packages/ai/test/](https://github.com/badlogic/pi-mono/tree/main/packages/ai/test):
...
> [!info] `deepseek` sends `thinking: { type: "enabled" | "disabled" }` and `reasoning_effort` when enabled. `qwen` is for DashScope-style top-level `enable_thinking`. Use `qwen-chat-template` for local Qwen-compatible servers that read `chat_template_kwargs.enable_thinking`.
...
#extension-api #model-provider #custom-api #llm-configuration #api-integration
