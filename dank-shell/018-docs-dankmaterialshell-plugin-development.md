...
source: sitemap
fetched_at: 2026-04-26T08:39:17.885361582-03:00
rendered_js: false
word_count: 1617
summary: This document provides a comprehensive guide for developers on how to create, configure, and debug plugins for the DankMaterialShell environment using QML.
tags:
    - dankmaterialshell
    - qml
    - plugin-development
...
> [!tip]
> Use [Run on Save](https://marketplace.visualstudio.com/items?itemName=emeraldwalk.RunOnSave) to auto-reload your plugin during development.
...
Widget plugins show up in DankBar or the Control Center and use `PluginComponent` as the base.
...
The widget pulls `customColor` from `pluginData`, which automatically syncs with your settings.

### Widget with Popout

Add a popout menu that opens when you click the widget. Add `layerNamespacePlugin: "<namespace>"` to set a layer namespace -- the shell adds `dms:plugins:` as a prefix automatically.

> [!warning]
> As of right now, layer namespace only works with popout widget plugins.
...
`PopoutComponent` gives you consistent header/footer and a `closePopout()` function.
...
A clock widget with analog and digital modes:
...
Use `closeLauncher: true` for paste operations:
...
Launcher plugins can support Shift+Enter to paste content directly.
...
Settings persist user preferences, but sometimes you need to persist runtime data -- command history, recent items, counters, cached results. That's what the state API is for.
...
`PluginSettings` provides convenience wrappers:
...
`PluginComponent` automatically provides these -- don't declare them yourself:
...
4. Hot-reload: `dms ipc call plugins reload myPlugin`
