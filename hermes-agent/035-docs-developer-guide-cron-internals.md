---
title: Cron Internals | Hermes Agent
url: https://hermes-agent.nousresearch.com/docs/developer-guide/cron-internals
source: crawler
fetched_at: 2026-04-24T17:01:04.513378375-03:00
rendered_js: false
word_count: 804
summary: This document details the functionality, structure, and execution mechanisms of a cron subsystem used for scheduling tasks. It outlines supported scheduling formats, job storage in JSON, the runtime tick cycle, various methods for attaching context (skills or scripts), and numerous delivery options.
tags:
    - cron-subsystem
    - scheduled-tasks
    - job-execution
...
StateMeaning`scheduled`Active, will fire at next scheduled time`paused`Suspended — won't fire until resumed`completed`Repeat count exhausted or one-shot that has fired`running`Currently executing (transient state)
...
TargetSyntaxExampleOrigin chat`origin`Deliver to the chat where the job was createdLocal file`local`Save to `~/.hermes/cron/output/`Telegram`telegram` or `telegram:<chat_id>``telegram:-1001234567890`Discord`discord` or `discord:#channel``discord:#engineering`Slack`slack`Deliver to Slack home channelWhatsApp`whatsapp`Deliver to WhatsApp homeSignal`signal`Deliver to SignalMatrix`matrix`Deliver to Matrix home roomMattermost`mattermost`Deliver to Mattermost homeEmail`email`Deliver via emailSMS`sms`Deliver via SMSHome Assistant`homeassistant`Deliver to HA conversationDingTalk`dingtalk`Deliver to DingTalkFeishu`feishu`Deliver to FeishuWeCom`wecom`Deliver to WeComWeixin`weixin`Deliver to Weixin (WeChat)BlueBubbles`bluebubbles`Deliver to iMessage via BlueBubblesQQ Bot`qqbot`Deliver to QQ (Tencent) via Official API v2
