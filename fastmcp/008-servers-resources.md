---
title: Resources & Templates - FastMCP
url: https://gofastmcp.com/servers/resources
source: crawler
fetched_at: 2026-01-22T22:21:56.357720078-03:00
rendered_js: false
word_count: 1884
summary: This document explains how to define and manage static and dynamic resources in FastMCP using decorators, classes, and URIs. It covers key concepts such as content types, visibility controls, asynchronous operations, and integration with the MCP context object.
tags:
    - fastmcp
    - mcp-resources
    - resource-templates
...
New in version `3.0.0` `ResourceResult` gives you explicit control over resource responses: multiple content items, per-item MIME types, and metadata at both the item and result level.
...
New in version `3.0.0` You can control which resources are enabled for clients using server-level enabled control. Disabled resources don’t appear in `list_resources` and can’t be read.
...
AnnotationTypeDefaultPurpose`readOnlyHint`booleantrueIndicates if the resource only provides data without side effects`idempotentHint`booleantrueIndicates if repeated reads have the same effect as a single read
...
New in version `2.13.0` FastMCP supports RFC 6570 form-style query parameters using the `{?param1,param2}` syntax. Query parameters provide a clean way to pass optional configuration to resources without cluttering the path. Query parameters must be optional function parameters (have default values), while path parameters map to required function parameters. This enforces a clear separation: required data goes in the path, optional configuration in query params.
...
New in version `2.2.0` FastMCP enforces these validation rules when creating resource templates:
