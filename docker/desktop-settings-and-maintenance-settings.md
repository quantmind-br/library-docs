...
source: llms
fetched_at: 2026-01-24T14:18:43.683545463-03:00
rendered_js: false
word_count: 3753
summary: This document provides instructions on how to access and configure Docker Desktop settings, covering general preferences, platform-specific features, and resource allocation across Mac, Windows, and Linux.
tags:
    - docker-desktop
    - configuration
    - settings-management
...
- Select the Docker menu ![whale menu](https://docs.docker.com/desktop/images/whale-x.svg) and then **Settings**
...
For additional information on settings that can be set in the Docker Desktop Dashboard and by administrators via the Admin Console, see the [Settings reference](https://docs.docker.com/enterprise/security/hardened-desktop/settings-management/settings-reference/).
...
#### [Synchronized file shares](#synchronized-file-shares)
...
#### [Shared folders on demand](#shared-folders-on-demand)
...
For more details on how it works, see [Using Docker Desktop with a proxy](https://docs.docker.com/desktop/features/networking/#useing-docker-desktop-with-a-proxy).
...
> 
> To manage proxy configurations for Docker Desktop, configure the settings in the Docker Desktop app or use [Settings Management](https://docs.docker.com/enterprise/security/hardened-desktop/settings-management/).
...
##### [Kerberos and NTLM authentication](#kerberos-and-ntlm-authentication)
...
Docker Desktop includes a standalone Kubernetes server, so that you can test deploying your Docker workloads on Kubernetes. To turn on Kubernetes support and install a standalone instance of Kubernetes running as a Docker container, select **Enable Kubernetes**. This can also be done from the **Kubernetes** view.
...
- **Allow the default Docker socket to be used (Requires password)**. Creates `/var/run/docker.sock` which some third party clients may use to communicate with Docker Desktop. For more information, see [permission requirements for macOS](https://docs.docker.com/desktop/setup/install/mac-permission-requirements/#installing-symlinks).
...
When signed in with a Docker account that has access to [Docker Offload](https://docs.docker.com/offload/), you can manage your Offload settings from the **Docker Offload** tab.

Use the **Docker Offload** tab to:

- Toggle **Enable Docker Offload**. When enabled, you can start Offload sessions.
- Select **Idle timeout**. This is the duration of time between no activity and Docker Offload entering idle mode.For details about idle timeout, see [Active and idle states](https://docs.docker.com/offload/configuration/#understand-active-and-idle-states)
