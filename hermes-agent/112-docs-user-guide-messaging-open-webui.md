...
url: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/open-webui
source: crawler
fetched_at: 2026-04-24T17:00:27.004175627-03:00
rendered_js: false
word_count: 982
summary: This document serves as a comprehensive guide explaining how to integrate and use Open WebUI, the popular self-hosted chat interface, with Hermes Agent's API server. It covers setup instructions using Docker/Docker Compose, configuration options via UI or environment variables, and details on supported API types like Chat Completions vs Responses.
tags:
    - open-webui
    - hermes-agent
    - ai-chat
...
### 3. Start Open WebUI[​](#3-start-open-webui "Direct link to 3. Start Open WebUI")

```bash
docker run -d-p3000:8080 \
-eOPENAI_API_BASE_URL=http://host.docker.internal:8642/v1 \
-eOPENAI_API_KEY=your-secret-key \
...
### 4. Open the UI[​](#4-open-the-ui "Direct link to 4. Open the UI")
...
ModeFormatWhen to use**Chat Completions** (default)`/v1/chat/completions`Recommended. Works out of the box.**Responses** (experimental)`/v1/responses`For server-side conversation state via `previous_response_id`.
...
VariableDefaultDescription`API_SERVER_ENABLED``false`Enable the API server`API_SERVER_PORT``8642`HTTP server port`API_SERVER_HOST``127.0.0.1`Bind address`API_SERVER_KEY`*(required)*Bearer token for auth. Match `OPENAI_API_KEY`.

### Open WebUI[​](#open-webui "Direct link to Open WebUI")

VariableDescription`OPENAI_API_BASE_URL`Hermes Agent's API URL (include `/v1`)`OPENAI_API_KEY`Must be non-empty. Match your `API_SERVER_KEY`.
...
ConnectionURLAPI KeyAlice`http://host.docker.internal:8643/v1``alice-secret`Bob`http://host.docker.internal:8644/v1``bob-secret`
...
docker run --network=host -eOPENAI_API_BASE_URL=http://localhost:8642/v1 ...

# Option 3: Use Docker bridge IP
docker run -eOPENAI_API_BASE_URL=http://172.17.0.1:8642/v1 ...
```