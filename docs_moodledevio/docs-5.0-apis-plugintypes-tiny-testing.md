...
url: https://moodledev.io/docs/5.0/apis/plugintypes/tiny/testing
source: sitemap
fetched_at: 2026-02-17T15:26:55.224016-03:00
rendered_js: false
word_count: 687
summary: This document provides instructions and best practices for writing Behat automated tests for Moodle plugins that interact with the TinyMCE editor. It details mandatory testing tags, generic and editor-specific step definitions, and how to create custom steps for plugin testing.
tags:
    - moodle
    - behat-testing
    - tinymce-editor
...
When writing tests which only *use* a text editor, and are not either tests specifically for that editor, or one of its plugins, you should attempt not to use editor-specific tests.
...
### Useful steps[​](#useful-steps "Direct link to Useful steps")
...
Use generic steps where possible

Typically, when interacting with an Editor, you should use the *generic* step definitions rather than writing specific steps, or writing sequences of steps to do so.

#### Generic steps to interact with editors[​](#generic-steps-to-interact-with-editors "Direct link to Generic steps to interact with editors")
...
##### Setting content of a field[​](#setting-content-of-a-field "Direct link to Setting content of a field")

Set the content of the field with the label 'Description'
...
Set the content of the field with the label 'Description' within the 'Create event' modal

```
Given I set the field "Description" in the "Create event""dialogue" to "<p>Some value which includes valid HTML</p>"
```

Set the content of the field with the label 'Description' along side some other fields
...
Set the content of the field with the label 'Description' along side some other fields within the 'Create event' modal
...
##### Checking the content of a field[​](#checking-the-content-of-a-field "Direct link to Checking the content of a field")

Check that the content of the field with the label 'Description' matches a value
...
Check that the content of the field with the label 'Description' does not match a value

```
Then the field "Description" does not match value "<p>Some value which includes valid HTML</p>"
```

Check that the content of the field with the label 'Description' within the 'Create event' modal matches a value

```
Then the field "Description" in the "Create event""dialogue" matches value "<p>Some value which includes valid HTML</p>"
```

Check that the content of a set of fields match values
...
Check that the content of a set of fields within the 'Create event' modal match values
...
#### TinyMCE specific steps[​](#tinymce-specific-steps "Direct link to TinyMCE specific steps")
...
Behat makes use of WebDriver to control browsers. The WebDriver specification limits the scope of a command to the current *context* where a context is a browser Window, Frame, or iFrame.

TinyMCE places the Editor UI within its own iFrame. To control any of its interface, you *may* need to switch to the iFrame for your editor first.

Most of the steps written for TinyMCE are already aware of the iFrame.

##### Working with toolbar buttons[​](#working-with-toolbar-buttons "Direct link to Working with toolbar buttons")

Expanding all toolbar buttons
...
Checking for a tag on your Feature or Scenario

```
if(!$this->has_tag('tiny_myplugin')){
thrownewDriverException(
...
Some of these steps will be improved in coming changes. See [MDL-75926](https://moodle.atlassian.net/browse/MDL-75926) and [MDL-76853](https://moodle.atlassian.net/browse/MDL-76853) for more information.