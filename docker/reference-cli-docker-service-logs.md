...
source: llms
fetched_at: 2026-01-24T14:40:54.743571817-03:00
rendered_js: false
word_count: 451
summary: This document explains how to use the docker service logs command to retrieve and stream logs from services or tasks within a Docker Swarm cluster. It covers various options for filtering, formatting, and monitoring log output from swarm services.
tags:
    - docker-swarm
    - service-logs
    - cluster-management
...
DescriptionFetch the logs of a service or taskUsage`docker service logs [OPTIONS] SERVICE|TASK`
...
OptionDefaultDescription`--details`API 1.30+ Show extra details provided to logs`-f, --follow`Follow log output`--no-resolve`Do not map IDs to Names in output`--no-task-ids`Do not include task IDs in output`--no-trunc`Do not truncate output`--raw`API 1.30+ Do not neatly format logs`--since`Show logs since timestamp (e.g. `2013-01-02T13:23:37Z`) or relative (e.g. `42m` for 42 minutes)  
`-n, --tail``all`Number of lines to show from the end of the logs`-t, --timestamps`Show timestamps