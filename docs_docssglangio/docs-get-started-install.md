---
title: Installation - SGLang Documentation
url: https://docs.sglang.io/docs/get-started/install
source: sitemap
fetched_at: 2026-05-11T05:51:08.486207531-03:00
rendered_js: false
word_count: 723
summary: This document provides comprehensive installation and deployment instructions for SGLang across various environments including local systems, Docker, Kubernetes, cloud platforms, and AWS SageMaker.
tags:
    - installation
    - deployment
    - docker
...
- If you want to develop SGLang, you can try the dev docker image. Please refer to [setup docker container](https://docs.sglang.io/docs/developer_guide/development_guide_using_docker#setup-docker-container). The docker image is `lmsysorg/sglang:dev`.
...
1. Option 1: For single node serving (typically when the model size fits into GPUs on one node) Execute command `kubectl apply -f docker/k8s-sglang-service.yaml`, to create k8s deployment and service, with llama-31-8b as example.
2. Option 2: For multi-node serving (usually when a large model requires more than one GPU node, such as `DeepSeek-R1`) Modify the LLM model path and arguments as necessary, then execute command `kubectl apply -f docker/k8s-sglang-distributed-sts.yaml`, to create two nodes k8s statefulset and serving service.
