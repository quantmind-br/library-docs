...
url: https://doc.rust-lang.org/stable/reference/expressions/loop-expr.html#loop-labels
source: crawler
fetched_at: 2026-05-06T21:32:10.169027005-03:00
rendered_js: false
word_count: 1118
summary: This document provides a technical reference for Rust's loop expressions, including loop, while, for, and labeled blocks, along with their associated control flow mechanisms like break, continue, and labels.
tags:
    - rust
...
- A [labeled block expression](https://doc.rust-lang.org/stable/reference/expressions/loop-expr.html#r-expr.loop.block-labels) runs a loop exactly once, but allows exiting the loop early with `break`.
...
A `loop` expression without an associated `break` expression is [diverging](https://doc.rust-lang.org/stable/reference/divergence.html#r-divergence) and has type [`!`](https://doc.rust-lang.org/stable/reference/types/never.html#r-type.never).
...
Condition operands must be either an [Expression](https://doc.rust-lang.org/stable/reference/expressions.html#grammar-Expression) with a [boolean type](https://doc.rust-lang.org/stable/reference/types/boolean.html) or a conditional `let` match. If all of the condition operands evaluate to `true` and all of the `let` patterns successfully match their [scrutinee](https://doc.rust-lang.org/stable/reference/glossary.html#scrutinee)s, then the loop body block executes.
...
A `while let` loop is equivalent to a `loop` expression containing a [`match` expression](https://doc.rust-lang.org/stable/reference/expressions/match-expr.html) as follows.
...
Multiple condition operands can be separated with `&&`. These have the same semantics and restrictions as [`if` condition chains](https://doc.rust-lang.org/stable/reference/expressions/if-expr.html#chains-of-conditions).
...
A `for` loop is equivalent to a `loop` expression containing a [`match` expression](https://doc.rust-lang.org/stable/reference/expressions/match-expr.html) as follows:
...
> Note
> 
> The outer `match` is used to ensure that any [temporary values](https://doc.rust-lang.org/stable/reference/expressions.html#temporaries) in `iter_expr` don’t get dropped before the loop is finished. `next` is declared before being assigned because it results in types being inferred correctly more often.
...
A `break` expression is [diverging](https://doc.rust-lang.org/stable/reference/divergence.html#r-divergence) and has a type of [`!`](https://doc.rust-lang.org/stable/reference/types/never.html#r-type.never).
...
In a [`loop` with break expressions](https://doc.rust-lang.org/stable/reference/expressions/loop-expr.html#r-expr.loop.break-value) or a [labeled block expression](https://doc.rust-lang.org/stable/reference/expressions/loop-expr.html#r-expr.loop.block-labels), a `break` without an expression is equivalent to `break ()`.
...
The type of a labeled block expression is the [least upper bound](https://doc.rust-lang.org/stable/reference/type-coercions.html#r-coerce.least-upper-bound) of all of the break operands and the final operand. If the final operand is omitted, the type of the final operand defaults to the [unit type](https://doc.rust-lang.org/stable/reference/types/tuple.html#r-type.tuple.unit), unless the block [diverges](https://doc.rust-lang.org/stable/reference/expressions/block-expr.html#r-expr.block.diverging), in which case it is the [never type](https://doc.rust-lang.org/stable/reference/types/never.html#r-type.never).
...
A `continue` expression is [diverging](https://doc.rust-lang.org/stable/reference/divergence.html#r-divergence) and has a type of [`!`](https://doc.rust-lang.org/stable/reference/types/never.html#r-type.never).
...
The type of a `loop` with associated `break` expressions is the [least upper bound](https://doc.rust-lang.org/stable/reference/type-coercions.html#r-coerce.least-upper-bound) of all of the break operands.
...
A `loop` with associated `break` expressions does not [diverge](https://doc.rust-lang.org/stable/reference/divergence.html#r-divergence) if any of the break operands do not diverge. If all of the `break` operands diverge, then the `loop` expression also diverges.
