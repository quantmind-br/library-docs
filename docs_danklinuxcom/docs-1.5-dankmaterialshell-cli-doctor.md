...
url: https://danklinux.com/docs/1.5/dankmaterialshell/cli-doctor
source: sitemap
fetched_at: 2026-02-22T18:44:01.556531-03:00
rendered_js: false
word_count: 827
summary: This document explains the usage and functionality of the dms doctor diagnostic command for verifying system dependencies and configuration. It details the various system, version, and installation checks performed to ensure the DankMaterialShell environment is correctly set up.
tags:
    - dms-doctor
    - diagnostics
    - troubleshooting
...
CompositorDetectionVersion Parsingniri`niri``niri --version`Hyprland`hyprland`, `Hyprland``hyprctl version`mangowc`mangowc``mangowc --version`labwc`labwc``labwc --version`Sway`sway``sway --version`Miracle WM`miracle-wm``miracle-wm --version`River`river``river -version`Wayfire`wayfire``wayfire --version`

### Active Compositor[​](#compositor "Direct link to Active Compositor")
...
### cups-pk-helper[​](#cups-pk-helper "Direct link to cups-pk-helper")
...
Required to use system monitoring widgets and applets (CPU, RAM, Disk, Network, Processes). See [dgop documentation](https://danklinux.com/docs/1.5/dgop/) for more information.
...
### gp-saml-gui[​](#gp-saml-gui "Direct link to gp-saml-gui")
...
### openconnect[​](#openconnect "Direct link to openconnect")
...
Used for indexed filesystem search in the launcher. Use `/` to initiate a fs search. See [DankSearch documentation](https://danklinux.com/docs/1.5/danksearch/) for more information.
...
## Configuration Files[​](#config-files "Direct link to Configuration Files")
...
Required for the Dank Greeter. (See [DankGreeter documentation](https://danklinux.com/docs/1.5/dankgreeter/) for more information.)
