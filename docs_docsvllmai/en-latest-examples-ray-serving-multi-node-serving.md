...
url: https://docs.vllm.ai/en/latest/examples/ray_serving/multi-node-serving/
source: sitemap
fetched_at: 2026-05-07T21:13:35.952078881-03:00
rendered_js: false
word_count: 6
summary: This script provides a command-line interface to automate the initialization of a multi-node Ray cluster for distributed vLLM model serving. It handles the deployment of Ray head and worker nodes while verifying cluster readiness before initiating model serving processes.
tags:
    - vllm
    - ray
...
[](https://github.com/vllm-project/vllm/edit/main/docs/examples/ray_serving/multi-node-serving.md "Edit this page")

Source [https://github.com/vllm-project/vllm/blob/main/examples/ray\_serving/multi-node-serving.sh](https://github.com/vllm-project/vllm/blob/main/examples/ray_serving/multi-node-serving.sh).
...
declare-astart_params# Parameters forwarded to the underlying 'ray start' command.
...
ifraystart--address="$ray_address":"$ray_port"--block"${start_params[@]}";then
...
raystart--head--port="$ray_port""${start_params[@]}"
