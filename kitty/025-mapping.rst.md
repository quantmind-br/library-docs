...
title: Mapping
word_count: 1975
summary: This document explains how to configure and customize keyboard mappings in the kitty terminal emulator, including actions, macros, remote control, and conditional logic.
category: guide
optimized: true
...
This will map the key press Ctrl+a to open a new window
...
map the key press to the combine action:
...
/kittens/unicode_input, /kittens/hints and
/kittens/themes. To learn about writing your own kittens, see
/kittens/custom.
...
square bracket key, etc.  For functional keys, such as Enter or
Escape, the names are present at :ref:`Functional key definitions
<functional>`. For modifier keys, the names are ctrl (control,
⌃), shift (⇧), alt (opt, option,
⌥), super (cmd, command, ⌘).

Additionally, you can use the name kitty_mod as a modifier, the default
...
with the kitty --debug-input option, kitty will output some debug text
...
actions for the full list of actions and the default shortcuts that map
...
This unmaps the default shortcut new_window to open a new window. Almost
all default shortcuts are of the form `modifier + key` where the
modifier defaults to Ctrl+Shift and can be changed using the kitty_mod setting
in kitty.conf.

If you want to clear all default shortcuts, you can use
clear_all_shortcuts in kitty.conf.
...
globally using the map_timeout option or per-mode using `--timeout`:
...
the focused window using very powerful criteria, see search_syntax for
...
shortcuts like Ctrl+C stop working because the key produces a non-Latin
...
This is accomplished by using `map` with send_text <send_text> in kitty.conf.
For example:
...
Now, if you press f1 when at shell prompt it will run the ``echo Hello,
world!`` command.
...
down arrow keys to focus the next/previous window. Press Shift+Up or
Shift+Down to move the active window up and down. Press t to make
