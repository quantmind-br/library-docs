---
title: Virtual Scroll - Crawl4AI Documentation (v0.8.x)
url: https://docs.crawl4ai.com/advanced/virtual-scroll/
source: sitemap
fetched_at: 2026-04-26T07:45:36.594210052-03:00
rendered_js: false
word_count: 521
summary: This document explains how to use the Virtual Scroll feature in Crawl4AI to capture data from websites that dynamically replace content during scrolling, such as social media feeds and virtualized tables.
tags:
    - crawl4ai
    - web-scraping
    - virtual-scrolling
...
Parameter Type Default Description `container_selector` `str` Required CSS selector for the scrollable container `scroll_count` `int` `10` Maximum number of scrolls to perform `scroll_by` `str` or `int` `"container_height"` Scroll amount per step `wait_after_scroll` `float` `0.5` Seconds to wait after each scroll
...
```
fromcrawl4aiimport AsyncWebCrawler, CrawlerRunConfig, VirtualScrollConfig, BrowserConfig
...
Feature Virtual Scroll scan\_full\_page **Purpose** Capture content that's replaced during scroll Load content that's appended during scroll **Use Case** Twitter, Instagram, virtual tables Traditional infinite scroll, lazy-loaded images **DOM Behavior** Replaces elements Adds elements **Memory Usage** Efficient (merges content) Can grow large **Configuration** Requires container selector Works on full page
...
Use **scan\_full\_page** when: - Content accumulates as you scroll - Images load lazily - Simple "load more" behavior - Full page scrolling

Virtual Scroll works seamlessly with extraction strategies:

```
fromcrawl4aiimport LLMExtractionStrategy, LLMConfig
...
# Run the examples
cddocs/examples
pythonvirtual_scroll_example.py
```
