...
CategoryAlert Type**LLM Performance**Hanging API calls, Slow API calls, Failed API calls, Model outage alerting**Budget & Spend**Budget tracking per key/user, Soft budget alerts, Weekly & Monthly spend reports per Team/Tag**System Health**Failed database read/writes**Daily Reports**Top 5 slowest LLM deployments, Top 5 LLM deployments with most failed requests, Weekly & Monthly spend per Team/Tag
...
### Step 1: Add a Slack Webhook URL to env[​](#step-1-add-a-slack-webhook-url-to-env "Direct link to Step 1: Add a Slack Webhook URL to env")
...
### Step 2: Setup Proxy[​](#step-2-setup-proxy "Direct link to Step 2: Setup Proxy")
...
### Redacting Messages from Alerts[​](#redacting-messages-from-alerts "Direct link to Redacting Messages from Alerts")
...
### Soft Budget Alerts for Virtual Keys[​](#soft-budget-alerts-for-virtual-keys "Direct link to Soft Budget Alerts for Virtual Keys")
...
### Add Metadata to alerts[​](#add-metadata-to-alerts "Direct link to Add Metadata to alerts")
...
### Select specific alert types[​](#select-specific-alert-types "Direct link to Select specific alert types")

Set `alert_types` if you want to Opt into only specific alert types. When alert\_types is not set, all Default Alert Types are enabled.
...
### Map slack channels to alert type[​](#map-slack-channels-to-alert-type "Direct link to Map slack channels to alert type")
...
### MS Teams Webhooks[​](#ms-teams-webhooks "Direct link to MS Teams Webhooks")
...
### Discord Webhooks[​](#discord-webhooks "Direct link to Discord Webhooks")
...
## \[BETA] Webhooks for Budget Alerts[​](#beta-webhooks-for-budget-alerts "Direct link to [BETA] Webhooks for Budget Alerts")
...
### API Spec for Webhook Event[​](#api-spec-for-webhook-event "Direct link to API Spec for Webhook Event")

- `spend` *float*: The current spend amount for the 'event\_group'.
- `max_budget` *float or null*: The maximum allowed budget for the 'event\_group'. null if not set.
...
- `projected_exceeded_date` *str or null*: The date when the budget is projected to be exceeded, returned when 'soft\_budget' is set for key (optional).
- `projected_spend` *float or null*: The projected spend amount, returned when 'soft\_budget' is set for key (optional).
- `event` *Literal\["budget\_crossed", "threshold\_crossed", "projected\_limit\_exceeded"]*: The type of event that triggered the webhook. Possible values are:
  
  - "spend\_tracked": Emitted whenever spend is tracked for a customer id.
  - "budget\_crossed": Indicates that the spend has exceeded the max budget.
  - "threshold\_crossed": Indicates that spend has crossed a threshold (currently sent when 85% and 95% of budget is reached).
  - "projected\_limit\_exceeded": For "key" only - Indicates that the projected spend is expected to exceed the soft budget threshold.
- `event_group` *Literal\["customer", "internal\_user", "key", "team", "proxy"]*: The group associated with the event. Possible values are:
  
  - "customer": The event is related to a specific customer
  - "internal\_user": The event is related to a specific internal user.
...
## Region-outage alerting (✨ Enterprise feature)[​](#region-outage-alerting--enterprise-feature "Direct link to Region-outage alerting (✨ Enterprise feature)")
