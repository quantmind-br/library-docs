---
title: Images and icons in themes | Moodle Developer Resources
url: https://moodledev.io/docs/5.1/apis/plugintypes/theme/images
source: sitemap
fetched_at: 2026-02-17T15:35:43.828108-03:00
rendered_js: false
word_count: 502
summary: This document explains how to manage, reference, and override images within Moodle themes using specific directory structures, templates, and CSS placeholders. It provides instructions on utilizing theme-specific assets and replacing core or plugin images through naming conventions.
tags:
    - moodle-development
    - theme-customization
    - image-handling
...
One of the theme features is the ability to override any of the standard images within Moodle when your theme is in use. At this point, let's explore how to utilize your own images within your theme and how to override the images being used by Moodle. So first up a bit about images within Moodle:
...
4. All plugins should store their images within their own `pix` directory.
...
## Use images in templates[​](#use-images-in-templates "Direct link to Use images in templates")
...
## Use images in CSS[​](#use-images-in-css "Direct link to Use images in CSS")
...
## Override images[​](#override-images "Direct link to Override images")

From within a theme you can **very** easily override any standard image within Moodle by simply adding the replacement image to the theme's pix directory in the same sub directory structure as it is in Moodle. So, for instance, if there is a need to override the following images:

1. `/pix/moodlelogo.png`
2. `/pix/i/user.svg`
3. `/mod/chat/pix/monologo.svg`

Simply add the replacement images to the theme in the following locations:

1. `/theme/themename/pix_core/moodlelogo.png`
2. `/theme/themename/pix_core/i/user.svg`
3. `/theme/themename/pix_plugins/mod/chat/monologo.svg`
...
Another noteworthy aspect is that Moodle not only searches for replacements of the same image type (svg, png, jpg, gif, and so on) but also replacements in any image format. This is why the image file extension was never specified when working with our images above. This means that the following would also work:

1. `/theme/themename/pix_core/moodlelogo.svg`
2. `/theme/themename/pix_core/i/user.jpg`
