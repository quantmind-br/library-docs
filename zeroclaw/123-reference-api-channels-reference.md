...
url: https://github.com/openagen/zeroclaw/blob/master/docs/reference/api/channels-reference.md
source: git
fetched_at: 2026-05-02T14:51:54.688554546-03:00
rendered_js: false
word_count: 2031
summary: This document serves as the comprehensive configuration and integration reference for managing various communication channels within the ZeroClaw platform.
tags:
    - configuration
    - integration
    - channel-management
...
Canonical reference for channel configuration in ZeroClaw.

For encrypted Matrix rooms, also read [[010-matrix-e2ee-guide|Matrix E2EE Guide]].

## Quick Paths

- Full config reference by channel: jump to [[#4-per-channel-config-examples]]
- No-response diagnosis flow: jump to [[#6-troubleshooting-checklist]]
- Matrix encrypted-room help: use [[010-matrix-e2ee-guide|Matrix E2EE Guide]]
- Nextcloud Talk bot setup: use [[110-nextcloud-talk-setup]]
- Deployment/network assumptions (polling vs webhook): use [[105-network-deployment]]

## FAQ: Matrix setup passes but no reply

Most common symptom (same class as issue #499). Check in order:
...
See [[010-matrix-e2ee-guide|Matrix E2EE Guide]] for encrypted-room troubleshooting.
...
Replies automatically use the same protocol the sender used. The private key is encrypted at rest via the `SecretStore` when `secrets.encrypt = true` (the default).
...
- See [[110-nextcloud-talk-setup]] for a full runbook.
...
- [[010-matrix-e2ee-guide|Matrix E2EE Guide]]
...
#configuration #integration #channel-management #zeroclaw #troubleshooting #multimodal
