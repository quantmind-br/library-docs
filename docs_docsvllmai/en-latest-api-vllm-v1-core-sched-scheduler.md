...
url: https://docs.vllm.ai/en/latest/api/vllm/v1/core/sched/scheduler/
source: sitemap
fetched_at: 2026-05-07T21:40:26.334254956-03:00
rendered_js: false
word_count: 58
summary: This document defines the initialization logic and state management for the vLLM request scheduler, including configurations for KV cache, request queuing, and multimodal processing.
tags:
    - vllm
    - scheduler
    - kv-cache
...
```
classScheduler(SchedulerInterface):
    def__init__(
        self,
    ...
    defschedule(self) -> SchedulerOutput:
    ...
    def_build_kv_connector_meta(
        self, connector: KVConnectorBase_V1, scheduler_output: SchedulerOutput
    ) -> KVConnectorMetadata:
        return connector.build_connector_meta(scheduler_output)

    def_preempt_request(self, request: Request, timestamp: float) -> None:
"""Preempt a request and put it back to the waiting queue.
    ...
    def_update_after_schedule(self, scheduler_output: SchedulerOutput) -> None:
...
"""
    ...
    def_try_schedule_encoder_inputs(
        self,
        request: Request,
...
"""
    ...
    @staticmethod
    def_is_blocked_waiting_status(status: RequestStatus) -> bool:
        return status in (
    ...
    def_enqueue_waiting_request(self, request: Request) -> None:
        if self._is_blocked_waiting_status(request.status):
            self.skipped_waiting.add_request(request)
        else:
            self.waiting.add_request(request)

    def_select_waiting_queue_for_scheduling(self) -> RequestQueue | None:
    ...
    def_handle_stopped_request(self, request: Request) -> bool:
"""Return True if finished (can be False for resumable requests)."""
    ...
    def_update_request_with_output(
    ...
    def_free_encoder_inputs(self, request: Request) -> None:
    ...
    defupdate_draft_token_ids(self, draft_token_ids: DraftTokenIds) -> None:
    ...
    defupdate_draft_token_ids_in_output(
    ...
    defget_request_counts(self) -> tuple[int, int]:
...
"""Handles the finish signal from outside the scheduler.
    ...
    def_free_blocks(self, request: Request):
        assert request.is_finished()
        self.kv_cache_manager.free(request)
        del self.requests[request.request_id]

    @property
    defpause_state(self) -> PauseState:
        return self._pause_state

    defset_pause_state(self, pause_state: PauseState) -> None:
        self._pause_state = pause_state

    defget_num_unfinished_requests(self) -> int:
        if self._pause_state == PauseState.PAUSED_ALL:
            return 0
    ...
    defhas_finished_requests(self) -> bool:
...
"""Reset the KV prefix cache.
    ...
    defreset_connector_cache(self) -> bool:
...
"""Reset the encoder cache to invalidate all cached encoder outputs.
    ...
    defget_kv_connector(self) -> KVConnectorBase_V1 | None:
        return self.connector

    def_connector_finished(
...
"""
    ...
    def_update_waiting_for_remote_kv(self, request: Request) -> None:
"""
    ...
    def_try_promote_blocked_waiting_request(self, request: Request) -> bool:
"""
    ...
    def_update_from_kv_xfer_finished(self, kv_connector_output: KVConnectorOutput):
"""
    ...
    def_update_requests_with_invalid_blocks(
        self,
