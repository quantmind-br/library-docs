...
source: llms
fetched_at: 2026-01-24T14:11:27.868625398-03:00
rendered_js: false
word_count: 600
summary: This document explains how to set up a containerized development environment for RAG applications by configuring a vector database and an LLM service using Docker Compose.
tags:
    - docker
    - docker-compose
    - rag
...
2. In the `docker-compose.yaml` file, you'll see the following:
   ...
   ```
   services:qdrant:image:qdrant/qdrantcontainer_name:qdrantports:- "6333:6333"volumes:- qdrant_data:/qdrant/storage
   ```
...
2. The `docker-compose.yaml` file already contains the necessary instructions. In your own apps, you'll need to add the Ollama service in your `docker-compose.yaml`. The following is the updated `docker-compose.yaml`:
   ...
   ```
   ollama:image:ollama/ollamacontainer_name:ollamaports:- "8000:8000"deploy:resources:reservations:devices:- driver:nvidiacount:1capabilities:[gpu]
   ```
