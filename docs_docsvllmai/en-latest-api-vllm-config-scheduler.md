...
source: sitemap
fetched_at: 2026-05-07T21:17:12.985706171-03:00
rendered_js: false
word_count: 219
summary: This document defines the configuration parameters for a model scheduler, detailing settings for token batching, chunked prefill, scheduling policies, and KV cache management.
tags:
    - scheduler
    - configuration
    - llm-inference
...
"""
    ...
    defget_scheduler_cls(self) -> type["SchedulerInterface"]:
        if self.scheduler_cls is None:
            if self.async_scheduling:
                fromvllm.v1.core.sched.async_schedulerimport AsyncScheduler

                return AsyncScheduler
            fromvllm.v1.core.sched.schedulerimport Scheduler
...
"""Skip validation if the value is `None` when initialisation is delayed."""
    ...
    def__post_init__(self, max_model_len: int, is_encoder_decoder: bool) -> None:
    ...
    defverify_max_model_len(self, max_model_len: int) -> Self:
        if (
