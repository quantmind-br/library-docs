...
source: git
fetched_at: 2026-04-26T05:48:12.405436395-03:00
rendered_js: false
word_count: 11696
summary: Structure, capabilities, and lifecycle of pi extensions — TypeScript modules for custom tools, commands, UI components, and event listeners.
tags:
    - typescript
    - automation
    - extensibility
...
> [!tip] Pi can create extensions — ask it to build one for your use case.

# Extensions

TypeScript modules that extend pi's behavior via lifecycle events, custom tools, commands, and more.

> [!note] Auto-discovery: place extensions in `~/.pi/agent/extensions/` (global) or `.pi/extensions/` (project-local). Use `pi -e ./path.ts` only for quick tests. Auto-discovered extensions support `/reload`.

**Key capabilities:**
- **Custom tools** — `pi.registerTool()`: tools the LLM can call
...
- **Custom UI components** — `ctx.ui.custom()`: full TUI with keyboard input
- **Custom commands** — `pi.registerCommand()`: commands like `/mycommand`
- **Session persistence** — `pi.appendEntry()`: state that survives restarts
- **Custom rendering** — control how tool calls/results and messages appear in TUI

**Example use cases:** permission gates, git checkpointing, path protection, custom compaction, conversation summaries, interactive/stateful tools, external integrations, games.
...
> [!danger] Extensions run with your full system permissions and can execute arbitrary code. Only install from sources you trust.
...
To share extensions via npm or git as pi packages, see [[016-packages-coding-agent-docs-packages|Packages]].
...
npm dependencies work too — add a `package.json` next to your extension, run `npm install`, and imports from `node_modules/` resolve automatically.

> [!info] For distributed pi packages (`pi install`), runtime deps must be in `dependencies`. Uses `npm install --omit=dev` by default. When `npmCommand` is configured, git packages use plain `install`.
...
> [!note] If the factory returns a `Promise`, pi awaits it before startup. Async init completes before `session_start`, `resources_discover`, and provider registrations queued via `pi.registerProvider()`.
...
This pattern makes fetched models available during normal startup and to `pi --list-models`.
...
See [[025-packages-coding-agent-docs-session|Session]] for session storage internals and the SessionManager API.
...
After a successful switch/new-session action: `session_shutdown` (old) → reload/rebind → `session_start` with `reason: "new" | "resume"` and `previousSessionFile`. Cleanup in `session_shutdown`, reestablish state in `session_start`.
...
After a successful fork/clone: `session_shutdown` (old) → reload/rebind → `session_start` with `reason: "fork"` and `previousSessionFile`.

#### session_before_compact / session_compact

Fired on compaction. See [[008-packages-coding-agent-docs-compaction|Compaction]] for details.
...
Fired on `/tree` navigation. See [[019-packages-coding-agent-docs-tree|Tree]] for tree navigation concepts.
...
The `systemPromptOptions` field gives extensions access to the same structured data Pi uses to build the system prompt — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources.

Inside `before_agent_start`, `event.systemPrompt` and `ctx.getSystemPrompt()` both reflect the chained system prompt as of the current handler. Later handlers can still modify it.
...
Fired before each LLM call. Modify messages non-destructively. See [[025-packages-coding-agent-docs-session|Session]] for message types.
...
Mainly useful for debugging provider serialization and cache behavior.
...
> [!info] Header availability depends on provider and transport. Providers that abstract HTTP responses may not expose headers.
...
Use to update UI elements or perform model-specific initialization when the active model changes.
...
Register a keyboard shortcut. See [[010-packages-coding-agent-docs-keybindings|Keybindings]] for the shortcut format and built-in keybindings.
...
Set the current model. Returns `false` if no API key is available. See [[011-packages-coding-agent-docs-models|Models]] for configuring custom models.
...
Register or override a model provider dynamically — proxies, custom endpoints, team-wide configs.
...
See [[021-packages-coding-agent-docs-custom-provider|Custom Provider]] for advanced topics: custom streaming APIs, OAuth details, model definition reference.
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [[020-packages-coding-agent-docs-tui|TUI]] for the full component API and [tool-execution.ts](https://github.com/badlogic/pi-mono/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
For the exhaustive list of keybinding ids and defaults, see [[010-packages-coding-agent-docs-keybindings|Keybindings]]. `keybindings.json` uses those same namespaced ids.
...
**For custom components, see [[020-packages-coding-agent-docs-tui|TUI]]** which has copy-paste patterns for:
...
Custom working-indicator frames are rendered verbatim. Add colors with `ctx.ui.theme.fg(...)`.
...
See [[020-packages-coding-agent-docs-tui|TUI]] for the full component API.
...
See [[020-packages-coding-agent-docs-tui|TUI]] for the full `OverlayOptions` API and [overlay-qa-tests.ts](../examples/extensions/overlay-qa-tests.ts) for examples.
...
See [[020-packages-coding-agent-docs-tui|TUI]] Pattern 7 for a complete example with mode indicator.
...
All render functions receive a `theme` object. See [[018-packages-coding-agent-docs-themes|Themes]] for creating custom themes and the full color palette.
...
| RPC (`--mode rpc`) | JSON protocol | Host handles UI, see [[023-packages-coding-agent-docs-rpc|RPC]] |
| JSON (`--mode json`) | No-op | Event stream to stdout, see [[022-packages-coding-agent-docs-json|JSON]] |
