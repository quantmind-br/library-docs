...
url: https://moodledev.io/docs/4.5/apis/plugintypes/ai/provider
source: sitemap
fetched_at: 2026-02-17T15:11:40.820984-03:00
rendered_js: false
word_count: 641
summary: This document explains how to develop and implement AI Provider plugins for the Moodle AI subsystem, detailing the required class structures, processing logic, and configuration settings. It outlines the architectural relationship between Providers, Managers, and Placements to ensure consistent communication with external AI services.
tags:
    - moodle-development
    - ai-subsystem
    - provider-plugins
...
Providers are the interface between the [AI subsystem](https://moodledev.io/docs/4.5/apis/subsystems/ai) and external AI. Their focus should be on converting the data requested into the format needed by the external AI, and then correctly providing the response back.

Incoming data to the Provider plugin arrives via the Manager `core_ai\manager`. The Manager is the connective tissue between the Provider and the [Placement](https://moodledev.io/docs/4.5/apis/plugintypes/ai/placement) plugins. Likewise, all responses from the Provider plugin are handed back to the Manager before being passed to the Placement plugin.
...
Each Provider **must** inherit from the `\core_ai\provider` abstract class. They must also implement the following methods:
...
Settings for the Provider should be defined in the `settings.php` file. Each Provider plugin should create a new admin settings page using `core_ai\admin\admin_settingspage_provider` class.
...
'aiprovider_openai',
newlang_string('pluginname','aiprovider_openai'),
'moodle/site:config',
true,
);
...
...
To assist with this, the AI subsystem provides a `core_ai\rate_limiter` class that can be used to implement rate limiting. This class supports both user and system level rate limiting.

This should be implemented in a `is_request_allowed()` method in the Provider class. For example, from the `aiprovider_openai` plugin:
 ...
 */
publicfunctionis_request_allowed(aiactions\base$action):array|bool{
...
$settings->add(newadmin_setting_configcheckbox(
'aiprovider_openai/enableglobalratelimit',
newlang_string('enableglobalratelimit','aiprovider_openai'),
newlang_string('enableglobalratelimit_desc','aiprovider_openai'),
0,
));
...
$settings->add(newadmin_setting_configtext(
'aiprovider_openai/globalratelimit',
newlang_string('globalratelimit','aiprovider_openai'),
newlang_string('globalratelimit_desc','aiprovider_openai'),
100,
PARAM_INT,
));
...
newlang_string('enableuserratelimit','aiprovider_openai'),
newlang_string('enableuserratelimit_desc','aiprovider_openai'),
0,
));
...
newlang_string('userratelimit','aiprovider_openai'),
newlang_string('userratelimit_desc','aiprovider_openai'),
10,
PARAM_INT,
));
