---
title: late_interaction_runner - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/worker/gpu/pool/late_interaction_runner/
source: sitemap
fetched_at: 2026-05-07T21:42:41.462587748-03:00
rendered_js: false
word_count: 10
summary: This class manages worker-side caching and post-processing for late-interaction scoring, handling the lifecycle of query embeddings and computing max-similarity scores for document requests.
tags:
    - late-interaction
    - embedding-scoring
    - worker-side-caching
...
"""Worker-side state and postprocessing for late-interaction scoring."""
    ...
    defon_requests_finished(self, finished_req_ids: Iterable[str]) -> None:
        for req_id in finished_req_ids:
            query_key = self._doc_query_keys.pop(req_id, None)
            if query_key is not None:
                self._release_query_use(query_key)

    defpostprocess_pooler_output(
        self,
    ...
    def_release_query_use(self, query_key: str) -> None:
    ...
    @staticmethod
    def_parse_late_interaction_meta(
