---
title: Writing a new service | Moodle Developer Resources
url: https://moodledev.io/docs/4.1/apis/subsystems/external/writing-a-service
source: sitemap
fetched_at: 2026-02-17T14:56:22.832412-03:00
rendered_js: false
word_count: 1036
summary: This document explains how to create and define custom web services within Moodle plugins using the External functions API. It covers naming conventions, service registration in configuration files, and the implementation of parameter and return value structures in PHP.
tags:
    - moodle-development
    - web-services
    - external-functions
...
## Functional specification[​](#functional-specification "Direct link to Functional specification")
...
### Exceptions and failures[​](#exceptions-and-failures "Direct link to Exceptions and failures")

If *any* group creation fails, the function will throw an exception, and no groups will be created.

## Technical specification[​](#technical-specification "Direct link to Technical specification")
...
## Declare the web service function[​](#declare-the-web-service-function "Direct link to Declare the web service function")
...
### Defining parameters[​](#defining-parameters "Direct link to Defining parameters")
...
classcreate_groupsextends\external_api{

/**
     * Returns description of method parameters
     * @return external_function_parameters
     */
publicstaticfunctionexecute_parameters(){
returnnewexternal_function_parameters([
'groups'=>newexternal_multiple_structure(
newexternal_single_structure([
'courseid'=>newexternal_value(PARAM_INT,'id of course'),
...
'enrolmentkey'=>newexternal_value(
PARAM_RAW,
...
An external\_multiple\_structure object (list) can be constructed with:
...
Each group values is a *external\_value* (primary type):
...
'courseid'=>newexternal_value(PARAM_INT,'id of course'),
'name'=>newexternal_value(PARAM_TEXT,'multilang compatible name, course unique'),
'description'=>newexternal_value(PARAM_RAW,'group description text'),
'enrolmentkey'=>newexternal_value(PARAM_RAW,'group enrol secret phrase'),
])
)
```

### execute\_returns()[​](#execute_returns "Direct link to execute_returns()")

It's similar to execute\_parameters(), but instead of describing the parameters, it describes the return values.

```
publicstaticfunctionexecute_returns(){
returnnewexternal_multiple_structure(
newexternal_single_structure([
'id'=>newexternal_value(PARAM_INT,'group record id'),
'courseid'=>newexternal_value(PARAM_INT,'id of course'),
'name'=>newexternal_value(PARAM_TEXT,'multilang compatible name, course unique'),
'description'=>newexternal_value(PARAM_RAW,'group description text'),
'enrolmentkey'=>newexternal_value(PARAM_RAW,'group enrol secret phrase'),
...
### Required, Optional or Default value[​](#required-optional-or-default-value "Direct link to Required, Optional or Default value")

A value can be `VALUE_REQUIRED`, `VALUE_OPTIONAL`, or `VALUE_DEFAULT`. If not mentioned, a value is `VALUE_REQUIRED` by default.

```
'yearofstudy'=>newexternal_value(PARAM_INT,'year of study',VALUE_DEFAULT,1979),
```

- `VALUE_REQUIRED` - if the value is not supplied =&gt; the server throws an error message
- `VALUE_OPTIONAL` - if the value is not supplied =&gt; the value is ignored. Note that VALUE\_OPTIONAL can't be used in top level parameters, it must be used only within array/objects key definition. If you need top level Optional parameters you should use VALUE\_DEFAULT instead.
...
```
publicstaticfunctionget_biscuit_parameters(){
returnnewexternal_function_parameters([
'chocolatechips'=>newexternal_value(PARAM_BOOL,PARAM_REQUIRED),
'glutenfree'=>newexternal_value(PARAM_BOOL,PARAM_DEFAULT,false),
// ERROR! top level optional parameter!!!
'icingsugar'=>newexternal_value(PARAM_BOOL,VALUE_OPTIONAL),
]);
}
```
...
publicstaticfunctionget_biscuit_parameters(){
returnnewexternal_function_parameters([
'ifeellike'=>newexternal_single_structure([
'chocolatechips'=>newexternal_value(PARAM_BOOL,VALUE_REQUIRED),
'glutenfree'=>newexternal_value(PARAM_BOOL,PARAM_DEFAULT,false),
...
## Implement the external function[​](#implement-the-external-function "Direct link to Implement the external function")
...
/**
     ...
     */
publicstaticfunctionexecute($groups){
global$CFG,$DB;
...
if(trim($group->name)==''){
thrownewinvalid_parameter_exception('Invalid group name');
}
if($DB->get_record('groups',['courseid'=>$group->courseid,'name'=>$group->name])){
thrownewinvalid_parameter_exception('Group with the same name already exists in the course');
}
...
This *validate\_parameters* function validates the external function parameters against the description. It will return an exception if some required parameters are missing, if parameters are not well-formed, and check the parameters validity. It is essential that you do this call to avoid potential hack.
...
### Context and Capability checks[​](#context-and-capability-checks "Direct link to Context and Capability checks")
...
Note: validate\_context() is required in all external functions before operating on any data belonging to a context. This function does sanity and security checks on the context that was passed to the external function - and sets up the global $PAGE and $OUTPUT for rendering return values. Do NOT use require\_login(), or $PAGE-&gt;set\_context() in an external function.
...
### Correct return values[​](#correct-return-values "Direct link to Correct return values")
...
- return values miss some required values (VALUE\_REQUIRED) =&gt; the server will return an error.
- return values types don't match the description (int != PARAM\_ALPHA) =&gt; the server will return an error **Note:** cast all your returned objects into arrays.

## Bump the plugin version[​](#bump-the-plugin-version "Direct link to Bump the plugin version")
...
/**
     ...
     * @return bool
     */
publicstaticfunctionexecute_is_deprecated(){
returntrue;
}
```
...
- [Web services developer documentation](https://moodledev.io/docs/4.1/apis/subsystems/external)
...
- Code example: [Adding a web service, using APIs](https://gist.github.com/timhunt/51987ad386faca61fe013904c242e9b4) by (Tim Hunt) Specification:
- [External services security](https://moodledev.io/docs/4.1/apis/subsystems/external/security)
- [External services description](https://moodledev.io/docs/4.1/apis/subsystems/external/description)
