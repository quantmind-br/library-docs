---
title: triton_turboquant_store - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/attention/ops/triton_turboquant_store/
source: sitemap
fetched_at: 2026-05-07T21:40:09.233597727-03:00
rendered_js: false
word_count: 107
summary: This document describes Triton kernels designed for high-performance KV cache storage, implementing FP8 key casting and uniform value quantization for the TurboQuant system.
tags:
    - triton-kernels
    - kv-cache
    - quantization
...
Two kernels: 1. \_tq\_fused\_store\_fp8: FP8 key scatter + value uniform quantization. 2. \_tq\_fused\_store\_mse: Fused binary-search bucketize + MSE index packing + value quantization.

The launcher `triton_turboquant_store` selects the appropriate kernel.

## \_store\_quantized\_value [¶](#vllm.v1.attention.ops.triton_turboquant_store._store_quantized_value "Permanent link")
...
Source code in `vllm/v1/attention/ops/triton_turboquant_store.py`

```
@triton.jit
def_store_quantized_value(
    Value_ptr,
    KV_cache_ptr,
...
## \_tq\_fused\_store\_fp8 [¶](#vllm.v1.attention.ops.triton_turboquant_store._tq_fused_store_fp8 "Permanent link")
...
_tq_fused_store_fp8(
    ...
    KV_cache_ptr,
    Slot_mapping_ptr,
    stride_cache_block: constexpr,
    stride_cache_pos: constexpr,
    stride_cache_head: constexpr,
    D: constexpr,
    H: constexpr,
    ...
    VAL_DATA_BYTES: constexpr,
    BLOCK_VAL: constexpr,
    BLOCK_GRP: constexpr = 16,
    FP8_E4B15: constexpr = 0,
)
```
...
## \_tq\_fused\_store\_mse [¶](#vllm.v1.attention.ops.triton_turboquant_store._tq_fused_store_mse "Permanent link")
...
Performs binary-search bucketize, MSE index packing, norm storage, and value quantization in one kernel.

Source code in `vllm/v1/attention/ops/triton_turboquant_store.py`

```
@triton.jit
def_tq_fused_store_mse(
