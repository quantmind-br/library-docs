---
title: Integration review | Moodle Developer Resources
url: https://moodledev.io/general/development/process/integration
source: sitemap
fetched_at: 2026-02-17T16:00:08.154839-03:00
rendered_js: false
word_count: 2494
summary: This document outlines the philosophical principles and technical procedures for the Moodle integration review process to ensure high-quality, secure code contributions. It details the criteria for backporting fixes, maintaining backwards compatibility, and following the weekly integration schedule.
tags:
    - moodle-development
    - integration-review
    - code-quality
...
## Integration Principles[​](#integration-principles "Direct link to Integration Principles")
...
## Component lead review process[​](#component-lead-review-process "Direct link to Component lead review process")

Please see the [Component Lead Review](https://moodledev.io/general/development/process/integration/clr) documentation for further information on the CLR process.

## Integration Review Process[​](#integration-review-process "Direct link to Integration Review Process")

01. Run automated pre-checks against the continuous integration server. (In future this can be automated and also moved into a publicly accessible domain.)
02. Final code review, much like the peer review, except that this is the final check. To include
    ...
    5. Security
03. Check purpose - the patch needs to fix the issue reported.
04. Verify target branches are appropriated. They must match 100% the rules defined for [backporting issues](https://moodledev.io/general/development/policies/backporting).
05. Ensure backwards compatibility is maintained. As a starting point backwards compatibility must always be maintained. Where backwards compatibility is affected it should be:
    
    1. Well discussed with evidence of justification
    2. Documented and communicated to the community
06. Ensure backwards compatibility with the Moodle mobile app. Especially in areas where the Moodle app uses pre-rendered content from the site (like Quiz or Lesson)
07. Verify that components are correct and check the right people have been involved (e.g. component maintainers)
08. For fundamental changes, check that a thread has been started in an appropriate forum, and other Moodlers, given enough time to comment.
09. Tests - must be written to guide tester to verify the fix is working.
...
10. Performance - we have to look at maintaining optimum code here, as far as simple patches that can affect performance. (simple optimisations)
...
14. [Tracker issue labels](https://moodledev.io/general/development/tracker/labels) which might need adding. Particularly:
...
### In normal periods[​](#in-normal-periods "Direct link to In normal periods")
...
- Monday to Thursday until [12:00 (UTC+8)](http://time.unitarium.com/utc/4): Integration and [Testing](https://moodledev.io/general/development/process/testing/integrated-issues#the-testing-process) happen. Note that 24h before the cutoff it's possible to pick issues out of order towards queues reduction.
...
Note that under the strict schedule above, it is specially important **to be as responsive as possible**, both when the issue is being integrated and when [testing](https://moodledev.io/general/development/process/testing/integrated-issues#expectation-from-tester). Any significant delay by any of the actors involved will result in the issue being moved out from current integration.

### During continuous integration/Freeze/QA period[​](#during-continuous-integrationfreezeqa-period "Direct link to During continuous integration/Freeze/QA period")
...
- Issues are picked on a one by one basis, prioritising [QA blockers](https://moodledev.io/general/development/process/testing/qa#resetting-tests) and main regressions (MUST FIX) issues.
...
- Monday: Integration and [testing](https://moodledev.io/general/development/process/testing/integrated-issues#differences-in-test-process-during-continuous-integration-periods) happens.
- Tuesday: Integration happens until [12:00 (UTC+8)](http://time.unitarium.com/utc/4), afterwards we try to [achieve 100% 'Test Passed'](https://moodledev.io/general/development/process/testing/integrated-issues#differences-in-test-process-during-continuous-integration-periods) and stop integrating any untested changes until a main release is produced.
...
More information about the last week before the release
...
### On-sync period[​](#on-sync-period "Direct link to On-sync period")
...
- The **environmental requirements for next major version** [must be agreed and resolved so they can land to main](https://moodledev.io/general/development/process/release#2-weeks-after) early in the process, remaining defined and stable over the next, 6 months of, development cycle. As part of the standard [Moodle release process](https://moodledev.io/general/development/process/release), at the beginning of the on-sync period, we "unhold" all bugs that were held during the last week before the release because they were unrelated to the release. At the end of the onsync period we "unhold" all new features that were submitted after the code freeze for the the release.

## Fixing issues identified during integration review/ testing[​](#fixing-issues-identified-during-integration-review-testing "Direct link to Fixing issues identified during integration review/ testing")
...
## Commit squashing[​](#commit-squashing "Direct link to Commit squashing")
...
You should pay close attention to [Git\_commits](https://moodledev.io/general/development/policies/codingstyle#git-commits) and intend to *"Tell a perfect, cleaned up version of the history. As if the code was written perfectly first time."*.
...
Considerations for your code review repository
...
### Useful configuration[​](#useful-configuration "Direct link to Useful configuration")
