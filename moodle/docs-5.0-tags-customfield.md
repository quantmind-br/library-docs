---
title: 2 docs tagged with "customfield" | Moodle Developer Resources
url: https://moodledev.io/docs/5.0/tags/customfield
source: sitemap
fetched_at: 2026-02-17T15:17:50.768252-03:00
rendered_js: false
word_count: 93
summary: This document introduces the Moodle Custom fields API and plugin type, detailing how developers can create and implement custom field types across various areas such as courses.
tags:
    - moodle-development
    - custom-fields
    - api-overview
    - plugin-types
    - field-types
category: api
---

## [Custom fields](https://moodledev.io/docs/5.0/apis/plugintypes/customfield)

Custom fields allow you to create field types to be used for custom fields. Instances of these field types can be added to the respective areas that implement Custom fields API. Currently in Moodle core only courses implement this API, however custom fields are also used in addon plugins for other areas. For example, if you want to display radio buttons on the course edit page, then you can add an instance of a radio custom field plugin to the Course custom fields configuration.

## [Custom fields API](https://moodledev.io/docs/5.0/apis/core/customfields)

Custom fields API overview