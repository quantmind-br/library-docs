---
title: reuse_manager - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/kv_offload/reuse_manager/
source: sitemap
fetched_at: 2026-05-07T21:41:06.475701078-03:00
rendered_js: false
word_count: 252
summary: This document describes the FilterReusedOffloadingManager, a decorator class that optimizes CPU KV-cache offloading by only storing blocks that meet a minimum reuse frequency threshold.
tags:
    - kv-cache
    - offloading
    - memory-management
...
## FilterReusedOffloadingManager [¶](#vllm.v1.kv_offload.reuse_manager.FilterReusedOffloadingManager "Permanent link")

Bases: `OffloadingManager`

An :class:`OffloadingManager` decorator that skips storing blocks whose reuse frequency is below *store\_threshold*.
...
- `lookup` — records the visited key in an internal LRU counter, then delegates to the backing manager. crossed the threshold *before* calling the backing `prepare_store`.

Parameters:

Name Type Description Default `backing` `OffloadingManager`

The underlying `OffloadingManager` to delegate to.

*required* `store_threshold` `int`

A block must be seen at least this many times in `lookup()` before it is eligible for offloading. Must be &gt;= 2 (a value of 1 would be equivalent to no filtering).

`2` `max_tracker_size` `int`
...
```
classFilterReusedOffloadingManager(OffloadingManager):
...
### lookup [¶](#vllm.v1.kv_offload.reuse_manager.FilterReusedOffloadingManager.lookup "Permanent link")
...
### prepare\_store [¶](#vllm.v1.kv_offload.reuse_manager.FilterReusedOffloadingManager.prepare_store "Permanent link")
