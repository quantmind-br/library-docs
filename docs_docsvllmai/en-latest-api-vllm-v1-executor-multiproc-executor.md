---
title: multiproc_executor - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/executor/multiproc_executor/
source: sitemap
fetched_at: 2026-05-07T21:40:46.484123551-03:00
rendered_js: false
word_count: 329
summary: The MultiprocExecutor class manages distributed model execution in vLLM by orchestrating multiple worker processes, handling inter-process communication via message queues, and monitoring worker health.
tags:
...
## MultiprocExecutor [¶](#vllm.v1.executor.multiproc_executor.MultiprocExecutor "Permanent link")
...
```
classMultiprocExecutor(Executor):
    supports_pp: bool = True

    def__init__(self, vllm_config: VllmConfig, monitor_workers: bool = True):
    ...
    def_get_parallel_sizes(self) -> tuple[int, int, int]:
    ...
    def_post_init_executor(self) -> None:
        pass

    def_is_driver_worker(self, rank: int) -> bool:
    ...
    defregister_failure_callback(self, callback: FailureCallback):
        if self.is_failed:
            callback()
        else:
...
"""Returns single result if unique_reply_rank and/or kv_output_aggregator
        ...
        defget_response():
            ...
            for mq in response_mqs:
                ...
                try:
                    ...
                    raise TimeoutError(f"RPC call to {method} timed out.") frome
    ...
    @staticmethod
    def_ensure_worker_termination(worker_procs: list[BaseProcess]):
"""Ensure that all worker processes are terminated. Assumes workers have
        received termination requests. Waits for processing, then sends
        termination and kill signals if needed."""

        defwait_for_termination(procs, timeout):
            if not time:
...
### \_ensure\_worker\_termination `staticmethod` [¶](#vllm.v1.executor.multiproc_executor.MultiprocExecutor._ensure_worker_termination "Permanent link")
...
### collective\_rpc [¶](#vllm.v1.executor.multiproc_executor.MultiprocExecutor.collective_rpc "Permanent link")

Returns single result if unique\_reply\_rank and/or kv\_output\_aggregator is provided, otherwise list.
...
### shutdown [¶](#vllm.v1.executor.multiproc_executor.MultiprocExecutor.shutdown "Permanent link")
...
## UnreadyWorkerProcHandle `dataclass` [¶](#vllm.v1.executor.multiproc_executor.UnreadyWorkerProcHandle "Permanent link")
...
## WorkerProc [¶](#vllm.v1.executor.multiproc_executor.WorkerProc "Permanent link")
...
"""Wrapper that runs one Worker in a separate process."""
    ...
    @staticmethod
    defwait_for_response_handle_ready(
    ...
    defmonitor_death_pipe(self, death_pipe, shutdown_requested: threading.Event):
        if death_pipe is None:
            return

        defdeath_pipe_monitor(queues_to_shutdown: list[MessageQueue]):
            try:
...
"""Worker initialization and execution loops.
    ...
    classResponseStatus(Enum):
        SUCCESS = auto()
        FAILURE = auto()

    defenqueue_output(self, output: Any):
"""Prepares output from the worker and enqueues it to the
    ...
    defhandle_output(self, output: Any):
"""Handles output from the worker. If async scheduling is enabled,
    ...
    defasync_output_busy_loop(self):
...
"""Main busy loop for Multiprocessing Workers"""
    ...
    @staticmethod
    defsetup_proc_title_and_log_prefix(enable_ep: bool) -> None:
...
### async\_output\_busy\_loop [¶](#vllm.v1.executor.multiproc_executor.WorkerProc.async_output_busy_loop "Permanent link")
...
### enqueue\_output [¶](#vllm.v1.executor.multiproc_executor.WorkerProc.enqueue_output "Permanent link")

```
enqueue_output(output: Any)
```

Prepares output from the worker and enqueues it to the worker\_response\_mq. If the output is an Exception, it is converted to a FAILURE response.
...
### handle\_output [¶](#vllm.v1.executor.multiproc_executor.WorkerProc.handle_output "Permanent link")

```
handle_output(output: Any)
```

Handles output from the worker. If async scheduling is enabled, it is passed to the async\_output\_busy\_loop thread. Otherwise, it is enqueued directly to the worker\_response\_mq.
...
### worker\_busy\_loop [¶](#vllm.v1.executor.multiproc_executor.WorkerProc.worker_busy_loop "Permanent link")
...
### worker\_main `staticmethod` [¶](#vllm.v1.executor.multiproc_executor.WorkerProc.worker_main "Permanent link")
...
## set\_multiprocessing\_worker\_envs [¶](#vllm.v1.executor.multiproc_executor.set_multiprocessing_worker_envs "Permanent link")
...
```
defset_multiprocessing_worker_envs():
