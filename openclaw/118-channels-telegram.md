...
source: sitemap
fetched_at: 2026-01-30T20:36:27.098578856-03:00
rendered_js: false
word_count: 3122
summary: This document provides comprehensive guidance on setting up and configuring Telegram bot integration using the Gateway, covering token management, privacy settings, group policies, and troubleshooting.
tags:
    - telegram-bot-api
    - bot-setup
    - configuration
...
Telegram distinguishes **voice notes** (round bubble) from **audio files** (metadata card). OpenClaw defaults to audio files for backward compatibility. To force a voice note bubble in agent replies, include this tag anywhere in the reply:
...
Telegram can stream **draft bubbles** while the agent is generating a response. OpenClaw uses Bot API `sendMessageDraft` (not real messages) and then sends the final reply as a normal message. Requirements (Telegram Bot API 9.3+):
...
- If `channels.telegram.streamMode` is `off`, reasoning stream is disabled. More context: [Streaming + chunking](https://docs.openclaw.ai/concepts/streaming).
