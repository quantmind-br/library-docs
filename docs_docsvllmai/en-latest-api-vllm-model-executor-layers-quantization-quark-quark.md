---
title: quark - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/quantization/quark/quark/
source: sitemap
fetched_at: 2026-05-07T21:27:32.40921548-03:00
rendered_js: false
word_count: 0
summary: This document defines the QuarkConfig class, which manages quantization configuration settings, layer-specific method resolution, and vLLM model structure mapping for model quantization.
tags:
    - quantization
    - vllm
    - model-config
...
```
classQuarkConfig(QuantizationConfig):
    def__init__(
        self,
    ...
    defget_linear_method(self) -> "QuarkLinearMethod":
...
"""
    ...
    @classmethod
    deffrom_config(cls, config: dict[str, Any]) -> "QuarkConfig":
    ...
    def_check_scheme_supported(self, min_capability: int, error: bool = True) -> bool:
...
"""
    ...
    def_find_matched_config(
        ...
        if proj_name in self.packed_modules_mapping:
            ...
            def_matches_pattern(layer_name, pattern):
                if "*" not in pattern:
