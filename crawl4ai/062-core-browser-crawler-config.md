...
source: sitemap
fetched_at: 2026-04-26T07:46:52.672531477-03:00
rendered_js: false
word_count: 1619
summary: This document provides a comprehensive overview of the BrowserConfig and CrawlerRunConfig classes in Crawl4AI, explaining how to configure browser behavior, crawler execution, and class-level defaults.
tags:
    - crawl4ai
    - web-scraping
...
Method Description `set_defaults(**kwargs)` Set class-level defaults. Invalid parameter names raise `ValueError`. `get_defaults()` Return a copy of the current class-level defaults. `reset_defaults()` Clear all class-level defaults. `reset_defaults("param1", "param2")` Clear only the named defaults.
