---
title: nvfp4_emulation_utils - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/quantization/utils/nvfp4_emulation_utils/
source: sitemap
fetched_at: 2026-05-07T21:28:06.790769543-03:00
rendered_js: false
word_count: 251
summary: This document describes Triton kernels and helper functions designed for NVFP4 quantization and dequantization, including optimized 2D tiling and E2M1 float conversion logic.
tags:
    - triton-kernel
    - nvfp4
    - quantization
...
## \_dequantize\_nvfp4\_kernel [¶](#vllm.model_executor.layers.quantization.utils.nvfp4_emulation_utils._dequantize_nvfp4_kernel "Permanent link")
...
## \_e2m1\_inline [¶](#vllm.model_executor.layers.quantization.utils.nvfp4_emulation_utils._e2m1_inline "Permanent link")
...
## \_e2m1\_lookup [¶](#vllm.model_executor.layers.quantization.utils.nvfp4_emulation_utils._e2m1_lookup "Permanent link")
...
## \_nvfp4\_quant\_dequant\_kernel [¶](#vllm.model_executor.layers.quantization.utils.nvfp4_emulation_utils._nvfp4_quant_dequant_kernel "Permanent link")
...
Uses a 2D grid (rows x tiles) to parallelize across both rows and quantization groups within a row. Each program handles TILE\_BLOCKS groups at once using vectorized 2D operations.

Source code in `vllm/model_executor/layers/quantization/utils/nvfp4_emulation_utils.py`

```
@triton.jit
def_nvfp4_quant_dequant_kernel(
    input_ptr,
    output_ptr,
    global_scale_ptr,
    k: tl.constexpr,
...
## \_round\_to\_fp4 [¶](#vllm.model_executor.layers.quantization.utils.nvfp4_emulation_utils._round_to_fp4 "Permanent link")
...
## \_triton\_dequantize\_nvfp4 [¶](#vllm.model_executor.layers.quantization.utils.nvfp4_emulation_utils._triton_dequantize_nvfp4 "Permanent link")

Dequantize NVFP4 using Triton (swizzle=False only).

Supports both 2D and 3D inputs: - 2D: \[m, packed\_k] -&gt; \[m, k] - 3D: \[dim0, m, packed\_k] -&gt; \[dim0, m, k]

Source code in `vllm/model_executor/layers/quantization/utils/nvfp4_emulation_utils.py`

```
def_triton_dequantize_nvfp4(
...
## \_triton\_nvfp4\_quant\_dequant [¶](#vllm.model_executor.layers.quantization.utils.nvfp4_emulation_utils._triton_nvfp4_quant_dequant "Permanent link")
...
```
def_triton_nvfp4_quant_dequant(
    x: torch.Tensor,
    global_scale: torch.Tensor,
    block_size: int,
) -> torch.Tensor:
...
## dequantize\_to\_dtype [¶](#vllm.model_executor.layers.quantization.utils.nvfp4_emulation_utils.dequantize_to_dtype "Permanent link")
...
## ref\_nvfp4\_quant\_dequant [¶](#vllm.model_executor.layers.quantization.utils.nvfp4_emulation_utils.ref_nvfp4_quant_dequant "Permanent link")
