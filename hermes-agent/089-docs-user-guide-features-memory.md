...
source: crawler
fetched_at: 2026-04-24T17:00:02.961579415-03:00
rendered_js: false
word_count: 1002
summary: This document details the persistent memory system of the Hermes Agent, explaining how its two memory files (`MEMORY.md` and `USER.md`) are structured, injected into the session prompt, and managed via `add`, `replace`, and `remove` tools.
tags:
    - memory-system
    - agent-context
    - hermes-agent
...
FilePurposeChar Limit**MEMORY.md**Agent's personal notes — environment facts, conventions, things learned2,200 chars (~800 tokens)**USER.md**User profile — your preferences, communication style, expectations1,375 chars (~500 tokens)
...
StoreLimitTypical entriesmemory2,200 chars8-15 entriesuser1,375 chars5-10 entries
...
FeaturePersistent MemorySession Search**Capacity**~1,300 tokens totalUnlimited (all sessions)**Speed**Instant (in system prompt)Requires search + LLM summarization**Use case**Key facts always availableFinding specific past conversations**Management**Manually curated by agentAutomatic — all sessions stored**Token cost**Fixed per session (~1,300 tokens)On-demand (searched when needed)
