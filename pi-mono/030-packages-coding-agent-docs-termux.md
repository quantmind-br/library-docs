...
source: git
fetched_at: 2026-05-03T09:31:26.348595453-03:00
rendered_js: false
word_count: 166
summary: Install and configure the Pi coding agent within the Termux Linux environment on Android devices.
tags:
    - termux
    - android
    - linux-environment
...
> [!warning]
> Image clipboard is not supported on Termux (`ctrl+v` image paste will not work).
...
- Use `pkg install termux-api` for the CLI tools
...
| No native binaries | Optional native deps (clipboard module) unavailable on Android ARM64 |
| Storage access | Run `termux-setup-storage` once to grant permissions for `/storage/emulated/0` |

## Troubleshooting

### Clipboard not working
Ensure both apps are installed (Termux + Termux:API from GitHub or F-Droid), then:
```bash
