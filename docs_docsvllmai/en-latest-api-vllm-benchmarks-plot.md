---
title: plot - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/benchmarks/plot/
source: sitemap
fetched_at: 2026-05-07T21:15:54.816766759-03:00
rendered_js: false
word_count: 258
summary: This document describes utility functions for processing benchmark request data and generating visualization plots, including timeline segments and dataset token distribution statistics.
tags:
    - benchmarking
    - data-visualization
    - vllm
...
## construct\_timeline\_data [¶](#vllm.benchmarks.plot.construct_timeline_data "Permanent link")

Construct timeline data from request results.

Parameters:

Name Type Description Default `requests_data` `list[dict[str, Any]]`

List of per-request result dictionaries

*required* `itl_thresholds` `list[float]`

ITL thresholds in seconds

*required* `labels` `list[str]`

Labels for ITL categories
...
## generate\_dataset\_stats\_plot [¶](#vllm.benchmarks.plot.generate_dataset_stats_plot "Permanent link")

Generate a matplotlib figure with dataset statistics.

Creates a figure with 4 subplots: - Top-left: Prompt tokens distribution (histogram) - Top-right: Output tokens distribution (histogram) - Bottom-left: Prompt+output tokens distribution (histogram) - Bottom-right: Stacked bar chart (request\_id vs tokens)

Parameters:

Name Type Description Default `results` `list[dict[str, Any]]`

List of per-request result dictionaries containing: - prompt\_len: Number of prompt tokens - output\_tokens: Number of output tokens
...
```
defgenerate_dataset_stats_plot(
...
## generate\_timeline\_plot [¶](#vllm.benchmarks.plot.generate_timeline_plot "Permanent link")
...
List of per-request result dictionaries containing: - start\_time: Request start time (seconds) - ttft: Time to first token (seconds) - itl: List of inter-token latencies (seconds) - latency: Total request latency (seconds) - prompt\_len: Number of prompt tokens - output\_tokens: Number of output tokens

*required* `output_path` `Path`

Path where the HTML file will be saved

*required* `colors` `list[str] | None`

List of colors for ITL categories (default: green, orange, red, black)

`None` `itl_thresholds` `list[float] | None`

ITL thresholds in seconds (default: \[1.0, 4.0, 6.0])

`None` `labels` `list[str] | None`
