...
source: sitemap
fetched_at: 2026-05-07T21:12:13.924286525-03:00
rendered_js: false
word_count: 852
summary: This document explains the Dual Batch Overlap (DBO) system in vLLM, which improves performance by interleaving compute and communication operations during MoE layer execution using microbatching.
tags:
    - vllm
    - dual-batch-overlap
    - mixture-of-experts
...
The [`UBatchContext`](https://docs.vllm.ai/en/latest/api/vllm/v1/worker/ubatching/#vllm.v1.worker.ubatching.UBatchContext "            UBatchContext") class is a `ForwardContext` wrapper class that is used by the `UBatchWrapper` class to synchronize the two UBatch threads. It should only be instantiated by using `make_ubatch_contexts`.
