---
title: Report builder API | Moodle Developer Resources
url: https://moodledev.io/docs/5.2/apis/core/reportbuilder
source: sitemap
fetched_at: 2026-02-17T15:43:43.284224-03:00
rendered_js: false
word_count: 2129
summary: This document explains the technical implementation of the Report Builder API, which provides a standardized framework for creating reports through columns, filters, and reusable entities. It details how to define data retrieval, formatting, and filtering logic using specific PHP classes and SQL fragments.
tags:
    - moodle-api
    - report-builder
    - data-entities
...
#### Column overview[​](#column-overview "Direct link to Column overview")
...
#### Creating columns[​](#creating-columns "Direct link to Creating columns")
...
Example of code for creating a column

```
$columns[]=(newcolumn(
'starttime',
newlang_string('task_starttime','admin'),
...
#### Filter overview[​](#filter-overview "Direct link to Filter overview")
...
- The type of filtering being performed (int, text, datetime, etc). Filter types are extendable, allowing for the addition of many more as suit each use case. We have provided common ones that cover most use cases.
...
- **Boolean Select** ([reportbuilder/classes/local/filters/boolean\_select.php](https://github.com/moodle/moodle/blob/main/reportbuilder/classes/local/filters/boolean_select.php))
...
- **Course selector** ([reportbuilder/classes/local/filters/course\_selector.php](https://github.com/moodle/moodle/blob/main/reportbuilder/classes/local/filters/course_selector.php))
...
#### Creating filters[​](#creating-filters "Direct link to Creating filters")
...
Example of code for creating a filter

```
$filters[]=(newfilter(
course_selector::class,
'courseselector',
newlang_string('courses'),
$this->get_entity_name(),
"{$tablealias}.id"
))
->add_joins($this->get_joins());
...
#### Entity overview[​](#entity-overview "Direct link to Entity overview")
...
#### Create an entity[​](#create-an-entity "Direct link to Create an entity")
...
##### get\_default\_tables()[​](#get_default_tables "Direct link to get_default_tables()")

Defines all the database tables that must be present in the main SQL or joins added to the entity.

##### get\_default\_entity\_title()[​](#get_default_entity_title "Direct link to get_default_entity_title()")

Defines the default title for this entity.

##### initialise()[​](#initialise "Direct link to initialise()")
...
Adding entity joins to a column
...
Example of getting the alias for a table
...
![Example of actions on the tasks logs system report](https://moodledev.io/assets/images/Actions-c0c8d1edca9686c818ebf90ab9fff10c.jpg)
...
newmoodle_url('/admin/tasklogs.php',['logid'=>':id']),
newpix_icon('e/search',''),
[],
true,
newlang_string('viewtasklog','report_tasklogs')
)));
```

## System reports[​](#system-reports "Direct link to System reports")
...
### Create a new system report using entities[​](#create-a-new-system-report-using-entities "Direct link to Create a new system report using entities")

To create a new system report just create a new class extending [reportbuilder/classes/system\_report.php](https://github.com/moodle/moodle/blob/main/reportbuilder/classes/system_report.php).
...
*/
protectedfunctioninitialise():void{
```
...
$this->add_action((newaction(
newmoodle_url('/admin/tasklogs.php',['logid'=>':id','download'=>true]),
newpix_icon('t/download',''),
[],
newlang_string('downloadtasklog','report_tasklogs')
)));
```

info

Note that the placeholders used here (:id in this example) have been previously added using **add\_base\_fields**();
...
### Use an entity[​](#use-an-entity "Direct link to Use an entity")

### Override display name for a column[​](#override-display-name-for-a-column "Direct link to Override display name for a column")
...
$column->set_title(newlang_string('user','admin'));
}
```

### Set a default initial sort direction[​](#set-a-default-initial-sort-direction "Direct link to Set a default initial sort direction")
...
### Create a new data source using entities[​](#create-a-new-data-source-using-entities "Direct link to Create a new data source using entities")
...
$entitycategory=newcourse_category();
...
### Setup report name[​](#setup-report-name "Direct link to Setup report name")
...
/**
     ...
     * @return string
     */
publicstaticfunctionget_name():string{
returnget_string('tasklogs','core_admin');
}
```

### Setup default columns[​](#setup-default-columns "Direct link to Setup default columns")
 ...
 * @return string[]
 */
publicfunctionget_default_columns():array{
return[
'task_log:name',
'task_log:starttime',
'task_log:duration',
 ...
 * @return int[]
 */
publicfunctionget_default_column_sorting():array{
return[
...
### Setup default filters[​](#setup-default-filters "Direct link to Setup default filters")
 ...
 * @return string[]
 */
publicfunctionget_default_filters():array{
return[
'task_log:timestart',
'task_log:result',
];
...
### Setup conditions[​](#setup-conditions "Direct link to Setup conditions")
 ...
 * @return string[]
 */
publicfunctionget_default_conditions():array{
return[
'task_log:type',
'task_log:timestart',
'task_log:result',
...
You may also optionally define the [initial values for any of the default conditions](https://moodledev.io/docs/5.2/apis/core/reportbuilder/filtering) by overriding the `get_default_condition_values()` method.
 ...
 * @return array
 */
publicfunctionget_default_condition_values():array{
return[
...
### Adding unit tests[​](#adding-unit-tests "Direct link to Adding unit tests")
 ...
 */
classcategories_testextendscore_reportbuilder_testcase{
```
...
#### Testing the default report[​](#testing-the-default-report "Direct link to Testing the default report")
...
/**
     ...
     */
publicfunctiontest_datasource_default():void{
$this->resetAfterTest();
...
#### Testing the non-default columns and filtering[​](#testing-the-non-default-columns-and-filtering "Direct link to Testing the non-default columns and filtering")
...
This would cause the report to return results for users with the first name of *Pedro* only.

#### The stress test[​](#the-stress-test "Direct link to The stress test")
...
/**
     ...
     */
publicfunctiontest_stress_datasource():void{
if(!PHPUNIT_LONGTEST){
...
#### Unit test examples[​](#unit-test-examples "Direct link to Unit test examples")
