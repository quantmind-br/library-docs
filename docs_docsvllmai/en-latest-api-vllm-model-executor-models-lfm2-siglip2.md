---
title: lfm2_siglip2 - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/models/lfm2_siglip2/
source: sitemap
fetched_at: 2026-05-07T21:31:23.026386822-03:00
rendered_js: false
word_count: 440
summary: This document provides a technical reference for the Siglip2 vision model components, including the attention mechanism, encoder, and encoder layers implemented within the vLLM framework.
tags:
    - siglip2
    - vision-model
    - transformer-encoder
...
## Siglip2Attention [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2Attention "Permanent link")
...
```
classSiglip2Attention(nn.Module):
"""Multi-headed attention from 'Attention Is All You Need' paper"""
...
## Siglip2Encoder [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2Encoder "Permanent link")
...
Name Type Description Default `config` `Siglip2VisionConfig`
...
## Siglip2EncoderLayer [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2EncoderLayer "Permanent link")
...
classSiglip2EncoderLayer(nn.Module):
    def__init__(
        self,
        config: Siglip2VisionConfig,
        quant_config: QuantizationConfig | None = None,
...
### forward [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2EncoderLayer.forward "Permanent link")

Parameters:

Name Type Description Default `hidden_states` `Tensor`

Input tensor of shape (batch, seq\_len, embed\_dim).

*required* `cu_seqlens` `Tensor`

Cumulative sequence lengths tensor.

*required* `max_seqlen` `int | Tensor`
...
## Siglip2Model [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2Model "Permanent link")
...
```
classSiglip2Model(torch.nn.Module):
    def__init__(
        self,
        config: Siglip2VisionConfig,
        quant_config: QuantizationConfig | None = None,
...
### forward [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2Model.forward "Permanent link")
...
Name Type Description Default `select_layers` `list[int] | None`

Layer indices to select hidden states from. Supports negative indices (e.g., \[-2] for second-to-last). If None, returns the last layer output with post\_layernorm. Multiple layers can be selected and will be concatenated.
...
## Siglip2VisionEmbeddings [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2VisionEmbeddings "Permanent link")
...
```
classSiglip2VisionEmbeddings(nn.Module):
    def__init__(self, config: Siglip2VisionConfig):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
...
"""Embed patchified pixel values in packed (unpadded) form.
    ...
    @staticmethod
    defresize_positional_embeddings_packed(
...
### forward [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2VisionEmbeddings.forward "Permanent link")
...
Name Type Description Default `pixel_values_packed` `FloatTensor`

(1, total\_tokens, patch\_dim) or (total\_tokens, patch\_dim), packed in tile order.

*required* `spatial_shapes` `LongTensor`

(num\_tiles, 2) on CPU (height, width) per tile.
...
(1, total\_tokens, embed\_dim) packed embeddings.
...
### resize\_positional\_embeddings\_packed `staticmethod` [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2VisionEmbeddings.resize_positional_embeddings_packed "Permanent link")
...
Name Type Description Default `positional_embeddings` `Tensor`

(height, width, embed\_dim) base grid.

*required* `spatial_shapes` `LongTensor`

(batch\_size, 2) on CPU, (height, width) per image.

*required* `lengths_list` `list[int]`

flattened token length per image (height * width).
...
(total\_tokens, embed\_dim) packed positional embeddings, concatenated
...
## Siglip2VisionTransformer [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2VisionTransformer "Permanent link")
...
```
classSiglip2VisionTransformer(nn.Module):
    def__init__(
        self,
        config: Siglip2VisionConfig,
        quant_config: QuantizationConfig | None = None,
...
### forward [¶](#vllm.model_executor.models.lfm2_siglip2.Siglip2VisionTransformer.forward "Permanent link")
...
spatial\_shapes (`torch.LongTensor` of shape `(batch_size, 2)`): Tensor containing the spatial dimensions (height, width) of the input images. select\_layers (`list[int]` or `None`, defaults to `None`): Layer indices to select hidden states from. Supports negative indices (e.g., -1 for last layer, -2 for second-to-last). If None, returns the last layer output.
