...
source: crawler
fetched_at: 2026-04-24T17:00:23.21889528-03:00
rendered_js: false
word_count: 555
summary: This document provides a tutorial and reference guide on automating workflows using the Hermes Agent, detailing five practical patterns such as website monitoring, weekly reporting, and multi-skill chaining.
tags:
    - hermes-agent
    - cron-jobs
    - workflow-automation
...
TargetExampleUse case`origin``--deliver origin`Same chat that created the job (default)`local``--deliver local`Save to local file only`telegram``--deliver telegram`Your Telegram home channel`discord``--deliver discord`Your Discord home channel`slack``--deliver slack`Your Slack home channelSpecific chat`--deliver telegram:-1001234567890`A specific Telegram groupThreaded`--deliver telegram:-1001234567890:17585`A specific Telegram topic thread
