---
title: Benchmark and Profiling — SGLang
url: https://docs.sglang.io/developer_guide/benchmark_and_profiling.html
source: crawler
fetched_at: 2026-02-04T08:47:13.987776848-03:00
rendered_js: false
word_count: 1510
summary: This document provides comprehensive instructions and command-line examples for benchmarking performance and profiling SGLang using tools like PyTorch Profiler. It covers measuring latency and throughput across various execution modes, including offline engines, online servers, and PD disaggregation environments.
tags:
    - benchmarking
    - profiling
...
## Benchmark and Profiling[#](#benchmark-and-profiling "Link to this heading")
...
- Benchmark the latency of running a single static batch without a server. The arguments are the same as for `launch_server.py`. Note that this is a simplified test script without a dynamic batching server, so it may run out of memory for a batch size that a real server can handle. A real server truncates the prefill into several batches, while this simplified script does not.
  ...
  - Without a server (do not need to launch a server)
    ...
    ```
    python-msglang.bench_one_batch--model-pathmeta-llama/Meta-Llama-3.1-8B-Instruct--batch32--input-len256--output-len32
    ```
  - With a server (please use `sglang.launch_server` to launch a server first and run the following command.)
    
    ```
    python-msglang.bench_one_batch_server--base-urlhttp://127.0.0.1:30000--model-pathmeta-llama/Meta-Llama-3.1-8B-Instruct--batch-size32--input-len256--output-len32
    ```
- Benchmark offline processing. This script will start an offline engine and run the benchmark.
  
  ```
  python3-msglang.bench_offline_throughput--model-pathmeta-llama/Meta-Llama-3.1-8B-Instruct--num-prompts10
  ```
- Benchmark online serving. Please use `sglang.launch_server` to launch a server first and run the following command.
  
  ```
  python3-msglang.bench_serving--backendsglang--num-prompt10
  ```

## Profile with PyTorch Profiler[#](#profile-with-pytorch-profiler "Link to this heading")
...
### Profile a server with `sglang.bench_serving`[#](#profile-a-server-with-sglang-bench-serving "Link to this heading")

```
# set trace path
exportSGLANG_TORCH_PROFILER_DIR=/root/sglang/profile_log

# start server
python-msglang.launch_server--model-pathmeta-llama/Llama-3.1-8B-Instruct

# send profiling request from client
python-msglang.bench_serving--backendsglang--modelmeta-llama/Llama-3.1-8B-Instruct--num-prompts10--sharegpt-output-len100--profile
```
...
For more details, please refer to [Bench Serving Guide](https://docs.sglang.io/developer_guide/bench_serving.html).

### Profile In PD Disaggregation Mode[#](#profile-in-pd-disaggregation-mode "Link to this heading")
...
#### Profile Prefill Workers[#](#profile-prefill-workers "Link to this heading")
...
python-msglang.launch_server--model-pathmeta-llama/Llama-3.1-8B-Instruct--disaggregation-modeprefill
python-msglang.launch_server--model-pathmeta-llama/Llama-3.1-8B-Instruct--disaggregation-modedecode--port30001--base-gpu-id1
...
python-msglang.bench_serving--backendsglang--modelmeta-llama/Llama-3.1-8B-Instruct--num-prompts10--sharegpt-output-len100--profile--pd-separated--profile-prefill-urlhttp://127.0.0.1:30000
```

#### Profile Decode Workers[#](#profile-decode-workers "Link to this heading")

```
# send profiling request targeting decode workers
python-msglang.bench_serving--backendsglang--modelmeta-llama/Llama-3.1-8B-Instruct--num-prompts10--sharegpt-output-len100--profile--pd-separated--profile-decode-urlhttp://127.0.0.1:30001
```
...
- Both options support multiple worker URLs for multi-instance setups:
  ...
  python-msglang.bench_serving--backendsglang--modelmeta-llama/Llama-3.1-8B-Instruct--num-prompts10--profile--pd-separated--profile-prefill-urlhttp://127.0.0.1:30000http://127.0.0.1:30002
  
  # Profile multiple decode workers
  python-msglang.bench_serving--backendsglang--modelmeta-llama/Llama-3.1-8B-Instruct--num-prompts10--profile--pd-separated--profile-decode-urlhttp://127.0.0.1:30001http://127.0.0.1:30003
  ```
- Make sure `SGLANG_TORCH_PROFILER_DIR` is set on all worker nodes before starting the servers
- For more details on setting up PD disaggregation, see [PD Disaggregation Guide](https://docs.sglang.io/advanced_features/pd_disaggregation.html)

### Profile a server with `sglang.bench_offline_throughput`[#](#profile-a-server-with-sglang-bench-offline-throughput "Link to this heading")
...
python3-msglang.bench_one_batch--model-pathmeta-llama/Llama-3.1-8B-Instruct--batch32--input-len1024--output-len10--profile

# profile multiple batches with bench_offline_throughput.py
python-msglang.bench_offline_throughput--model-pathmeta-llama/Llama-3.1-8B-Instruct--dataset-namerandom--num-prompts10--profile--mem-frac=0.8
```

### Profile a server with `sglang.profiler`[#](#profile-a-server-with-sglang-profiler "Link to this heading")
...
### Profile a server with HTTP API endpoints[#](#profile-a-server-with-http-api-endpoints "Link to this heading")
...
#### Using `/start_profile` endpoint[#](#using-start-profile-endpoint "Link to this heading")
...
curl-XPOSThttp://127.0.0.1:30000/start_profile\
...
#### Using `/end_profile` endpoint[#](#using-end-profile-endpoint "Link to this heading")

The `/end_profile` endpoint stops an ongoing profiling session and saves the trace file.

```
# Stop profiling and save traces
curl-XPOSThttp://127.0.0.1:30000/end_profile
```
...
exportSGLANG_TORCH_PROFILER_DIR=/tmp/profiles
python-msglang.launch_server--model-pathmeta-llama/Llama-3.1-8B-Instruct
...
python-msglang.bench_serving--backendsglang--num-prompts100

# Terminal 2: Stop profiling when done
curl-XPOSThttp://127.0.0.1:30000/end_profile
```

### Profiler Trace Merger for Distributed Traces[#](#profiler-trace-merger-for-distributed-traces "Link to this heading")
...
#### Multi-Node Profiling and Shared Storage Considerations[#](#multi-node-profiling-and-shared-storage-considerations "Link to this heading")
...
#### HTTP API Usage[#](#http-api-usage "Link to this heading")

```
# Start profiling with automatic trace merging enabled
curl-XPOST<BASE_URL>/start_profile\
-H"Content-Type: application/json"\
-d'{
...
### Possible PyTorch bugs[#](#possible-pytorch-bugs "Link to this heading")

If in any cases you encounter the following error (for example, using qwen 2.5 VL):

```
RuntimeError:!stack.empty()INTERNALASSERTFAILEDat"/pytorch/torch/csrc/autograd/profiler_python.cpp":983,pleasereportabugtoPyTorch.Pythonreplaystackisempty.
```
...
```
exportSGLANG_PROFILE_WITH_STACK=False
python-msglang.bench_offline_throughput--model-pathmeta-llama/Llama-3.1-8B-Instruct--dataset-namerandom--num-prompts10--profile--mem-frac=0.8
```
...
```
python-msglang.bench_serving--backendsglang--modelmeta-llama/Llama-3.1-8B-Instruct--num-prompts2--sharegpt-output-len100--profile
```
...
## Profile with Nsight[#](#profile-with-nsight "Link to this heading")
...
1. Prerequisite:
   ...
   aptupdate
   aptinstall-y--no-install-recommendsgnupg
   echo"deb http://developer.download.nvidia.com/devtools/repos/ubuntu$(source/etc/lsb-release;echo"$DISTRIB_RELEASE"|tr-d.)/$(dpkg--print-architecture) /"|tee/etc/apt/sources.list.d/nvidia-devtools.list
   apt-keyadv--fetch-keyshttp://developer.download.nvidia.com/compute/cuda/repos/ubuntu1804/x86_64/7fa2af80.pub
   aptupdate
   aptinstallnsight-systems-cli
   ```
2. To profile a single batch, use
   
   ```
   nsysprofile--trace-fork-before-exec=true--cuda-graph-trace=nodepython3-msglang.bench_one_batch--modelmeta-llama/Meta-Llama-3-8B--batch-size64--input-len512
   ```
3. To profile a server, e.g.
   ...
   nsysprofile--trace-fork-before-exec=true--cuda-graph-trace=node-osglang.out--delay60--duration70python3-msglang.launch_server--model-pathmeta-llama/Llama-3.1-8B-Instruct--disable-radix-cache
   
   # client
   python3-msglang.bench_serving--backendsglang--num-prompts1000--dataset-namerandom--random-input1024--random-output512
   ```
   ...
   ```
   nsysstop--session=profile-XXXXX
   ```
...
### Layer-wise NVTX Profiling with Nsight Systems[#](#layer-wise-nvtx-profiling-with-nsight-systems "Link to this heading")
...
#### Using `--enable-layerwise-nvtx-marker` with Nsight Systems and `/start_profile`[#](#using-enable-layerwise-nvtx-marker-with-nsight-systems-and-start-profile "Link to this heading")
...
1. Launch the server with layerwise NVTX enabled under Nsight Systems:
   ...
   nsysprofile--trace-fork-before-exec=true\
...
--cuda-graph-trace=node\
-olayerwise_profile\
python-msglang.bench_serving--backendsglang--num-prompts10
```
...
```
nsys-uilayerwise_profile.qdrep
```
...
## Other tips[#](#other-tips "Link to this heading")
...
2. You can benchmark a model with modified configs (e.g., less layers) by using `--json-model-override-args`. For example, you can benchmark a model with only 2 layers and 2 kv heads using:
   ...
   ```
   python-msglang.bench_one_batch--model-pathmeta-llama/Meta-Llama-3.1-8B-Instruct--batch32--input-len256--output-len32--load-formatdummy--json-model-override-args'{"num_hidden_layers": 1, "num_key_value_heads": 1}'
   ```
