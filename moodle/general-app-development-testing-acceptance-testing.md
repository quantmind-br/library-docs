---
title: Acceptance testing for the Moodle App | Moodle Developer Resources
url: https://moodledev.io/general/app/development/testing/acceptance-testing
source: sitemap
fetched_at: 2026-02-17T15:55:10.117581-03:00
rendered_js: false
word_count: 3131
summary: This document provides comprehensive instructions for setting up and running automated acceptance tests for the Moodle App using the Behat framework. It details environment configuration, plugin requirements, and the process for writing and executing mobile-specific test scenarios.
tags:
    - behat
    - moodle-app
    - acceptance-testing
...
In order to run tests that carry out automated functionality testing for the Moodle App, you can write [Acceptance tests](https://moodledev.io/general/development/tools/behat). This can be useful if you want to test plugins that are compatible with the app, or you're contributing to the app core. Behat tests for the app work the same way as tests for Moodle core, but they are not run as part of a normal Behat execution and there are some differences that we'll go through in this page.

A key point is that these tests are run using the Moodle Behat infrastructure, and don't depend only on the app codebase. Therefore, you will need a Moodle development setup as described in [Setting up development environment](https://moodledev.io/general/development/gettingstarted).
...
### Configuring the Moodle site[​](#configuring-the-moodle-site "Direct link to Configuring the Moodle site")

You can learn how to run a Moodle site locally in [Setting up development environment](https://moodledev.io/general/development/gettingstarted).
...
### Configuring the Moodle App[​](#configuring-the-moodle-app "Direct link to Configuring the Moodle App")

If you are going to modify the application code, you can learn how to run it locally in [Setting up your development environment for the Moodle App](https://moodledev.io/general/app/development/setup). You only need to run the application in a browser, so you can skip the instructions for Android/iOS. Make sure to launch the application on the testing environment, running `npm run serve:test`.

If you only intend to run the app with the goal of executing Behat tests, you can use [the Docker images for the Mobile App](https://moodledev.io/general/app/development/setup/docker-images). Again, make sure that you're running them on the testing environment using the `-test` suffix.
...
### Configuring Behat[​](#configuring-behat "Direct link to Configuring Behat")
...
The Moodle App [only works in Chromium-based browsers](https://moodledev.io/general/app/development/setup/app-in-browser), so mobile tests will be ignored if you are using any other browser. You can learn how to configure the browser used in your tests in the [Running acceptance test](https://moodledev.io/general/development/tools/behat/running) page.
...
## Running Behat[​](#running-behat "Direct link to Running Behat")
...
This page assumes you already know all about [Writing acceptance tests](https://moodledev.io/general/development/tools/behat/writing) in general.
...
### Start the app[​](#start-the-app "Direct link to Start the app")
...
### Log in to the app[​](#log-in-to-the-app "Direct link to Log in to the app")
...
### Standard steps[​](#standard-steps "Direct link to Standard steps")
...
### Leaving the app[​](#leaving-the-app "Direct link to Leaving the app")
...
WhenI enter the app
AndI log in as "student"
AndI press "Course 1" near "Course overview" in the app
Then the header should be "Course 1" in the app
```
...
### Testing against multiple app versions[​](#testing-against-multiple-app-versions "Direct link to Testing against multiple app versions")
...
If you pause a test (for example, using the `I wait "2000" seconds` step) and open the developer tools, you can debug the application like you would during development. You can learn how to do that in [Using the Moodle App in a browser](https://moodledev.io/general/app/development/setup/app-in-browser).
...
### Writing custom steps[​](#writing-custom-steps "Direct link to Writing custom steps")

If you find something missing to test your code, you can always implement custom steps.

If you're writing a plugin, you can include a new class under `tests/behat/behat\_{yourpluginname}.php`. If you're working on application code, you can always update [behat\_app.php](https://github.com/moodlehq/moodleapp/blob/latest/local_moodleappbehat/tests/behat/behat_app.php) as well.

You can learn more about writing custom steps in the [Writing new acceptance test step definitions](https://docs.moodle.org/dev/Writing_new_acceptance_test_step_definitions) page, and if you want to see how the steps that are specific to the app work, you should look into [behat\_app.php](https://github.com/moodlehq/moodleapp/blob/latest/local_moodleappbehat/tests/behat/behat_app.php) and [behat-runtime.ts](https://github.com/moodlehq/moodleapp/tree/latest/src/testing/services/behat-runtime.ts).

### Testing graphical regressions[​](#testing-graphical-regressions "Direct link to Testing graphical regressions")

If you want to test against graphical regressions, you can use the [local\_behatsnapshots](https://github.com/NoelDeMartin/moodle-local_behatsnapshots/) plugin. It allows you to store UI snapshots in your repository and check against them every time Behat tests are run:
...
### Unable to load app version from [https://localhost:8100/assets/env.json](https://localhost:8100/assets/env.json)[​](#unable-to-load-app-version-from-httpslocalhost8100assetsenvjson "Direct link to unable-to-load-app-version-from-httpslocalhost8100assetsenvjson")
...
### The plugins required by this course could not be loaded correctly...[​](#the-plugins-required-by-this-course-could-not-be-loaded-correctly "Direct link to The plugins required by this course could not be loaded correctly...")
...
To clear the timeout message, open the app in your [development browser](https://moodledev.io/general/app/development/setup/app-in-browser), open the Inspector, open the Application tab, select Clear storage, press Clear site data, close Inspector, close the tab with mobile site, re-open mobile site in new tab and log in. Then in a separate tab, log in to your behat site (you can find the url in `$CFG->behat_wwwroot` within your `config.php` file) and make sure you can get into the course without seeing the error.

### Fatal error: Maximum execution time of 30 seconds exceeded in...[​](#fatal-error-maximum-execution-time-of-30-seconds-exceeded-in "Direct link to Fatal error: Maximum execution time of 30 seconds exceeded in...")
...
### Test fails because of the browser language[​](#test-fails-because-of-the-browser-language "Direct link to Test fails because of the browser language")
...
### Application build gets killed without any error information[​](#application-build-gets-killed-without-any-error-information "Direct link to Application build gets killed without any error information")
...
### MacOS: running moodle-docker commands show grep usage options and do nothing else[​](#macos-running-moodle-docker-commands-show-grep-usage-options-and-do-nothing-else "Direct link to MacOS: running moodle-docker commands show grep usage options and do nothing else")
