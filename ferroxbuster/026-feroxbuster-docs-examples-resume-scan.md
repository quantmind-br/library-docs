...
url: https://epi052.github.io/feroxbuster-docs/examples/resume-scan
source: github_pages
fetched_at: 2026-02-06T10:56:07.981667102-03:00
rendered_js: true
word_count: 221
summary: This document explains how to pause and resume scans in feroxbuster using state files, covering command-line flags, automated offset calculations, and configuration options.
tags:
    - feroxbuster
    - scan-resumption
    - state-management
...
[Section titled “Stop and Resume Scans”](#stop-and-resume-scans)
...
![save-state](https://epi052.github.io/feroxbuster-docs/images/examples/save-state.png)
...
Based on the example image above, the same scan can be resumed by using `feroxbuster --resume-from ferox-http_localhost-1606947491.state`. Directories that were already complete are not rescanned.

(updated behavior for **2.8.0**) Partially complete scans are started from roughly where they left off. More specifically, 10% less than the number of requests that were made already (`requests_made_so_far`) becomes the starting offset into the wordlist. This is to help ensure that no responses were lost mid-flight when cancelling/resuming.
...
![resumed-scan](https://epi052.github.io/feroxbuster-docs/images/examples/resumed-scan.gif)

## Opt-out of saving state

[Section titled “Opt-out of saving state”](#opt-out-of-saving-state)
...
[Section titled “Configure name of the state file”](#configure-name-of-the-state-file)
...
STATE_FILENAME=/some/path/to/a/fileferoxbuster-u...
```