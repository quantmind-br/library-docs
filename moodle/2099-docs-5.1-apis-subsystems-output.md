---
title: Output API | Moodle Developer Resources
url: https://moodledev.io/docs/5.1/apis/subsystems/output
source: sitemap
fetched_at: 2026-02-17T15:36:56.14801-03:00
rendered_js: false
word_count: 3306
summary: This document explains the Moodle Output API, detailing how renderers, renderables, and templates collaborate to display visual content. It provides a technical overview of the page output lifecycle, the $PAGE object, and the implementation of renderable classes.
tags:
    - moodle-development
    - output-api
    - php-renderers
...
## Page Output Journey[​](#page-output-journey "Direct link to Page Output Journey")
...
$pagetitle=$title;
$url=newmoodle_url("/admin/tool/demo/index.php");
$PAGE->set_url($url);
...
protectedreadonly\core\output\renderer_helper$rendererhelper,
){
}

publicfunctiondo_something_with_my_renderer(){
/** @var \tool_demo\output\renderer $renderer */
$renderer=$this->rendererhelper->get_renderer('tool_demo');
// Do something with the renderer.
}

publicfunctiondo_something_with_core_renderer(){
...
/admin/tool/demo/classes/output/index\_page.php

```
<?php
// Standard GPL and phpdocs

namespacetool_demo\output;
...
classindex_pageimplementsrenderable, templatable {
/** @var string $sometext Some text to show how to pass data to a template. */
private$sometext=null;

publicfunction__construct($sometext):void{
...
/**
     ...
     * @return stdClass
     */
publicfunctionexport_for_template(renderer_base$output):stdClass{
$data=newstdClass();
...
Example implementation of get\_template\_name()
...
useplugin_renderer_base;

classrendererextendsplugin_renderer_base{
/**
     * Defer to template.
     *
     ...
     */
publicfunctionrender_index_page($page):string{
$data=$page->export_for_template($this);
returnparent::render_from_template('tool_demo/index_page',$data);
}
}
```
...
admin/tool/demo/templates/index\_page.mustache
...
This is the mustache template for this demo. It uses some bootstrap classes directly to position and style the content on the page. `{{sometext}}` is replaced with the variable from the context when this template is rendered. For more information on templates see [Templates](https://moodledev.io/docs/5.1/guides/templates).

## Output Functions[​](#output-functions "Direct link to Output Functions")
...
### String formatting functions[​](#string-formatting-functions "Direct link to String formatting functions")
...
#### p() and s()[​](#p-and-s "Direct link to p() and s()")

```
functions($var,$strip=false)
functionp($var,$strip=false)
```
...
#### format\_text()[​](#format_text "Direct link to format_text()")
...
#### format\_string()[​](#format_string "Direct link to format_string()")

```
functionformat_string(string$text,$striplinks=true,['context'=>$context,'escape'=>true,'filter'=>true])
```
...
### Simple elements rendering[​](#simple-elements-rendering "Direct link to Simple elements rendering")
...
While to render complex elements, you should use [templates](https://moodledev.io/docs/5.1/guides/templates), some simple elements can be rendered using the following functions:

#### container()[​](#container "Direct link to container()")

```
functioncontainer(string$contents,?string$classes=null,?string$id=null):string
```
...
```
functionparagraph(string$contents,?string$classes=null,?string$id=null):string
```
...
#### visually\_hidden\_text()[​](#visually_hidden_text "Direct link to visually_hidden_text()")

```
functionvisually_hidden_text(string$contents):string
```
...
```
<spanclass="visually-hidden">Contents</span>
```

### Other[​](#other "Direct link to Other")

#### Progress Bars[​](#progress-bars "Direct link to Progress Bars")

There are two types of progress bars you can use in Moodle.

##### Standard Progress Bars[​](#standard-progress-bars "Direct link to Standard Progress Bars")
...
##### Stored Progress Bars[​](#stored-progress-bars "Direct link to Stored Progress Bars")
...
```
classstored_progress_scheduled_task_exampleextends\core\task\scheduled_task{
...
$task=newstored_progress_scheduled_task_example();
...
## Reusing Output Classes in Web Services[​](#reusing-output-classes-in-web-services "Direct link to Reusing Output Classes in Web Services")
...
Here is an example of how to implement the `externable` interface in your output class.

```

namespacemod_MYPLUGIN\output;

usecm_info;
usecore\output\externable;
usecore\output\named_templatable;
usecore\output\renderable;
usecore\output\renderer_base;
usecore_courseformat\baseas course_format;
usemod_MYPLUGIN\external\myname_exporter;
usestdClass;

classmynameimplementsexternable, named_templatable, renderable {
publicfunction__construct(
...
#[\Override]
publicfunctionget_exporter(?\core\context$context=null):myname_exporter{
$context=$context??\core\context\system::instance();
returnnewmyname_exporter($this,['context'=>$context]);
}

#[\Override]
publicstaticfunctionget_read_structure(
int$required=VALUE_REQUIRED,
mixed$default=null
):\core_external\external_single_structure{
returnmyname_exporter::get_read_structure($required,$default);
}

#[\Override]
publicstaticfunctionread_properties_definition():array{
returnmyname_exporter::read_properties_definition();
}
...
### Using `exporters` with `exportable` objects.[​](#using-exporters-with-exportable-objects "Direct link to using-exporters-with-exportable-objects")
...
Here is an example of how to implement an `exporter` to an `externable` output.

This is an example of an exporter for the `myname` output class we created above.

```
namespacemod_MYPLUGIN\external;

usecore\external\exporter;
usemod_MYPLUGIN\output\myname;

classmyname_exporterextendsexporter{
/**
...
#[\Override]
protectedstaticfunctiondefine_properties():array{
return[];
}

#[\Override]
protectedstaticfunctiondefine_related(){
...
#[\Override]
protectedstaticfunctiondefine_other_properties(){
return[
'activityname'=>[
'type'=>PARAM_TEXT,
...
#[\Override]
protectedfunctionget_other_values(\renderer_base$output){
/** @var \cm_info $cm */
$cm=$this->data->cm;
...
Here is an example of how to implement a webservice using an `externable` output.

```
namespacemod_MYPLUGIN\external;

usecore_external\external_api;
usecore_external\external_function_parameters;
usecore_external\external_single_structure;
usecore_external\external_value;
usecore\output\renderer_helper;
usemod_MYPLUGIN\output\myname;
usestdClass;

classget_my_nameextendsexternal_api{
publicstaticfunctionexecute_parameters():external_function_parameters{
returnnewexternal_function_parameters([
'cmid'=>newexternal_value(PARAM_INT,'Course module id',VALUE_REQUIRED),
]);
}

publicstaticfunctionexecute(int$cmid):stdClass{
[
'cmid'=>$cmid,
...
returnmyname::get_read_structure();
}
}
```
...
- [Templates](https://moodledev.io/docs/5.1/guides/templates)