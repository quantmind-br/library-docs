---
title: hermes_tool_parser - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/tool_parsers/hermes_tool_parser/
source: sitemap
fetched_at: 2026-05-07T21:36:05.841873101-03:00
rendered_js: false
word_count: 55
summary: This document defines a parser class for extracting tool calls from Hermes 2 Pro model outputs, including support for both batch extraction and streaming delta processing.
tags:
    - hermes-2-pro
    - tool-calling
    - parsing
...
```
classHermes2ProToolParser(ToolParser):
...
"""Return unsent non-tool-call text, or None.
    ...
    def_extract_tool_call_jsons(self, text: str) -> list[tuple[str, bool]]:
"""Extract (json_text, is_complete) for each <tool_call> region."""
    ...
    @staticmethod
    def_extract_tool_name(tc_json: str) -> str | None:
"""Extract tool name, or None if the name isn't complete yet."""
        match = re.search(r'"name"\s*:\s*"([^"]+)"', tc_json)
        return match.group(1) if match else None

    @staticmethod
    def_extract_tool_args(tc_json: str, is_complete: bool) -> str | None:
