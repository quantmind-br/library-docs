---
title: Openclaw Migration — Migrate a user's OpenClaw customization footprint into Hermes Agent | Hermes Agent
url: https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/migration/migration-openclaw-migration
source: crawler
fetched_at: 2026-04-24T17:01:23.142437336-03:00
rendered_js: false
word_count: 2013
summary: This document details the Hermes Agent skill designed to migrate a user's configuration and assets from an OpenClaw setup into Hermes. It explains both the CLI command interface for migration and the interactive workflow supported by the agent, covering what is imported, path resolutions, and required user interaction protocols.
tags:
    - migration
    - openclaw-hermes
    - agent-skill
...
SourceOptional — install with `hermes skills install official/migration/openclaw-migration`Path`optional-skills/migration/openclaw-migration`Version`1.0.0`AuthorHermes Agent (Nous Research)LicenseMITTags`Migration`, `OpenClaw`, `Hermes`, `Memory`, `Persona`, `Import`Related skills[`hermes-agent`](https://hermes-agent.nousresearch.com/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent)
...
01. Base all counts on `report.summary`.
02. Only list an item under "Successfully Migrated" if its `status` is exactly `migrated`.
03. Do not claim a conflict was resolved unless the report shows that item as `migrated`.
04. Do not say `SOUL.md` was overwritten unless the report item for `kind="soul"` has `status="migrated"`.
05. If `report.summary.conflict > 0`, include a conflict section instead of silently implying success.
06. If counts and listed items disagree, fix the list to match the report before responding.
07. Include the `output_dir` path from the report when available so the user can inspect `report.json`, `summary.md`, backups, and archived files.
08. For memory or user-profile overflow, do not say the entries were archived unless the report explicitly shows an archive path. If `details.overflow_file` exists, say the full overflow list was exported there.
09. If a skill was imported under a renamed folder, report the final destination and mention `details.renamed_from`.
...
01. Run a dry run before writing unless the user explicitly says to proceed immediately.
02. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Hermes unless the user explicitly asks for secret migration.
03. Do not silently overwrite non-empty Hermes targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
04. Always give the user the skipped-items report. That report is part of the migration, not an optional extra.
05. Prefer the primary OpenClaw workspace (`~/.openclaw/workspace/`) over `workspace.default/`. Only use the default workspace as fallback when the primary files are missing.
06. Even in secret-migration mode, only migrate secrets with a clean Hermes destination. Unsupported auth blobs must still be reported as skipped.
07. If the dry run shows a large asset copy, a conflicting `SOUL.md`, or overflowed memory entries, call those out separately before execution.
08. Default to `user-data only` if the user is unsure.
09. Only include `workspace-agents` when the user has explicitly provided a destination workspace path.
