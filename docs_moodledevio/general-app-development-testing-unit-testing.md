---
title: Unit testing for the Moodle App | Moodle Developer Resources
url: https://moodledev.io/general/app/development/testing/unit-testing
source: sitemap
fetched_at: 2026-02-17T15:55:13.213405-03:00
rendered_js: false
word_count: 1014
summary: This document provides instructions on how to write and run unit tests using Jest, covering specialized helpers for mocking TypeScript classes, services, and Angular components.
tags:
    - unit-testing
    - jest
    - typescript
...
Unit tests are written in JavaScript using [Jest](https://jestjs.io/). If you want to create a new one, Jest is already configured and you only need to create a file ending with `.test.ts` within the project. If you're going to do so, remember to follow the [file location conventions](https://moodledev.io/general/app/development/development-guide#test-files).
...
## Testing plain TypeScript[​](#testing-plain-typescript "Direct link to Testing plain TypeScript")
...
If you are testing some code that uses [Service Singletons](https://moodledev.io/general/app/development/development-guide#service-singletons), it is likely that you want to mock some of them. You can achieve it by using the `mockSingleton` helper. This method takes a Service Singleton and creates a mock for the instance underneath, mocking the methods and properties that you specify along the way.

For example, let's say that you have the following test:

```
it('App provider checks current platform',()=>{
const appService =newCoreAppProvider();
...
// Arrange.
const domUtils =newCoreUtilsProvider(mock<NgZone>());
...
const fixture =awaitrenderComponent(UsersListComponent);
...
const fixture =awaitrenderTemplate(
        UsersListComponent,
...
```
it('Renders a list of users',async()=>{
const fixture =awaitrenderWrapperComponent(
        UsersListComponent,
'users-list',
{ users:['Amy']('John',)},
);
...
## What about integration tests?[​](#what-about-integration-tests "Direct link to What about integration tests?")
...
If you want to write even more realistic tests, that are actually running the complete application and interacting with it like a real user would, you should check out the [Acceptance testing for the Moodle App](https://moodledev.io/general/app/development/testing/acceptance-testing) page.