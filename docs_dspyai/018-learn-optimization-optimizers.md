---
title: Optimizers - DSPy
url: https://dspy.ai/learn/optimization/optimizers/
source: sitemap
fetched_at: 2026-01-23T08:03:18.486616004-03:00
rendered_js: false
word_count: 1513
summary: This document provides an overview of DSPy optimizers, which are algorithms designed to tune prompt instructions and language model weights to maximize program accuracy. It categorizes and describes available optimization methods for few-shot learning, instruction optimization, and model fine-tuning.
tags:
    - dspy-framework
    - prompt-optimization
    - few-shot-learning
...
## DSPy Optimizers (formerly Teleprompters)[¶](#dspy-optimizers-formerly-teleprompters "Permanent link")
...
## What does a DSPy Optimizer tune? How does it tune them?[¶](#what-does-a-dspy-optimizer-tune-how-does-it-tune-them "Permanent link")
...
## What DSPy Optimizers are currently available?[¶](#what-dspy-optimizers-are-currently-available "Permanent link")

Optimizers can be accessed via `from dspy.teleprompt import *`.

### Automatic Few-Shot Learning[¶](#automatic-few-shot-learning "Permanent link")
...
### Automatic Instruction Optimization[¶](#automatic-instruction-optimization "Permanent link")
...
2. [**`MIPROv2`**](https://dspy.ai/api/optimizers/MIPROv2/) : Generates instructions *and* few-shot examples in each step. The instruction generation is data-aware and demonstration-aware. Uses Bayesian Optimization to effectively search over the space of generation instructions/demonstrations across your modules.
3. [**`SIMBA`**](https://dspy.ai/api/optimizers/SIMBA/) : Uses stochastic mini-batch sampling to identify challenging examples with high output variability, then applies the LLM to introspectively analyze failures and generate self-reflective improvement rules or add successful demonstrations.
4. [**`GEPA`**](https://dspy.ai/api/optimizers/GEPA/overview/) : Uses LM's to reflect on the DSPy program's trajectory, to identify what worked, what didn't and propose prompts addressing the gaps. Additionally, GEPA can leverage domain-specific textual feedback to rapidly improve the DSPy program. Detailed tutorials on using GEPA are available at [dspy.GEPA Tutorials](https://dspy.ai/tutorials/gepa_ai_program/).

### Automatic Finetuning[¶](#automatic-finetuning "Permanent link")

This optimizer is used to fine-tune the underlying LLM(s).

1. [**`BootstrapFinetune`**](https://dspy.ai/api/optimizers/BootstrapFinetune) : Distills a prompt-based DSPy program into weight updates. The output is a DSPy program that has the same steps, but where each step is conducted by a finetuned model instead of a prompted LM. [See the classification fine-tuning tutorial](https://dspy.ai/tutorials/classification_finetuning/) for a complete example.

### Program Transformations[¶](#program-transformations "Permanent link")
...
## Which optimizer should I use?[¶](#which-optimizer-should-i-use "Permanent link")
...
- If you prefer to do **instruction optimization only** (i.e. you want to keep your prompt 0-shot), use `MIPROv2` [configured for 0-shot optimization](https://dspy.ai/api/optimizers/MIPROv2/).
...
## How do I use an optimizer?[¶](#how-do-i-use-an-optimizer "Permanent link")

They all share this general interface, with some differences in the keyword arguments (hyperparameters). A full list can be found in the [API reference](https://dspy.ai/api/optimizers/BetterTogether/).
...
```
importdspy
fromdspy.datasetsimport HotPotQA

dspy.configure(lm=dspy.LM('openai/gpt-4o-mini'))

defsearch(query: str) -> list[str]:
...
For a complete RAG example that you can run, start this [tutorial](https://dspy.ai/tutorials/rag/). It improves the quality of a RAG system over a subset of StackExchange communities from 53% to 61%.
...
importdspy
fromdspy.datasetsimport DataLoader
...
## Saving and loading optimizer output[¶](#saving-and-loading-optimizer-output "Permanent link")
