---
title: async_llm - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/engine/async_llm/
source: sitemap
fetched_at: 2026-05-07T21:40:31.429155037-03:00
rendered_js: false
word_count: 98
summary: This document defines the AsyncLLM class, which acts as an asynchronous wrapper for the vLLM engine to manage background engine processes, input/output processing, and system observability.
tags:
    - vllm
    - asynchronous
    - engine-client
...
"""Add new request to the AsyncLLM."""
    ...
    async def_add_streaming_input_request(
        self,
        request_id: str,
    ...
    @staticmethod
    def_validate_streaming_input_sampling_params(
        params: SamplingParams | PoolingParams,
    ):
        if (
...
"""
        ...
        # Unexpected error in the generate() task (possibly recoverable).
            ...
            raise EngineGenerateError() frome
        finally:
            if q is not None:
                q.close()

    def_run_output_handler(self):
...
"""
    ...
    async defstart_profile(self, profile_prefix: str | None = None) -> None:
...
"""Load a new LoRA adapter into the engine for future requests."""
    ...
    async defremove_lora(self, lora_id: int) -> bool:
"""Remove an already loaded LoRA adapter."""
        return await self.engine_core.remove_lora_async(lora_id)

    async deflist_loras(self) -> set[int]:
"""List all registered adapters."""
        return await self.engine_core.list_loras_async()

    async defpin_lora(self, lora_id: int) -> bool:
...
"""
    ...
    async defwait_for_requests_to_drain(self, drain_timeout: int = 300):
