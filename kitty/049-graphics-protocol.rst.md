...
word_count: 7793
summary: This document defines the technical specification for the kitty graphics protocol, a standard for rendering raster graphics directly within a terminal emulator using pixel-based data.
category: reference
optimized: true
...
- [bicat](https://github.com/stevenxxiu/bicat) - a terminal image viewer that also works in the Vifm file manager, with nested Tmux support
...
- [nvim](https://github.com/neovim/neovim/issues/30889) - A TUI editor that can display images in the terminal
...
is arbitrary binary data, base64 <4648> encoded to prevent interoperation problems
...
specifying the `o` key. Currently, only 1950 ZLIB based deflate
...
`t`                 A temporary file, the terminal emulator will delete the file after reading the pixel data. For security reasons
                      ...
                      is in a known temporary directory, such as /tmp,
                      /dev/shm, TMPDIR env var if present and any platform
                      specific temporary directories and the file has the
                      string tty-graphics-protocol in its full file path.
`s`                 A *shared memory object*, which on POSIX systems is a
                      [POSIX shared memory object](https://pubs.opengroup.org/onlinepubs/9699919799/functions/shm_open.html)
                      and on Windows is a
                      [Named shared memory object](https://docs.microsoft.com/en-us/windows/win32/memory/creating-named-shared-memory).
...
base64 <4648> encoded then chunked up into chunks no larger than `4096` bytes. All
...
rowcolumn-diacritics.txt <../gen/rowcolumn-diacritics.txt>.  For
...
graphics_unicode_placeholders above. It can be used to specify a single
...
1. Animations often consist of small changes from one frame to the next, the
...
transferring_pixel_data above. The main difference is that the image
