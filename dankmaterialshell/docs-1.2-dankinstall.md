...
url: https://danklinux.com/docs/1.2/dankinstall
source: sitemap
fetched_at: 2026-02-22T18:42:30.386946-03:00
rendered_js: false
word_count: 731
summary: This document provides instructions for using the dankinstall script to deploy the DankMaterialShell desktop environment and its dependencies across multiple Linux distributions. It outlines the core components installed, supported operating systems, and specific configuration steps for each distribution.
tags:
    - dankmaterialshell
    - linux-installation
    - desktop-environment
...
- [**DankMaterialShell**](https://danklinux.com/docs/1.2/dankmaterialshell/overview) - The desktop shell built with quickshell & Go.
...
Packages come from the [DankLinux PPA](https://danklinux.com/docs/1.2/danklinux#ubuntu) (`ppa:avengemedia/danklinux`). Hyprland comes from `ppa:cppiber/hyprland`.

### Debian[​](#debian "Direct link to Debian")

**Supported:** Debian 13+ (Trixie), Debian Testing, Debian Sid

Packages come from the [DankLinux OBS repository](https://danklinux.com/docs/1.2/danklinux#debian). **niri only** - Debian doesn't have Hyprland packages yet.

### openSUSE Tumbleweed[​](#opensuse-tumbleweed "Direct link to openSUSE Tumbleweed")

Good package availability out of the box. niri, hyprland, ghostty, and most tools are in standard repos. Additional packages come from the [DankLinux OBS repository](https://danklinux.com/docs/1.2/danklinux#opensuse).
...
Most distros now have pre-built packages via the [DankLinux Repository](https://danklinux.com/docs/1.2/danklinux). The installer handles any remaining builds automatically:
...
See [Managing Your Installation](https://danklinux.com/docs/1.2/dankmaterialshell/managing) for details on switching between systemd and manual startup, environment variable configuration, and more.
