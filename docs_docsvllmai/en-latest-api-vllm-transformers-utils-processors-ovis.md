---
title: ovis - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/transformers_utils/processors/ovis/
source: sitemap
fetched_at: 2026-05-07T21:38:09.82886778-03:00
rendered_js: false
word_count: 8
summary: The OvisProcessor class integrates a Qwen2 tokenizer and image processor into a unified interface for preparing multi-modal inputs for the Ovis model.
tags:
...
```
classOvisProcessor(ProcessorMixin):
r"""
...
"""
    ...
    def_tokenize_with_image_symbol(self, text_list: list[str]) -> torch.LongTensor:
        batch_token_ids = []
    ...
    defget_token_value(self, tok):
        return self.extra_special_tokens[tok]

    defconstruct_image_indicators(self, grid):
        image_placeholders = [
    ...
    defconstruct_image_placeholders(self, grid):
    ...
    defpreprocess_image(
        ...
        def_preprocess(img: PIL.Image.Image, side):
        ...
        def_partition(img, grid) -> list[tuple[int, int, int, int]]:
            w, h = img.size
            row_height = h // grid[0]
            col_width = w // grid[1]
        ...
        def_covering_area(left, upper, right, lower, side):
            w = right - left
            h = lower - upper
            w, h = max(w, h), min(w, h)
            if w > side:
...
"""
    ...
    defpost_process_image_text_to_text(self, generated_outputs):
"""
