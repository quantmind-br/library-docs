---
title: Neuroskill Bci | Hermes Agent
url: https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/health/health-neuroskill-bci
source: crawler
fetched_at: 2026-04-24T17:01:21.942669748-03:00
rendered_js: false
word_count: 1643
summary: This document serves as a comprehensive reference and guide for integrating NeuroSkill's Brain-Computer Interface (BCI) metrics into an agent system. It details prerequisites, CLI commands, and provides instructions on interpreting real-time cognitive state data like focus, relaxation, and heart rate.
tags:
    - neuroskill
    - bci-integration
    - cognitive-state
...
SourceOptional — install with `hermes skills install official/health/neuroskill-bci`Path`optional-skills/health/neuroskill-bci`Version`1.0.0`AuthorHermes Agent + Nous ResearchLicenseMITTags`BCI`, `neurofeedback`, `health`, `focus`, `EEG`, `cognitive-state`, `biometrics`, `neuroskill`
...
npx neuroskill search --k10--json# 10 nearest neighbors
...
```bash
npx neuroskill search-labels "deep focus"--k10--json
...
npx neuroskill sleep0--json# most recent sleep session
...
```bash
npx neuroskill listen --seconds30--json
npx neuroskill listen --seconds5--json| jq '[.[] | select(.event == "scores")]'
```
...
ErrorLikely CauseFix`npx neuroskill status` hangsNeuroSkill app not runningOpen NeuroSkill desktop app`device.state: "disconnected"`BCI device not connectedCheck Bluetooth, device batteryAll scores return 0Poor electrode contactReposition headband, moisten electrodes`signal_quality` values &lt; 0.7Loose electrodesAdjust fit, clean electrode contactsSNR &lt; 3 dBNoisy signalMinimize head movement, check environment`command not found: npx`Node.js not installedInstall Node.js 20+
