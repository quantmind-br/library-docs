---
title: Applications | Dokploy
url: https://docs.dokploy.com/docs/core/applications
source: crawler
fetched_at: 2026-02-08T04:01:29.830602726-03:00
rendered_js: false
word_count: 527
summary: This document provides a comprehensive overview of managing applications in Dokploy, detailing deployment methods, environment configurations, monitoring tools, and advanced container management settings.
tags:
    - dokploy
...
To help speed up navigating there are some built in keyboard shortcuts for navigating tabs on application pages. Similar to GitHub these are all prefixed with the `g` key so to use them press `g` and then the shortcut key.

KeyTab`g`General`e`Environment`u`Domains`p`Preview Deployments`s`Schedules`v`Volume Backups`d`Deployments`l`Logs`m`Monitoring`a`Advanced
