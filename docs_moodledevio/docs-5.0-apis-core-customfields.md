---
title: Custom fields API | Moodle Developer Resources
url: https://moodledev.io/docs/5.0/apis/core/customfields
source: sitemap
fetched_at: 2026-02-17T15:24:26.23158-03:00
rendered_js: false
word_count: 795
summary: This document provides an overview of the Custom fields API, explaining how developers can implement handler classes to manage custom metadata for various system components. It details the processes for configuring fields, integrating them into forms, and ensuring compliance with the Privacy API.
tags:
    - moodle-development
    - custom-fields-api
    - plugin-development
...
## Custom fields API overview[​](#custom-fields-api-overview "Direct link to Custom fields API overview")
...
## How to use custom fields[​](#how-to-use-custom-fields "Direct link to How to use custom fields")

Component/plugin that uses custom fields must define a **handler class** for each area and a **configuration page**. Handler class must be called `<PLUGINNAME>/customfield/<AREA>_handler` and be placed in autoloaded location `<PLUGINDIR>/classes/customfield/<AREA>_handler.php`. This class must extend **\\core\_customfield\\handler** . Configuration page may be located anywhere. For course custom fields configuration the admin settings page is used [/course/customfield.php](https://github.com/moodle/moodle/blob/main/course/customfield.php). If the area uses `itemid` this page should take `itemid` as a parameter.
...
### Add custom fields to the instance edit form[​](#add-custom-fields-to-the-instance-edit-form "Direct link to Add custom fields to the instance edit form")
...
### Retrieving instances custom fields[​](#retrieving-instances-custom-fields "Direct link to Retrieving instances custom fields")

How custom fields are used depends entirely on the situation.

Handler methods to retrieve custom fields values for the given instance(s)
...
To retrieve the list of custom fields used in the given component/area/itemid
...
Example code for course custom fields. This function will return all the custom fields for a given courseid

```
functionget_course_metadata($courseid){
...
### Privacy API[​](#privacy-api "Direct link to Privacy API")
...
Plugins that store user information in custom fields should link subsystem in their get\_metadata

```
$collection->link_subsystem('core_customfield','privacy:metadata:customfieldpurpose');
```

They can use the following methods in the export/delete functions

```
usecore_customfield\privacy\provideras customfield_provider;
...
## Custom fields plugins[​](#custom-fields-plugins "Direct link to Custom fields plugins")
...
Except for common [Plugin files](https://moodledev.io/docs/5.0/apis/commonfiles) and tests the following classes must be present in `customfield` plugins (in respective autoloaded locations):

```
namespacecustomfield_<PLUGINNAME>;
classfield_controllerextends\core_customfield\field_controller;
classdata_controllerextends\core_customfield\data_controller;

namespacecustomfield_<PLUGINNAME>\privacy;
classproviderimplements\core_privacy\local\metadata\null_provider, \core_customfield\privacy\customfield_provider;
```
