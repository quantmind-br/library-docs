...
source: llms
fetched_at: 2026-01-24T14:25:42.404026564-03:00
rendered_js: false
word_count: 898
summary: This document explains how to use tmpfs mounts in Docker on Linux to store non-persistent data in host memory for improved performance and security. It details the differences between tmpfs, volumes, and bind mounts while providing configuration options for the --mount and --tmpfs flags.
tags:
    - docker
    - tmpfs-mounts
    - linux-containers
...
OptionDescription`destination`, `dst`, `target`Container path to mount into a tmpfs.`tmpfs-size`Size of the tmpfs mount in bytes. If unset, the default maximum size of a tmpfs volume is 50% of the host's total RAM.`tmpfs-mode`File mode of the tmpfs in octal. For instance, `700` or `0770`. Defaults to `1777` or world-writable.
