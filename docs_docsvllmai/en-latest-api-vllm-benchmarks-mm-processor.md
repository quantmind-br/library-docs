...
url: https://docs.vllm.ai/en/latest/api/vllm/benchmarks/mm_processor/
source: sitemap
fetched_at: 2026-05-07T21:15:53.007714645-03:00
rendered_js: false
word_count: 220
summary: This document describes the vLLM utility for benchmarking the latency and performance metrics of multimodal processor modules. It provides details on configuring CLI arguments and the execution flow for measuring processing statistics.
tags:
    - vllm
    - multimodal
    - benchmarking
...
## add\_cli\_args [¶](#vllm.benchmarks.mm_processor.add_cli_args "Permanent link")
...
```
defadd_cli_args(parser: argparse.ArgumentParser) -> None:
"""Add CLI arguments for the multimodal processor benchmark."""
    ...
    fromvllm.benchmarks.datasetsimport (
...
## benchmark\_multimodal\_processor [¶](#vllm.benchmarks.mm_processor.benchmark_multimodal_processor "Permanent link")
...
```
defbenchmark_multimodal_processor(
...
## calculate\_mm\_processor\_metrics [¶](#vllm.benchmarks.mm_processor.calculate_mm_processor_metrics "Permanent link")
...
```
defcalculate_mm_processor_metrics(
...
## collect\_mm\_processor\_stats [¶](#vllm.benchmarks.mm_processor.collect_mm_processor_stats "Permanent link")
...
```
defcollect_mm_processor_stats(llm_engine: LLMEngine) -> dict[str, list[float]]:
"""
...
## get\_timing\_stats\_from\_engine [¶](#vllm.benchmarks.mm_processor.get_timing_stats_from_engine "Permanent link")

Get all multimodal timing stats from the LLM engine.

Collects both preprocessing stats (HF processor, hashing, cache lookup, prompt update) and encoder forward pass timing, merged by request\_id.

Parameters:

Name Type Description Default `llm_engine` `LLMEngine`

The LLM engine (has input\_processor and workers).

*required*

Returns:

Type Description `dict[str, dict[str, float]]`

Dictionary mapping request\_id to merged stats dict containing
...
{ 'request-123': { 'get\_mm\_hashes\_secs': 0.02, 'get\_cache\_missing\_items\_secs': 0.01, 'apply\_hf\_processor\_secs': 0.45, 'merge\_mm\_kwargs\_secs': 0.01, 'apply\_prompt\_updates\_secs': 0.03, 'preprocessor\_total\_secs': 0.51, 'encoder\_forward\_secs': 0.23, 'num\_encoder\_calls': 1 } }

Source code in `vllm/benchmarks/mm_processor.py`

```
defget_timing_stats_from_engine(llm_engine: LLMEngine) -> dict[str, dict[str, float]]:
"""
    Get all multimodal timing stats from the LLM engine.
...
## main [¶](#vllm.benchmarks.mm_processor.main "Permanent link")
...
## validate\_args [¶](#vllm.benchmarks.mm_processor.validate_args "Permanent link")

Validate command-line arguments for mm\_processor benchmark.

Source code in `vllm/benchmarks/mm_processor.py`

```
defvalidate_args(args):
"""
