---
title: Qwen3-VL - SGLang Documentation
url: https://docs.sglang.io/cookbook/autoregressive/Qwen/Qwen3-VL
source: sitemap
fetched_at: 2026-05-11T05:50:06.417702671-03:00
rendered_js: false
word_count: 840
summary: This document provides an overview and deployment guide for the Qwen3-VL vision-language model series using the SGLang framework, including configuration tips for hardware optimization and instructions for multi-modal API invocation.
tags:
    - sglang
    - qwen3-vl
...
[Qwen3-VL series](https://github.com/QwenLM/Qwen3-VL) are the most powerful vision-language models in the Qwen series to date, featuring advanced capabilities in multi-modal understanding, reasoning, and agentic applications. This generation delivers comprehensive upgrades across the board:
...
B200H100H200MI300XMI325XMI355X

Model Size

235BMOE30BMOE32BDense8BDense4BDense2BDense
...
- **Expert Parallelism** : SGLang supports Expert Parallelism (EP) via `--ep`, allowing experts in MoE models to be deployed on separate GPUs for better throughput. One thing to note is that, for quantized models, you need to set `--ep` to a value that satisfies the requirement: `(moe_intermediate_size / moe_tp_size) % weight_block_size_n == 0, where moe_tp_size is equal to tp_size divided by ep_size.` Note that EP may perform worse in low concurrency scenarios due to additional communication overhead. Check out [Expert Parallelism Deployment](https://docs.sglang.io/docs/advanced_features/expert_parallelism) for more details.
...
**Multi-Image Input Example:** Qwen3-VL can process multiple images in a single request for comparison or analysis:
...
**Video Input Example:** Qwen3-VL supports video understanding by processing video URLs:
...
**Optimized Results (with CUDA IPC Transport):** For further TTFT optimization, enable CUDA IPC Transport for multimodal features by setting `SGLANG_USE_CUDA_IPC_TRANSPORT=1`. This significantly reduces TTFT by using CUDA IPC for transferring multimodal features.
...
- **Test Results:** With `SGLANG_USE_CUDA_IPC_TRANSPORT=1`, TTFT improves significantly:
