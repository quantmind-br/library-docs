...
source: sitemap
fetched_at: 2026-04-17T06:39:20.656091318-03:00
rendered_js: false
word_count: 626
summary: This document explains how to utilize, switch between, and customize opencode modes to suit various development workflows. It details the configuration of built-in modes like build and plan, as well as the creation of custom modes via JSON or Markdown files.
tags:
    - opencode
    - configuration
...
ToolDescription`bash`Execute shell commands`edit`Modify existing files`write`Create new files`read`Read file contents`grep`Search file contents`glob`Find files by pattern`patch`Apply patches to files`todowrite`Manage todo lists`webfetch`Fetch web content
