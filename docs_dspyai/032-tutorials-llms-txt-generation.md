...
url: https://dspy.ai/tutorials/llms_txt_generation/
source: sitemap
fetched_at: 2026-01-23T08:04:03.974204888-03:00
rendered_js: false
word_count: 433
summary: This tutorial explains how to build a DSPy program that analyzes a GitHub repository's structure and content to automatically generate an llms.txt file for LLM-friendly documentation.
tags:
    - dspy
    - llms-txt
...
[](https://github.com/stanfordnlp/dspy/blob/main/docs/docs/tutorials/llms_txt_generation/index.md "Edit this page")

## Generating llms.txt for Code Documentation with DSPy[¶](#generating-llmstxt-for-code-documentation-with-dspy "Permanent link")
...
## What is llms.txt?[¶](#what-is-llmstxt "Permanent link")
...
## Building a DSPy Program for llms.txt Generation[¶](#building-a-dspy-program-for-llmstxt-generation "Permanent link")

Let's create a DSPy program that analyzes a repository and generates comprehensive `llms.txt` documentation.

### Step 1: Define Our Signatures[¶](#step-1-define-our-signatures "Permanent link")
...
classAnalyzeRepository(dspy.Signature):
...
classAnalyzeCodeStructure(dspy.Signature):
...
classGenerateLLMsTxt(dspy.Signature):
...
### Step 2: Create the Repository Analyzer Module[¶](#step-2-create-the-repository-analyzer-module "Permanent link")

```
classRepositoryAnalyzer(dspy.Module):
    def__init__(self):
        super().__init__()
    ...
    defforward(self, repo_url, file_tree, readme_content, package_files):
...
### Step 3: Gather Repository Information[¶](#step-3-gather-repository-information "Permanent link")
...
defget_github_file_tree(repo_url):
...
defget_github_file_content(repo_url, file_path):
...
defgather_repository_info(repo_url):
...
### Step 4: Configure DSPy and Generate llms.txt[¶](#step-4-configure-dspy-and-generate-llmstxt "Permanent link")

```
defgenerate_llms_txt_for_dspy():
...
## Expected Output Structure[¶](#expected-output-structure "Permanent link")
