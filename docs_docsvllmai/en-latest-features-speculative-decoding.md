...
source: sitemap
fetched_at: 2026-05-07T21:14:36.186705036-03:00
rendered_js: false
word_count: 1016
summary: This document provides an overview of speculative decoding methods supported by vLLM, including configuration details for optimizing inference latency and maintaining output accuracy.
tags:
    - vllm
...
[](https://github.com/vllm-project/vllm/edit/main/docs/features/speculative_decoding/README.md "Edit this page")
...
To train your own draft models for optimized speculative decoding, see [vllm-project/speculators](https://docs.vllm.ai/en/latest/features/speculative_decoding/speculators/) for seamless training and integration with vLLM.

## vLLM Speculation Methods[¶](#vllm-speculation-methods "Permanent link")
...
- [EAGLE](https://docs.vllm.ai/en/latest/features/speculative_decoding/eagle/)
- [Multi-Token Prediction (MTP)](https://docs.vllm.ai/en/latest/features/speculative_decoding/mtp/)
- [Draft Model](https://docs.vllm.ai/en/latest/features/speculative_decoding/draft_model/)
- [Parallel Draft Model (PARD)](https://docs.vllm.ai/en/latest/features/speculative_decoding/parallel_draft_model/)
- [Multi-Layer Perceptron](https://docs.vllm.ai/en/latest/features/speculative_decoding/mlp/)
- [N-Gram](https://docs.vllm.ai/en/latest/features/speculative_decoding/n_gram/)
- [Suffix Decoding](https://docs.vllm.ai/en/latest/features/speculative_decoding/suffix/)

## Method Selection at a Glance[¶](#method-selection-at-a-glance "Permanent link")
...
Method Low QPS (latency focused) High QPS (throughput focused) Notes EAGLE High gain Medium to high gain Strong general-purpose model-based method. MTP High gain Medium to high gain Best when the target model has native MTP support. Draft model High gain Medium gain Needs a separate draft model. Parallel Draft Model High gain Medium to high gain Low draft model latency. MLP speculator Medium to high gain Medium gain Good when compatible MLP speculators are available. N-gram Low to medium gain Medium gain Lightweight and easy to enable. Suffix decoding Low to medium gain Medium gain No extra draft model; dynamic speculation depth.

For reproducible measurements in your environment, use [`examples/features/speculative_decoding/spec_decode_offline.py`](https://github.com/vllm-project/vllm/blob/main/examples/features/speculative_decoding/spec_decode_offline.py) or the [benchmark CLI guide](https://docs.vllm.ai/en/latest/benchmarking/cli/).

## `--speculative-config` schema[¶](#-speculative-config-schema "Permanent link")

Use `--speculative-config` to pass speculative decoding settings as a JSON object on the CLI:

```
vllmserve<target-model>\
--speculative-config'{
    "method": "draft_model",
    "model": "<draft-model>",
...
2. **Algorithmic Losslessness** - vLLM’s implementation of speculative decoding is algorithmically validated to be lossless. Key validation tests include:
   ...
   > - **Greedy Sampling Equality**: Confirms that greedy sampling with speculative decoding matches greedy sampling without it. This verifies that vLLM's speculative decoding framework, when integrated with the vLLM forward pass and the vLLM rejection sampler, provides a lossless guarantee. Almost all of the tests in [tests/spec\_decode/e2e](https://docs.vllm.ai/tests/v1/spec_decode). verify this property using [this assertion implementation](https://github.com/vllm-project/vllm/blob/b67ae00cdbbe1a58ffc8ff170f0c8d79044a684a/tests/spec_decode/e2e/conftest.py#L291)
3. **vLLM Logprob Stability** - vLLM does not currently guarantee stable token log probabilities (logprobs). This can result in different outputs for the same request across runs. For more details, see the FAQ section titled *Can the output of a prompt vary across runs in vLLM?* in the [FAQs](https://docs.vllm.ai/en/latest/usage/faq/).
...
For mitigation strategies, please refer to the FAQ entry *Can the output of a prompt vary across runs in vLLM?* in the [FAQs](https://docs.vllm.ai/en/latest/usage/faq/).

## Known Feature Incompatibility[¶](#known-feature-incompatibility "Permanent link")
