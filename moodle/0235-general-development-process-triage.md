---
title: Bug triage | Moodle Developer Resources
url: https://moodledev.io/general/development/process/triage
source: sitemap
fetched_at: 2026-02-17T16:00:48.192251-03:00
rendered_js: false
word_count: 2811
summary: This document outlines the procedures and criteria for the bug triage process in Moodle, explaining how to screen, prioritize, and manage tracker issues effectively.
tags:
    - moodle-tracker
    - bug-triage
    - issue-management
...
## The triage process[​](#the-triage-process "Direct link to The triage process")

### Initial screening[​](#initial-screening "Direct link to Initial screening")
...
- **Has the issue been reported previously?** If so, link to a duplicate issue and close the newly reported issue as a `Duplicate` with no fix version set. Encourage the reporter to search before reporting. If a newer issue has a patch or more voters/watchers, consider closing the older issue. Checking for duplicates first will save you having to check the rest of the issue. See [Tracker tips](https://moodledev.io/general/development/tracker/tips) for help with effective search of tracker.
- **Does the problem only affect unsupported versions?** If so, the issue should be closed using `Fixed` (preferred as it sounds better) when the issue is resolved in current versions, or `Not a bug` when the issue has disappeared due to changes leading to current versions. See the [Releases](https://moodledev.io/general/releases) page to for the list of currently supported versions
...
- **Labels**
  ...
  - Remove functionality tags that some reporters add as labels, only [standard labels or partner-specific labels](https://moodledev.io/general/development/tracker/labels) are used in Moodle
...
### Following up on issues[​](#following-up-on-issues "Direct link to Following up on issues")
...
### Revisiting old issues[​](#revisiting-old-issues "Direct link to Revisiting old issues")
...
- **Does the issue have a patch and if yes, is it still applicable?** If the issue has a patch that still works but the `patch` label is missing, look through comments or history to see if the `patch` label was removed after reviewing the current patch. If you find that label was never added, do it yourself. If the issue has the `patch` label but the patch is no longer applicable or not sufficient, remove the label and comment explaining why.
...
- **Closing: en\_fix**, when it's a correction to English language strings.
...
You can search, re-use and adapt any of the messages in the **Canned response** selector, placed in the tracker editor: ![Canned responses in Tracker](https://moodledev.io/assets/images/cannedresponses-76a21a267934b0074e5e48ef3a3196bc.png)

## Triaging priorities and the Triaging Dashboard[​](#triaging-priorities-and-the-triaging-dashboard "Direct link to Triaging priorities and the Triaging Dashboard")
...
## Creating a filter and gadget for triaging[​](#creating-a-filter-and-gadget-for-triaging "Direct link to Creating a filter and gadget for triaging")
...
In the [Tracker tips and tricks](https://moodledev.io/general/development/tracker/tips#creating-a-filter) page you'll find how to create a filter and add it to a dashboard.

## See also[​](#see-also "Direct link to See also")

- [Tracker guide](https://moodledev.io/general/development/tracker/guide)
