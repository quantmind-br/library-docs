...
source: llms
fetched_at: 2026-01-24T14:04:41.07488253-03:00
rendered_js: false
word_count: 280
summary: This guide explains how to execute .NET unit tests within Docker containers during local development and image building. It demonstrates using Docker Compose for interactive testing and modifying Dockerfiles to automate tests in multi-stage builds.
tags:
    - dotnet
    - docker
    - unit-testing
...
```
# syntax=docker/dockerfile:1FROM--platform=$BUILDPLATFORM mcr.microsoft.com/dotnet/sdk:8.0-alpine AS buildARG TARGETARCHCOPY . /sourceWORKDIR/source/srcRUN --mount=type=cache,id=nuget,target=/root/.nuget/packages \
    dotnet publish -a ${TARGETARCH/amd64/x64} --use-current-runtime --self-contained false -o /appRUN dotnet test /source/testsFROMmcr.microsoft.com/dotnet/sdk:8.0-alpineASdevelopmentCOPY . /sourceWORKDIR/source/srcCMD dotnet run --no-launch-profileFROMmcr.microsoft.com/dotnet/aspnet:8.0-alpineASfinalWORKDIR/appCOPY --from=build /app .ARG UID=10001
RUN adduser \
    --disabled-password \
    --gecos "" \
    --home "/nonexistent" \
    ...
    --no-create-home \
    --uid "${UID}" \
    appuserUSERappuserENTRYPOINT ["dotnet", "myWebApp.dll"]
```
