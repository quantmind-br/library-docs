...
source: sitemap
fetched_at: 2026-01-30T20:30:04.991291839-03:00
rendered_js: false
word_count: 703
summary: This document explains how OpenClaw uses Docker containers to sandbox tool execution, detailing configuration options for controlling sandbox behavior, scope, workspace access, and security considerations.
tags:
    - docker
    - sandboxing
    - security
...
By default, sandbox containers run with **no network**. Override with `agents.defaults.sandbox.docker.network`. Docker installs and the containerized gateway live here: [Docker](https://docs.openclaw.ai/install/docker)
...
Each agent can override sandbox + tools: `agents.list[].sandbox` and `agents.list[].tools` (plus `agents.list[].tools.sandbox.tools` for sandbox tool policy). See [Multi-Agent Sandbox & Tools](https://docs.openclaw.ai/multi-agent-sandbox-tools) for precedence.
