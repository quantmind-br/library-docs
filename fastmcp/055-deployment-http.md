...
url: https://gofastmcp.com/deployment/http#oauth-token-security
source: crawler
fetched_at: 2026-01-22T22:21:52.993469368-03:00
rendered_js: false
word_count: 2206
summary: This document explains how to deploy FastMCP servers as remote HTTP services using either direct server runs or ASGI applications. It covers essential configuration for production environments, including security, health monitoring, and middleware integration.
tags:
    - fastmcp
    - remote-deployment
    - asgi
...
When running tools that take a long time to complete, you may encounter issues with load balancers or proxies terminating connections that stay idle too long. [SEP-1699](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/1699) introduces SSE polling to solve this by allowing the server to gracefully close connections and have clients automatically reconnect. To enable SSE polling, configure an `EventStore` when creating your HTTP application:
...
New in version `2.10.2` When deploying FastMCP behind a load balancer or running multiple server instances, you need to understand how the HTTP transport handles sessions and configure your server appropriately.
