---
title: serving - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/entrypoints/openai/completion/serving/
source: sitemap
fetched_at: 2026-05-07T21:19:59.960142815-03:00
rendered_js: false
word_count: 16
summary: This document defines the OpenAIServingCompletion class, which implements a server-side handler for OpenAI-compatible completion API requests, managing request validation, tokenization, engine scheduling, and response formatting.
tags:
    - openai-api
    - completion-handler
    - request-processing
...
```
classOpenAIServingCompletion(OpenAIServing):
    def__init__(
        self,
...
"""Completion API similar to OpenAI's API.
        ...
        if request.stream:
            ...
            async deffake_stream_generator() -> AsyncGenerator[str, None]:
    ...
    async defcompletion_stream_generator(
        self,
        request: CompletionRequest,
    ...
    defrequest_output_to_completion_response(
        self,
    ...
    def_create_completion_logprobs(
        self,
        token_ids: GenericSequence[int],
