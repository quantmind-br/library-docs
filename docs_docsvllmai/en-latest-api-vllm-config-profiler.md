...
source: sitemap
fetched_at: 2026-05-07T21:17:09.76608229-03:00
rendered_js: false
word_count: 643
summary: This document defines the ProfilerConfig dataclass in vLLM, which manages settings for engine performance profiling, including PyTorch and CUDA profiler configurations and scheduling options.
tags:
    - vllm
    - profiler
    - performance-tuning
...
## ProfilerConfig [¶](#vllm.config.profiler.ProfilerConfig "Permanent link")
...
"""
    ...
    def_validate_profiler_config(self) -> Self:
...
### active\_iterations `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.active_iterations "Permanent link")
...
### delay\_iterations `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.delay_iterations "Permanent link")

```
delay_iterations: int = Field(default=0, ge=0)
```

Number of engine iterations to skip before starting profiling. Defaults to 0, meaning profiling starts immediately after receiving /start\_profile.

### ignore\_frontend `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.ignore_frontend "Permanent link")
...
### max\_iterations `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.max_iterations "Permanent link")
...
### profiler `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.profiler "Permanent link")
...
### torch\_profiler\_dir `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.torch_profiler_dir "Permanent link")
...
### torch\_profiler\_dump\_cuda\_time\_total `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.torch_profiler_dump_cuda_time_total "Permanent link")
...
### torch\_profiler\_record\_shapes `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.torch_profiler_record_shapes "Permanent link")
...
### torch\_profiler\_use\_gzip `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.torch_profiler_use_gzip "Permanent link")
...
### torch\_profiler\_with\_flops `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.torch_profiler_with_flops "Permanent link")
...
### torch\_profiler\_with\_memory `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.torch_profiler_with_memory "Permanent link")
...
### torch\_profiler\_with\_stack `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.torch_profiler_with_stack "Permanent link")

```
torch_profiler_with_stack: bool = True
```

If `True`, enables stack tracing in the torch profiler. Enabled by default as it is useful for debugging. Can be disabled via --profiler-config.torch\_profiler\_with\_stack=false CLI flag.

### wait\_iterations `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.wait_iterations "Permanent link")
...
### warmup\_iterations `class-attribute` `instance-attribute` [¶](#vllm.config.profiler.ProfilerConfig.warmup_iterations "Permanent link")
...
### compute\_hash [¶](#vllm.config.profiler.ProfilerConfig.compute_hash "Permanent link")
...
## \_is\_uri\_path [¶](#vllm.config.profiler._is_uri_path "Permanent link")
