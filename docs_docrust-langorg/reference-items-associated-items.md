...
url: https://doc.rust-lang.org/reference/items/associated-items.html#grammar-AssociatedItem
source: crawler
fetched_at: 2026-05-06T21:36:02.494484137-03:00
rendered_js: false
word_count: 1434
summary: This document explains the concept of associated items in the Rust programming language, detailing how associated functions, methods, and associated types are declared and implemented within traits and types.
tags:
    - rust-programming
    - associated-items
...
*Associated Items* are the items declared in [traits](https://doc.rust-lang.org/reference/items/traits.html) or defined in [implementations](https://doc.rust-lang.org/reference/items/implementations.html). They are called this because they are defined on an associate type — the type in the implementation.

They are a subset of the kinds of items you can declare in a module. Specifically, there are [associated functions](#associated-functions-and-methods) (including methods), [associated types](#associated-types), and [associated constants](#associated-constants).
...
*Associated functions* are [functions](https://doc.rust-lang.org/reference/items/functions.html) associated with a type.
...
An *associated function definition* defines a function associated with another type. It is written the same as a [function item](https://doc.rust-lang.org/reference/types/function-item.html).
...
When the associated function is declared on a trait, the function can also be called with a [path](https://doc.rust-lang.org/reference/paths.html) that is a path to the trait appended by the name of the trait. When this happens, it is substituted for `<_ as Trait>::function_name`.
...
Associated functions whose first parameter is named `self` are called *methods* and may be invoked using the [method call operator](https://doc.rust-lang.org/reference/expressions/method-call-expr.html), for example, `x.foo()`, as well as the usual function call notation.
...
ShorthandEquivalent `self``self: Self` `&'lifetime self``self: &'lifetime Self` `&'lifetime mut self``self: &'lifetime mut Self`

> Note
> 
> Lifetimes can be, and usually are, elided with this shorthand.

If the `self` parameter is prefixed with `mut`, it becomes a mutable variable, similar to regular parameters using a `mut` [identifier pattern](https://doc.rust-lang.org/reference/patterns.html#identifier-patterns). For example:
...
This defines a trait with two methods. All values that have [implementations](https://doc.rust-lang.org/reference/items/implementations.html) of this trait while the trait is in scope can have their `draw` and `bounding_box` methods called.
...
Attributes on method parameters follow the same rules and restrictions as [regular function parameters](https://doc.rust-lang.org/reference/items/functions.html#attributes-on-function-parameters).

## [Associated types](#associated-types)

*Associated types* are [type aliases](https://doc.rust-lang.org/reference/items/type-aliases.html) associated with another type.

Associated types cannot be defined in [inherent implementations](https://doc.rust-lang.org/reference/items/implementations.html#inherent-implementations) nor can they be given a default implementation in traits.
...
There is an implicit [`Sized`](https://doc.rust-lang.org/reference/special-types-and-traits.html#sized) bound on associated types that can be relaxed using the special `?Sized` bound.
...
*Associated constants* are [constants](https://doc.rust-lang.org/reference/items/constant-items.html) associated with a type.
...
An *associated constant definition* defines a constant associated with a type. It is written the same as a [constant item](https://doc.rust-lang.org/reference/items/constant-items.html).

Associated constant definitions undergo [constant evaluation](https://doc.rust-lang.org/reference/const_eval.html) only when referenced. Further, definitions that include [generic parameters](https://doc.rust-lang.org/reference/items/generics.html) are evaluated after monomorphization.
