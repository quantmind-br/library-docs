---
title: 2 docs tagged with "core_availability" | Moodle Developer Resources
url: https://moodledev.io/docs/4.5/tags/core-availability
source: sitemap
fetched_at: 2026-02-17T15:06:30.7879-03:00
rendered_js: false
word_count: 73
summary: This document explains the Moodle Availability API and its conditions, which allow for controlling and restricting access to course activities and sections based on defined criteria.
tags:
    - moodle-api
    - access-control
    - availability-conditions
    - plugin-development
    - course-management
category: api
---

## [Availability API](https://moodledev.io/docs/4.5/apis/subsystems/availability)

The availability API controls access to activities and sections. For example, a teacher could restrict access so that an activity cannot be accessed until a certain date, or so that a section cannot be accessed unless users have a certain grade in a quiz.

## [Availability conditions](https://moodledev.io/docs/4.5/apis/plugintypes/availability)

Availability conditions allow teachers to restrict an activity or section so that only certain users can access it. These are accessed using the Availability API.