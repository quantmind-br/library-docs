---
title: elastic_state - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/distributed/elastic_ep/elastic_state/
source: sitemap
fetched_at: 2026-05-07T21:17:51.949200433-03:00
rendered_js: false
word_count: 0
summary: This document defines the ElasticEPScalingState class, which manages state transitions and synchronization barriers during the elastic scaling process of distributed model engines.
tags:
    - distributed-systems
    - vllm
    - elastic-scaling
...
classElasticEPScalingState:
    ...
    @property
    defmodel_executor(self) -> "Executor":
    ...
    defrun_pre_kv_init_states(self) -> None:
    ...
    def_staged_barrier(self, use_new_group: bool, barrier_name: str) -> bool:
"""
        ...
        try:
            ...
            if timeout is None:
                raise RuntimeError("Unexpected timeout encountered") frome
            dp_store.compare_set(sync_key, "", b"1")
            return False

    def_progress_existing_engine(self) -> bool:
        state = self.state
    ...
    def_progress_new_engine(self) -> bool:
        state = self.state
        assert self.new_dp_group is not None and self.new_dp_store is not None
    ...
    def_progress_remaining_engine(self) -> bool:
        state = self.state
        assert self.old_dp_group is not None and self.old_dp_store is not None
    ...
    def_progress_removing_engine(self) -> bool:
        state = self.state
        assert self.old_dp_group is not None and self.old_dp_store is not None
    ...
    defhandle_notification(self, notification_type: EEPNotificationType):
    ...
    def_create_standby_groups(self):
    ...
    def_transfer_expert_mapping(self):
        assert self.old_dp_group is not None
        self.model_executor.collective_rpc(
    ...
    def_sync_kv_cache_memory_size(self):
    ...
    def_eplb_reshuffle_before_scale_down(self):
        assert self.reconfig_request is not None and self.old_dp_group is not None
        self.model_executor.collective_rpc(
            "elastic_ep_execute",
            args=(
    ...
    def_switch_and_remove(self):
        self.model_executor.collective_rpc(
            "elastic_ep_execute", args=("switch_and_remove",)
        )

    def_update_parallel_config(self):
