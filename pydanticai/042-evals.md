---
title: Pydantic Evals - Pydantic AI
url: https://ai.pydantic.dev/evals/
source: sitemap
fetched_at: 2026-01-22T22:23:26.472148955-03:00
rendered_js: false
word_count: 1175
summary: Pydantic Evals is a code-first framework for systematically evaluating AI systems through datasets, experiments, and various scoring evaluators. It enables developers to test LLM outputs and agent behaviors using a structured Python-based data model.
tags:
    - pydantic-evals
    - llm-evaluation
    - ai-testing
...
- [Span-Based Evaluation](https://ai.pydantic.dev/evals/evaluators/span-based/) - Evaluate internal agent behavior (tool calls, execution flow) using OpenTelemetry traces. Essential for complex agents where correctness depends on *how* the answer was reached, not just the final output. Also ensures eval assertions align with production telemetry.
...
In Pydantic Evals, everything begins with [`Dataset`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset "Dataset")s and [`Case`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Case "Case            dataclass   ")s:
...
- [**`Case`**](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Case "Case            dataclass   "): A single test scenario corresponding to Task inputs, with optional expected outputs, metadata, and case-specific evaluators
...
[`Evaluator`](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.Evaluator "Evaluator            dataclass   ")s analyze and score the results of your Task when tested against a Case.
...
1. You can add built-in evaluators to a dataset using the [`add_evaluator`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.add_evaluator "add_evaluator") method.
...
1. Create a [test case](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Case "Case            dataclass   ") as above
2. Create a [`Dataset`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset "Dataset") with test cases and [`evaluators`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluators "evaluators            class-attribute       instance-attribute   ")
3. Our function to evaluate.
4. Run the evaluation with [`evaluate_sync`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluate_sync "evaluate_sync"), which runs the function against all test cases in the dataset, and returns an [`EvaluationReport`](https://ai.pydantic.dev/api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport "EvaluationReport            dataclass   ") object.
5. Print the report with [`print`](https://ai.pydantic.dev/api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport.print "print"), which shows the results of the evaluation. We have omitted duration here just to keep the printed output from changing from run to run.
