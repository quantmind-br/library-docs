...
source: sitemap
fetched_at: 2026-01-30T20:36:53.896864722-03:00
rendered_js: false
word_count: 2744
summary: This document provides comprehensive instructions for setting up and configuring the Microsoft Teams plugin for OpenClaw, covering installation, Azure bot setup, Teams app configuration, and access control policies.
tags:
    - microsoft-teams
    - plugin-setup
    - azure-bot
...
If you choose Teams during configure/onboarding and a git checkout is detected, OpenClaw will offer the local install path automatically. Details: [Plugins](https://docs.openclaw.ai/plugin)
...
Without Graph permissions, channel messages with images will be received as text-only (the image content is not accessible to the bot). By default, OpenClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
