...
source: llms
fetched_at: 2026-02-05T21:41:10.458139217-03:00
rendered_js: false
word_count: 963
summary: This document explains how to configure and use custom LLM providers and API keys within the Factory CLI using the Bring Your Own Key (BYOK) feature.
tags:
    - byok
    - factory-cli
    - llm-configuration
...
<Note>
  **Legacy support**: Custom models in `~/.factory/config.json` using snake\_case field names (`custom_models`, `base_url`, etc.) are still supported for backwards compatibility. Both files are loaded and merged, with `settings.json` taking priority.
</Note>
