---
title: Adaptive Speculative Decoding - SGLang Documentation
url: https://docs.sglang.io/docs/advanced_features/adaptive_speculative_decoding
source: sitemap
fetched_at: 2026-05-11T05:49:44.326084552-03:00
rendered_js: false
word_count: 582
summary: This document describes the adaptive speculative decoding mechanism in SGLang, which dynamically adjusts the number of draft steps at runtime based on workload acceptance patterns to optimize inference performance.
tags:
    - sglang
...
KeyDefaultMeaning`candidate_steps``[1, 3, 7]`Discrete `speculative_num_steps` tiers that adaptive mode can switch between`ema_alpha``0.2`EMA smoothing factor for accepted draft length`update_interval``5`Recompute interval, in verify batches, after warmup`warmup_batches``10`Number of verify batches to observe before switching`down_hysteresis``-0.25`Extra margin before moving to a smaller step`up_hysteresis``0.0`Extra margin before moving to a larger step
