...
url: https://hyprland-community.github.io/pyprland/versions/2.3.5/monitors
source: github_pages
fetched_at: 2026-01-31T16:01:15.186974037-03:00
rendered_js: false
word_count: 363
summary: Provides configuration instructions for setting up relative monitor placement in Hyprland using a TOML-based syntax, including support for monitor positioning, settings, and hotplug commands.
tags:
    - monitor-placement
    - hyprland
    - configuration
...
You can separate the terms with "\_" to improve the readability, as in "top\_center\_of".

#### monitor settings (since 2.3.2) [​](#monitor-settings-since-2-3-2)
...
### `startup_relayout` (optional) [​](#startup-relayout-optional)

Default to `ŧrue`.

When set to `false`, do not initialize the monitor layout on startup or when configuration is reloaded.

### `full_relayout` (legacy - always enabled now) [​](#full-relayout-legacy-always-enabled-now)

### `new_monitor_delay` (optional) [​](#new-monitor-delay-optional)
...
### `hotplug_command` (optional) [​](#hotplug-command-optional)
...
### `hotplug_commands` (optional) [​](#hotplug-commands-optional)
...
### `unknown` (optional) [​](#unknown-optional)

> *Added in 2.3.0*

None by default, allows to run a command when no monitor layout has been changed (no rule applied).
...
### `trim_offset` (optional) [​](#trim-offset-optional)
