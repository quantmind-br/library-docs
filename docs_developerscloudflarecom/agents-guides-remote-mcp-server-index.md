...
url: https://developers.cloudflare.com/agents/guides/remote-mcp-server/index.md
source: llms
fetched_at: 2026-01-24T13:59:49.009677499-03:00
rendered_js: false
word_count: 1444
summary: This guide explains how to deploy and configure a remote Model Context Protocol (MCP) server on Cloudflare Workers using Streamable HTTP transport. It covers setup, local development, and the implementation of user authentication using OAuth.
tags:
    - mcp-server
    - cloudflare-workers
    - streamable-http
...
Once deployed, this server will be live at your workers.dev subdomain (e.g. remote-mcp-server-authless.your-account.workers.dev/mcp). You can connect to it immediately using the [AI Playground](https://playground.ai.cloudflare.com/) (a remote MCP client), [MCP inspector](https://github.com/modelcontextprotocol/inspector) or [other MCP clients](https://developers.cloudflare.com/agents/guides/remote-mcp-server/#connect-your-remote-mcp-server-to-claude-and-other-mcp-clients-via-a-local-proxy). Then, once you're ready, you can customize the MCP server and add your own [tools](https://developers.cloudflare.com/agents/model-context-protocol/tools/).
...
Now that you've added the ID and secret of your production OAuth app, you should now be able to connect to your MCP server running at `worker-name.account-name.workers.dev/mcp` using the [AI Playground](https://playground.ai.cloudflare.com/), MCP inspector or ([other MCP clients](https://developers.cloudflare.com/agents/guides/remote-mcp-server/#connect-your-mcp-server-to-claude-and-other-mcp-clients)), and authenticate with GitHub.
