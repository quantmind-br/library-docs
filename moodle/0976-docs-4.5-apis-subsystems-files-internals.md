---
title: File API internals | Moodle Developer Resources
url: https://moodledev.io/docs/4.5/apis/subsystems/files/internals
source: sitemap
fetched_at: 2026-02-17T15:14:10.501975-03:00
rendered_js: false
word_count: 1495
summary: This document outlines the architecture and core components of Moodle's File API, explaining how it handles file storage, metadata, and serving through a consistent interface. It details the use of content hashing for deduplication and the various sub-APIs responsible for file management and user interface integration.
tags:
    - moodle-file-api
    - file-storage
    - content-hashing
...
See the [File API](https://moodledev.io/docs/4.5/apis/subsystems/files) documentation for information on using the File API.
...
## File API internals[​](#file-api-internals "Direct link to File API internals")
...
### Files table[​](#files-table "Direct link to Files table")
...
## Implementation of basic operations[​](#implementation-of-basic-operations "Direct link to Implementation of basic operations")
...
Details of common operations are documented in the [File System API](https://moodledev.io/docs/4.5/apis/subsystems/files) documentation
...
- links to these files are constructed on the fly from the relative links stored in database (see [Generating a URL to your files](https://moodledev.io/docs/4.5/apis/subsystems/files#generating-a-url-to-your-files) for further information).
...
- The **file picker** is an interface, often accessed using the *file manager*, to select files for upload to Moodle. The file picker makes use of [file repositories](https://moodledev.io/docs/4.5/apis/plugintypes/repository).

### Form fields[​](#form-fields "Direct link to Form fields")
...
### Integration with the HTML editor[​](#integration-with-the-html-editor "Direct link to Integration with the HTML editor")
...
## Other issues[​](#other-issues "Direct link to Other issues")

### Unicode support in zip format[​](#unicode-support-in-zip-format "Direct link to Unicode support in zip format")
...
#### Client software[​](#client-software "Direct link to Client software")
...
#### Tar Alternative[​](#tar-alternative "Direct link to Tar Alternative")
...
2. using single byte encoding "garbage in/garbage out" approach for encoding of files in zip archives; add new `zipencoding` string into lang packs (for example `cp852` DOS charset for Czech locale) and use it during extraction (we might support true unicode later when PHP Zip extension does that)

### Tar packer[​](#tar-packer "Direct link to Tar packer")

In addition to the zip packer, a tar packer is also available. This creates archives in a compressed tar format (similar to the file created by `tar -czf example.tar.gz mycontent`).
...
- [Repository API](https://moodledev.io/docs/4.5/apis/plugintypes/repository)
