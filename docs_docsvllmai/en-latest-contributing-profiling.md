...
source: sitemap
fetched_at: 2026-05-07T21:11:20.813672091-03:00
rendered_js: false
word_count: 972
summary: This document provides instructions for vLLM developers on how to use PyTorch Profiler and NVIDIA Nsight Systems to analyze code performance and identify bottlenecks.
tags:
    - profiling
    - vllm
    - performance-tuning
...
Traces can be visualized using [https://ui.perfetto.dev/](https://ui.perfetto.dev/).
...
To stop the profiler - it flushes out all the profile trace files to the directory. This takes time, for example for about 100 requests worth of data for a llama 70b, it takes about 10 minutes to flush out on a H100. Set the env variable VLLM\_RPC\_TIMEOUT to a big number before you start the server. Say something like 30 minutes. `export VLLM_RPC_TIMEOUT=1800000`

### Example commands and usage[¶](#example-commands-and-usage "Permanent link")

#### Offline Inference[¶](#offline-inference "Permanent link")

Refer to [examples/features/profiling/simple\_profiling\_offline.py](https://github.com/vllm-project/vllm/blob/main/examples/features/profiling/simple_profiling_offline.py) for an example.

#### OpenAI Server[¶](#openai-server "Permanent link")

```
vllmservemeta-llama/Llama-3.1-8B-Instruct--profiler-config'{"profiler": "torch", "torch_profiler_dir": "./vllm_profile"}'
```
...
$curl-XPOSThttp://localhost:8000/start_profile

# Call model generate.
curl-XPOSThttp://localhost:8000/v1/chat/completions\
...
$curl-XPOSThttp://localhost:8000/stop_profile
```
...
--capture-range-endrepeat\
vllmservemeta-llama/Llama-3.1-8B-Instruct--profiler-config.profilercuda
...
```
nsysstatsreport1.nsys-rep
...
**CUDAGPUKernelSummary(cuda_gpu_kern_sum):

Time(%)TotalTime(ns)InstancesAvg(ns)Med(ns)Min(ns)Max(ns)StdDev(ns)Name
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
46.310,327,352,33817,505589,965.9144,383.027,0403,126,460944,263.8sm90_xmma_gemm_bf16bf16_bf16f32_f32_tn_n_tilesize128x128x64_warpgroupsize1x1x1_execute_segment_k_of…
14.83,305,114,7645,152641,520.7293,408.0287,2962,822,716867,124.9sm90_xmma_gemm_bf16bf16_bf16f32_f32_tn_n_tilesize256x128x64_warpgroupsize2x1x1_execute_segment_k_of…
12.12,692,284,87614,280188,535.483,904.019,3282,862,237497,999.9sm90_xmma_gemm_bf16bf16_bf16f32_f32_tn_n_tilesize64x128x64_warpgroupsize1x1x1_execute_segment_k_off…
9.52,116,600,57833,92062,399.821,504.015,3262,532,285290,954.1sm90_xmma_gemm_bf16bf16_bf16f32_f32_tn_n_tilesize64x64x64_warpgroupsize1x1x1_execute_segment_k_off_…
5.01,119,749,16518,91259,208.49,056.06,7842,578,366271,581.7voidvllm::act_and_mul_kernel<c10::BFloat16,&vllm::silu_kernel<c10::BFloat16>,(bool)1>(T1*,cons…
4.1916,662,51521,31243,011.619,776.08,9282,586,205199,790.1voidcutlass::device_kernel<flash::enable_sm90_or_later<flash::FlashAttnFwdSm90<flash::CollectiveMa…
2.6587,283,11337,82415,526.73,008.02,7192,517,756139,091.1std::enable_if<T2>(int)0&&vllm::_typeConvert<T1>::exists,void>::typevllm::fused_add_rms_norm_kern…
1.9418,362,60518,91222,121.53,871.03,3282,523,870175,248.2voidvllm::rotary_embedding_kernel<c10::BFloat16,(bool)1>(constlong*,T1*,T1*,constT1*,in…
0.7167,083,06918,8808,849.72,240.01,4712,499,996101,436.1voidvllm::reshape_and_cache_flash_kernel<__nv_bfloat16,__nv_bfloat16,(vllm::Fp8KVCacheDataType)0…
...
```
...
The `vllm.utils.profiling` helpers are deprecated and will be removed in `v0.21`. Please use Python's `cProfile` module directly instead.
...
```
fromvllm.utils.profilingimport cprofile
...
```
fromvllm.utils.profilingimport cprofile_context
...
```
pipinstallsnakeviz
snakevizexpensive_function.prof
```

### Analyzing Garbage Collection Costs[¶](#analyzing-garbage-collection-costs "Permanent link")

Leverage VLLM\_GC\_DEBUG environment variable to debug GC costs.

- VLLM\_GC\_DEBUG=1: enable GC debugger with gc.collect elapsed times
- VLLM\_GC\_DEBUG='{"top\_objects":5}': enable GC debugger to log top 5 collected objects for each gc.collect