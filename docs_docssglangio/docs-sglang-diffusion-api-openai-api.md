---
title: OpenAI API - SGLang Documentation
url: https://docs.sglang.io/docs/sglang-diffusion/api/openai_api
source: sitemap
fetched_at: 2026-05-11T05:51:26.416481482-03:00
rendered_js: false
word_count: 945
summary: This document provides a comprehensive guide to using the SGLang diffusion HTTP server, which offers an OpenAI-compatible API for image and video generation and LoRA adapter management.
tags:
    - sglang
    - diffusion
    - openai-api
...
The server implements an OpenAI-compatible Images API under the `/v1/images` namespace. **Create an image** **Endpoint:** `POST /v1/images/generations` **Python Example (b64\_json response):**
...
**Edit an image** **Endpoint:** `POST /v1/images/edits` This endpoint accepts a multipart form upload with input images and a text prompt. The server can return either a base64-encoded image or a URL to download the image. **Curl Example (b64\_json response):**
