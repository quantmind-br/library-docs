---
title: MCP servers with Qwen Code
url: https://qwenlm.github.io/qwen-code-docs/en/developers/tools/mcp-server
source: github_pages
fetched_at: 2026-04-09T09:04:59.505113249-03:00
rendered_js: true
word_count: 3287
summary: This document serves as a comprehensive guide detailing how to configure and utilize Model Context Protocol (MCP) servers with Qwen Code, explaining the architecture, transport mechanisms, and required settings in `settings.json` for successful integration.
tags:
    - mcp-protocol
    - qwen-code
    - configuration
...
### Discovery Layer (`mcp-client.ts`)[](#discovery-layer-mcp-clientts)
...
### Execution Layer (`mcp-tool.ts`)[](#execution-layer-mcp-toolts)
...
### Configure the MCP server in settings.json[](#configure-the-mcp-server-in-settingsjson)
...
#### Required (one of the following)[](#required-one-of-the-following)
...
#### Browser Redirect Requirements[](#browser-redirect-requirements)
...
#### Managing OAuth Authentication[](#managing-oauth-authentication)
...
#### OAuth Configuration Properties[](#oauth-configuration-properties)
...
#### Authentication Provider Type[](#authentication-provider-type)
...
#### Python MCP Server (Stdio)[](#python-mcp-server-stdio)
...
#### Node.js MCP Server (Stdio)[](#nodejs-mcp-server-stdio)
...
#### Docker-based MCP Server[](#docker-based-mcp-server)
...
#### HTTP-based MCP Server with Custom Headers[](#http-based-mcp-server-with-custom-headers)
...
#### MCP Server with Tool Filtering[](#mcp-server-with-tool-filtering)
...
### SSE MCP Server with SA Impersonation[](#sse-mcp-server-with-sa-impersonation)
...
### 2. Confirmation Process[](#2-confirmation-process)

Each `DiscoveredMCPTool` implements sophisticated confirmation logic:

#### Trust-based Bypass[](#trust-based-bypass)
...
#### Dynamic Allow-listing[](#dynamic-allow-listing)
...
#### User Choice Handling[](#user-choice-handling)
...
## How to interact with your MCP server[](#how-to-interact-with-your-mcp-server)

### Using the `/mcp` Command[](#using-the-mcp-command)
...
### Example `/mcp` Output[](#example-mcp-output)
...
## Status Monitoring and Troubleshooting[](#status-monitoring-and-troubleshooting)

### Connection States[](#connection-states)

The MCP integration tracks several states:

#### Server Status (`MCPServerStatus`)[](#server-status-mcpserverstatus)

- **`DISCONNECTED`:** Server is not connected or has errors
- **`CONNECTING`:** Connection attempt in progress
- **`CONNECTED`:** Server is connected and ready

#### Discovery State (`MCPDiscoveryState`)[](#discovery-state-mcpdiscoverystate)
...
#### Server Won’t Connect[](#server-wont-connect)
...
### Example: Returning Text and an Image[](#example-returning-text-and-an-image)
...
### Defining Prompts on the Server[](#defining-prompts-on-the-server)
...
## Managing MCP Servers with `qwen mcp`[](#managing-mcp-servers-with-qwen-mcp)
...
#### Adding an stdio server[](#adding-an-stdio-server)
...
### Managing Servers (`qwen mcp`)[](#managing-servers-qwen-mcp)
