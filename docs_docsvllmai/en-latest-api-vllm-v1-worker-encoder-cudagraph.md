---
title: encoder_cudagraph - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/worker/encoder_cudagraph/
source: sitemap
fetched_at: 2026-05-07T21:42:14.459294074-03:00
rendered_js: false
word_count: 118
summary: This class manages the lifecycle of CUDA graphs for vision encoders, including initialization, automated token budget calculation, graph capture, and optimized execution based on token usage.
tags:
    - cuda-graphs
    - vision-encoder
...
"""Initialize CUDA graph manager with provided token budgets
    ...
    @staticmethod
    def_generate_budgets(min_budget: int, max_budget: int) -> list[int]:
"""Generate power-of-2 token budgets from min_budget to max_budget."""
    ...
    defsupports_modality(self, modality: str) -> bool:
...
"""Capture CUDA graphs for all token budgets."""
    ...
    def_capture_budget_graph(self, token_budget: int):
"""Capture CUDA graph for a single token budget."""
    ...
    def_find_smallest_fitting_budget_given_tokens(
...
"""Find smallest budget >= total_tokens.
    ...
    def_get_per_item_out_tokens(self, mm_kwargs: dict[str, Any]) -> list[int]:
...
"""Execute encoder using CUDA graph with optional DP.
    ...
    defget_cumulative_stats(self) -> dict[str, Any]:
