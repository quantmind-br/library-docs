---
title: encoder_cache_manager - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/core/encoder_cache_manager/
source: sitemap
fetched_at: 2026-05-07T21:40:16.399570962-03:00
rendered_js: false
word_count: 1244
summary: This document describes the EncoderCacheManager class, which provides memory-aware caching for multimodal encoder outputs in vLLM V1 to prevent redundant computation. It outlines the mechanisms for managing cache lifecycle, shared embedding storage, and LRU-style eviction policies for multimodal inputs.
tags:
    - multimodal-models
    - caching-mechanism
    - vllm-framework
...
## EncoderCacheManager [¶](#vllm.v1.core.encoder_cache_manager.EncoderCacheManager "Permanent link")
...
Name Type Description Default `cache_size` `int`

Limit the size of the cache, measured by the number of encoder embeddings from the input sequence.

*required*

Attributes:

Name Type Description `cache_size`

Total cache capacity in encoder embeddings.

`num_free_slots`

Current available cache capacity in encoder embeddings.

`num_freeable_slots`

Capacity that can be immediately reclaimed by evicting entries with zero references (in encoder embeddings).

`cached` `dict[str, set[str]]`

Mapping from mm\_hash to a set of request IDs that currently reference the cached entry. If the set is empty, the entry exists but is not referenced by any request and is eligible for reclamation.

`freeable` `OrderedDict[str, int]`

List of tuples (mm\_hash, num\_encoder\_embeds) representing entries whose no current running request is needed and that can be freed to make space when needed.

`freed` `list[str]`

List of mm\_hash strings that were actually evicted since the last call to get\_freed\_mm\_hashes(). This list is cleared on return.

Source code in `vllm/v1/core/encoder_cache_manager.py`

```
classEncoderCacheManager:
"""Manages caching of encoder outputs for multimodal models in vLLM V1.
    ...
    def__init__(self, cache_size: int):
...
"""Reset the encoder cache to its initial state.
    ...
    defcheck_and_update_cache(self, request: Request, input_id: int) -> bool:
...
"""Check if there's sufficient cache space for a multimodal input.
    ...
    defallocate(self, request: Request, input_id: int) -> None:
"""Allocate cache space for a multimodal input's encoder output.
    ...
    defget_cached_input_ids(self, request: Request) -> set[int]:
"""Get all cached multimodal input IDs for a request.
    ...
    deffree_encoder_input(self, request: Request, input_id: int) -> None:
"""Free the request's reference to the encoder input (`mm_data`)
    ...
    deffree(self, request: Request) -> None:
"""Free all encoder input cache reference held by *request*.
    ...
    defget_freed_mm_hashes(self) -> list[str]:
...
### allocate [¶](#vllm.v1.core.encoder_cache_manager.EncoderCacheManager.allocate "Permanent link")
...
This method assumes can\_allocate() returned True for the same input.
...
### can\_allocate [¶](#vllm.v1.core.encoder_cache_manager.EncoderCacheManager.can_allocate "Permanent link")
...
If there is not enough free space in `num_free_slots` but there is enough reclaimable space in `num_freeable_slots`, entries will be evicted from `freeable` (their mm\_hash appended to `freed`) until enough space is available, and then this method returns True. Older entries are evicted first.
...
*required* `input_id` `int`

Index of the multimodal input within the request.

*required* `encoder_compute_budget` `int`

Number of encoder embeddings allowed to be computed when this method is invoked.

*required* `num_embeds_to_schedule` `int`

Number of encoder embeddings already scheduled to be allocated with cache space when this method is invoked.
...
### check\_and\_update\_cache [¶](#vllm.v1.core.encoder_cache_manager.EncoderCacheManager.check_and_update_cache "Permanent link")
...
### free [¶](#vllm.v1.core.encoder_cache_manager.EncoderCacheManager.free "Permanent link")

Free all encoder input cache reference held by *request*.

For each cached input ID, `free_encoder_input` is invoked. The data stays in memory until eviction is triggered by a future attempt allocation called by 'can\_allocate'.
...
### free\_encoder\_input [¶](#vllm.v1.core.encoder_cache_manager.EncoderCacheManager.free_encoder_input "Permanent link")
...
### get\_cached\_input\_ids [¶](#vllm.v1.core.encoder_cache_manager.EncoderCacheManager.get_cached_input_ids "Permanent link")

Get all cached multimodal input IDs for a request.

Returns the set of input IDs whose `mm_hash` exists in the cache map. This includes entries that are currently unreferenced (and thus present in `freeable`); for such entries, freeing for this request will be a no-op.
...
### get\_freed\_mm\_hashes [¶](#vllm.v1.core.encoder_cache_manager.EncoderCacheManager.get_freed_mm_hashes "Permanent link")
...
List of mm\_hash strings that were actually evicted since the last
...
### reset [¶](#vllm.v1.core.encoder_cache_manager.EncoderCacheManager.reset "Permanent link")
...
## EncoderDecoderCacheManager [¶](#vllm.v1.core.encoder_cache_manager.EncoderDecoderCacheManager "Permanent link")
...
```
classEncoderDecoderCacheManager(EncoderCacheManager):
    def__init__(self, cache_size: int):
        self.cache_size = cache_size
        self.num_free_slots = cache_size
...
### reset [¶](#vllm.v1.core.encoder_cache_manager.EncoderDecoderCacheManager.reset "Permanent link")
...
## compute\_mm\_encoder\_budget [¶](#vllm.v1.core.encoder_cache_manager.compute_mm_encoder_budget "Permanent link")

Compute the encoder cache budget based on the model and scheduler configurations for a multimodal model.

Parameters:

Name Type Description Default `scheduler_config` `SchedulerConfig`

Scheduler configuration.

*required* `mm_max_toks_per_item` `Mapping[str, int]`

The maximum number of tokens per item for each non-text modality.
...
- Compute budget for encoder execution, measured in number of tokens from the input sequence.

`int`

- Space budget for encoder cache size, measured in number of tokens from the input sequence.

Source code in `vllm/v1/core/encoder_cache_manager.py`

```
defcompute_mm_encoder_budget(
