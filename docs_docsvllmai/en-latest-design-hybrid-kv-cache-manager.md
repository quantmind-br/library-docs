...
source: sitemap
fetched_at: 2026-05-07T21:12:19.03733458-03:00
rendered_js: false
word_count: 2256
summary: This document outlines the design for a hybrid KV Cache Manager in vLLM, explaining how to allocate memory for models that combine different attention types like full attention, sliding window, and Mamba layers.
tags:
    - vllm
    - kv-cache
    - memory-management
...
To serve these models efficiently, our [KVCacheManager](https://docs.vllm.ai/en/latest/api/vllm/v1/core/kv_cache_manager/#vllm.v1.core.kv_cache_manager.KVCacheManager "            KVCacheManager") must:
...
4. **page size**: the physical memory size of a block, defined as:
   ...
   \\\[ \\text{num\_layers} \\times \\text{block\_size} \\times \\text{kv\_hidden\_size} \\]
...
We use a single memory pool for all layer types. The memory pool is split into multiple blocks with the same page size. [KVCacheManager](https://docs.vllm.ai/en/latest/api/vllm/v1/core/kv_cache_manager/#vllm.v1.core.kv_cache_manager.KVCacheManager "            KVCacheManager") allocates different numbers of blocks to different layers according to its attention type.
...
\\\[ \\text{page\_size} = \\text{kv\_hidden\_size} \\times \\text{block\_size} \\]

[KVCacheManager](https://docs.vllm.ai/en/latest/api/vllm/v1/core/kv_cache_manager/#vllm.v1.core.kv_cache_manager.KVCacheManager "            KVCacheManager") allocates a different number of blocks to each layer.
...
[![Allocation Result](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/basic_grouping_example.png)](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/basic_grouping_example.png)
...
1. Increase the `block_size` of attention layers until $$ \\text{block\_size} \\times \\text{kv\_hidden\_size}*{\\text{att}} \\ge \\text{state\_size}* $$}
2. Pad the mamba state per layer to $$ \\text{block\_size} \\times \\text{kv\_hidden\_size}\_{\\text{att}} $$
...
KV sharing refers to a layer using the KV cache of another layer, e.g., gemma-3n. In these models, [KVCacheManager](https://docs.vllm.ai/en/latest/api/vllm/v1/core/kv_cache_manager/#vllm.v1.core.kv_cache_manager.KVCacheManager "            KVCacheManager") ignores all layers with kv sharing and only allocates KV cache for layers that need kv cache, and some patches are made in model runner to apply the allocation result to kv sharing layers.
...
For full attention layers, blocks are allocated for all tokens in the request. For details on the underlying design, see [Prefix Caching](https://docs.vllm.ai/en/latest/design/prefix_caching/)
...
[![Prefix Caching of Full Attention](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/full_attn.png)](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/full_attn.png)
...
[![Prefix Caching of Sliding Window Attention](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/sw_attn.png)](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/sw_attn.png)
...
[![Overview of Hybrid KV Cache Manager](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/overview.png)](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/overview.png)

The `KVCacheManager` is organized into 3 layers:

- [**KVCacheManager**](https://docs.vllm.ai/en/latest/api/vllm/v1/core/kv_cache_manager/#vllm.v1.core.kv_cache_manager.KVCacheManager "            KVCacheManager"): The interface between the scheduler and kv cache management system.
- [**KVCacheCoordinator**](https://docs.vllm.ai/en/latest/api/vllm/v1/core/kv_cache_coordinator/#vllm.v1.core.kv_cache_coordinator.KVCacheCoordinator "            KVCacheCoordinator"): coordinate per-group SingleTypeKVCacheManagers to generate the allocation result of a request. Depending on the model's configuration, one of these coordinators is chosen:
  ...
  - [**KVCacheCoordinatorNoPrefixCache**](https://docs.vllm.ai/en/latest/api/vllm/v1/core/kv_cache_coordinator/#vllm.v1.core.kv_cache_coordinator.KVCacheCoordinatorNoPrefixCache "            KVCacheCoordinatorNoPrefixCache"): Used when prefix caching is disabled.
  - [**UnitaryKVCacheCoordinator**](https://docs.vllm.ai/en/latest/api/vllm/v1/core/kv_cache_coordinator/#vllm.v1.core.kv_cache_coordinator.UnitaryKVCacheCoordinator "            UnitaryKVCacheCoordinator"): If only one KV cache group. The prefix caching logic is simplified as no intersection is needed.
...
- [**SingleTypeKVCacheManager**](https://docs.vllm.ai/en/latest/api/vllm/v1/core/single_type_kv_cache_manager/#vllm.v1.core.single_type_kv_cache_manager.SingleTypeKVCacheManager "            SingleTypeKVCacheManager"): Each instance manages allocation and prefix caching for one KV cache group, implementing the attention‑type–specific logic (e.g., full attention, sliding window, Mamba).
...
- use [`HybridKVCacheCoordinator`](https://docs.vllm.ai/en/latest/api/vllm/v1/core/kv_cache_coordinator/#vllm.v1.core.kv_cache_coordinator.HybridKVCacheCoordinator "            HybridKVCacheCoordinator")
...
With such an example, the physical memory is divided into 10 buffers ([`KVCacheTensor`](https://docs.vllm.ai/en/latest/api/vllm/v1/kv_cache_interface/#vllm.v1.kv_cache_interface.KVCacheTensor "            KVCacheTensor            dataclass   ") 0 - [`KVCacheTensor`](https://docs.vllm.ai/en/latest/api/vllm/v1/kv_cache_interface/#vllm.v1.kv_cache_interface.KVCacheTensor "            KVCacheTensor            dataclass   ") 9). Each buffer is shared by 3 layers (e.g., [`KVCacheTensor`](https://docs.vllm.ai/en/latest/api/vllm/v1/kv_cache_interface/#vllm.v1.kv_cache_interface.KVCacheTensor "            KVCacheTensor            dataclass   ") 0 is shared by full.0 from group 0, sw.0 from group 1, and sw.10 from group 2) and is divided into pieces with size `block_size * kv_hidden_size`. The KV cache of these 3 attention layers are saved to different pieces of the buffer based on the allocated `block_ids`:

[![Example Memory Layout](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/memory_layout.png)](https://docs.vllm.ai/en/latest/assets/design/hybrid_kv_cache_manager/memory_layout.png)
