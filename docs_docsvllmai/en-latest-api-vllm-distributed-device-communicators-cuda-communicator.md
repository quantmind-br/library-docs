...
source: sitemap
fetched_at: 2026-05-07T21:17:30.004676869-03:00
rendered_js: false
word_count: 53
summary: This document defines the CudaCommunicator class, which manages distributed communication backends for parallel processing, including custom all-reduce, FlashInfer, and NCCL implementations for CUDA and ROCM devices.
tags:
...
```
classCudaCommunicator(DeviceCommunicatorBase):
    def__init__(
        self,
        ...
        if "tp" not in unique_name:
            ...
            fromvllm.distributed.parallel_stateimport _ENABLE_CUSTOM_ALL_REDUCE
        ...
        fromvllm.distributed.device_communicators.custom_all_reduceimport (
            CustomAllreduce,
        )
        fromvllm.distributed.device_communicators.flashinfer_all_reduceimport (
            FlashInferAllReduce,
        )
        fromvllm.distributed.device_communicators.pyncclimport PyNcclCommunicator
        fromvllm.distributed.device_communicators.quick_all_reduceimport (
            QuickAllReduce,
        )
        fromvllm.distributed.device_communicators.symm_memimport SymmMemCommunicator
    ...
    defreduce_scatter(self, input_: torch.Tensor, dim: int = -1):
...
"""Broadcast a tensor from source rank to all ranks."""
    ...
    defall_gatherv(
        ...
        def_all_gather_single(input_: torch.Tensor, sizes: list[int] | None = None):
