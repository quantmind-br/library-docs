...
source: sitemap
fetched_at: 2026-01-23T08:03:27.075689517-03:00
rendered_js: false
word_count: 631
summary: This document introduces DSPy modules as the foundational building blocks for language model programs, explaining how to declare, configure, and execute various prompting techniques through a unified interface.
tags:
    - dspy
    - language-models
    - prompt-engineering
...
## How do I use a built-in module, like `dspy.Predict` or `dspy.ChainOfThought`?[¶](#how-do-i-use-a-built-in-module-like-dspypredict-or-dspychainofthought "Permanent link")
...
## What other DSPy modules are there? How can I use them?[¶](#what-other-dspy-modules-are-there-how-can-i-use-them "Permanent link")
...
```
defevaluate_math(expression: str) -> float:
    return dspy.PythonInterpreter({}).execute(expression)

defsearch_wikipedia(query: str) -> str:
    results = dspy.ColBERTv2(url='http://20.102.90.50:2017/wiki17_abstracts')(query, k=3)
    return [x['text'] for x in results]
...
## How do I compose multiple modules into a bigger program?[¶](#how-do-i-compose-multiple-modules-into-a-bigger-program "Permanent link")
...
```
classHop(dspy.Module):
    def__init__(self, num_docs=10, num_hops=4):
        self.num_docs, self.num_hops = num_docs, num_hops
        self.generate_query = dspy.ChainOfThought('claim, notes -> query')
        self.append_notes = dspy.ChainOfThought('claim, notes, context -> new_notes: list[str], titles: list[str]')

    defforward(self, claim: str) -> list[str]:
        notes = []
        titles = []
...
## How do I track LM usage?[¶](#how-do-i-track-lm-usage "Permanent link")
...
classMyProgram(dspy.Module):
    ...
    def__call__(self, question: str) -> str:
