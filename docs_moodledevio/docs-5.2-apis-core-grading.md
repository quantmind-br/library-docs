---
title: Advanced grading API | Moodle Developer Resources
url: https://moodledev.io/docs/5.2/apis/core/grading
source: sitemap
fetched_at: 2026-02-17T15:43:31.326581-03:00
rendered_js: false
word_count: 447
summary: This document outlines the architecture and implementation of Moodle's advanced grading system, defining key entities and providing a step-by-step guide for developers to integrate it into activity modules.
tags:
    - moodle-development
    - advanced-grading
    - grading-api
...
### Grading area[​](#grading-area "Direct link to Grading area")
...
### Grading form definition[​](#grading-form-definition "Direct link to Grading form definition")

Grading form definitions are the set of rules defining how the grading is performed. For example, in rubrics this is the set of criteria and levels and their display options. The basic information about definition is stored in the DB table grading\_definitions. A separate entry is created for each grading area (that is for each module). Users with permission `moodle/grade:managegradingforms` are able to edit the definitions and mark them as "Ready".

### Grading form instance[​](#grading-form-instance "Direct link to Grading form instance")

A grading form instance is created for each evaluation of a submission, using advanced grading. One instance (usually the latest) has the status INSTANCE\_STATUS\_ACTIVE. Sometimes it may be the case that a teacher wants to change the definition after some students have already been graded. In this case their instances change status to `INSTANCE_STATUS_NEEDUPDATE`. The grade pushed to the gradebook remains unchanged but students will not see the grade in advanced grading format until teacher updates them. Plugins are also welcome to use these status levels.

## Functions[​](#functions "Direct link to Functions")

## Examples[​](#examples "Direct link to Examples")

### Using advanced grading in grade-able modules[​](#using-advanced-grading-in-grade-able-modules "Direct link to Using advanced grading in grade-able modules")
...
1. In order for module to support advanced grading, its function **`[activityname]_supports(FEATURE_ADVANCED_GRADING)`** must return true.
   ...
   switch($feature){
   // ...
   caseFEATURE_ADVANCED_GRADING:
   returntrue;
   // ...
   }
   }
   ```
2. The module should define a function called **`[activityname]_grading_areas_list()`** .
...
5. During population of the grading form, simple grading elements can now be substituted with advanced grading element.
   ...
   ```
6. On submission of a grading form, the advanced grading information shall be also submitted. The grade for the gradebook retrieved from plugin and saved to the gradebook.
