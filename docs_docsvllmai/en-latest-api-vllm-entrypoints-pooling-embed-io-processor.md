...
url: https://docs.vllm.ai/en/latest/api/vllm/entrypoints/pooling/embed/io_processor/
source: sitemap
fetched_at: 2026-05-07T21:20:59.648442738-03:00
rendered_js: false
word_count: 68
summary: This document defines a class for processing embedding tasks with support for chunked input handling and online mean pooling aggregation. It manages the segmentation of long-text inputs into smaller chunks and reconstructs the final embedding result after inference.
tags:
    - embedding-processor
    - chunked-processing
    - pooling-aggregation
...
```
classEmbedIOProcessor(PoolingIOProcessor):
    name = "embed"
    ...
    def_pre_process_chunked(self, ctx: PoolingServeContext) -> None:
    ...
    def_post_process_chunked(self, ctx: PoolingServeContext) -> None:
    ...
    @staticmethod
    def_load_task_instructions(hf_config: Any) -> dict[str, str] | None:
...
"""Load ``task_instructions`` from ``config_sentence_transformers.json``."""
        fromvllm.transformers_utils.repo_utilsimport get_hf_file_to_dict
...
"""Return ``(truncate_prompt_tokens, truncation_side)``."""
    ...
    def_pre_process_cohere_online(self, ctx: PoolingServeContext) -> None:
"""Convert a ``CohereEmbedRequest`` into engine prompts.
    ...
    def_has_chat_template(self) -> bool:
        return (
    ...
    def_preprocess_cohere_text_completion(
        self,
        request: CohereEmbedRequest,
        texts: list[str],
...
"""Batch-render multiple conversations through the chat template."""
    ...
    def_validate_input_type(self, input_type: str | None) -> None:
...
"""Prepend the task-instruction prefix for *input_type*.
    ...
    def_get_task_instruction_prefix(self, input_type: str | None) -> str | None:
"""Return the task-instruction prefix for *input_type*, or ``None``."""
    ...
    def_enforce_cohere_max_tokens(self, ctx: PoolingServeContext) -> None:
        if isinstance(ctx.request, CohereEmbedRequest):
            request = ctx.request
