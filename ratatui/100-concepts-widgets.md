...
source: crawler
fetched_at: 2026-02-01T21:12:29.091065935-03:00
rendered_js: false
word_count: 1205
summary: This document explains the role and implementation of widgets in Ratatui, covering built-in components, core traits for stateful and stateless rendering, and the rendering process.
tags:
    - ratatui
    - rust-tui
    - widgets
...
pubtrait WidgetRef {
fnrender_ref(&self, area: Rect, buf:&mut Buffer);
}
pubtrait StatefulWidgetRef {
type State;
fnrender_ref(&self, area: Rect, buf:&mut Buffer, state:&mutSelf::State);
}
```
...
terminal.draw(|frame| {
frame.render_stateful_widget(some_stateful_widget, frame.area(), &mutsome_state);
});
```
...
fnrender(self, area: Rect, buf:&mut Buffer) {
letgreeting=format!("Hello, {}!", self.name);
...
impl Widget for GreetingWidget {
fnrender(self, area: Rect, buf:&mut Buffer) {
lethello= Span::raw("Hello, ");
letname= Span::styled(self.name, Modifier::BOLD);
letline= Line::from(vec![hello, name]);
...
type State = i32;
fnrender(self, area: Rect, buf:&mut Buffer, state:&mut i32) {
*state+=1;
lettext=format!("Frame count: {state}");
...
letwidgets: Vec<Box<dyn WidgetRef>> =vec![
