...
url: https://doc.rust-lang.org/cargo/reference/config.html#buildincremental
source: crawler
fetched_at: 2026-05-06T21:25:03.171221203-03:00
rendered_js: false
word_count: 6881
summary: This document describes the hierarchical configuration system in Cargo, including how configuration files are merged and the various settings available for customizing Cargo behavior.
tags:
    - cargo
    - rust
    - configuration
...
This document explains how Cargo’s configuration system works, as well as available keys or configuration. For configuration of a package through its manifest, see the [manifest format](https://doc.rust-lang.org/cargo/reference/manifest.html).
...
In addition to the system above, Cargo recognizes a few other specific [environment variables](https://doc.rust-lang.org/cargo/reference/environment-variables.html).
...
## [Including extra configuration files](#including-extra-configuration-files)

Configuration can include other configuration files using the top-level `include` key. This allows sharing configuration across multiple projects or splitting complex configurations into multiple files.
...
Loads additional configuration files. Paths are relative to the configuration file that includes them. Only paths ending with `.toml` are accepted.
...
- `optional` (boolean, default: false): If `true`, missing files are silently skipped instead of causing an error.
...
1. Config values are first loaded from the `include` paths.
   ...
   - Included files are loaded left to right, with values from later files taking precedence over earlier ones.
   - This step recurses if included config files also contain `include` keys.
2. Then, the config file’s own values are merged on top of the included config, taking highest precedence.
...
Configuration values with sensitive information are stored in the `$CARGO_HOME/credentials.toml` file. This file is automatically created and updated by [`cargo login`](https://doc.rust-lang.org/cargo/commands/cargo-login.html) and [`cargo logout`](https://doc.rust-lang.org/cargo/commands/cargo-logout.html) when using the [`cargo:token`](https://doc.rust-lang.org/cargo/reference/registry-authentication.html#cargotoken) credential provider.

Tokens are used by some Cargo commands such as [`cargo publish`](https://doc.rust-lang.org/cargo/commands/cargo-publish.html) for authenticating with remote registries. Care should be taken to protect the tokens and to keep them secret.
...
This section documents all configuration keys. The description for keys with variable parts are annotated with angled brackets like `target.<triple>` where the `<triple>` part can be any [target triple](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)') like `target.x86_64-pc-windows-msvc`.
...
An array of paths to local packages which are to be used as overrides for dependencies. For more information see the [Overriding Dependencies guide](https://doc.rust-lang.org/cargo/reference/overriding-dependencies.html#paths-overrides).
...
The default [target platform triples](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)') to compile to.
...
For more information see the [build cache documentation](https://doc.rust-lang.org/cargo/reference/build-cache.html).
...
If the `--target` flag (or [`build.target`](#buildtarget)) is used, then the flags will only be passed to the compiler for the target. Things being built for the host, such as build scripts or proc macros, will not receive the args. Without `--target`, the flags will be passed to all compiler invocations (including build scripts and proc macros) because dependencies are shared. If you have args that you do not want to pass to build scripts or proc macros and are building for the host, pass `--target` with the [host triple](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)').

It is not recommended to pass in flags that Cargo itself usually manages. For example, the flags driven by [profiles](https://doc.rust-lang.org/cargo/reference/profiles.html) are best handled by setting the appropriate profile setting.
...
Whether or not to perform [incremental compilation](https://doc.rust-lang.org/cargo/reference/profiles.html#incremental). The default if not set is to use the value from the [profile](https://doc.rust-lang.org/cargo/reference/profiles.html#incremental). Otherwise this overrides the setting of all profiles.
...
Strips the given path prefix from [dep info](https://doc.rust-lang.org/cargo/reference/build-cache.html#dep-info-files) file paths. This config setting is intended to convert absolute paths to relative paths for tools that require relative paths.
...
See [Registry Authentication](https://doc.rust-lang.org/cargo/reference/registry-authentication.html) for more information.

### [`[doc]`](#doc)

The `[doc]` table defines options for the [`cargo doc`](https://doc.rust-lang.org/cargo/commands/cargo-doc.html) command.
...
This option sets the browser to be used by [`cargo doc`](https://doc.rust-lang.org/cargo/commands/cargo-doc.html), overriding the `BROWSER` environment variable when opening documentation with the `--open` option.

### [`[cargo-new]`](#cargo-new)

The `[cargo-new]` table defines defaults for the [`cargo new`](https://doc.rust-lang.org/cargo/commands/cargo-new.html) command.
...
The `[future-incompat-report]` table controls setting for [future incompat reporting](https://doc.rust-lang.org/cargo/reference/future-incompat-report.html)
...
> **Note**: This tracking is currently only implemented for the global cache in Cargo’s home directory. This includes registry indexes and source files downloaded from registries and git dependencies. Support for tracking build artifacts is not yet implemented, and tracked in [cargo#13136](https://github.com/rust-lang/cargo/issues/13136).
> 
> Additionally, there is an unstable feature to support *manually* triggering cache cleaning, and to further customize the configuration options. See the [Unstable chapter](https://doc.rust-lang.org/cargo/reference/unstable.html#gc) for more information.
...
This option defines how often Cargo will automatically delete unused files in the global cache. This does *not* define how old the files must be, those thresholds are described [above](#global-caches).
...
The `[install]` table defines defaults for the [`cargo install`](https://doc.rust-lang.org/cargo/commands/cargo-install.html) command.
...
Sets the path to the root directory for installing executables for [`cargo install`](https://doc.rust-lang.org/cargo/commands/cargo-install.html). Executables go into a `bin` directory underneath the root.
...
The `[profile]` table can be used to globally change profile settings, and override settings specified in `Cargo.toml`. It has the same syntax and options as profiles specified in `Cargo.toml`. See the [Profiles chapter](https://doc.rust-lang.org/cargo/reference/profiles.html) for details about the options.

#### [`[profile.<name>.build-override]`](#profilenamebuild-override)

- Environment: `CARGO_PROFILE_<name>_BUILD_OVERRIDE_<key>`

The build-override table overrides settings for build scripts, proc macros, and their dependencies. It has the same keys as a normal profile. See the [overrides section](https://doc.rust-lang.org/cargo/reference/profiles.html#overrides) for more details.

#### [`[profile.<name>.package.<name>]`](#profilenamepackagename)

- Environment: not supported

The package table overrides settings for specific packages. It has the same keys as a normal profile, minus the `panic`, `lto`, and `rpath` settings. See the [overrides section](https://doc.rust-lang.org/cargo/reference/profiles.html#overrides) for more details.
...
See [codegen-units](https://doc.rust-lang.org/cargo/reference/profiles.html#codegen-units).
...
See [debug](https://doc.rust-lang.org/cargo/reference/profiles.html#debug).
...
See [split-debuginfo](https://doc.rust-lang.org/cargo/reference/profiles.html#split-debuginfo).
...
See [debug-assertions](https://doc.rust-lang.org/cargo/reference/profiles.html#debug-assertions).
...
See [incremental](https://doc.rust-lang.org/cargo/reference/profiles.html#incremental).
...
See [lto](https://doc.rust-lang.org/cargo/reference/profiles.html#lto).
...
See [overflow-checks](https://doc.rust-lang.org/cargo/reference/profiles.html#overflow-checks).
...
See [opt-level](https://doc.rust-lang.org/cargo/reference/profiles.html#opt-level).
...
See [strip](https://doc.rust-lang.org/cargo/reference/profiles.html#strip).

### [`[resolver]`](#resolver)

The `[resolver]` table overrides [dependency resolution behavior](https://doc.rust-lang.org/cargo/reference/resolver.html) for local development (e.g. excludes `cargo install`).

#### [`resolver.incompatible-rust-versions`](#resolverincompatible-rust-versions)

- Type: string
- Default: See [`resolver`](https://doc.rust-lang.org/cargo/reference/resolver.html#resolver-versions) docs
...
The `[registries]` table is used for specifying additional [registries](https://doc.rust-lang.org/cargo/reference/registries.html). It consists of a sub-table for each named registry.
...
Specifies the authentication token for the given registry. This value should only appear in the [credentials](#credentials) file. This is used for registry commands like [`cargo publish`](https://doc.rust-lang.org/cargo/commands/cargo-publish.html) that require authentication.
...
More information about registry protocols may be found in the [Registries chapter](https://doc.rust-lang.org/cargo/reference/registries.html).
...
The name of the registry (from the [`registries` table](#registries)) to use by default for registry commands like [`cargo publish`](https://doc.rust-lang.org/cargo/commands/cargo-publish.html).
...
Specifies the authentication token for [crates.io](https://crates.io/). This value should only appear in the [credentials](#credentials) file. This is used for registry commands like [`cargo publish`](https://doc.rust-lang.org/cargo/commands/cargo-publish.html) that require authentication.
...
The `[source]` table defines the registry sources available. See [Source Replacement](https://doc.rust-lang.org/cargo/reference/source-replacement.html) for more information. It consists of a sub-table for each named source. A source should only define one kind (directory, registry, local-registry, or git).
...
The `[target]` table is used for specifying settings for specific platform targets. It consists of a sub-table which is either a [platform triple](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)') or a [`cfg()` expression](https://doc.rust-lang.org/reference/conditional-compilation.html). The given values will be used if the target platform matches either the `<triple>` value or the `<cfg>` expression.
...
`cfg` values come from those built-in to the compiler (run `rustc --print=cfg` to view) and extra `--cfg` flags passed to `rustc` (such as those defined in `RUSTFLAGS`). Do not try to match on `debug_assertions`, `test`, Cargo features like `feature="foo"`, or values set by [build scripts](https://doc.rust-lang.org/cargo/reference/build-scripts.html).

If using a target spec JSON file, the [`<triple>`](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)') value is the filename stem. For example `--target foo/bar.json` would match `[target.bar]`.
...
Specifies the linker which is passed to `rustc` (via [`-C linker`](https://doc.rust-lang.org/rustc/codegen-options/index.html#linker)) when the [`<triple>`](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)') is being compiled for. By default, the linker is not overridden.

#### [`target.<cfg>.linker`](#targetcfglinker)

This is similar to the [target linker](#targettriplelinker), but using a [`cfg()` expression](https://doc.rust-lang.org/reference/conditional-compilation.html). If both a [`<triple>`](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)') and `<cfg>` runner match, the `<triple>` will take precedence. It is an error if more than one `<cfg>` runner matches the current target.
...
If a runner is provided, executables for the target [`<triple>`](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)') will be executed by invoking the specified runner with the actual executable passed as an argument. This applies to [`cargo run`](https://doc.rust-lang.org/cargo/commands/cargo-run.html), [`cargo test`](https://doc.rust-lang.org/cargo/commands/cargo-test.html) and [`cargo bench`](https://doc.rust-lang.org/cargo/commands/cargo-bench.html) commands. By default, compiled executables are executed directly.
...
Passes a set of custom flags to the compiler for this [`<triple>`](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)'). The value may be an array of strings or a space-separated string.
...
This is similar to the [target rustflags](#targettriplerustflags), but using a [`cfg()` expression](https://doc.rust-lang.org/reference/conditional-compilation.html). If several `<cfg>` and [`<triple>`](https://doc.rust-lang.org/cargo/appendix/glossary.html#target '"target" (glossary)') entries match the current target, the flags are joined together.
...
The links sub-table provides a way to [override a build script](https://doc.rust-lang.org/cargo/reference/build-scripts.html#overriding-build-scripts). When specified, the build script for the given `links` library will not be run, and the given values will be used instead.
