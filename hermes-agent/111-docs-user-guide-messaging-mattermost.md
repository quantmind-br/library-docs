...
url: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/mattermost
source: crawler
fetched_at: 2026-04-24T17:00:15.18379541-03:00
rendered_js: false
word_count: 1567
summary: This document serves as a comprehensive guide detailing how the Hermes Agent integrates with Mattermost, explaining its conversational behavior across DMs and channels, the session management model, and providing step-by-step instructions for configuration.
tags:
    - hermes-agent
...
ContextBehavior**DMs**Hermes responds to every message. No `@mention` needed. Each DM has its own session.**Public/private channels**Hermes responds when you `@mention` it. Without a mention, Hermes ignores the message.**Threads**If `MATTERMOST_REPLY_MODE=thread`, Hermes replies in a thread under your message. Thread context stays isolated from the parent channel.**Shared channels with multiple users**By default, Hermes isolates session history per user inside the channel. Two people talking in the same channel do not share one transcript unless you explicitly disable that.
...
ModeBehavior`off` (default)Hermes posts flat messages in the channel, like a normal user.`thread`Hermes replies in a thread under your original message. Keeps channels clean when there's lots of back-and-forth.
...
VariableDefaultDescription`MATTERMOST_REQUIRE_MENTION``true`Set to `false` to respond to all messages in channels (DMs always work).`MATTERMOST_FREE_RESPONSE_CHANNELS`*(none)*Comma-separated channel IDs where the bot responds without `@mention`, even when require\_mention is true.
