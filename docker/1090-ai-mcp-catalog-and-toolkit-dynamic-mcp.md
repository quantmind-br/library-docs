...
source: llms
fetched_at: 2026-01-24T14:13:54.703377851-03:00
rendered_js: false
word_count: 713
summary: A guide on implementing and utilizing the Model Context Protocol (MCP) for dynamic server connections and real-time data integration.
tags:
    - MCP
...
ToolDescription`mcp-find`Search for MCP servers in the catalog by name or description`mcp-add`Add a new MCP server to the current session`mcp-config-set`Configure settings for an MCP server`mcp-remove`Remove an MCP server from the session`mcp-exec`Execute a tool by name that exists in the current session`code-mode`Create a JavaScript-enabled tool that combines multiple MCP server tools
