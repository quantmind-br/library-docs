...
url: https://docs.yasb.dev/dev/keybindings
source: crawler
fetched_at: 2026-02-23T05:28:53.767081-03:00
rendered_js: false
word_count: 756
---
...
Key Name Description `win`, `windows`, `super`, `meta` Windows key `alt` Alt key `ctrl`, `control` Control key `shift` Shift key
...
Configuration Valid `"alt+a"` Yes `"ctrl+shift+f1"` Yes `"a+b"` No - two non-modifier keys `"f13+h"` No - two non-modifier keys
