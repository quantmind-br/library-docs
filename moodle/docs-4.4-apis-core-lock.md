---
title: Lock API | Moodle Developer Resources
url: https://moodledev.io/docs/4.4/apis/core/lock
source: sitemap
fetched_at: 2026-02-17T15:02:30.846363-03:00
rendered_js: false
word_count: 330
summary: This document explains the purpose and implementation of the Moodle Locking API to prevent concurrent access to shared resources across processes or cluster nodes. It provides guidance on usage, performance considerations, and instructions for configuring or extending lock factories.
tags:
    - moodle
    - locking-api
    - concurrency-control
...
## When to use locking[​](#when-to-use-locking "Direct link to When to use locking")
...
The locking API is used by getting an instance of a lock\_factory, and then using it to retrieve locks, and eventually releasing them. You are required to release all your locks, even on the event of failures.
...
thrownewmoodle_exception('locktimeout');
}
```

## Use a different lock type from the default[​](#use-a-different-lock-type-from-the-default "Direct link to Use a different lock type from the default")

Change the $CFG-&gt;lock\_factory setting to one of the other lock types included with core. These are all documented in config-dist.php.

## Implementing new lock types[​](#implementing-new-lock-types "Direct link to Implementing new lock types")
...
Add a new local\_XXX plugin with an autoloaded class that implements \\core\\lock\\lock\_factory. Set the site configuration variable "lock\_factory" to the full namespaced path to your class in the config.php for example
...
See `lib/tests/lock_test.php` for an example of unit tests which can be run on a custom lock instance to verify it for correctness (run\_on\_lock\_factory).