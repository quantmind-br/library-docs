...
## API Contract[​](#api-contract "Direct link to API Contract")
...
### `tools` Parameter[​](#tools-parameter "Direct link to tools-parameter")
...
### `tool_calls` Parameter[​](#tool_calls-parameter "Direct link to tool_calls-parameter")
...
### `structured_messages` Parameter[​](#structured_messages-parameter "Direct link to structured_messages-parameter")
...
## LiteLLM Configuration[​](#litellm-configuration "Direct link to LiteLLM Configuration")
...
### Example: Pillar Security[​](#example-pillar-security "Direct link to Example: Pillar Security")
...
See the [Pillar Security documentation](https://docs.litellm.ai/docs/proxy/guardrails/pillar_security) for full configuration options.
...
See [mock\_bedrock\_guardrail\_server.py](https://github.com/BerriAI/litellm/blob/main/cookbook/mock_guardrail_server/mock_bedrock_guardrail_server.py) for a complete reference implementation.
...
classGuardrailRequest(BaseModel):
    texts: List[str]
...
classGuardrailResponse(BaseModel):
...
asyncdefapply_guardrail(request: GuardrailRequest):
...
if"file_path"in arguments and".."instr(arguments["file_path"]):
return GuardrailResponse(
                            action="BLOCKED",
