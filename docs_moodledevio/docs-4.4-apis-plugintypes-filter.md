...
url: https://moodledev.io/docs/4.4/apis/plugintypes/filter
source: sitemap
fetched_at: 2026-02-17T15:03:03.657933-03:00
rendered_js: false
word_count: 950
summary: This document explains how to develop and structure filter plugins in Moodle to automatically transform content before it is displayed. It covers mandatory file structures, the main filter class implementation, and best practices for ensuring filter performance.
tags:
    - moodle-development
    - filter-plugins
    - plugin-structure
...
Filters are applied to content passed into the `format_string()` and `format_text()` functions, which are part of the [Output API](https://moodledev.io/docs/4.4/apis/subsystems/output).
...
Some of the important files for the filter plugintype are described below. See the [common plugin files](https://moodledev.io/docs/4.4/apis/commonfiles) documentation for details of other files which may be useful in your plugin.

### filter.php[​](#filterphp "Direct link to filter.php")

#### Filter main class

##### File path: /filter.php

The filter file contains the code for the main filter class. Unlike more complex plugins like activities or repositories, filters only have one mandatory class extending the core moodle\_text\_filter class.

View example

public/filter/pluginname/filter.php
...
classfilter_pluginnameextendsmoodle_text_filter{
...
To enable a filter, go to the [filters administration screen](https://moodledev.io/docs/4.4/apis/plugintypes/filter) and set the filter active to "On".
...
```
classpluginfile_filter_local_settings_formextendsfilter_local_settings_form{
