...
url: https://hyprland-community.github.io/pyprland/versions/2.3.5/scratchpads
source: github_pages
fetched_at: 2026-01-31T16:04:29.368722625-03:00
rendered_js: false
word_count: 580
summary: This document explains how to configure and use scratchpads in Hyprland using pyprland, allowing users to easily toggle the visibility of frequently used applications in floating windows.
tags:
    - scratchpad
    - hyprland
    - window-management
...
If you wish to have a more generic space for any application you may run, check [toggle\_special](https://hyprland-community.github.io/pyprland/versions/2.3.5/toggle_special.html).

## Commands [​](#commands)

- `toggle <scratchpad name>` toggles the given scratchpads
  
  - use multiple *space separated* names to synchronize visibility based on the first scratchpad provided
...
In version 2.3.5 and up, you can use `"*"` as a *scratchpad name* to target every scratchpad when using `show` or `hide`. You'll need to quote or escape the `*` character to avoid interpretation from your shell.
...
It supports [Variables](https://hyprland-community.github.io/pyprland/versions/2.3.5/Variables.html)

### `animation` (optional) [​](#animation-optional)
...
### `size` (recommended) [​](#size-recommended)
...
### `multi` (optional) [​](#multi-optional)

> *Added in 2.2.13*

Defaults to `true`.

When set to `false`, only one client window is supported for this scratchpad. Otherwise other matching windows will be **attach**ed to the scratchpad.

## Advanced configuration [​](#advanced-configuration)

To go beyond the basic setup and have a look at every configuration item, you can read the following pages:

- [Advanced](https://hyprland-community.github.io/pyprland/versions/2.3.5/scratchpads_advanced.html) contains options for fine-tuners or specific tastes (eg: i3 compatibility)
- [Non-Standard](https://hyprland-community.github.io/pyprland/versions/2.3.5/scratchpads_nonstandard.html) contains options for "broken" applications like progressive web apps (PWA) or emacsclient, use only if you can't get it to work otherwise

## Monitor specific overrides [​](#monitor-specific-overrides)
