---
title: Deprecation | Moodle Developer Resources
url: https://moodledev.io/general/development/policies/deprecation
source: sitemap
fetched_at: 2026-02-17T15:59:09.052547-03:00
rendered_js: false
word_count: 2129
summary: This document explains the technical deprecation policy and multi-stage implementation process for the Moodle codebase, detailing how to phase out legacy code through specific lifecycle stages.
tags:
    - moodle-core
    - deprecation-policy
    - api-lifecycle
...
## Why is deprecation needed?[​](#why-is-deprecation-needed "Direct link to Why is deprecation needed?")
...
## What is Moodle's deprecation policy?[​](#what-is-moodles-deprecation-policy "Direct link to What is Moodle's deprecation policy?")
...
- All deprecations should be noted with an [upgrade note](https://moodledev.io/general/development/upgradenotes).
...
What does it mean for an API to be considered "Public"
...
## Moodle Core deprecation process[​](#moodle-core-deprecation-process "Direct link to Moodle Core deprecation process")
...
### Step 1. Immediate action - Initial deprecation[​](#step-1-immediate-action---initial-deprecation "Direct link to Step 1. Immediate action - Initial deprecation")

Deprecation affects only the current `main` version, in other words, the deprecation only becomes effective after the next [major release](https://moodledev.io/general/releases).
...
- If a public file is being deprecated (for example, `badges/newbadge.php` in [MDL-43938](https://moodle.atlassian.net/browse/MDL-43938)), the following actions should be done:
...
- If the entire class is being moved (for example, moving multiple class definitions from a monolithic file in to individual files), follow the process for [renaming classes](https://moodledev.io/docs/5.2/apis/commonfiles#dbrenamedclassesphp).
...
- Using the \\core\\deprecation API from Moodle 4.4
- Deprecations using debugging

```
#[\core\attribute\deprecated('foobar::blah()',since:'4.4',mdl:'MDL-XXXXX')]
publicfunctionfoobar(int$old,array$params):array{
\core\deprecation::emit_deprecation_if_present([$this,__FUNCTION__]);
// Call new method if possible or retain existing code.
returnfoobar::blah($old,$params);
}
```
...
- Last but not least, every deprecation should be documented in an [upgrade note](https://moodledev.io/general/development/upgradenotes) **at least** once but, **ideally**, both on this initial/immediate deprecation and also on the final deprecation/removal.
...
### Step 2. Final deprecation[​](#step-2-final-deprecation "Direct link to Step 2. Final deprecation")
...
- Function signatures *may* be removed, that is:
...
- Last but not least, every deprecation should be documented in an [upgrade note](https://moodledev.io/general/development/upgradenotes) **at least** once but, **ideally**, both on the initial/immediate deprecation and also on this final deprecation/removal.
...
### Step 3. Removal[​](#step-3-removal "Direct link to Step 3. Removal")

#### Policy[​](#policy-1 "Direct link to Policy")

A code removal step was added to the deprecation process in Moodle 5.0 and is aligned with the LTS release cycle.

- Any feature or functionality which went through the Step 2 (Final deprecation) process *before* an LTS release, may be removed in the next major version *after* the LTS release.
- Removals must be documented in an [upgrade note](https://moodledev.io/general/development/upgradenotes).
...
## Parameters deprecation[​](#parameters-deprecation "Direct link to Parameters deprecation")
...
- Add an [upgrade note](https://moodledev.io/general/development/upgradenotes), documenting that the deprecated parameter should not be used any more
...
Changes to default values and types
...
When deprecating a method which is *likely* to be extended, you should strongly consider deprecating the entire method and creating a replacement method with the updated arguments instead. This includes all renderer methods.

An example of correct behaviour

- Original
- Deprecate a parameter
- Deprecating the method instead
...
Deprecations for core methods in Moodle 4.1 and earlier
...
- [String deprecation](https://moodledev.io/general/projects/api/string-deprecation)
- [Deprecation attributes](https://moodledev.io/docs/5.2/apis/core/deprecation)
- [External functions deprecation](https://moodledev.io/docs/5.2/apis/subsystems/external/writing-a-service#deprecation)
- [Capabilities deprecation](https://moodledev.io/docs/5.2/apis/subsystems/access#deprecating-a-capability)
- [Plugin type deprecation](https://moodledev.io/docs/5.2/apis/plugintypes#plugin-type-deprecation)
- [Icon deprecation](https://moodledev.io/general/development/policies/deprecation/icon-deprecation)
- [SCSS deprecation](https://moodledev.io/general/development/policies/deprecation/scss-deprecation)
- [Behat step definition deprecation](https://moodledev.io/general/development/tools/behat/writing#deprecating-a-step-definition)
