---
title: Pipeline Parallelism for Long Context — SGLang
url: https://docs.sglang.io/advanced_features/pipeline_parallelism.html
source: crawler
fetched_at: 2026-02-04T08:46:55.386486719-03:00
rendered_js: false
word_count: 1596
summary: This document explains how SGLang implements pipeline parallelism and dynamic chunked prefill to optimize long-context LLM serving by reducing latency and improving throughput.
tags:
...
## Pipeline Parallelism for Long Context[#](#pipeline-parallelism-for-long-context "Link to this heading")

## Why Pipeline Parallelism?[#](#why-pipeline-parallelism "Link to this heading")
...
## Implementation Refactoring based on Async Communication[#](#implementation-refactoring-based-on-async-communication "Link to this heading")
...
## Guidance about Dynamic Chunking[#](#guidance-about-dynamic-chunking "Link to this heading")

### Why Dynamic Chunking[#](#why-dynamic-chunking "Link to this heading")
...
### Chunked Prefill Size and Smoothing Factor[#](#chunked-prefill-size-and-smoothing-factor "Link to this heading")
...
## Best Practice for Long Context[#](#best-practice-for-long-context "Link to this heading")

### Tuning the Chunked Prefill Size[#](#tuning-the-chunked-prefill-size "Link to this heading")
...
### Enable Dynamic Chunking and Adjust Smoothing Factor for Ultra-long ITL[#](#enable-dynamic-chunking-and-adjust-smoothing-factor-for-ultra-long-itl "Link to this heading")
...
### Case Study on NVIDIA H20[#](#case-study-on-nvidia-h20 "Link to this heading")
...
#### DeepSeek-V3.1 with 128K Input Token Length[#](#deepseek-v3-1-with-128k-input-token-length "Link to this heading")

```
# prefill node 0 (fixed chunked prefill size)
python3-msglang.launch_server\
--model-pathdeepseek-ai/DeepSeek-V3.1--trust-remote-code\
--nnodes4--node-rank0--tp8--pp-size4\
--port30000--dist-init-addr<MASTER_NODE_IP>\
--disable-radix-cache--mem-fraction-static0.8\
--attention-backendfa3--host0.0.0.0--watchdog-timeout3600\
--max-running-requests128--chunked-prefill-size4096
```

```
# prefill node 0 (with dynamic chunking)
exportSGLANG_DYNAMIC_CHUNKING_SMOOTH_FACTOR=0.65
python3-msglang.launch_server\
--model-pathdeepseek-ai/DeepSeek-V3.1--trust-remote-code\
--nnodes4--node-rank0--tp8--pp-size4\
--port30000--dist-init-addr<MASTER_NODE_IP>\
--disable-radix-cache--mem-fraction-static0.8\
--attention-backendfa3--host0.0.0.0--watchdog-timeout3600\
--max-running-requests128--chunked-prefill-size12288--enable-dynamic-chunking
```

#### Qwen3-235B-A22B-FP8 with 128K Input Token Length[#](#qwen3-235b-a22b-fp8-with-128k-input-token-length "Link to this heading")

```
# prefill node 0 (fixed chunked prefill size)
python3-msglang.launch_server\
--model-pathQwen/Qwen3-235B-A22B-FP8--trust-remote-code\
--nnodes4--node-rank0--tp4--pp-size8\
--port30000--dist-init-addr<MASTER_NODE_IP>\
--disable-radix-cache--mem-fraction-static0.8\
--attention-backendfa3--host0.0.0.0--watchdog-timeout3600\
--max-running-requests128--chunked-prefill-size6144
```

```
# prefill node 0 (with dynamic chunking)
exportSGLANG_DYNAMIC_CHUNKING_SMOOTH_FACTOR=0.8
python3-msglang.launch_server\
--model-pathQwen/Qwen3-235B-A22B-FP8--trust-remote-code\
--nnodes4--node-rank0--tp4--pp-size8\
--port30000--dist-init-addr<MASTER_NODE_IP>\
--disable-radix-cache--mem-fraction-static0.8\
--attention-backendfa3--host0.0.0.0--watchdog-timeout3600\
--max-running-requests128--chunked-prefill-size18432--enable-dynamic-chunking
```
...
## Best Practice for Pipeline Parallelism with PD Disaggregation[#](#best-practice-for-pipeline-parallelism-with-pd-disaggregation "Link to this heading")
