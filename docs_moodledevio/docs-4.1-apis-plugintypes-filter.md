...
url: https://moodledev.io/docs/4.1/apis/plugintypes/filter
source: sitemap
fetched_at: 2026-02-17T14:55:22.032042-03:00
rendered_js: false
word_count: 950
summary: This document provides a guide on developing filter plugins in Moodle to automatically transform text content before it is rendered. It outlines the required file structure, class implementation, and performance best practices for efficient content processing.
tags:
    - moodle-development
    - filter-plugins
    - php-coding
...
Filters are applied to content passed into the `format_string()` and `format_text()` functions, which are part of the [Output API](https://moodledev.io/docs/4.1/apis/subsystems/output).
...
Some of the important files for the filter plugintype are described below. See the [common plugin files](https://moodledev.io/docs/4.1/apis/commonfiles) documentation for details of other files which may be useful in your plugin.
...
To enable a filter, go to the [filters administration screen](https://moodledev.io/docs/4.1/apis/plugintypes/filter) and set the filter active to "On".
