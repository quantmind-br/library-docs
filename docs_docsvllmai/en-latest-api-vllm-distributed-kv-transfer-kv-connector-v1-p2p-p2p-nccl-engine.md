...
source: sitemap
fetched_at: 2026-05-07T21:18:55.152032271-03:00
rendered_js: false
word_count: 4
summary: This document defines the P2pNcclEngine class, which manages point-to-point tensor communication between processes using NCCL and ZMQ. It handles connection initialization, synchronization modes, memory pooling, and background thread management for asynchronous data transfer.
tags:
    - python
    - nccl
    - zmq
...
classP2pNcclEngine:
    ...
    defcreate_connect(self, remote_address: str | None = None):
    ...
    defhave_sent_tensor_id(self, tensor_id: str):
    ...
    defhave_received_tensor_id(self, tensor_id: str):
        request_id = tensor_id.split("#")[0]
    ...
    defsend_sync(self, item: SendQueueItem) -> bool:
...
"""
    ...
    defsend(self, comm, tensor: torch.Tensor, dst: int, stream=None):
    ...
    defrecv(self, comm, tensor: torch.Tensor, src: int, stream=None):
        assert tensor.device == self.device, (
            f"this nccl communicator is created to work on {self.device}, "
            f"but the input tensor is on {tensor.device}"
        )
