---
title: thinking_budget_state - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/sample/thinking_budget_state/
source: sitemap
fetched_at: 2026-05-07T21:41:36.187592902-03:00
rendered_js: false
word_count: 46
summary: This class manages reasoning token budgets for LLM generation by tracking thinking segments and enforcing termination tokens when the allotted budget is exceeded.
tags:
    - token-management
    - llm-inference
    - reasoning-budget
...
```
classThinkingBudgetStateHolder:
"""Tracks thinking sections and forces end tokens when budget is exceeded."""
    ...
    defhas_tracked_requests(self) -> bool:
"""True when ``sync_batch`` has state for a ``thinking_token_budget`` row.
    ...
    defsync_batch(self, batch_update: BatchUpdate | None) -> None:
...
"""Mask and bump logits for forced end-of-thinking tokens."""
    ...
    @staticmethod
    def_find_last_sequence_index(target_list: list[int], token_ids: list[int]) -> int:
        if not token_ids:
            return -1
    ...
    def_update_think_state(self, state: dict[str, Any]) -> None:
