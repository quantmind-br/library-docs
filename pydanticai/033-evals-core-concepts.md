---
title: Core Concepts - Pydantic AI
url: https://ai.pydantic.dev/evals/core-concepts/
source: sitemap
fetched_at: 2026-01-22T22:25:17.624229035-03:00
rendered_js: false
word_count: 1119
summary: This document explains the core components and architectural workflow of Pydantic Evals, including how to define datasets, test cases, and run evaluation experiments.
tags:
    - pydantic-evals
    - llm-evaluation
    - testing-framework
...
- [**`Case`**](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Case "Case            dataclass   ") - A single test scenario with inputs and optional expected outputs
- [**`Evaluator`**](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.Evaluator "Evaluator            dataclass   ") - Logic for scoring or validating outputs
...
- [**`EvaluationReport`**](https://ai.pydantic.dev/api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport "EvaluationReport            dataclass   ") - The results from running an experiment
...
Unit Testing Pydantic Evals Test function [`Case`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Case "Case            dataclass   ") + [`Evaluator`](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.Evaluator "Evaluator            dataclass   ") Test suite [`Dataset`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset "Dataset") Running tests (`pytest`) **Experiment** (`dataset.evaluate(task)`) Test report [`EvaluationReport`](https://ai.pydantic.dev/api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport "EvaluationReport            dataclass   ") `assert` Evaluator returning `bool`
...
You run an experiment by calling [`evaluate()`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluate "evaluate            async   ") or [`evaluate_sync()`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluate_sync "evaluate_sync") on a dataset:
...
4. **Reporting**: All results are aggregated into an [`EvaluationReport`](https://ai.pydantic.dev/api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport "EvaluationReport            dataclass   ")
...
A [`Case`](https://ai.pydantic.dev/api/pydantic_evals/dataset/#pydantic_evals.dataset.Case "Case            dataclass   ") represents a single test scenario with specific inputs and optional expected outputs.
...
The expected result, used by evaluators like [`EqualsExpected`](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.EqualsExpected "EqualsExpected            dataclass   "):
...
Arbitrary data that evaluators can access via [`EvaluatorContext`](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.EvaluatorContext "EvaluatorContext            dataclass   "):
...
Cases can have their own evaluators that only run for that specific case. This is particularly powerful for building comprehensive evaluation suites where different cases have different requirements - if you could write one evaluator rubric that worked perfectly for all cases, you'd just incorporate it into your agent instructions. Case-specific [`LLMJudge`](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.LLMJudge "LLMJudge            dataclass   ") evaluators are especially useful for quickly building maintainable golden datasets by describing what "good" looks like for each scenario. See [Case-specific evaluators](https://ai.pydantic.dev/evals/evaluators/overview/#case-specific-evaluators) for a more detailed explanation and examples.

## Evaluator

An [`Evaluator`](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.Evaluator "Evaluator            dataclass   ") assesses the output of your task and returns one or more scores, labels, or assertions. Each score, label or assertion can also have an optional string-value reason associated.

### Evaluator Types

Evaluators return different types of results:

Return Type Purpose Example `bool` **Assertion** - Pass/fail check `True` → ✔, `False` → ✗ `int` or `float` **Score** - Numeric quality metric `0.95`, `87` `str` **Label** - Categorical result `"correct"`, `"hallucination"`
...
Evaluators can also return instances of [`EvaluationReason`](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.EvaluationReason "EvaluationReason            dataclass   "), and dictionaries mapping labels to output values. See the [custom evaluator return types](https://ai.pydantic.dev/evals/evaluators/custom/#return-types) docs for more detail.

### EvaluatorContext

All evaluators receive an [`EvaluatorContext`](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.EvaluatorContext "EvaluatorContext            dataclass   ") containing:
...
Add explanations to your evaluations using [`EvaluationReason`](https://ai.pydantic.dev/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.EvaluationReason "EvaluationReason            dataclass   "):
...
An [`EvaluationReport`](https://ai.pydantic.dev/api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport "EvaluationReport            dataclass   ") is the result of running an experiment. It contains all the data from executing your task against the dataset's cases and running all evaluators.
...
The [`EvaluationReport`](https://ai.pydantic.dev/api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport "EvaluationReport            dataclass   ") contains:
