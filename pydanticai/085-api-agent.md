---
title: pydantic_ai.agent - Pydantic AI
url: https://ai.pydantic.dev/api/agent/
source: sitemap
fetched_at: 2026-01-22T22:23:53.170564096-03:00
rendered_js: false
word_count: 9967
summary: This reference defines the Agent dataclass, which serves as the primary interface for configuring and managing structured conversations with Large Language Models.
tags:
    - pydantic-ai
    - llm-agent
    - python-api
...
Agents are generic in the dependency type they take [`AgentDepsT`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.AgentDepsT "AgentDepsT            module-attribute   ") and the output type they return, [`OutputDataT`](https://ai.pydantic.dev/api/output/#pydantic_ai.output.OutputDataT "OutputDataT            module-attribute   ").
...
"""A contextmanager which can be used to iterate over the agent graph's nodes as they are executed.
    ...
    def_resolve_and_store_metadata(
        self,
...
"""Context manager to temporarily override agent name, dependencies, model, toolsets, tools, or instructions.
    ...
    @overload
    definstructions(self, func: Callable[[], str | None], /) -> Callable[[], str | None]: ...

    @overload
    definstructions(self, func: Callable[[], Awaitable[str | None]], /) -> Callable[[], Awaitable[str | None]]: ...
...
"""Decorator to register an instructions function.
    ...
    @overload
    defsystem_prompt(self, func: Callable[[], str | None], /) -> Callable[[], str | None]: ...

    @overload
    defsystem_prompt(self, func: Callable[[], Awaitable[str | None]], /) -> Callable[[], Awaitable[str | None]]: ...
...
"""All toolsets registered on the agent, including a function toolset holding tools that were registered on the agent directly.
    ...
    @overload
    def_prepare_output_schema(self, output_type: None) -> _output.OutputSchema[OutputDataT]: ...
    ...
    def_prepare_output_schema(self, output_type: OutputSpec[Any] | None) -> _output.OutputSchema[Any]:
...
"""Create a Starlette app that serves a web chat UI for this agent.
        ...
        Example:
            ...
            ```
        """
        from..ui._webimport create_web_app
...
Name Type Description Default `model` `Model | KnownModelName | str | None`
...
`None` `output_type` `OutputSpec[OutputDataT]`

The type of the output data, used to validate the data returned by the model, defaults to `str`.

`str` `instructions` `Instructions[AgentDepsT]`

Instructions to use for this agent, you can also register instructions via a function with [`instructions`](#pydantic_ai.agent.Agent.instructions "instructions") or pass additional, temporary, instructions when executing a run.

`None` `system_prompt` `str | Sequence[str]`

Static system prompts to use for this agent, you can also register system prompts via a function with [`system_prompt`](#pydantic_ai.agent.Agent.system_prompt "system_prompt").
...
The default number of retries to allow for tool calls and output validation, before raising an error. For model request retries, see the [HTTP Request Retries](https://ai.pydantic.dev/retries/) documentation.

`1` `validation_context` `Any | Callable[[RunContext[AgentDepsT]], Any]`
...
`None` `output_retries` `int | None`

The maximum number of retries to allow for output validation, defaults to `retries`.

`None` `tools` `Sequence[Tool[AgentDepsT] | ToolFuncEither[AgentDepsT, ...]]`

Tools to register with the agent, you can also register tools via the decorators [`@agent.tool`](#pydantic_ai.agent.Agent.tool "tool") and [`@agent.tool_plain`](#pydantic_ai.agent.Agent.tool_plain "tool_plain").
...
`None` `toolsets` `Sequence[AbstractToolset[AgentDepsT] | ToolsetFunc[AgentDepsT]] | None`

Toolsets to register with the agent, including MCP servers and functions which take a run context and return a toolset. See [`ToolsetFunc`](https://ai.pydantic.dev/api/toolsets/#pydantic_ai.toolsets.ToolsetFunc "ToolsetFunc            module-attribute   ") for more information.

`None` `defer_model_check` `bool`

by default, if you provide a [named](https://ai.pydantic.dev/api/models/base/#pydantic_ai.models.KnownModelName "KnownModelName            module-attribute   ") model, it's evaluated to create a [`Model`](https://ai.pydantic.dev/api/models/base/#pydantic_ai.models.Model "Model") instance immediately, which checks for the necessary environment variables. Set this to `false` to defer the evaluation until the first run. Useful if you want to [override the model](#pydantic_ai.agent.Agent.override "override") for testing.

`False` `end_strategy` `EndStrategy`

Strategy for handling tool calls that are requested alongside a final result. See [`EndStrategy`](#pydantic_ai.agent.EndStrategy "EndStrategy            module-attribute   ") for more information.

`'early'` `instrument` `InstrumentationSettings | bool | None`

Set to True to automatically instrument with OpenTelemetry, which will use Logfire if it's configured. Set to an instance of [`InstrumentationSettings`](#pydantic_ai.agent.InstrumentationSettings "InstrumentationSettings            dataclass   ") to customize. If this isn't set, then the last value set by [`Agent.instrument_all()`](#pydantic_ai.agent.Agent.instrument_all "instrument_all            staticmethod   ") will be used, which defaults to False. See the [Debugging and Monitoring guide](https://ai.pydantic.dev/logfire/) for more info.

`None` `metadata` `AgentMetadata[AgentDepsT] | None`

Optional metadata to store with each run. Provide a dictionary of primitives, or a callable returning one computed from the [`RunContext`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") on each run. Metadata is resolved when a run starts and recomputed after a successful run finishes so it can reflect the final state. Resolved metadata can be read after the run completes via [`AgentRun.metadata`](#pydantic_ai.agent.AgentRun "AgentRun            dataclass   "), [`AgentRunResult.metadata`](#pydantic_ai.agent.AgentRunResult "AgentRunResult            dataclass   "), and [`StreamedRunResult.metadata`](https://ai.pydantic.dev/api/result/#pydantic_ai.result.StreamedRunResult "StreamedRunResult            dataclass   "), and is attached to the agent run span when instrumentation is enabled.

`None` `history_processors` `Sequence[HistoryProcessor[AgentDepsT]] | None`
...
`None` `output_type` `OutputSpec[Any] | None`
...
`UNSET` `metadata` `AgentMetadata[AgentDepsT] | Unset`
...
Optionally takes [`RunContext`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") as its only argument. Can decorate a sync or async functions.
...
Name Type Description Default `func` `SystemPromptFunc[AgentDepsT] | None`

The function to decorate

`None` `dynamic` `bool`

If True, the system prompt will be reevaluated even when `messages_history` is provided, see [`SystemPromptPart.dynamic_ref`](https://ai.pydantic.dev/api/messages/#pydantic_ai.messages.SystemPromptPart.dynamic_ref "dynamic_ref            class-attribute       instance-attribute   ")
...
Optionally takes [`RunContext`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") as its first argument. Can decorate a sync or async functions.
...
Decorator to register a tool function which takes [`RunContext`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") as its first argument.
...
Name Type Description Default `func` `ToolFuncContext[AgentDepsT, ToolParams] | None`
...
`None` `retries` `int | None`

The number of retries to allow for this tool, defaults to the agent's default retries, which defaults to 1.

`None` `prepare` `ToolPrepareFunc[AgentDepsT] | None`

custom method to prepare the tool definition for each step, return `None` to omit this tool from a given step. This is useful if you want to customise a tool at call time, or omit it completely from a step. See [`ToolPrepareFunc`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.ToolPrepareFunc "ToolPrepareFunc            module-attribute   ").

`None` `docstring_format` `DocstringFormat`

The format of the docstring, see [`DocstringFormat`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.DocstringFormat "DocstringFormat            module-attribute   "). Defaults to `'auto'`, such that the format is inferred from the structure of the docstring.

`'auto'` `require_parameter_descriptions` `bool`

If True, raise an error if a parameter description is missing. Defaults to False.

`False` `schema_generator` `type[GenerateJsonSchema]`

The JSON schema generator class to use for this tool. Defaults to `GenerateToolJsonSchema`.

`GenerateToolJsonSchema` `strict` `bool | None`

Whether to enforce JSON schema compliance (only affects OpenAI). See [`ToolDefinition`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.ToolDefinition "ToolDefinition            dataclass   ") for more info.

`None` `sequential` `bool`

Whether the function requires a sequential/serial execution environment. Defaults to False.

`False` `requires_approval` `bool`

Whether this tool requires human-in-the-loop approval. Defaults to False. See the [tools documentation](https://ai.pydantic.dev/deferred-tools/#human-in-the-loop-tool-approval) for more info.

`False` `metadata` `dict[str, Any] | None`
...
Name Type Description Default `func` `ToolFuncPlain[ToolParams] | None`
...
Decorator to register a toolset function which takes [`RunContext`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") as its only argument.
...
Name Type Description Default `func` `ToolsetFunc[AgentDepsT] | None`

The toolset function to register.

`None` `per_run_step` `bool`
...
```
frompydantic_aiimport Agent
frompydantic_ai.builtin_toolsimport WebSearchTool
...
Deprecated: use [`async with agent`](#pydantic_ai.agent.Agent.__aenter__ "__aenter__            async   ") instead. If you need to set a sampling model on all MCP servers, use [`agent.set_mcp_sampling_model()`](#pydantic_ai.agent.Agent.set_mcp_sampling_model "set_mcp_sampling_model").
...
Abstract superclass for [`Agent`](#pydantic_ai.agent.Agent "Agent            dataclass   "), [`WrapperAgent`](#pydantic_ai.agent.WrapperAgent "WrapperAgent"), and your own custom agent implementations.
...
"""All toolsets registered on the agent.
    ...
    defoutput_json_schema(self, output_type: OutputSpec[OutputDataT | RunOutputDataT] | None = None) -> JsonSchema:
...
This is a convenience method that wraps [`self.run`](#pydantic_ai.agent.AbstractAgent.run "run            async   ") with `loop.run_until_complete(...)`. You therefore can't use this method inside async code or if there's an active event loop.
...
As this method will consider the first output matching the `output_type` to be the final output, it will stop running the agent graph and will not execute any tool calls made by the model after this "final" output. If you want to always run the agent graph to completion and stream events and output at the same time, use [`agent.run()`](#pydantic_ai.agent.AbstractAgent.run "run            async   ") with an `event_stream_handler` or [`agent.iter()`](#pydantic_ai.agent.AbstractAgent.iter "iter            abstractmethod       async   ") instead.
...
This is a convenience method that wraps [`run_stream()`](#pydantic_ai.agent.AbstractAgent.run_stream "run_stream            async   ") with `loop.run_until_complete(...)`. You therefore can't use this method inside async code or if there's an active event loop.
...
Type Description `StreamedRunResultSync[AgentDepsT, Any]`
...
This is a convenience method that wraps [`self.run`](#pydantic_ai.agent.AbstractAgent.run "run            async   ") and uses the `event_stream_handler` kwarg to get a stream of events from the run.
...
Arguments are the same as for [`self.run`](#pydantic_ai.agent.AbstractAgent.run "run            async   "), except that `event_stream_handler` is now allowed.
...
```
uvicornapp:app--host0.0.0.0--port8000
```
...
Name Type Description Default `output_type` `OutputSpec[OutputDataT] | None`
...
`None` `lifespan` `Lifespan[AGUIApp[AgentDepsT, OutputDataT]] | None`
...
Type Description `AGUIApp[AgentDepsT, OutputDataT]`
...
Name Type Description Default `deps` `AgentDepsT`
...
`'pydantic-ai'` `message_history` `Sequence[ModelMessage] | None`
...
A stateful, async-iterable run of an [`Agent`](#pydantic_ai.agent.Agent "Agent            dataclass   ").

You generally obtain an `AgentRun` instance by calling `async with my_agent.iter(...) as agent_run:`.

Once you have an instance, you can use it to iterate through the run's nodes as they execute. When an [`End`](https://ai.pydantic.dev/api/pydantic_graph/nodes/#pydantic_graph.nodes.End "End            dataclass   ") is reached, the run finishes and [`result`](#pydantic_ai.agent.AgentRun.result "result            property   ") becomes available.
...
You can also manually drive the iteration using the [`next`](#pydantic_ai.agent.AgentRun.next "next            async   ") method for more granular control.
...
Once the run returns an [`End`](https://ai.pydantic.dev/api/pydantic_graph/nodes/#pydantic_graph.nodes.End "End            dataclass   ") node, `result` is populated with an [`AgentRunResult`](#pydantic_ai.agent.AgentRunResult "AgentRunResult            dataclass   ").
...
Return all messages from [`all_messages`](#pydantic_ai.agent.AgentRun.all_messages "all_messages") as JSON bytes.
...
Return new messages from [`new_messages`](#pydantic_ai.agent.AgentRun.new_messages "new_messages") as JSON bytes.
...
This lets you inspect or mutate the node before continuing execution, or skip certain nodes under dynamic conditions. The agent run should be stopped when you return an [`End`](https://ai.pydantic.dev/api/pydantic_graph/nodes/#pydantic_graph.nodes.End "End            dataclass   ") node.
...
Name Type Description Default `node` `AgentNode[AgentDepsT, OutputDataT]`
...
Type Description `AgentNode[AgentDepsT, OutputDataT] | End[FinalResult[OutputDataT]]`

The next node returned by the graph logic, or an [`End`](https://ai.pydantic.dev/api/pydantic_graph/nodes/#pydantic_graph.nodes.End "End            dataclass   ") node if
...
Name Type Description Default `output_tool_return_content` `str | None`
...
Type Description `list[ModelMessage]`
...
Return all messages from [`all_messages`](#pydantic_ai.agent.AgentRunResult.all_messages "all_messages") as JSON bytes.
...
Return new messages from [`new_messages`](#pydantic_ai.agent.AgentRunResult.new_messages "new_messages") as JSON bytes.
...
Context manager to access the messages used in a [`run`](#pydantic_ai.agent.AbstractAgent.run "run            async   "), [`run_sync`](#pydantic_ai.agent.AbstractAgent.run_sync "run_sync"), or [`run_stream`](#pydantic_ai.agent.AbstractAgent.run_stream "run_stream            async   ") call.
...
- `Agent(instrument=...)`
- [`Agent.instrument_all()`](#pydantic_ai.agent.Agent.instrument_all "instrument_all            staticmethod   ")
- [`InstrumentedModel`](https://ai.pydantic.dev/api/models/instrumented/#pydantic_ai.models.instrumented.InstrumentedModel "InstrumentedModel            dataclass   ")
...
Name Type Description Default `tracer_provider` `TracerProvider | None`
...
`None` `meter_provider` `MeterProvider | None`
...
`'attributes'` `logger_provider` `LoggerProvider | None`
...
Name Type Description Default `messages` `list[ModelMessage]`

The messages to convert.

*required* `parameters` `ModelRequestParameters | None`
...
Type Description `list[LogRecord]`
...
A function that receives agent [`RunContext`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") and an async iterable of events from the model's streaming response and the agent's execution of tools.