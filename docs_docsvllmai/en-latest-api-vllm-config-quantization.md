...
url: https://docs.vllm.ai/en/latest/api/vllm/config/quantization/
source: sitemap
fetched_at: 2026-05-07T21:17:10.934705248-03:00
rendered_js: false
word_count: 190
summary: This document defines the configuration structures and enums for managing online quantization schemes in the vLLM library, including support for fp8 and int8 precision. It provides the classes and functions required to resolve and apply quantization settings to model layers.
tags:
    - vllm
    - quantization
    - fp8
...
## vllm.config.quantization [¶](#vllm.config.quantization "Permanent link")

## OnlineQuantScheme [¶](#vllm.config.quantization.OnlineQuantScheme "Permanent link")
...
## OnlineQuantizationConfigArgs [¶](#vllm.config.quantization.OnlineQuantizationConfigArgs "Permanent link")
...
```
@config
classOnlineQuantizationConfigArgs:
"""Configuration for online quantization.
...
### global\_scheme `class-attribute` `instance-attribute` [¶](#vllm.config.quantization.OnlineQuantizationConfigArgs.global_scheme "Permanent link")
...
### ignore `class-attribute` `instance-attribute` [¶](#vllm.config.quantization.OnlineQuantizationConfigArgs.ignore "Permanent link")

Layers to skip quantization for. Supports exact names and regex patterns with `re:` prefix (e.g. `re:.*attn.*`), consistent with compressed\_tensors layer skipping.

### linear\_scheme\_override `class-attribute` `instance-attribute` [¶](#vllm.config.quantization.OnlineQuantizationConfigArgs.linear_scheme_override "Permanent link")
...
### moe\_scheme\_override `class-attribute` `instance-attribute` [¶](#vllm.config.quantization.OnlineQuantizationConfigArgs.moe_scheme_override "Permanent link")
...
## resolve\_online\_quant\_config [¶](#vllm.config.quantization.resolve_online_quant_config "Permanent link")
...
```
defresolve_online_quant_config(
    quantization: str | None,
