---
title: Pagination - FastMCP
url: https://gofastmcp.com/servers/pagination
source: crawler
fetched_at: 2026-01-22T22:21:55.340551571-03:00
rendered_js: false
word_count: 335
summary: This document explains how to implement and manage pagination for tools, resources, and prompts within the FastMCP framework, covering both server-side configuration and client-side consumption.
tags:
    - mcp
    - fastmcp
    - pagination
...
New in version `3.0.0` When a server exposes many tools, resources, or prompts, returning them all in a single response can be impractical. MCP supports pagination for list operations, allowing servers to return results in manageable chunks that clients can fetch incrementally.
...
OperationConvenience MethodManual MethodTools`list_tools()``list_tools_mcp(cursor=...)`Resources`list_resources()``list_resources_mcp(cursor=...)`Resource Templates`list_resource_templates()``list_resource_templates_mcp(cursor=...)`Prompts`list_prompts()``list_prompts_mcp(cursor=...)`
