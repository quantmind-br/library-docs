---
title: base_linear - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/lora/layers/base_linear/
source: sitemap
fetched_at: 2026-05-07T21:22:33.1002275-03:00
rendered_js: false
word_count: 0
summary: This document defines the BaseLinearLayerWithLoRA class, which implements Low-Rank Adaptation (LoRA) for linear layers, supporting both synchronous and asynchronous CUDA execution paths for efficient integration with deep learning models.
tags:
    - lora
    - cuda
    - linear-layer
...
```
classBaseLinearLayerWithLoRA(BaseLayerWithLoRA):
    def__init__(self, base_layer: LinearBase):
        super().__init__()
    ...
    def_init_lora_stream_context(self) -> None:
    ...
    def_apply_base_forward(self, x: torch.Tensor) -> torch.Tensor:
        base_output = self.base_layer(x)
        output = base_output[0] if isinstance(base_output, tuple) else base_output
        return self._apply_lora_to_output(x, output)

    def_apply_lora_to_output(
...
"""
    ...
    @property
    defbias(self) -> torch.Tensor | None:
