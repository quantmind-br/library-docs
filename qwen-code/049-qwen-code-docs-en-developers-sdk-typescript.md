...
source: github_pages
fetched_at: 2026-04-09T09:04:36.248155576-03:00
rendered_js: true
word_count: 928
summary: This document serves as an overview and reference for the @qwen-code/sdk, detailing its installation, usage with a quick start example, and comprehensive API documentation covering query parameters, timeouts, message handling, permission modes, and instance methods.
tags:
    - sdk-reference
    - typescript
    - programmatic-access
...
### Query Instance Methods[](#query-instance-methods)
...
### Permission Priority Chain[](#permission-priority-chain)
...
#### `createSdkMcpServer(options)`[](#createsdkmcpserveroptions)

Creates an SDK-embedded MCP server instance.

OptionTypeDefaultDescription`name``string`RequiredUnique name for the MCP server`version``string``'1.0.0'`Server version`tools``SdkMcpToolDefinition[]`-Array of tools created with `tool()`
