---
title: Drug Discovery — Pharmaceutical research assistant for drug discovery workflows | Hermes Agent
url: https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/research/research-drug-discovery
source: crawler
fetched_at: 2026-04-24T17:01:31.534308857-03:00
rendered_js: false
word_count: 325
summary: This document details a comprehensive pharmaceutical research skill for drug discovery workflows, enabling users to perform tasks such as searching bioactive compounds in ChEMBL, calculating molecular properties like Lipinski's Rule of Five and QED, looking up drug interactions via OpenFDA, and querying literature from OpenTargets.
tags:
    - drug-discovery
    - cheminformatics
    - pharmacology
...
SourceOptional — install with `hermes skills install official/research/drug-discovery`Path`optional-skills/research/drug-discovery`Version`1.0.0`AuthorbennytimzLicenseMITTags`science`, `chemistry`, `pharmacology`, `research`, `health`
...
TaskAPIEndpointFind targetChEMBL`/api/data/target/search?q=`Get bioactivityChEMBL`/api/data/activity?target_chembl_id=`Molecule propertiesPubChem`/rest/pug/compound/name/{name}/property/`Drug interactionsOpenFDA`/drug/label.json?search=drug_interactions:`Adverse eventsOpenFDA`/drug/event.json?search=...&count=reaction`Gene-diseaseOpenTargetsGraphQL POST `/api/v4/graphql`