---
title: resampler - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/resampler/
source: sitemap
fetched_at: 2026-05-07T21:28:10.807547011-03:00
rendered_js: false
word_count: 171
summary: This document describes the implementation of a perceiver-resampler network and utility functions for generating 2D sine-cosine positional embeddings in multimodal models.
tags:
    - perceiver-resampler
    - multimodal-models
...
## BaseResampler [¶](#vllm.model_executor.layers.resampler.BaseResampler "Permanent link")

Bases: `Module`

A 2D perceiver-resampler network with one cross attention layers by (grid\_size**2) learnable queries and 2d sincos pos\_emb. Outputs: A tensor with the shape of (grid\_size**2, embed\_dim)

Source code in `vllm/model_executor/layers/resampler.py`

```
classBaseResampler(nn.Module):
"""
    ...
    def_repeat(self, query, N: int):
        return query.unsqueeze(1).repeat(1, N, 1)
```

## Resampler2 [¶](#vllm.model_executor.layers.resampler.Resampler2 "Permanent link")

Bases: `BaseResampler`

Resampler-perceiver network to be used for a variety of model types, e.g., Qwen-vl / Minicpmv 2.0. The main difference is the addition of the do\_post\_projection arg, which indicates whether or not there should be a post layer normalization and projector after the attention. This is present in minicpmv2.0, but not qwen-vl.

Source code in `vllm/model_executor/layers/resampler.py`

```
classResampler2(BaseResampler):
...
## get\_1d\_sincos\_pos\_embed\_from\_grid [¶](#vllm.model_executor.layers.resampler.get_1d_sincos_pos_embed_from_grid "Permanent link")

embed\_dim: output dimension for each position pos: a list of positions to be encoded: size (M,) / (H, W) out: (M, D) / (H, W, D)

Source code in `vllm/model_executor/layers/resampler.py`

```
defget_1d_sincos_pos_embed_from_grid(
...
## get\_2d\_sincos\_pos\_embed [¶](#vllm.model_executor.layers.resampler.get_2d_sincos_pos_embed "Permanent link")
