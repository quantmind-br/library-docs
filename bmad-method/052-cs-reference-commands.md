---
title: Skills
url: https://docs.bmad-method.org/cs/reference/commands/
source: sitemap
fetched_at: 2026-04-08T11:29:33.624537293-03:00
rendered_js: false
word_count: 651
summary: This document explains the concept and usage differences between 'Skills' and 'Agent Offer Triggers' within the BMad system. It details how these skills are generated by an installer, where their files reside in the project structure, and provides guidance on how to discover and troubleshoot them.
tags:
    - skills
    - bmad-system
    - workflow-automation
...
Skills jsou předpřipravené prompty, které načítají agenty, spouštějí workflow nebo provádějí úkoly ve vašem IDE. Instalátor BMad je generuje z vašich nainstalovaných modulů při instalaci. Pokud později přidáte, odeberete nebo změníte moduly, přeinstalujte pro synchronizaci skills (viz [Řešení problémů](#%C5%99e%C5%A1en%C3%AD-probl%C3%A9m%C5%AF)).

## Skills vs. spouštěče nabídky agentů

[Section titled “Skills vs. spouštěče nabídky agentů”](#skills-vs-spou%C5%A1t%C4%9B%C4%8De-nab%C3%ADdky-agent%C5%AF)

BMad nabízí dva způsoby zahájení práce a slouží k různým účelům.

MechanismusJak se vyvoláváCo se stane**Skill**Zadejte název skillu (např. `bmad-help`) ve vašem IDEPřímo načte agenta, spustí workflow nebo provede úkol**Spouštěč nabídky agenta**Nejprve načtěte agenta, pak zadejte krátký kód (např. `DS`)Agent interpretuje kód a spustí odpovídající workflow, přičemž zůstává v charakteru
...
[Section titled “Jak se skills generují”](#jak-se-skills-generuj%C3%AD)
...
Typ skilluCo generovaný soubor dělá**Spouštěč agenta**Načte soubor persony agenta, aktivuje jeho nabídku a zůstává v charakteru**Workflow skill**Načte konfiguraci workflow a následuje jeho kroky**Task skill**Načte samostatný soubor úkolu a následuje jeho instrukce**Tool skill**Načte samostatný soubor nástroje a následuje jeho instrukce

## Kde žijí soubory skills

[Section titled “Kde žijí soubory skills”](#kde-%C5%BEij%C3%AD-soubory-skills)
...
IDE / CLIAdresář skillsClaude Code`.claude/skills/`Cursor`.cursor/skills/`Windsurf`.windsurf/skills/`Další IDEViz výstup instalátoru pro cílovou cestu
...
[Section titled “Jak objevit vaše skills”](#jak-objevit-va%C5%A1e-skills)
...
Příklad skilluAgentRole`bmad-agent-dev`Amelia (Developer)Implementuje stories s přísným dodržováním specifikací`bmad-pm`John (Product Manager)Vytváří a validuje PRD`bmad-architect`Winston (Architect)Navrhuje systémovou architekturu

Viz [Agenti](https://docs.bmad-method.org/cs/reference/agents/) pro úplný seznam výchozích agentů a jejich spouštěčů.
...
Příklad skilluÚčel`bmad-product-brief`Vytvoření product briefu — řízené discovery, když je váš koncept jasný`bmad-prfaq`Working Backwards PRFAQ výzva pro zátěžový test vašeho produktového konceptu`bmad-create-prd`Vytvoření dokumentu požadavků (PRD)`bmad-create-architecture`Návrh systémové architektury`bmad-create-epics-and-stories`Vytvoření epiců a stories`bmad-dev-story`Implementace story`bmad-code-review`Spuštění revize kódu`bmad-quick-dev`Sjednocený quick flow — vyjasnění záměru, plán, implementace, revize, prezentace

Viz [Mapa pracovních postupů](https://docs.bmad-method.org/cs/reference/workflow-map/) pro kompletní referenci workflow organizovanou podle fází.

### Task a tool skills

[Section titled “Task a tool skills”](#task-a-tool-skills)
...
Základní modul zahrnuje 11 vestavěných nástrojů — revize, komprese, brainstorming, správa dokumentů a další. Viz [Základní nástroje](https://docs.bmad-method.org/cs/reference/core-tools/) pro kompletní referenci.

## Konvence pojmenování

[Section titled “Konvence pojmenování”](#konvence-pojmenov%C3%A1n%C3%AD)

Všechny skills používají prefix `bmad-` následovaný popisným názvem (např. `bmad-dev`, `bmad-create-prd`, `bmad-help`). Viz [Moduly](https://docs.bmad-method.org/cs/reference/modules/) pro dostupné moduly.
