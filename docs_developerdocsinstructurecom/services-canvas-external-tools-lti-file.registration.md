---
title: Registration | Instructure Developer Documentation Portal
url: https://developerdocs.instructure.com/services/canvas/external-tools/lti/file.registration
source: sitemap
fetched_at: 2026-02-15T09:13:37.345453-03:00
rendered_js: false
word_count: 1050
summary: This document explains how to implement LTI Dynamic Registration in Canvas to enable automated tool installation and configuration using the LTI 1.3 specification.
tags:
...
In order to enable an Administrator to install an application in Canvas, a tool can support the automated registration process, built on the Dynamic Registration specification mentioned [herearrow-up-right](https://www.imsglobal.org/activity/learning-tools-interoperability-lti%C2%AE). This replaces the [manual configuration](https://developerdocs.instructure.com/services/canvas/external-tools/lti/file.lti_dev_key_config) process previously used to install applications to Canvas.
...
Canvas includes a URL in the OpenID configuration under the `registration_endpoint` key which can be used by the tool to create a registration. The tool must send a `POST` request to this endpoint with the tool's [LTI Registration](https://developerdocs.instructure.com/services/canvas/external-tools/lti/file.registration#lti-registration-schema) in the body and the `registration_token` as the bearer token in the `Authorization` http header.
...
https://canvas.instructure.com/lti/privacy\_level

"public" | "name\_only" | "email\_only" | "anonymous"

The tool's default privacy level, (determines the PII fields the tool is sent.) defaults to "anonymous"

https://canvas.instructure.com/lti/tool\_id
