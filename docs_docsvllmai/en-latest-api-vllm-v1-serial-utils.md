---
title: serial_utils - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/serial_utils/
source: sitemap
fetched_at: 2026-05-07T21:41:37.441897227-03:00
rendered_js: false
word_count: 497
summary: The MsgpackDecoder provides a specialized interface for deserializing msgpack-encoded data, with built-in support for converting custom torch tensor and numpy array structures.
tags:
    - msgpack
    - serialization
    - tensor-processing
...
## MsgpackDecoder [¶](#vllm.v1.serial_utils.MsgpackDecoder "Permanent link")
...
`oob_tensor_provider` must be used when an OOBTensorConsumer is used on the encoder side.
...
"""Decoder with custom torch tensor and numpy array serialization.
    ...
    defdecode(self, bufs: bytestr | Sequence[bytestr]) -> Any:
    ...
    defdec_hook(self, t: type, obj: Any) -> Any:
    ...
    def_decode_utility_result(self, obj: Any) -> UtilityResult:
    ...
    def_convert_result(self, result_type: Sequence[str], result: Any) -> Any:
    ...
    def_decode_ndarray(self, arr: Any) -> np.ndarray:
    ...
    def_decode_tensor(self, arr: Any) -> torch.Tensor:
        dtype, shape, data = arr
    ...
    def_decode_mm_items(self, obj: dict[str, Any]) -> MultiModalKwargsItems:
    ...
    def_decode_mm_item(self, obj: dict[str, Any]) -> MultiModalKwargsItem:
        return MultiModalKwargsItem(
            {key: self._decode_mm_field_elem(elem) for key, elem in obj.items()}
        )

    def_decode_mm_field_elem(self, obj: dict[str, Any]) -> MultiModalFieldElem:
    ...
    def_decode_nested_tensors(self, obj: Any) -> NestedTensors:
...
## MsgpackEncoder [¶](#vllm.v1.serial_utils.MsgpackEncoder "Permanent link")
...
When a `oob_tensor_consumer` is provided, tensors (CUDA and CPU) will be offered to it for out-of-band handling.
...
"""Encoder with custom torch tensor and numpy array serialization.
    ...
    defencode(self, obj: Any) -> Sequence[bytestr]:
        try:
    ...
    defencode_into(self, obj: Any, buf: bytearray) -> Sequence[bytestr]:
        try:
            if self.oob_tensor_consumer is not None:
                self.oob_tensor_consumer.new_message()
    ...
    def_encode_mm_items(self, items: MultiModalKwargsItems) -> dict[str, Any]:
        return {
            modality: [self._encode_mm_item(item) for item in itemlist]
            for modality, itemlist in items.items()
        }

    def_encode_mm_item(self, item: MultiModalKwargsItem) -> dict[str, Any]:
        return {key: self._encode_mm_field_elem(elem) for key, elem in item.items()}

    def_encode_mm_field_elem(self, elem: MultiModalFieldElem) -> dict[str, Any]:
        return {
            "data": (
    ...
    def_encode_nested_tensors(self, nt: NestedTensors) -> Any:
    ...
    def_encode_mm_field(self, field: BaseMultiModalField):
...
## OOBTensorConsumer [¶](#vllm.v1.serial_utils.OOBTensorConsumer "Permanent link")
...
```
classOOBTensorConsumer(ABC):
    @abstractmethod
    def__call__(self, tensor: torch.Tensor) -> dict | None:
"""
...
### \_\_call\__ `abstractmethod` [¶](#vllm.v1.serial_utils.OOBTensorConsumer.__call__ "Permanent link")

Called with tensors for the current message. Returns None to reject the tensor (falls back to regular serialization), otherwise a dict with arbitrary placeholder data to be included in the serialized message.
...
### new\_message `abstractmethod` [¶](#vllm.v1.serial_utils.OOBTensorConsumer.new_message "Permanent link")
...
## PydanticMsgspecMixin [¶](#vllm.v1.serial_utils.PydanticMsgspecMixin "Permanent link")

Make a `msgspec.Struct` compatible with Pydantic for both **validation** (JSON/dict -&gt; Struct) and **serialization** (Struct -&gt; JSON-safe dict).

Subclasses may set `__pydantic_msgspec_exclude__` (a `set[str]`) to list non-underscore field names that should also be stripped from serialized output. Fields whose names start with `_` are always excluded automatically.

Source code in `vllm/v1/serial_utils.py`

```
classPydanticMsgspecMixin:
...
"""
    ...
    @classmethod
    def_validate_msgspec(cls, value: Any) -> Any:
"""Validate and convert input to msgspec.Struct instance."""
    ...
    @staticmethod
    def_serialize_msgspec(value: Any) -> Any:
...
### \_\_get\_pydantic\_core\_schema\__ `classmethod` [¶](#vllm.v1.serial_utils.PydanticMsgspecMixin.__get_pydantic_core_schema__ "Permanent link")
...
### \_serialize\_msgspec `staticmethod` [¶](#vllm.v1.serial_utils.PydanticMsgspecMixin._serialize_msgspec "Permanent link")

```
_serialize_msgspec(value: Any) -> Any
```

Serialize a msgspec.Struct to a JSON-compatible dict, stripping private (`_`-prefixed) and explicitly excluded fields.

Uses `msgspec.to_builtins` which respects `omit_defaults=True`, so only fields that differ from their declared defaults are included.
...
### \_validate\_msgspec `classmethod` [¶](#vllm.v1.serial_utils.PydanticMsgspecMixin._validate_msgspec "Permanent link")
...
## UtilityResult [¶](#vllm.v1.serial_utils.UtilityResult "Permanent link")
...
"""Wrapper for special handling when serializing/deserializing."""
    ...
    def__init__(self, r: Any = None):
        self.result = r
```

## \_decode\_type\_info\_recursive [¶](#vllm.v1.serial_utils._decode_type_info_recursive "Permanent link")
...
```
def_decode_type_info_recursive(
...
## \_encode\_type\_info\_recursive [¶](#vllm.v1.serial_utils._encode_type_info_recursive "Permanent link")
...
```
def_encode_type_info_recursive(obj: Any) -> Any:
...
## run\_method [¶](#vllm.v1.serial_utils.run_method "Permanent link")
