---
title: 2. GEPA Advanced - DSPy
url: https://dspy.ai/api/optimizers/GEPA/GEPA_Advanced/
source: sitemap
fetched_at: 2026-01-23T08:02:26.641832494-03:00
rendered_js: false
word_count: 785
summary: This document explains how to customize and implement instruction proposers within the dspy.GEPA optimizer for advanced prompt refinement and multi-modal handling.
tags:
    - dspy
    - gepa
...
[](https://github.com/stanfordnlp/dspy/blob/main/docs/docs/api/optimizers/GEPA/GEPA_Advanced.md "Edit this page")

## dspy.GEPA - Advanced Features[¶](#dspygepa-advanced-features "Permanent link")

## Custom Instruction Proposers[¶](#custom-instruction-proposers "Permanent link")

### What is instruction\_proposer?[¶](#what-is-instruction_proposer "Permanent link")
...
### Default Implementation[¶](#default-implementation "Permanent link")
...
### When to Use Custom instruction\_proposer[¶](#when-to-use-custom-instruction_proposer "Permanent link")
...
- **Domain-specific information**: Inject specialized knowledge, terminology, or context that the default proposer lacks and cannot be provided via feedback\_func. This is an advanced feature, and most users should not need to use this.
...
- **Coupled component updates**: Handle situations where 2 or more components need to be updated together in a coordinated manner, rather than optimizing each component independently (refer to component\_selector parameter, in [Custom Component Selection](#custom-component-selection) section, for related functionality)
- **External knowledge integration**: Connect to databases, APIs, or knowledge bases during instruction generation

### Available Options[¶](#available-options "Permanent link")
...
```
fromdspy.teleprompt.gepa.instruction_proposalimport MultiModalInstructionProposer
...
### How to Implement Custom Instruction Proposers[¶](#how-to-implement-custom-instruction-proposers "Permanent link")
...
```
fromdspy.teleprompt.gepa.gepa_utilsimport ReflectiveExample

classCustomInstructionProposer:
    def__call__(
        self,
...
# Or as a function:
defcustom_instruction_proposer(candidate, reflective_dataset, components_to_update):
    # Your custom instruction generation logic here
    return updated_instructions
```
...
#### Basic Example: Word Limit Proposer[¶](#basic-example-word-limit-proposer "Permanent link")

```
importdspy
fromgepa.core.adapterimport ProposalFn
fromdspy.teleprompt.gepa.gepa_utilsimport ReflectiveExample

classGenerateWordLimitedInstruction(dspy.Signature):
...
classWordLimitProposer(ProposalFn):
    def__init__(self, max_words: int = 1000):
        self.max_words = max_words
        self.instruction_improver = dspy.ChainOfThought(GenerateWordLimitedInstruction)

    def__call__(self, candidate: dict[str, str], reflective_dataset: dict[str, list[ReflectiveExample]], components_to_update: list[str]) -> dict[str, str]:
        updated_components = {}
...
#### Advanced Example: RAG-Enhanced Instruction Proposer[¶](#advanced-example-rag-enhanced-instruction-proposer "Permanent link")
...
classGenerateDocumentationQuery(dspy.Signature):
...
classGenerateRAGEnhancedInstruction(dspy.Signature):
...
"""Module that uses RAG to improve instructions with specialized documentation."""
    ...
    defforward(self, current_instruction: str, component_examples: list):
...
classDocumentationEnhancedProposer(ProposalFn):
"""Instruction proposer that accesses specialized documentation via RAG."""

    def__init__(self, documentation_retriever):
"""
        Args:
...
```
classExternalLMProposer(ProposalFn):
    def__init__(self):
        # Manage your own LM instance
        self.external_lm = dspy.LM('gemini/gemini-2.5-pro')

    def__call__(self, candidate, reflective_dataset, components_to_update):
        updated_components = {}
...
## Custom Component Selection[¶](#custom-component-selection "Permanent link")

### What is component\_selector?[¶](#what-is-component_selector "Permanent link")
...
### Built-in Selection Strategies[¶](#built-in-selection-strategies "Permanent link")
...
### When to Use Custom Component Selection[¶](#when-to-use-custom-component-selection "Permanent link")
...
### Custom Component Selector Protocol[¶](#custom-component-selector-protocol "Permanent link")
...
```
fromdspy.teleprompt.gepa.gepa_utilsimport GEPAState, Trajectory

classCustomComponentSelector:
    def__call__(
        self,
...
# Or as a function:
defcustom_component_selector(state, trajectories, subsample_scores, candidate_idx, candidate):
    # Your custom component selection logic here
    return selected_components
```

### Custom Implementation Example[¶](#custom-implementation-example "Permanent link")

Here's a simple function that alternates between optimizing different halves of your components:

```
defalternating_half_selector(state, trajectories, subsample_scores, candidate_idx, candidate):
...
### Integration with Custom Instruction Proposers[¶](#integration-with-custom-instruction-proposers "Permanent link")
