...
source: sitemap
fetched_at: 2026-05-07T21:12:28.759955824-03:00
rendered_js: false
word_count: 2682
summary: This document provides a high-level technical overview of the multi-head query attention kernel implementation used in vLLM, explaining memory layout, core concepts, and data access patterns.
tags:
    - vllm
    - cuda-kernel
    - paged-attention
...
```
template<typenamescalar_t,intHEAD_SIZE,intBLOCK_SIZE,intNUM_THREADS,intPARTITION_SIZE=0>
__device__voidpaged_attention_kernel(
...// Other side args.
constscalar_t*__restrict__out,// [num_seqs, num_heads, max_num_partitions, head_size]
constscalar_t*__restrict__q,// [num_seqs, num_heads, head_size]
constscalar_t*__restrict__k_cache,// [num_blocks, num_kv_heads, head_size/x, block_size, x]
constscalar_t*__restrict__v_cache,// [num_blocks, num_kv_heads, head_size, block_size]
...// Other side args.
)
```
...
```
constscalar_t*q_ptr=q+seq_idx*q_stride+head_idx*HEAD_SIZE;
```

[![query](https://docs.vllm.ai/en/latest/assets/design/paged_attention/query.png)](https://docs.vllm.ai/en/latest/assets/design/paged_attention/query.png)
...
[![q_vecs](https://docs.vllm.ai/en/latest/assets/design/paged_attention/q_vecs.png)](https://docs.vllm.ai/en/latest/assets/design/paged_attention/q_vecs.png)

```
__shared__Q_vecq_vecs[THREAD_GROUP_SIZE][NUM_VECS_PER_THREAD];
```
...
```
constscalar_t*k_ptr=k_cache+physical_block_number*kv_block_stride
...
[![key](https://docs.vllm.ai/en/latest/assets/design/paged_attention/key.png)](https://docs.vllm.ai/en/latest/assets/design/paged_attention/key.png)
...
[![k_vecs](https://docs.vllm.ai/en/latest/assets/design/paged_attention/k_vecs.png)](https://docs.vllm.ai/en/latest/assets/design/paged_attention/k_vecs.png)

```
K_veck_vecs[NUM_VECS_PER_THREAD]
```
...
...
floatqk=scale*Qk_dot<scalar_t,THREAD_GROUP_SIZE>::dot(q_vecs[thread_group_offset],k_vecs);
}
```
...
\\\[ \\begin{gather\*} m(x):=\\max \_i \\quad x\_i \\\\ \\quad f(x):=\\left\[\\begin{array}{lll}e^{x\_1-m(x)} & \\ldots & e^{x\_B-m(x)}\\end{array}\\right]\\\\ \\quad \\ell(x):=\\sum\_i f(x)\_i \\\\ \\quad \\operatorname{softmax}(x):=\\frac{f(x)}{\\ell(x)} \\end{gather\*} \\]
...
constboolmask=token_idx>=context_len;
...
```
for(intmask=WARP_SIZE/2;mask>=THREAD_GROUP_SIZE;mask/=2){
...
```
for(intmask=NUM_WARPS/2;mask>=1;mask/=2){
qk_max=fmaxf(qk_max,VLLM_SHFL_XOR_SYNC(qk_max,mask));
}
...
```
for(inti=thread_idx;i<num_tokens;i+=NUM_THREADS){
floatval=__expf(logits[i]-qk_max);
logits[i]=val;
exp_sum+=val;
}
...
...
```
constfloatinv_sum=__fdividef(1.f,exp_sum+1e-6f);
for(inti=thread_idx;i<num_tokens;i+=NUM_THREADS){
logits[i]*=inv_sum;
}
```
...
[![value](https://docs.vllm.ai/en/latest/assets/design/paged_attention/value.png)](https://docs.vllm.ai/en/latest/assets/design/paged_attention/value.png)

[![logits_vec](https://docs.vllm.ai/en/latest/assets/design/paged_attention/logits_vec.png)](https://docs.vllm.ai/en/latest/assets/design/paged_attention/logits_vec.png)

[![v_vec](https://docs.vllm.ai/en/latest/assets/design/paged_attention/v_vec.png)](https://docs.vllm.ai/en/latest/assets/design/paged_attention/v_vec.png)
...
```
for(inti=0;i<NUM_ROWS_PER_THREAD;i++){
floatacc=accs[i];
for(intmask=NUM_V_VECS_PER_ROW/2;mask>=1;mask/=2){
...
constfloat*src=&out_smem[warp_idx*HEAD_SIZE];
for(inti=0;i<NUM_ROWS_PER_THREAD;i++){
...
accs[i]+=src[row_idx];
}
...
```
for(inti=0;i<NUM_ROWS_PER_THREAD;i++){
constintrow_idx=lane/NUM_V_VECS_PER_ROW+i*NUM_ROWS_PER_ITER;
