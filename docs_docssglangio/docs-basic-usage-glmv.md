---
title: GLM-4.6V / GLM-4.5V Usage - SGLang Documentation
url: https://docs.sglang.io/docs/basic_usage/glmv
source: sitemap
fetched_at: 2026-05-11T05:49:10.625956762-03:00
rendered_js: false
word_count: 345
summary: This document provides instructions for launching and configuring the SGLang server to support multimodal models, including optimization flags for hardware-specific deployments and methods for submitting image and video inference requests.
tags:
    - sglang
...
In SGLang, we can implement thinking budget with `CustomLogitProcessor`. Launch a server with the `--enable-custom-logit-processor` flag. Then, use `Glm4MoeThinkingBudgetLogitProcessor` in the request, similar to the `GLM-4.6` example in [glm45.md](https://docs.sglang.io/docs/basic_usage/glm45).