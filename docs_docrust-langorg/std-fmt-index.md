---
title: std::fmt - Rust
url: https://doc.rust-lang.org/std/fmt/index.html
source: crawler
fetched_at: 2026-05-06T21:24:37.087190137-03:00
rendered_js: false
word_count: 2576
summary: This module provides the runtime support and syntax for Rust's string formatting macros, enabling flexible data representation through positional parameters, named arguments, and customizable formatting specifiers.
tags:
    - rust
    - string-formatting
    - macros
...
1.0.0 · [Source](https://doc.rust-lang.org/src/alloc/lib.rs.html#226)
...
The width can also be provided dynamically by referencing another argument with a `$` suffix. Use `{:N$}` to reference the Nth positional argument (where N is an integer), or `{:name$}` to reference a named argument. The referenced argument must be of type [`usize`](https://doc.rust-lang.org/std/primitive.usize.html "primitive usize").
...
2. An integer or name followed by dollar sign `.N$`:
   ...
   use the value of format *argument* `N` (which must be a `usize`) as the precision. An integer refers to a positional argument, and a name refers to a named argument.
3. An asterisk `.*`:
...
- `identifier` is an `IDENTIFIER_OR_KEYWORD` (not an `IDENTIFIER`) as defined by the [Rust language reference](https://doc.rust-lang.org/reference/identifiers.html), except for a bare `_`.
...
- *nothing* ⇒ [`Display`](https://doc.rust-lang.org/std/fmt/trait.Display.html "trait std::fmt::Display")
...
What this means is that any type of argument which implements the [`fmt::Binary`](https://doc.rust-lang.org/std/fmt/trait.Binary.html "trait std::fmt::Binary") trait can then be formatted with `{:b}`. Implementations are provided for these traits for a number of primitive types by the standard library as well. If no format is specified (as in `{}` or `{:6}`), then the format trait used is the [`Display`](https://doc.rust-lang.org/std/fmt/trait.Display.html "trait std::fmt::Display") trait.
...
Your type will be passed as `self` by-reference, and then the function should emit output into the Formatter `f` which implements `fmt::Write`. It is up to each format trait implementation to correctly adhere to the requested formatting parameters. The values of these parameters can be accessed with methods of the [`Formatter`](https://doc.rust-lang.org/std/fmt/struct.Formatter.html "struct std::fmt::Formatter") struct. In order to help with this, the [`Formatter`](https://doc.rust-lang.org/std/fmt/struct.Formatter.html "struct std::fmt::Formatter") struct also provides some helper methods.

Additionally, the return value of this function is [`fmt::Result`](https://doc.rust-lang.org/std/fmt/type.Result.html "fmt::Result") which is a type alias of `Result<(), std::fmt::Error>`. Formatting implementations should ensure that they propagate errors from the [`Formatter`](https://doc.rust-lang.org/std/fmt/struct.Formatter.html "struct std::fmt::Formatter") (e.g., when calling [`write!`](https://doc.rust-lang.org/std/macro.write.html "macro std::write")). However, they should never return errors spuriously. That is, a formatting implementation must and may only return an error if the passed-in [`Formatter`](https://doc.rust-lang.org/std/fmt/struct.Formatter.html "struct std::fmt::Formatter") returns an error. This is because, contrary to what the function signature might suggest, string formatting is an infallible operation. This function only returns a [`Result`](https://doc.rust-lang.org/std/fmt/type.Result.html "type std::fmt::Result") because writing to the underlying stream might fail and it must provide a way to propagate the fact that an error has occurred back up the stack.
...
- [`fmt::Display`](https://doc.rust-lang.org/std/fmt/trait.Display.html "trait std::fmt::Display") implementations assert that the type can be faithfully represented as a UTF-8 string at all times. It is **not** expected that all types implement the [`Display`](https://doc.rust-lang.org/std/fmt/trait.Display.html "trait std::fmt::Display") trait.
- [`fmt::Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") implementations should be implemented for **all** public types. Output will typically represent the internal state as faithfully as possible. The purpose of the [`Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") trait is to facilitate debugging Rust code. In most cases, using `#[derive(Debug)]` is sufficient and recommended.
...
There are a number of related macros in the [`format!`](https://doc.rust-lang.org/std/macro.format.html "macro std::format") family. The ones that are currently implemented are:
...
[`write!`](https://doc.rust-lang.org/std/macro.write.html "macro std::write") and [`writeln!`](https://doc.rust-lang.org/std/macro.writeln.html "macro std::writeln") are two macros which are used to emit the format string to a specified stream. This is used to prevent intermediate allocations of format strings and instead directly write the output. Under the hood, this function is actually invoking the [`write_fmt`](https://doc.rust-lang.org/std/io/trait.Write.html#method.write_fmt) function defined on the [`std::io::Write`](https://doc.rust-lang.org/std/io/trait.Write.html) and the [`std::fmt::Write`](https://doc.rust-lang.org/std/fmt/trait.Write.html) trait. Example usage is:
...
This and [`println!`](https://doc.rust-lang.org/std/macro.println.html "println!") emit their output to stdout. Similarly to the [`write!`](https://doc.rust-lang.org/std/macro.write.html "macro std::write") macro, the goal of these macros is to avoid intermediate allocations when printing output. Example usage is:
...
The [`eprint!`](https://doc.rust-lang.org/std/macro.eprint.html "eprint!") and [`eprintln!`](https://doc.rust-lang.org/std/macro.eprintln.html "eprintln!") macros are identical to [`print!`](https://doc.rust-lang.org/std/macro.print.html "print!") and [`println!`](https://doc.rust-lang.org/std/macro.println.html "println!"), respectively, except they emit their output to stderr.
...
The result of the [`format_args!`](https://doc.rust-lang.org/std/macro.format_args.html "format_args!") macro is a value of type [`fmt::Arguments`](https://doc.rust-lang.org/std/fmt/struct.Arguments.html "fmt::Arguments"). This structure can then be passed to the [`write`](https://doc.rust-lang.org/std/fmt/fn.write.html "fmt::write") and [`format`](https://doc.rust-lang.org/std/fmt/fn.format.html "fmt::format") functions inside this module in order to process the format string. The goal of this macro is to even further prevent intermediate allocations when dealing with formatting strings.
...
[Arguments](https://doc.rust-lang.org/std/fmt/struct.Arguments.html "struct std::fmt::Arguments")

This structure represents a safely precompiled version of a format string and its arguments. This cannot be generated at runtime because it cannot safely be done, so no constructors are given and the fields are private to prevent modification.

[DebugList](https://doc.rust-lang.org/std/fmt/struct.DebugList.html "struct std::fmt::DebugList")

A struct to help with [`fmt::Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") implementations.

[DebugMap](https://doc.rust-lang.org/std/fmt/struct.DebugMap.html "struct std::fmt::DebugMap")

A struct to help with [`fmt::Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") implementations.

[DebugSet](https://doc.rust-lang.org/std/fmt/struct.DebugSet.html "struct std::fmt::DebugSet")

A struct to help with [`fmt::Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") implementations.

[DebugStruct](https://doc.rust-lang.org/std/fmt/struct.DebugStruct.html "struct std::fmt::DebugStruct")

A struct to help with [`fmt::Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") implementations.

[DebugTuple](https://doc.rust-lang.org/std/fmt/struct.DebugTuple.html "struct std::fmt::DebugTuple")

A struct to help with [`fmt::Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") implementations.

[Error](https://doc.rust-lang.org/std/fmt/struct.Error.html "struct std::fmt::Error")

The error type which is returned from formatting a message into a stream.

[Formatter](https://doc.rust-lang.org/std/fmt/struct.Formatter.html "struct std::fmt::Formatter")

Configuration for formatting.

[FromFn](https://doc.rust-lang.org/std/fmt/struct.FromFn.html "struct std::fmt::FromFn")

Implements [`fmt::Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") and [`fmt::Display`](https://doc.rust-lang.org/std/fmt/trait.Display.html "trait std::fmt::Display") via the provided closure.

[FormattingOptions](https://doc.rust-lang.org/std/fmt/struct.FormattingOptions.html "struct std::fmt::FormattingOptions")Experimental

Options for formatting.

[Alignment](https://doc.rust-lang.org/std/fmt/enum.Alignment.html "enum std::fmt::Alignment")

Possible alignments returned by `Formatter::align`

[DebugAsHex](https://doc.rust-lang.org/std/fmt/enum.DebugAsHex.html "enum std::fmt::DebugAsHex")Experimental

Specifies whether the [`Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") trait should use lower-/upper-case hexadecimal or normal integers.

[Sign](https://doc.rust-lang.org/std/fmt/enum.Sign.html "enum std::fmt::Sign")Experimental

The signedness of a [`Formatter`](https://doc.rust-lang.org/std/fmt/struct.Formatter.html "struct std::fmt::Formatter") (or of a [`FormattingOptions`](https://doc.rust-lang.org/std/fmt/struct.FormattingOptions.html "struct std::fmt::FormattingOptions")).

[Binary](https://doc.rust-lang.org/std/fmt/trait.Binary.html "trait std::fmt::Binary")
...
[Display](https://doc.rust-lang.org/std/fmt/trait.Display.html "trait std::fmt::Display")

Format trait for an empty format, `{}`.

[LowerExp](https://doc.rust-lang.org/std/fmt/trait.LowerExp.html "trait std::fmt::LowerExp")

`e` formatting.

[LowerHex](https://doc.rust-lang.org/std/fmt/trait.LowerHex.html "trait std::fmt::LowerHex")

`x` formatting.

[Octal](https://doc.rust-lang.org/std/fmt/trait.Octal.html "trait std::fmt::Octal")

`o` formatting.

[Pointer](https://doc.rust-lang.org/std/fmt/trait.Pointer.html "trait std::fmt::Pointer")

`p` formatting.

[UpperExp](https://doc.rust-lang.org/std/fmt/trait.UpperExp.html "trait std::fmt::UpperExp")

`E` formatting.

[UpperHex](https://doc.rust-lang.org/std/fmt/trait.UpperHex.html "trait std::fmt::UpperHex")

`X` formatting.

[Write](https://doc.rust-lang.org/std/fmt/trait.Write.html "trait std::fmt::Write")

A trait for writing or formatting into Unicode-accepting buffers or streams.

[format](https://doc.rust-lang.org/std/fmt/fn.format.html "fn std::fmt::format")

Takes an [`Arguments`](https://doc.rust-lang.org/std/fmt/struct.Arguments.html "struct std::fmt::Arguments") struct and returns the resulting formatted string.

[from\_fn](https://doc.rust-lang.org/std/fmt/fn.from_fn.html "fn std::fmt::from_fn")

Creates a type whose [`fmt::Debug`](https://doc.rust-lang.org/std/fmt/trait.Debug.html "trait std::fmt::Debug") and [`fmt::Display`](https://doc.rust-lang.org/std/fmt/trait.Display.html "trait std::fmt::Display") impls are forwarded to the provided closure.

[write](https://doc.rust-lang.org/std/fmt/fn.write.html "fn std::fmt::write")

Takes an output stream and an `Arguments` struct that can be precompiled with the `format_args!` macro.

[Result](https://doc.rust-lang.org/std/fmt/type.Result.html "type std::fmt::Result")

The type returned by formatter methods.

[Debug](https://doc.rust-lang.org/std/fmt/derive.Debug.html "derive std::fmt::Debug")
