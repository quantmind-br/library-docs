---
title: base_router - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/fused_moe/router/base_router/
source: sitemap
fetched_at: 2026-05-07T21:25:27.051664408-03:00
rendered_js: false
word_count: 0
summary: This document defines the BaseRouter class, which provides a template-based architecture for implementing expert routing logic in Mixture-of-Experts (MoE) models, including support for expert load balancing (EPLB). It delegates the core routing computation to subclasses while managing state validation, index dtype conversion, and logical-to-physical expert ID mapping.
tags:
    - mixture-of-experts
    - deep-learning
    - routing-algorithm
...
```
classBaseRouter(FusedMoERouter):
"""
...
"""
    ...
    defset_capture_fn(self, capture_fn: Callable[[torch.Tensor], None] | None) -> None:
"""Set a capture callback for logical routed expert IDs."""
        self.capture_fn = capture_fn

    def_validate_eplb_state(self) -> None:
"""Validate that EPLB state is properly initialized if EPLB is enabled."""
    ...
    def_get_indices_type(self) -> torch.dtype | None:
"""Get the desired indices dtype from the getter function."""
    ...
    def_apply_eplb_mapping(self, topk_ids: torch.Tensor) -> torch.Tensor:
