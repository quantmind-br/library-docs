---
title: Analytics API | Moodle Developer Resources
url: https://moodledev.io/docs/5.2/apis/subsystems/analytics
source: sitemap
fetched_at: 2026-02-17T15:45:11.427853-03:00
rendered_js: false
word_count: 5667
summary: This document explains the architecture and core components of the Moodle Analytics API, detailing how prediction models, indicators, and targets interact to generate actionable insights. It provides technical overviews of data flow, API class structures, and machine learning concepts relevant to extending Moodle's analytical capabilities.
tags:
    - moodle-api
    - analytics-api
    - machine-learning
...
### API components[​](#api-components "Direct link to API components")

This diagram shows the main components of the analytics API and the interactions between them.

![Analytics API components and their interactions](https://moodledev.io/assets/images/Inspire_API_components-e456d58a4230fca6d50f7450ceeb6ae2.png)
...
![Data flow at different stages](https://moodledev.io/assets/images/Inspire_data_flow-567a94675fdd615f15dac23ad11702a0.png)

### API classes diagram[​](#api-classes-diagram "Direct link to API classes diagram")
...
![API Class Structure](https://moodledev.io/assets/images/Analytics_API_classes_diagram_%28summary%29-79d7ad9966f55e510015cc027a18608e.svg)

## Built-in models[​](#built-in-models "Direct link to Built-in models")
...
### Prediction model[​](#prediction-model "Direct link to Prediction model")
...
The relationship between indicators and targets is stored in *analytics\_models* database table.
...
#### Static models[​](#static-models "Direct link to Static models")
...
### Indicator[​](#indicator "Direct link to Indicator")
...
### Analysis intervals[​](#analysis-intervals "Direct link to Analysis intervals")
...
### Machine learning backends[​](#machine-learning-backends "Direct link to Machine learning backends")

Documentation available in [Machine learning backends](https://moodledev.io/docs/5.2/apis/plugintypes/mlbackend).
...
[Machine learning backends](https://moodledev.io/docs/5.2/apis/plugintypes/mlbackend) is a new Moodle plugin type. They are stored in `lib/mlbackend`.
...
This section does not include Machine learning backend interfaces. See [Machine learning backends](https://moodledev.io/docs/5.2/apis/plugintypes/mlbackend#interfaces) for more information on these.

#### Analysable (core\_analytics\\analysable)[​](#analysable-core_analyticsanalysable "Direct link to Analysable (core_analytics\analysable)")
 ...
 * @return \Iterator
 */
publicfunctionget_analysables_iterator(?string$action=null)
```
 ...
 */
abstractpublicfunctionget_all_samples(\core_analytics\analysable$analysable);
```
 ...
 * @return array array[0] = int[] (sampleids) and array[1] = array (samplesdata)
 */
abstractpublicfunctionget_samples($sampleids);
```
 ...
 */
abstractpublicfunctionget_sample_analysable($sampleid);
```
 ...
 * @return string
 */
abstractpublicfunctionget_samples_origin();
```
 ...
 * @return \context
 */
abstractpublicfunctionsample_access_context($sampleid);
```
 ...
 */
abstractpublicfunctionsample_description($sampleid,$contextid,$sampledata);
```
 ...
 * @return bool
 */
publicfunctionprocesses_user_data();
```
 ...
 * @return string
 */
publicfunctionjoin_sample_user($sampletablealias);
```
 ...
 * @return bool
 */
publicstaticfunctionone_sample_per_analysable(){
returntrue;
}
```

#### Indicator (core\_analytics\\local\\indicator\\base)[​](#indicator-core_analyticslocalindicatorbase "Direct link to Indicator (core_analytics\local\indicator\base)")

Indicators should generally extend one of these 3 classes, depending on the values they can return: *core\_analytics\\local\\indicator\\binary* for **yes/no** indicators, *core\_analytics\\local\\indicator\\linear* for indicators that return linear values and *core\_analytics\\local\\indicator\\discrete* for categorised indicators. In case you want your activity module to implement a [community of inquiry](https://docs.moodle.org/en/Students_at_risk_of_dropping_out#Indicators) indicator you can extend *core\_analytics\\local\\indicator\\community\_of\_inquiry\_indicator* look for examples in Moodle core.

You can use `required_sample_data` to specify what your indicator needs to be calculated; you may need a *user* object, a *course*, a *grade item*... The default implementation does not require anything. Models which analysers do not return the required data will not be able to use your indicator so only list here what you really need. e.g. if you need a grade\_grades record mark it as required, but there is no need to require the *user* object and the *course* as well because you can obtain them from the grade\_grades item. It is very likely that the analyser will provide them as well because the principle they follow is to include as much related data as possible but do not flag related objects as required because an analyser may, for example, chose to not include the *user* object because it is too big and sites can have memory problems.
 ...
 */
publicstaticfunctionrequired_sample_data(){
returnnull;
}
```
 ...
 * @return float|null
 */
abstractprotectedfunctioncalculate_sample($sampleid,$sampleorigin,$starttime,$endtime);
```
...
- You can also overwrite *fill\_per\_analysable\_caches* method if necessary (keep in mind though that PHP memory is not unlimited).
...
#### Target (core\_analytics\\local\\target\\base)[​](#target-core_analyticslocaltargetbase "Direct link to Target (core_analytics\local\target\base)")
 ...
 */
abstractpublicfunctionget_analyser_class();
```
 ...
 * @return true|string
 */
publicfunctionis_valid_analysable(\core_analytics\analysable$analysable,$fortraining=true);
```
 ...
 * @return bool
 */
publicfunctionis_valid_sample($sampleid,\core_analytics\analysable$analysable,$fortraining=true);
```
 ...
 * @return float|null
 */
protectedfunctioncalculate_sample($sampleid,\core_analytics\analysable$analysable,$starttime=false,$endtime=false);
```
 ...
 */
publicfunctionprediction_actions(\core_analytics\prediction$prediction,$includedetailsaction=false,
$isinsightuser=false)
```
 ...
 * @return array
 */
publicfunctionget_insights_users(\context$context)
```
 ...
 * @return bool
 */
publicfunctionalways_update_analysis_time()
```
 ...
 * @return array
 */
publicfunctionignored_predicted_classes()
```
 ...
 * @return string
 */
publicfunctionget_insight_subject(int$modelid,\context$context)
```

You can also override the URL to the insight with get\_insight\_context\_url():
 ...
 * @return \moodle_url
 */
publicfunctionget_insight_context_url($modelid,$context)
```

#### Analysis interval (core\_analytics\\local\\time\_splitting\\base)[​](#analysis-interval-core_analyticslocaltime_splittingbase "Direct link to Analysis interval (core_analytics\local\time_splitting\base)")

Analysis intervals are used to define when the analytics API will train the predictions processor and when it will generate predictions. As explained above in the [Analysis intervals](https://moodledev.io/docs/5.2/apis/subsystems/analytics#analysis-intervals) documentation, they define time ranges based on analysable elements start and end timestamps.
 ...
 */
protectedfunctiondefine_ranges();
```
 ...
 * @return \lang_string
 */
publicstaticfunctionget_name():\lang_string;
```

Analysis intervals can now override the following:

`valid_for_evaluation()`. You can return false if the time-splitting method can not be used to evaluate prediction models or if it does not make sense to evaluate prediction models with it, as for example upcoming\_periodic children classes.

`include_range_info_in_training_data()` and `get_training_ranges()`. They can be used to create time splitting methods with a pre-defined number of ranges.
...
#### Calculable (core\_analytics\\calculable)[​](#calculable-core_analyticscalculable "Direct link to Calculable (core_analytics\calculable)")
...
## How to create a model[​](#how-to-create-a-model "Direct link to How to create a model")

New models can be created and implemented in php, and can be packaged as a part of a Moodle plugin for distribution. An example of model components and models are provided at [https://github.com/dmonllao/moodle-local\_testanalytics](https://github.com/dmonllao/moodle-local_testanalytics).
...
### Define the problem[​](#define-the-problem "Direct link to Define the problem")
...
### How many predictions for each sample?[​](#how-many-predictions-for-each-sample "Direct link to How many predictions for each sample?")
...
- If you want a single prediction for each sample at a specific point in time you can reuse a site-wide analyser or define your own and control samples validity through your target's *is\_valid\_sample()* method.
- If you want multiple predictions at different points in time for each sample reuse an analysable element or define your own (extending `\core_analytics\analysable`) and reuse or define your own analyser to retrieve these analysable elements. You can control analysers validity through your target's *is\_valid\_analysable()* method.
- If you want predictions at activity level use a "by\_course" analyser as otherwise you may have scalability problems (imagine storing in memory calculations for each grade\_grades record in your site, though processing elements by courses help as we clean memory after each course is processed)
...
### Create the target[​](#create-the-target "Direct link to Create the target")

As specified in the [Target](https://moodledev.io/docs/5.2/apis/subsystems/analytics#target-core_analyticslocaltargetbase) section.
...
## API usage examples[​](#api-usage-examples "Direct link to API usage examples")
...
- **Time splitting:** single range
- **Analyser samples:** student grade on an activity, aka grade\_grades id (analysable elements are courses)
- `target::is_valid_analysable`
...
## Clustered environments[​](#clustered-environments "Direct link to Clustered environments")
