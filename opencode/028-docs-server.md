...
source: sitemap
fetched_at: 2026-04-17T06:40:29.879685336-03:00
rendered_js: false
word_count: 762
summary: This document provides a technical reference for the opencode server, detailing its CLI usage, authentication methods, and comprehensive API endpoints for managing projects, sessions, and messages.
tags:
    - opencode-server
    - api-reference
    - cli-usage
...
FlagDescriptionDefault`--port`Port to listen on`4096``--hostname`Hostname to listen on`127.0.0.1``--mdns`Enable mDNS discovery`false``--mdns-domain`Custom domain name for mDNS service`opencode.local``--cors`Additional browser origins to allow`[]`
