...
````yaml openapi/openapi.json get /api/governance/rate-limits
openapi: 3.1.0
info:
  title: Bifrost API
  description: >
...
paths:
  /api/governance/rate-limits:
    get:
      ...
      description: >-
        Returns a list of all rate limits. Use the `from_memory` query parameter
        to get data from in-memory cache.
      operationId: listRateLimits
      parameters:
        - name: from_memory
          in: query
          description: >-
            If true, returns rate limits from in-memory cache instead of
            database
          schema:
            type: boolean
            default: false
