...
source: crawler
fetched_at: 2026-04-24T17:00:14.447598021-03:00
rendered_js: false
word_count: 1183
summary: This document serves as a comprehensive guide detailing how Hermes connects to WhatsApp via an unofficial Baileys bridge. It explains the setup process through a wizard, outlines operational modes (bot or self-chat), configuration options for access control and behavior, and describes features like session persistence and voice message handling.
tags:
    - hermes-whatsapp
    - baileys-bridge
...
ModeHow it worksBest for**Separate bot number** (recommended)Dedicate a phone number to the bot. People message that number directly.Clean UX, multiple users, lower ban risk**Personal self-chat**Use your own WhatsApp. You message yourself to talk to the agent.Quick setup, single user, testing
...
OptionCostNotes**Google Voice**FreeUS only. Get a number at [voice.google.com](https://voice.google.com). Verify WhatsApp via SMS through the Google Voice app.**Prepaid SIM**$5–15 one-timeAny carrier. Activate, verify WhatsApp, then the SIM can sit in a drawer. Number must stay active (make a call every 90 days).**VoIP services**Free–$5/monthTextNow, TextFree, or similar. Some VoIP numbers are blocked by WhatsApp — try a few if the first doesn't work.
...
MarkdownWhatsAppRenders as`**bold**``*bold*`**bold**`~~strikethrough~~``~strikethrough~`strikethrough`# Heading``*Heading*`Bold text (no native headings)`[link text](url)``link text (url)`Inline URL
