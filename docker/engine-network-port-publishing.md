...
source: llms
fetched_at: 2026-01-24T14:24:42.030055055-03:00
rendered_js: false
word_count: 1839
summary: This document explains how Docker handles port publishing and network routing for containers on bridge networks, covering both default NAT behavior and advanced direct routing configurations. it details how to use the publish flag and gateway modes to manage external and inter-container network access.
tags:
    - docker-networking
    - port-publishing
    - bridge-networks
...
Flag valueDescription`-p 8080:80`Map port `8080` on the Docker host to TCP port `80` in the container.`-p 192.168.1.100:8080:80`Map port `8080` on the Docker host IP `192.168.1.100` to TCP port `80` in the container.`-p 8080:80/udp`Map port `8080` on the Docker host to UDP port `80` in the container.`-p 8080:80/tcp -p 8080:80/udp`Map TCP port `8080` on the Docker host to TCP port `80` in the container, and map UDP port `8080` on the Docker host to UDP port `80` in the container.
