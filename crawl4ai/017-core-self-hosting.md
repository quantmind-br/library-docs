---
title: Hosting Guide - Crawl4AI Documentation (v0.8.x)
url: https://docs.crawl4ai.com/core/self-hosting/
source: sitemap
fetched_at: 2026-04-26T07:47:24.822472422-03:00
rendered_js: false
word_count: 4183
summary: This document provides comprehensive instructions for installing, configuring, and managing a self-hosted Crawl4AI web scraping infrastructure using Docker.
tags:
    - crawl4ai
    - web-scraping
    - docker
...
dockerpullunclecode/crawl4ai:0.8.0
...
```
dockerstopcrawl4ai&&dockerrmcrawl4ai
```
...
cpdeploy/docker/.llm.env.example.llm.env
...
The system automatically selects the appropriate API key based on the provider. LiteLLM handles finding the correct environment variable for each provider (e.g., OPENAI\_API\_KEY for OpenAI, GEMINI\_API\_TOKEN for Google Gemini, etc.).
...
- **Run Pre-built Image from Docker Hub:**
  ...
  IMAGE=unclecode/crawl4ai:latestdockercomposeup-d
  ```
...
- **Customize the Build:**
  ...
  INSTALL_TYPE=alldockercomposeup--build-d
  
  # Build with GPU support (for AMD64 platforms)
  ENABLE_GPU=truedockercomposeup--build-d
  ```
...
dockerbuildxbuild-tcrawl4ai-local:latest--load.

# Or build for multiple architectures (useful for publishing)
dockerbuildxbuild--platformlinux/amd64,linux/arm64-tcrawl4ai-local:latest--load.

# Build with additional options
dockerbuildxbuild\
--build-argINSTALL_TYPE=all\
--build-argENABLE_GPU=false\
-tcrawl4ai-local:latest--load.
```
...
```
dockerstopcrawl4ai-standalone&&dockerrmcrawl4ai-standalone
```
...
claudemcpadd--transportssec4ai-ssehttp://localhost:11235/mcp/sse
...
pythontests/mcp/test_mcp_socket.py
```
...
```
curlhttp://localhost:11235/hooks/info
```
...
01. **Never Trust User Input**: If accepting hook code from users, always validate and sandbox it
02. **Avoid Phishing Sites**: Never use hooks on suspicious or unverified websites
03. **Protect Credentials**:
04. Never hardcode passwords, tokens, or API keys in hook code
05. Use environment variables or secure secret management
06. Rotate credentials regularly
07. **Network Isolation**: Run the Docker container in an isolated network when testing
08. **Audit Hook Code**: Always review hook code before execution
09. **Limit Permissions**: Use the least privileged access needed
...
```
fromcrawl4aiimport Crawl4aiDockerClient

async defmy_hook(page, context, **kwargs):
    await page.set_viewport_size({"width": 1920, "height": 1080})
    return page
...
```
fromcrawl4aiimport hooks_to_string

# Define your hooks as functions
async defsetup_hook(page, context, **kwargs):
    await page.set_viewport_size({"width": 1920, "height": 1080})
    await context.add_cookies([{
        "name": "session",
        "value": "token",
...
async defscroll_hook(page, context, **kwargs):
    await page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
    return page
...
async defauth_hook(page, context, **kwargs):
...
async defperformance_hook(page, context, **kwargs):
...
"""Block analytics and tracking scripts"""
    ...
    @staticmethod
    async defscroll_infinite(page, context, **kwargs):
"""Handle infinite scroll to load more content"""
    ...
    @staticmethod
    async defwait_for_dynamic_content(page, context, url, response, **kwargs):
...
fromhooks_libraryimport CrawlHooks
fromcrawl4aiimport Crawl4aiDockerClient

async defcrawl_with_optimizations(url):
...
Approach Best For IDE Support Language **String-based** Non-Python clients, REST APIs, other languages ❌ None Any **Function-based** Python applications, local development ✅ Full Python only **Docker Client** Python apps with automatic conversion ✅ Full Python only
...
```
fromcrawl4aiimport Crawl4aiDockerClient, BrowserConfig, CrawlerRunConfig, CacheMode

# Define hooks as regular Python functions
async defsetup_environment(page, context, **kwargs):
...
async defextract_content(page, context, **kwargs):
...
Parameter Type Required Description `webhook_url` string Yes Your HTTP(S) endpoint to receive notifications `webhook_data_in_payload` boolean No Include full result data in webhook payload (default: false) `webhook_headers` object No Custom headers for authentication/identification
...
```
fromflaskimport Flask, request, jsonify
importrequests
...
defsubmit_llm_job_with_webhook():
    response = requests.post(
...
Argument Description Default Options INSTALL\_TYPE Feature set `default` `default`, `all`, `torch`, `transformer` ENABLE\_GPU GPU support (CUDA for AMD64) `false` `true`, `false` APP\_HOME Install path inside container (advanced) `/app` any valid path USE\_LOCAL Install library from local source `true` `true`, `false` GITHUB\_REPO Git repo to clone if USE\_LOCAL=false *(see Dockerfile)* any git URL GITHUB\_BRANCH Git branch to clone if USE\_LOCAL=false `main` any branch name

*(Note: PYTHON\_VERSION is fixed by the `FROM` instruction in the Dockerfile)*
...
```
importasyncio
fromcrawl4ai.docker_clientimport Crawl4aiDockerClient
fromcrawl4aiimport BrowserConfig, CrawlerRunConfig, CacheMode
...
**Advanced Crawler Configuration** *(Keep example, ensure cache\_mode uses valid enum value like "bypass")*
...
```
async defsmart_extraction(url: str, content_type: str):
...
```
importjson
importhttpx # Use httpx for async streaming example

async deftest_stream_crawl(token: str = None): # Made token optional
...
![Monitoring Dashboard](https://via.placeholder.com/800x400?text=Crawl4AI%20Monitoring%20Dashboard)
...
Type Signature Age Last Used Hits Actions permanent abc12345 2h 5s ago 1,247 Restart hot def67890 45m 2m ago 89 Kill / Restart cold ghi11213 30m 15m ago 3 Kill / Restart
...
{
"sig":"abc12345"//First8charsofbrowsersignature
}
```
...
{
"sig":"permanent"//Orfirst8charsofsignature
}
```
...
curlhttp://localhost:11235/metrics
```
...
importwebsockets
importjson
fromyour_monitoringimport push_metric

async defintegrate_monitoring():
...
01. **Memory Usage Trends**
02. Track `container.memory_percent` over time
03. Alert when consistently above 80%
04. Prevents OOM kills
05. **Request Success Rates**
06. Monitor per-endpoint success rates
07. Alert when below 95%
08. Indicates crawling issues
09. **Average Latency**
...
```
curlhttp://localhost:11235/health
```
...
2. Mount it when running the container:
   ...
   - **Using `docker run`:**
     ...
     dockerrun-d-p11235:11235\
     --namecrawl4ai-custom-config\
     --env-file.llm.env\
     --shm-size=1g\
...
01. **Security First** 🔒
02. Always enable security in production
03. Use specific trusted\_hosts instead of wildcards
04. Set up proper rate limiting to protect your server
05. Consider your environment before enabling HTTPS redirect
06. **Resource Management** 💻
07. Adjust memory\_threshold\_percent based on available RAM
08. Set timeouts according to your content size and network conditions
09. Use Redis for rate limiting in multi-container setups
10. **Monitoring** 📊
...
16. Increase batch\_process timeout for large content
17. Adjust stream\_init timeout based on initial response times
