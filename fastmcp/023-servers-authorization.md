---
title: Authorization - FastMCP
url: https://gofastmcp.com/servers/authorization
source: crawler
fetched_at: 2026-01-22T22:21:41.246462921-03:00
rendered_js: false
word_count: 502
summary: This document explains how to implement authorization in FastMCP using callable-based checks and middleware to control access to tools, resources, and prompts. It covers built-in authorization patterns, custom check creation, and the distinction between component-level filtering and server-wide enforcement.
tags:
    - fastmcp
    - authorization
    - middleware
...
BehaviorComponent-level `auth``AuthMiddleware`Filters list responsesYesYesBlocks executionNoYes (raises `AuthorizationError`)
...
PropertyTypeDescription`token``str`The raw token string`client_id``str | None`OAuth client identifier`scopes``list[str]`Granted OAuth scopes`expires_at``datetime | None`Token expiration time`claims``dict[str, Any]`All JWT claims or custom token data

### AuthContext

The `AuthContext` dataclass is passed to all auth check functions.

PropertyTypeDescription`token``AccessToken | None`Current access token, or `None` if unauthenticated`component``Tool | Resource | Prompt`The component being accessed
