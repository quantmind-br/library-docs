---
title: Troubleshooting and Frequently Asked Questions — SGLang
url: https://docs.sglang.io/references/faq.html
source: crawler
fetched_at: 2026-02-04T08:47:15.947031605-03:00
rendered_js: false
word_count: 494
summary: Provides solutions for common errors such as CUDA out-of-memory and server hangs, along with explanations for non-deterministic outputs in the SGLang inference engine.
tags:
    - troubleshooting
    - cuda-oom
    - sglang
...
## Troubleshooting and Frequently Asked Questions[#](#troubleshooting-and-frequently-asked-questions "Link to this heading")

## Troubleshooting[#](#troubleshooting "Link to this heading")

This page lists common errors and tips for resolving them.

### CUDA Out of Memory[#](#cuda-out-of-memory "Link to this heading")
...
### CUDA Error: Illegal Memory Access Encountered[#](#cuda-error-illegal-memory-access-encountered "Link to this heading")
...
### The server hangs[#](#the-server-hangs "Link to this heading")
...
- If the server hangs during initialization or running, it can be memory issues (out of memory), network issues (nccl errors), or other bugs in sglang.
  ...
  - If it is out of memory, you might see that `avail mem` is very low during the initialization or right after initialization. In this case, you can try to decrease `--mem-fraction-static`, decrease `--cuda-graph-max-bs`, or decrease `--chunked-prefill-size`.
- Other bugs, please file an issue on GitHub.

## Frequently Asked Questions[#](#frequently-asked-questions "Link to this heading")

### The results are not deterministic, even with a temperature of 0[#](#the-results-are-not-deterministic-even-with-a-temperature-of-0 "Link to this heading")
...
**Update**: Recently, we also introduced a deterministic mode, you can enable it with `--enable-deterministic-inference`. Please find more details in this blog post: https://lmsys.org/blog/2025-09-22-sglang-deterministic/