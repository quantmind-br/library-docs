...
url: https://docs.vllm.ai/en/latest/api/vllm/tokenizers/kimi_audio/
source: sitemap
fetched_at: 2026-05-07T21:35:43.719427158-03:00
rendered_js: false
word_count: 41
summary: This document defines the KimiAudioTokenizer class, which provides a custom implementation for loading and managing Tiktoken-based tokenization with specialized support for Kimi-Audio specific tokens and configurations.
tags:
    - python
    - tokenizer
    - huggingface
...
```
classKimiAudioTokenizer(TokenizerLike):
"""TikToken tokenizer for Kimi-Audio."""
    ...
    def_add_kimiaudio_special_tokens(self) -> None:
"""Add Kimi-Audio special tokens to the tokenizer."""
    ...
    @property
    defall_special_tokens(self) -> list[str]:
        return list(self._added_tokens_decoder.values())

    @property
    defall_special_ids(self) -> list[int]:
        return list(self._added_tokens_decoder.keys())

    @property
    defbos_token_id(self) -> int:
        return self._bos_token_id

    @property
    defeos_token_id(self) -> int:
        return self._eos_token_id

    @property
    defpad_token_id(self) -> int:
        return self._pad_token_id

    @property
    defis_fast(self) -> bool:
        return False
    ...
    @property
    defmax_token_id(self) -> int:
        return self._tokenizer.n_vocab - 1

    @property
    defmax_chars_per_token(self) -> int:
        return self._max_chars_per_token

    @property
    deftruncation_side(self) -> str:
        return self._truncation_side

    @property
    defadded_tokens_decoder(self) -> dict[int, Any]:
        return self._added_tokens_decoder

    @added_tokens_decoder.setter
    defadded_tokens_decoder(self, value: dict[int, Any]) -> None:
"""Set added tokens decoder and update special token IDs."""
    ...
    defget_vocab(self) -> dict[str, int]:
...
"""Return vocab size for compatibility with HF tokenizer interface."""
    ...
    def_maybe_truncate(self, tokens: list[int], max_length: int | None) -> list[int]:
...
"""Decode token IDs to text, optionally skipping special tokens."""
    ...
    @overload
    defconvert_tokens_to_ids(self, tokens: str) -> int: ...

    @overload
    defconvert_tokens_to_ids(self, tokens: list[str]) -> list[int]: ...

    defconvert_tokens_to_ids(self, tokens: str | list[str]) -> int | list[int]:
    ...
    defconvert_tokens_to_string(self, tokens: list[str]) -> str:
