...
source: crawler
fetched_at: 2026-04-24T17:00:18.740941676-03:00
rendered_js: false
word_count: 531
summary: This document serves as a reference guide detailing all the specific commands available under `hermes profile`, allowing users to manage, view, create, delete, and manipulate profiles within the Hermes agent environment.
tags:
    - cli-commands
    - profile-management
    - hermes-agent
...
ArgumentDescription`<name>`Profile name to activate. Use `default` to return to the base profile.
...
Argument / OptionDescription`<name>`Name for the new profile. Must be a valid directory name (alphanumeric, hyphens, underscores).`--clone`Copy `config.yaml`, `.env`, and `SOUL.md` from the current profile.`--clone-all`Copy everything (config, memories, skills, sessions, state) from the current profile.`--clone-from <profile>`Clone from a specific profile instead of the current one. Used with `--clone` or `--clone-all`.`--no-alias`Skip wrapper script creation.
...
Argument / OptionDescription`<name>`Profile to delete.`--yes`, `-y`Skip confirmation prompt.
...
ArgumentDescription`<name>`Profile to inspect.
...
Argument / OptionDescription`<name>`Profile to create/update the alias for.`--remove`Remove the wrapper script instead of creating it.`--name <alias>`Custom alias name (default: profile name).
...
ArgumentDescription`<old-name>`Current profile name.`<new-name>`New profile name.
...
Argument / OptionDescription`<name>`Profile to export.`-o`, `--output <path>`Output file path (default: `<name>.tar.gz`).
...
Argument / OptionDescription`<archive>`Path to the tar.gz archive to import.`--name <name>`Name for the imported profile (default: inferred from archive).
...
OptionDescription`-p <name>`, `--profile <name>`Profile to use for this command.
...
ArgumentDescription`<shell>`Shell to generate completions for: `bash` or `zsh`.
