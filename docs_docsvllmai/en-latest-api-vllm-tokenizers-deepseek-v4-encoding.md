---
title: deepseek_v4_encoding - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/tokenizers/deepseek_v4_encoding/
source: sitemap
fetched_at: 2026-05-07T21:35:38.864364346-03:00
rendered_js: false
word_count: 838
summary: This document provides the technical implementation for encoding and decoding DeepSeek-V4 chat messages, including support for tool calling, reasoning mode management, and prompt formatting.
tags:
    - deepseek-v4
    - encoding
    - tokenization
...
## \_drop\_thinking\_messages [¶](#vllm.tokenizers.deepseek_v4_encoding._drop_thinking_messages "Permanent link")

Drop reasoning and non-essential messages before the last user message.

Behavior: - Messages with role in \["user", "system", "tool", "latest\_reminder"] are always kept. - Messages at or after the last user index are always kept. - Assistant messages before the last user get reasoning removed. - Developer messages before the last user are dropped entirely.

Source code in `vllm/tokenizers/deepseek_v4_encoding.py`

```
def_drop_thinking_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
"""
    Drop reasoning and non-essential messages before the last user message.
...
## \_read\_until\_stop [¶](#vllm.tokenizers.deepseek_v4_encoding._read_until_stop "Permanent link")

Read text from index until one of the stop strings is found.

Returns:

Type Description `Tuple[int, str, Optional[str]]`

Tuple of (new\_index, content\_before\_stop, matched\_stop\_string\_or\_None).

Source code in `vllm/tokenizers/deepseek_v4_encoding.py`

```
def_read_until_stop(index: int, text: str, stop: List[str]) -> Tuple[int, str, Optional[str]]:
"""
    Read text from index until one of the stop strings is found.
...
## decode\_dsml\_to\_arguments [¶](#vllm.tokenizers.deepseek_v4_encoding.decode_dsml_to_arguments "Permanent link")

Decode DSML parameters back to a tool call dict.

Parameters:

Name Type Description Default `tool_name` `str`

Name of the tool.

*required* `tool_args` `Dict[str, Tuple[str, str]]`

Dict mapping param\_name -&gt; (value, is\_string\_flag).
...
```
defdecode_dsml_to_arguments(tool_name: str, tool_args: Dict[str, Tuple[str, str]]) -> Dict[str, str]:
"""
    Decode DSML parameters back to a tool call dict.
    ...
    Returns:
        Dict with "name" and "arguments" (JSON string) keys.
    """
    def_decode_value(key: str, value: str, string: str):
        if string == "true":
            value = to_json(value)
...
## encode\_arguments\_to\_dsml [¶](#vllm.tokenizers.deepseek_v4_encoding.encode_arguments_to_dsml "Permanent link")

Encode tool call arguments into DSML parameter format.

Parameters:

Name Type Description Default `tool_call` `Dict[str, Any]`
...
```
defencode_arguments_to_dsml(tool_call: Dict[str, Any]) -> str:
"""
    Encode tool call arguments into DSML parameter format.
...
## encode\_messages [¶](#vllm.tokenizers.deepseek_v4_encoding.encode_messages "Permanent link")
...
Name Type Description Default `messages` `List[Dict[str, Any]]`

List of message dicts to encode.

*required* `thinking_mode` `str`

Either "chat" or "thinking".

*required* `context` `Optional[List[Dict[str, Any]]]`

Optional preceding context messages (already encoded prefix).

`None` `drop_thinking` `bool`

If True, drop reasoning from earlier assistant turns (only keep reasoning for messages after the last user message).

`True` `add_default_bos_token` `bool`

Whether to prepend BOS token at conversation start.

`True` `reasoning_effort` `Optional[str]`

Optional reasoning effort level ("max", "high", or None).
...
## find\_last\_user\_index [¶](#vllm.tokenizers.deepseek_v4_encoding.find_last_user_index "Permanent link")
...
```
deffind_last_user_index(messages: List[Dict[str, Any]]) -> int:
"""Find the index of the last user/developer message."""
    last_user_index = -1
...
Merge tool messages into the preceding user message using content\_blocks format.

DeepSeek-V4 does not have a standalone "tool" role; instead, tool results are encoded as blocks within user messages.
...
```
defmerge_tool_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
"""
...
## parse\_message\_from\_completion\_text [¶](#vllm.tokenizers.deepseek_v4_encoding.parse_message_from_completion_text "Permanent link")
...
This function takes the raw text output from the model (a single assistant turn) and extracts: - reasoning (thinking block) - content (summary/response) - tool\_calls (if any)
...
The raw completion text (including EOS token).
...
Dict with keys: "role", "content", "reasoning", "tool\_calls".

`Dict[str, Any]`

tool\_calls are in OpenAI format.

Source code in `vllm/tokenizers/deepseek_v4_encoding.py`

```
defparse_message_from_completion_text(text: str, thinking_mode: str) -> Dict[str, Any]:
"""
    Parse a model completion text into a structured assistant message.
...
Tuple of (new\_index, last\_stop\_token, list\_of\_tool\_call\_dicts).
...
```
defparse_tool_calls(index: int, text: str) -> Tuple[int, Optional[str], List[Dict[str, str]]]:
"""
    Parse DSML tool calls from text starting at the given index.
...
## render\_message [¶](#vllm.tokenizers.deepseek_v4_encoding.render_message "Permanent link")
...
Index of the message to render.

*required* `messages` `List[Dict[str, Any]]`
...
*required* `drop_thinking` `bool`

Whether to drop reasoning content from earlier turns.
...
```
defrender_message(index: int, messages: List[Dict[str, Any]], thinking_mode: str, drop_thinking: bool = True, reasoning_effort: Optional[str] = None) -> str:
"""
    Render a single message at the given index into its encoded string form.
...
Name Type Description Default `tools` `List[Dict[str, Union[str, Dict[str, Any]]]]`

List of tool schema dicts (each with name, description, parameters).
...
```
defrender_tools(tools: List[Dict[str, Union[str, Dict[str, Any]]]]) -> str:
"""
    Render tool schemas into the system prompt format.
...
Sort tool\_result blocks within user messages by the order of tool\_calls in the preceding assistant message.

Parameters:

Name Type Description Default `messages` `List[Dict[str, Any]]`

Preprocessed message list (after merge\_tool\_messages).
...
```
defsort_tool_results_by_call_order(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
"""
...
## to\_json [¶](#vllm.tokenizers.deepseek_v4_encoding.to_json "Permanent link")
...
```
deftool_calls_from_openai_format(tool_calls):
"""Convert OpenAI-format tool calls to internal format."""
    return [
        {
...
```
deftool_calls_to_openai_format(tool_calls):
"""Convert internal tool calls to OpenAI format."""
    return [
        {
            "type": "function",
...
```
deftools_from_openai_format(tools):
"""Extract function definitions from OpenAI-format tool list."""
