...
url: https://moodledev.io/docs/5.0/apis/plugintypes/filter
source: sitemap
fetched_at: 2026-02-17T15:25:34.817981-03:00
rendered_js: false
word_count: 761
summary: This document explains how to develop filter plugins for Moodle to automatically transform content before output. It covers the mandatory file structure, implementation of the text_filter class, and performance best practices.
tags:
    - moodle-development
    - filter-plugins
    - plugin-development
...
Filters are applied to content passed into the `format_string()` and `format_text()` functions, which are part of the [Output API](https://moodledev.io/docs/5.0/apis/subsystems/output).
...
Some of the important files for the filter plugintype are described below. See the [common plugin files](https://moodledev.io/docs/5.0/apis/commonfiles) documentation for details of other files which may be useful in your plugin.
...
To enable a filter, go to the [filters administration screen](https://moodledev.io/docs/5.0/apis/plugintypes/filter) and set the filter active to "On".
