---
title: Repomix Extension | goose
url: https://block.github.io/goose/docs/mcp/repomix-mcp
source: github_pages
fetched_at: 2026-01-22T22:15:47.134033921-03:00
rendered_js: true
word_count: 712
summary: This tutorial explains how to integrate the Repomix MCP Server with the goose AI agent to enable automated repository packing and codebase analysis while optimizing for LLM context limits.
tags:
    - goose-extension
    - repomix
    - mcp-server
...
it('should handle async interceptors correctly',asyncfunction(){
const instance =newAxios({});
const order =[];

      instance.interceptors.request.use(asyncfunction(config){
...
return config;
      ...
      instance.interceptors.response.use(asyncfunction(response){
awaitnewPromise(resolve=>setTimeout(resolve,100));
        order.push(3);
return response;
});
...
const instance =newAxios({});
const error =newError('Interceptor Error');
...
{method:'patch',data:newURLSearchParams('foo=bar')},
{method:'post',data:newFormData()},
{method:'put',data:newBlob(['test'])},
...
_request(configOrUrl, config){
try{
returnthis.request(configOrUrl, config);
}catch(err){
// Error handling
}
}
...
// Suggested
async_request(configOrUrl, config){
try{
returnawaitthis.request(configOrUrl, config);
}catch(err){
if(err instanceofAxiosError){
throw err;
}
throwAxiosError.from(err);
}
}
```
...
returnthis._processRequest(normalizedConfig);
}
```
