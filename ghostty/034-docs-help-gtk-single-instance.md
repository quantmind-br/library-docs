...
source: crawler
fetched_at: 2026-03-10T06:35:58.665066-03:00
rendered_js: true
word_count: 414
summary: This document explains how Ghostty utilizes GTK single instance mode by default when not launched from a command-line interface, and details the heuristics used for detection and how users can force this behavior on or off.
tags:
    - gtk
    - single-instance
    - ghostty
...
Ghostty has this behavior because CLI terminal usage is common (i.e. `ghostty -e "some command"`) and in those cases the expected behavior is to launch a new process and block until it exits. Single instance mode doesn't allow us to easily do this so we only want to enable single instance mode when launched from a non-CLI environment.
...
2. If any command line arguments are passed to Ghostty, we assume we want instance-specific configuration and disable GTK single instance mode.

You can force GTK single instance mode on or off by setting the [`gtk-single-instance`](https://ghostty.org/docs/config/reference#gtk-single-instance) option:

```ini
gtk-single-instance = true
```

A value of `true` forces Ghostty to run in single instance mode. In this mode, each new `ghostty` process will result in a new window in an existing instance of Ghostty if one is running.
...
- [CLI Environment Detection](#cli-environment-detection)
- [Forcing GTK Single Instance Mode On or Off](#forcing-gtk-single-instance-mode-on-or-off)
- [Relationship to Startup Performance and Memory](#relationship-to-startup-performance-and-memory)