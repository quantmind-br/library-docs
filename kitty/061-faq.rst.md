...
title: Faq
word_count: 3038
summary: This document provides troubleshooting and configuration guidance for common issues encountered when using the kitty terminal emulator, including character rendering, remote connectivity, key bindings, and theme management.
category: reference
optimized: true
...
can be turned off for specific symbols using narrow_symbols.
...
If this does not work, see manual_terminfo_copy for alternative ways to
...
This will make TERMINFO available
...
likely they have not added support for /keyboard-protocol.
...
at-set-colors. The shortcut mapping technique has the same syntax as the
remote control command, for details, see at-set-colors.

To change colors when SSHing into a remote host, use the :opt:`color_scheme
<kitten-ssh.color_scheme>` setting for the ssh kitten <kittens/ssh>.

Additionally, you can use the escape code described in color-stack
...
See color-stack for details on the syntax for specifying colors and
...
window_padding_width. This padding can be particularly noticeable when
...
them to change the terminal default background color <color-stack>
...
closing sequence. Press reset_terminal to reset the terminal.
...
Use the panel kitten <quake>, this allows you to use kitty as a quick
...
![icon_256x256](https://github.com/k0nserv/kitty-icon/raw/main/kitty.iconset/icon_256x256.png)

![kitty-dark](https://github.com/DinkDonk/kitty-icon/raw/main/kitty-dark.png)

![kitty-light](https://github.com/DinkDonk/kitty-icon/raw/main/kitty-light.png)

![kitty_icon](https://github.com/hristost/kitty-alternative-icon/raw/main/kitty_icon.png)

![whiskers](https://github.com/igrmk/whiskers/raw/main/whiskers.svg)

![whiskers](https://github.com/samholmes/whiskers/raw/main/whiskers.png)

![a37d7830-4a8c-45a8-988a-3e98a41ea541](https://github.com/user-attachments/assets/a37d7830-4a8c-45a8-988a-3e98a41ea541)

![kitty-preview](https://github.com/eccentric-j/eccentric-icons/raw/main/icons/kitty-terminal/2d/kitty-preview.png)

![kitty-preview](https://github.com/eccentric-j/eccentric-icons/raw/main/icons/kitty-terminal/3d/kitty-preview.png)

![kitty.app](https://github.com/sodapopcan/kitty-icon/raw/main/kitty.app.png)

![icon_128x128@2x](https://github.com/sfsam/some_icons/raw/main/kitty.app.iconset/icon_128x128@2x.png)

![twiskers](https://github.com/igrmk/twiskers/raw/main/icon/twiskers.svg)

![kitty-nyan](https://github.com/mtklr/kitty-nyan-icon/raw/main/kitty-nyan.svg)

You can put kitty.app.icns (macOS only) or kitty.app.png in the
kitty configuration directory <confloc>, and this icon will be applied
...
This is accomplished by using `map` with send_key in kitty.conf.
For example:
...
than a key press, see send_text <send_text> instead.
...
configuration files such as `XDG_*`, KITTY_CONFIG_DIRECTORY and,
most importantly, `PATH` to locate binaries.

The simplest way to fix this is to have kitty load the environment variables
from your shell configuration at startup using the env directive,
...
First, terminal multiplexers are a bad idea <391#issuecomment-638320745>,
...
styled underlines </underlines>, :doc:`desktop notifications
</desktop-notifications>`, variable sized text </text-sizing-protocol>,
