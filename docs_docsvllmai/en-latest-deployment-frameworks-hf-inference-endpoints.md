...
source: sitemap
fetched_at: 2026-05-07T21:11:47.649452449-03:00
rendered_js: false
word_count: 899
summary: This document provides instructions for deploying vLLM-compatible models on Hugging Face Inference Endpoints using catalog-based, guided, and manual configuration methods.
tags:
    - vllm
    - hugging-face
    - inference-endpoints
...
1. Go to [Endpoints Catalog](https://endpoints.huggingface.co/catalog) and in the **Inference Server** options, select `vLLM`.This will display the current list of models with optimized preconfigured options.
   ...
   [![Endpoints Catalog](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-catalog.png)](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-catalog.png)
2. Select the desired model and click **Create Endpoint**.
   
   [![Create Endpoint](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-create-endpoint.png)](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-create-endpoint.png)
...
2. Locate the **Deploy** button. The button appears for models tagged with `transformers` at the top right of the [model card](https://huggingface.co/ibm-granite/granite-docling-258M).
   ...
   [![Locate deploy button](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-locate-deploy-button.png)](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-locate-deploy-button.png)
3. Click the **Deploy** button &gt; **HF Inference Endpoints**. You will be taken to the Inference Endpoints interface to configure the deployment.
   ...
   [![Click deploy button](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-click-deploy-button.png)](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-click-deploy-button.png)
4. Select the Hardware (we choose AWS&gt;GPU&gt;T4 for the example) and Container Configuration. Choose `vLLM` as the container type and finalize the deployment pressing **Create Endpoint**.
   ...
   [![Select Hardware](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-select-hardware.png)](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-select-hardware.png)
...
1. Start a new deployment. Go to [Inference Endpoints](https://endpoints.huggingface.co/) and click `New`.
   ...
   [![New Endpoint](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-new-endpoint.png)](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-new-endpoint.png)
2. Search the model in the Hub. In the dialog, switch to **Hub** and search for the desired model.
   
   [![Select model](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-select-model.png)](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-select-model.png)
3. Choosing infrastructure. On the configuration page, select the cloud provider and hardware from the available options.  
   ...
   [![Choose Infra](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-choose-infra.png)](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-choose-infra.png)
4. Configure the container. Scroll to the **Container Configuration** and select `vLLM` as the container type.
   
   [![Configure Container](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-configure-container.png)](https://docs.vllm.ai/en/latest/assets/deployment/hf-inference-endpoints-configure-container.png)
