...
source: sitemap
fetched_at: 2026-05-07T21:13:15.818353316-03:00
rendered_js: false
word_count: 284
summary: This document describes a disaggregated proxy for vLLM that manages multi-turn chat requests by facilitating bidirectional KV cache transfer between separate Prefill and Decode nodes. It explains the request routing architecture and the mechanism for scoping KV caches to specific conversations via unique IDs.
tags:
    - vllm
    - kv-cache
    - proxy-server
...
"""Per-conversation KV block cache.
    ...
    def__init__(self, ttl_seconds: float = 600.0) -> None:
        self._store: dict[str, CachedKVEntry] = {}
        self._ttl = ttl_seconds

    defget(self, conversation_id: str) -> dict[str, Any] | None:
"""Retrieve and consume cached KV params for a conversation.
    ...
    defput(self, conversation_id: str, kv_params: dict[str, Any]) -> None:
...
def_make_headers(request_id: str) -> dict[str, str]:
...
async def_stream_from_decode_sse(
    client: ServiceClient,
    endpoint: str,
    req_data: dict[str, Any],
    request_id: str,
...
def_next_client(app_state, role: str) -> ServiceClient:
    if role == "prefill":
...
# Request handler
async def_handle_request(api_path: str, request: Request):
...
async defcompletions(request: Request):
