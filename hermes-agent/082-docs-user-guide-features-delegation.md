...
source: crawler
fetched_at: 2026-04-24T17:00:07.094438802-03:00
rendered_js: false
word_count: 830
summary: This document details the `delegate_task` function, which spawns independent subagent instances to handle tasks concurrently or sequentially within a parent agent's context. It explains how subagents maintain fresh contexts, defines various delegation modes (single/parallel batch), and outlines configuration options like toolset limitations, iteration counts, and nested orchestration.
tags:
    - agent-delegation
    - subagent-workflow
    - task-management
...
Toolset PatternUse Case`["terminal", "file"]`Code work, debugging, file editing, builds`["web"]`Research, fact-checking, documentation lookup`["terminal", "file", "web"]`Full-stack tasks (default)`["file"]`Read-only analysis, code review without execution`["terminal"]`System administration, process management
...
Factordelegate\_taskexecute\_code**Reasoning**Full LLM reasoning loopJust Python code execution**Context**Fresh isolated conversationNo conversation, just script**Tool access**All non-blocked tools with reasoning7 tools via RPC, no reasoning**Parallelism**3 concurrent subagents by default (configurable)Single script**Best for**Complex tasks needing judgmentMechanical multi-step pipelines**Token cost**Higher (full LLM loop)Lower (only stdout returned)**User interaction**None (subagents can't clarify)None
