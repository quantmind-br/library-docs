...
source: sitemap
fetched_at: 2026-04-26T08:39:05.79637657-03:00
rendered_js: false
word_count: 1813
summary: This document provides a comprehensive reference for the IPC command-line interface of DankMaterialShell, enabling external control over system settings such as audio, display, themes, and peripheral management.
tags:
    - dankmaterialshell
    - ipc-commands
    - shell-control
...
DankMaterialShell IPC enables external control via:
...
| `openQuery <widgetId> <query>` | Open widget popout with pre-filled search query | widget ID, query text |
| `toggleQuery <widgetId> <query>` | Toggle widget popout with pre-filled search query | widget ID, query text |
...
> [!note]
> The `set` function supports boolean, number, and string values. Objects and arrays are not currently supported.
...
See [[014-docs-dankmaterialshell-cli-keybinds-cheatsheets]] for more details on providers and creating custom cheatsheets.
...
> [!warning]
> Returns an error if running on an unsupported compositor.
...
> [!info]
> Available on niri, Hyprland, and MangoWC compositors.
