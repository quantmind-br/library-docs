---
title: SIS Imports | Instructure Developer Documentation Portal
url: https://developerdocs.instructure.com/services/canvas/resources/sis_imports
source: sitemap
fetched_at: 2026-02-15T09:08:22.36011-03:00
rendered_js: false
word_count: 1313
summary: This document defines the data structures and API endpoints for importing Student Information System (SIS) data, including details on import states, batch processing, and statistics.
tags:
    - sis-import
    - data-integration
    - api-reference
...
[SisImportsApiController#indexarrow-up-right](https://github.com/instructure/canvas-lms/blob/master/app/controllers/sis_imports_api_controller.rb)
...
Returns a list of [SisImport](https://developerdocs.instructure.com/services/canvas/resources/sis_imports#sisimport) objects.

[SisImportsApiController#importingarrow-up-right](https://github.com/instructure/canvas-lms/blob/master/app/controllers/sis_imports_api_controller.rb)
...
Returns a [SisImport](https://developerdocs.instructure.com/services/canvas/resources/sis_imports#sisimport) object.

[SisImportsApiController#createarrow-up-right](https://github.com/instructure/canvas-lms/blob/master/app/controllers/sis_imports_api_controller.rb)
...
Choose the data format for reading SIS data. With a standard Canvas install, this option can only be ‘instructure\_csv’, and if unprovided, will be assumed to be so. Can be part of the query string.
...
```
curl -F attachment=@<filename> -H "Authorization: Bearer <token>" <br>    https://<canvas>/api/v1/accounts/<account_id>/sis_imports.json?import_type=instructure_csv
```
...
```
curl -H 'Content-Type: application/octet-stream' --data-binary @<filename>.zip <br>    -H "Authorization: Bearer <token>" <br>    https://<canvas>/api/v1/accounts/<account_id>/sis_imports.json?import_type=instructure_csv&extension=zip
...
Runs batch mode against all terms in terms file. Requires change\_threshold.
...
This option, if present, will process all changes as if they were UI changes. This means that “stickiness” will be added to changed fields. This option is only processed if ‘override\_sis\_stickiness’ is also provided.

This option, if present, will clear “stickiness” from all fields processed by this import. Requires that ‘override\_sis\_stickiness’ is also provided. If ‘add\_sis\_stickiness’ is also provided, ‘clear\_sis\_stickiness’ will overrule the behavior of ‘add\_sis\_stickiness’
...
If set on a CSV import, Canvas will attempt to optimize the SIS import by comparing this set of CSVs to the previous set that has the same data set identifier, and only applying the difference between the two. See the SIS CSV Format documentation for more details. Diffing cannot be used with batch\_mode

`diffing_remaster_data_set`

If true, and diffing\_data\_set\_identifier is sent, this SIS import will be part of the data set, but diffing will not be performed. See the SIS CSV Format documentation for details.

If diffing\_drop\_status is passed, this SIS import will use this status for enrollments that are not included in the sis\_batch. Defaults to ‘deleted’

Allowed values: `deleted`, `completed`, `inactive`

`diffing_user_remove_status`

For users removed from one batch to the next one using the same diffing\_data\_set\_identifier, set their status to the value of this argument. Defaults to ‘deleted’.

Allowed values: `deleted`, `suspended`

`batch_mode_enrollment_drop_status`

If batch\_mode\_enrollment\_drop\_status is passed, this SIS import will use this status for enrollments that are not included in the sis\_batch. This will have an effect if multi\_term\_batch\_mode is set. Defaults to ‘deleted’ This will still mark courses and sections that are not included in the sis\_batch as deleted, and subsequently enrollments in the deleted courses and sections as deleted.

Allowed values: `deleted`, `completed`, `inactive`

If set with batch\_mode, the batch cleanup process will not run if the number of items deleted is higher than the percentage set. If set to 10 and a term has 200 enrollments, and batch would delete more than 20 of the enrollments the batch will abort before the enrollments are deleted. The change\_threshold will be evaluated for course, sections, and enrollments independently. If set with diffing, diffing will not be performed if the files are greater than the threshold as a percent. If set to 5 and the file is more than 5% smaller or more than 5% larger than the file that is being compared to, diffing will not be performed. If the files are less than 5%, diffing will be performed. The way the percent is calculated is by taking the size of the current import and dividing it by the size of the previous import. The formula used is:
...
[SisImportsApiController#showarrow-up-right](https://github.com/instructure/canvas-lms/blob/master/app/controllers/sis_imports_api_controller.rb)
...
[SisImportsApiController#restore\_statesarrow-up-right](https://github.com/instructure/canvas-lms/blob/master/app/controllers/sis_imports_api_controller.rb)
...
If set, will only restore items that were deleted from batch\_mode.
...
[SisImportsApiController#abortarrow-up-right](https://github.com/instructure/canvas-lms/blob/master/app/controllers/sis_imports_api_controller.rb)
...
```
curlhttps://<canvas>/api/v1/accounts/<account_id>/sis_imports/<sis_import_id>/abort\
-H'Authorization: Bearer <token>'
```

Returns a [SisImport](https://developerdocs.instructure.com/services/canvas/resources/sis_imports#sisimport) object.

[SisImportsApiController#abort\_all\_pendingarrow-up-right](https://github.com/instructure/canvas-lms/blob/master/app/controllers/sis_imports_api_controller.rb)
...
```
curlhttps://<canvas>/api/v1/accounts/<account_id>/sis_imports/abort_all_pending\
-H'Authorization: Bearer <token>'
```
