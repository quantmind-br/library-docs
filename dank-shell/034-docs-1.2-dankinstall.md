...
source: sitemap
fetched_at: 2026-04-26T08:35:23.633717563-03:00
rendered_js: false
word_count: 766
summary: This document provides an installation guide and overview for DankMaterialShell, a desktop environment for Linux that includes automated configuration for compositors, terminal emulators, and desktop utilities.
tags:
    - linux
    - desktop-environment
    - hyprland
...
- [[039-docs-1.2-danklinux|DankMaterialShell]] — desktop shell built with quickshell & Go
...
Uses `pacman` for system packages from official repos. Builds quickshell, matugen (pre-compiled binary), and dgop from AUR using `makepkg` — no AUR helper needed. niri and hyprland are in official repos.
...
Packages come from the [[039-docs-1.2-danklinux#ubuntu|Ubuntu PPA]] (`ppa:avengemedia/danklinux`). Hyprland comes from `ppa:cppiber/hyprland`.

### Debian

**Supported:** Debian 13+ (Trixie), Debian Testing, Debian Sid

Packages come from the [[039-docs-1.2-danklinux#debian|DankLinux OBS repository]]. **niri only** — Debian doesn't have Hyprland packages yet.

### openSUSE Tumbleweed

Good package availability out of the box. niri, hyprland, ghostty, and most tools are in standard repos. Additional packages come from the [[039-docs-1.2-danklinux#opensuse|DankLinux OBS repository]].
...
Most distros have pre-built packages via the [[039-docs-1.2-danklinux|DankLinux Repository]]. The installer handles remaining builds automatically:
