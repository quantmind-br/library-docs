...
url: https://moodledev.io/docs/4.4/apis
source: sitemap
fetched_at: 2026-02-17T15:01:51.54203-03:00
rendered_js: false
word_count: 1482
summary: This document provides an overview of Moodle's core and subsystem APIs, categorizing essential tools and libraries available for plugin development.
tags:
    - moodle-development
    - core-apis
    - plugin-development
...
The [Access API](https://moodledev.io/docs/4.4/apis/subsystems/access) gives you functions so you can determine what the current user is allowed to do, and it allows modules to extend Moodle with new capabilities.

### Data manipulation API (dml)[​](#data-manipulation-api-dml "Direct link to Data manipulation API (dml)")

The [Data manipulation API](https://moodledev.io/docs/4.4/apis/core/dml) allows you to read/write to databases in a consistent and safe way.

### File API (files)[​](#file-api-files "Direct link to File API (files)")

The [File API](https://moodledev.io/docs/4.4/apis/subsystems/files) controls the storage of files in connection to various plugins.

### Form API (form)[​](#form-api-form "Direct link to Form API (form)")

The [Form API](https://moodledev.io/docs/4.4/apis/subsystems/form) defines and handles user data via web forms.
...
The [Navigation API](https://moodledev.io/docs/4.4/apis/core/navigation) allows you to manipulate the navigation tree to add and remove items as you wish.
...
The [Output API](https://moodledev.io/docs/4.4/apis/subsystems/output) is used to render the HTML for all parts of the page.
...
The [Upgrade API](https://moodledev.io/docs/4.4/guides/upgrade) is how your module installs and upgrades itself, by keeping track of its own version.
...
The [Admin settings](https://moodledev.io/docs/4.4/apis/subsystems/admin) API deals with providing configuration options for each plugin and Moodle core.
...
The [Analytics API](https://moodledev.io/docs/4.4/apis/subsystems/analytics) allow you to create prediction models and generate insights.

### Availability API (availability)[​](#availability-api-availability "Direct link to Availability API (availability)")

The [Availability API](https://moodledev.io/docs/4.4/apis/subsystems/availability) controls access to activities and sections.

### Backup API (backup)[​](#backup-api-backup "Direct link to Backup API (backup)")

The [Backup API](https://moodledev.io/docs/4.4/apis/subsystems/backup) defines exactly how to convert course data into XML for backup purposes, and the [Restore API](https://moodledev.io/docs/4.4/apis/subsystems/backup/restore) describes how to convert it back the other way.

### Cache API (cache)[​](#cache-api-cache "Direct link to Cache API (cache)")

The [The Moodle Universal Cache (MUC)](https://docs.moodle.org/dev/The_Moodle_Universal_Cache_%28MUC%29) is the structure for storing cache data within Moodle. [Cache API](https://moodledev.io/docs/4.4/apis/subsystems/muc) explains some of what is needed to use a cache in your code.

### Calendar API (calendar)[​](#calendar-api-calendar "Direct link to Calendar API (calendar)")

The [Calendar API](https://moodledev.io/docs/4.4/apis/core/calendar) allows you to add and modify events in the calendar for user, groups, courses, or the whole site.

### Check API (check)[​](#check-api-check "Direct link to Check API (check)")

The [Check API](https://moodledev.io/docs/4.4/apis/subsystems/check) allows you to add security, performance or health checks to your site.
...
The [Communication API](https://moodledev.io/docs/4.4/apis/subsystems/communication) provides access to the messaging system and other communication providers (such as Matrix).
...
The [Data definition API](https://moodledev.io/docs/4.4/apis/core/dml/ddl) is what you use to create, change and delete tables and fields in the database during upgrades.

### Editor API[​](#editor-api "Direct link to Editor API")

The [Editor API](https://moodledev.io/docs/4.4/apis/subsystems/editor) is used to control HTML text editors.

### Enrolment API (enrol)[​](#enrolment-api-enrol "Direct link to Enrolment API (enrol)")

The [Enrolment API](https://moodledev.io/docs/4.4/apis/subsystems/enrol) deals with course participants.
...
The [Hooks API](https://moodledev.io/docs/4.4/apis/core/hooks) allows core and plugins to communicate indirectly with other plugins.
...
The [External functions API](https://moodledev.io/docs/4.4/apis/subsystems/external/functions) allows you to create fully parametrised methods that can be accessed by external programs (such as [Web services](https://moodledev.io/docs/4.4/apis/subsystems/external)).

### Favourites API[​](#favourites-api "Direct link to Favourites API")

The [Favourites API](https://moodledev.io/docs/4.4/apis/subsystems/favourites) allows you to mark items as favourites for a user and manage these favourites. This is often referred to as 'Starred'.
...
The [Lock API](https://moodledev.io/docs/4.4/apis/core/lock) lets you synchronise processing between multiple requests, even for separate nodes in a cluster.

### Message API (message)[​](#message-api-message "Direct link to Message API (message)")

The [Message API](https://moodledev.io/docs/4.4/apis/core/message) lets you post messages to users. They decide how they want to receive them.
...
The [Preference API](https://moodledev.io/docs/4.4/apis/core/preference) is a simple way to store and retrieve preferences for individual users.
...
The [Report builder API](https://moodledev.io/docs/4.4/apis/core/reportbuilder) allows you to create reports in your plugin, as well as providing custom reporting data which users can use to build their own reports.
...
The [Tag API](https://moodledev.io/docs/4.4/apis/subsystems/tag) allows you to store tags (and a tag cloud) to items in your module.

### Task API (task)[​](#task-api-task "Direct link to Task API (task)")

The [Task API](https://moodledev.io/docs/4.4/apis/subsystems/task) lets you run jobs in the background. Either once off, or on a regular schedule.

### Time API (time)[​](#time-api-time "Direct link to Time API (time)")

The [Time API](https://moodledev.io/docs/4.4/apis/subsystems/time) takes care of translating and displaying times between users in the site.
...
This is a rather informal grouping of miscellaneous [User-related APIs](https://moodledev.io/docs/4.4/apis/core/user) relating to sorting and searching lists of users.

### Web services API (webservice)[​](#web-services-api-webservice "Direct link to Web services API (webservice)")

The [Web services API](https://moodledev.io/docs/4.4/apis/subsystems/external/writing-a-service) allows you to expose particular functions (usually external functions) as web services.
...
The [Custom fields API](https://moodledev.io/docs/4.4/apis/core/customfields) allows you to configure and add custom fields for different entities
...
The [Activity completion API](https://moodledev.io/docs/4.4/apis/core/activitycompletion) is to indicate to the system how activities are completed.

### Advanced grading API (grading)[​](#advanced-grading-api-grading "Direct link to Advanced grading API (grading)")

The [Advanced grading API](https://moodledev.io/docs/4.4/apis/core/grading) allows you to add more advanced grading interfaces (such as rubrics) that can produce simple grades for the gradebook.
...
The deprecated [Conditional activities API](https://moodledev.io/docs/4.4/apis/core/conditionalactivities) used to provide conditional access to modules and sections in Moodle 2.6 and below. It has been replaced by the [Availability API](https://moodledev.io/docs/4.4/apis/subsystems/availability).

### Groups API (group)[​](#groups-api-group "Direct link to Groups API (group)")

The [Groups API](https://moodledev.io/docs/4.4/apis/subsystems/group) allows you to check the current activity group mode and set the current group.
...
The [Plagiarism API](https://moodledev.io/docs/4.4/apis/subsystems/plagiarism) allows your activity module to send files and data to external services to have them checked for plagiarism.
