...
url: https://ratatui.rs/recipes/apps/log-with-tracing/
source: crawler
fetched_at: 2026-02-01T21:13:10.605783277-03:00
rendered_js: false
word_count: 269
summary: This document provides a comprehensive guide for setting up and initializing structured logging and error handling in Rust applications using the tracing ecosystem.
tags:
    - rust
    - logging
    - tracing
...
cargoaddtracing-errortracing
cargoaddtracing-subscriber--featuresenv-filter
cargoadddirectorieslazy_staticcolor-eyre# (optional)
```
...
pubstaticrefLOG_ENV: String =format!("{}_LOGLEVEL", PROJECT_NAME.clone());
pubstaticrefLOG_FILE: String =format!("{}.log", env!("CARGO_PKG_NAME"));
}
...
pubfninitialize_logging() -> Result<()> {
letdirectory=get_data_dir();
std::fs::create_dir_all(directory.clone())?;
letlog_path=directory.join(LOG_FILE.clone());
letlog_file= std::fs::File::create(log_path)?;
letlog_filter= std::env::var("RUST_LOG")
.or_else(|_| std::env::var(LOG_ENV.clone()))
.unwrap_or_else(|_|format!("{}=info", env!("CARGO_CRATE_NAME")));
letfile_subscriber= tracing_subscriber::fmt::layer()
.with_file(true)
...
Additionally, the location of the log files would be decided by your environment variables. See [the section on XDG directories](https://ratatui.rs/recipes/apps/config-directories/) for more information.
