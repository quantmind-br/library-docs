...
source: sitemap
fetched_at: 2026-05-07T21:15:10.470069097-03:00
rendered_js: false
word_count: 1173
summary: This document explains how to configure and deploy vLLM models using data parallel strategies, including internal, hybrid, and external load balancing options for both single-node and multi-node environments.
tags:
    - vllm
    - data-parallelism
    - model-deployment
...
```
vllmserve$MODEL--data-parallel-size4--tensor-parallel-size2
```
...
vllmserve$MODEL--data-parallel-size4--data-parallel-size-local2\
--data-parallel-address10.99.48.128--data-parallel-rpc-port13345
# Node 1
vllmserve$MODEL--headless--data-parallel-size4--data-parallel-size-local2\
--data-parallel-start-rank2\
--data-parallel-address10.99.48.128--data-parallel-rpc-port13345
```
...
```
# Node 0  (with ip address 10.99.48.128)
vllmserve$MODEL--data-parallel-size4--data-parallel-size-local0\
--data-parallel-address10.99.48.128--data-parallel-rpc-port13345
# Node 1
vllmserve$MODEL--headless--data-parallel-size4--data-parallel-size-local4\
--data-parallel-address10.99.48.128--data-parallel-rpc-port13345
```
...
[![DP Internal LB Diagram](https://docs.vllm.ai/en/latest/assets/deployment/dp_internal_lb.png)](https://docs.vllm.ai/en/latest/assets/deployment/dp_internal_lb.png)
...
```
# Rank 0
CUDA_VISIBLE_DEVICES=0vllmserve$MODEL--data-parallel-size2--data-parallel-rank0\
--port8000
# Rank 1
CUDA_VISIBLE_DEVICES=1vllmserve$MODEL--data-parallel-size2--data-parallel-rank1\
--port8001
```
...
vllmserve$MODEL--data-parallel-size2--data-parallel-rank0\
--data-parallel-address10.99.48.128--data-parallel-rpc-port13345
# Rank 1
vllmserve$MODEL--data-parallel-size2--data-parallel-rank1\
--data-parallel-address10.99.48.128--data-parallel-rpc-port13345
```

The coordinator process also runs in this scenario, co-located with the DP rank 0 engine.

[![DP External LB Diagram](https://docs.vllm.ai/en/latest/assets/deployment/dp_external_lb.png)](https://docs.vllm.ai/en/latest/assets/deployment/dp_external_lb.png)
