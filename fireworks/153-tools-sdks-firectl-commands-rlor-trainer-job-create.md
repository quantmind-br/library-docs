---
title: firectl rlor-trainer-job create - Fireworks AI Docs
url: https://docs.fireworks.ai/tools-sdks/firectl/commands/rlor-trainer-job-create
source: sitemap
fetched_at: 2026-04-27T20:16:41.043713362-03:00
rendered_js: false
word_count: 460
summary: This document describes the command and available flags for creating an rlor trainer job using `firectl`. It details various parameters that control aspects of the fine-tuning process, such as model selection, training hyperparameters, logging, and cloud credentials.
tags:
    - rlor-trainer-job
    - command-line
    - fine-tuning
...
category: reference
optimized: true
optimized_at: 2026-04-27T23:16:41Z
---
# firectl rlor-trainer-job create

Create an RLOR (Reinforcement Learning with Rewards) trainer job.
...
| `--rewards` | strings | Reward weights as `key=value` pairs (e.g., `--rewards key1=1.0,key2=0.5`) |
| `--source-job` | string | Source trainer job to copy configuration from |
| `--training-shape` | string | Training shape selector for service-mode launches |
