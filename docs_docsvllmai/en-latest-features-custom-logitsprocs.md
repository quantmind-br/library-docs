...
source: sitemap
fetched_at: 2026-05-07T21:14:07.957711166-03:00
rendered_js: false
word_count: 2528
summary: This document provides technical instructions for extending vLLM by creating custom logits processors to modify token probability distributions at runtime without modifying the source code.
tags:
    - vllm
    - logits-processor
    - llm-inference
...
- `validate_params(cls, sampling_params: SamplingParams)`:
  ...
  - Raise `ValueError` if [`SamplingParams`](https://docs.vllm.ai/en/latest/api/vllm/sampling_params/#vllm.sampling_params.SamplingParams "            SamplingParams") has invalid arguments (especially custom arguments) used by logits processor.
  - When request is sent to entrypoint, `validate_params()` will validate [`SamplingParams`](https://docs.vllm.ai/en/latest/api/vllm/sampling_params/#vllm.sampling_params.SamplingParams "            SamplingParams") and refuse request with invalid arguments.
...
- `update_state(self, batch_update: Optional["BatchUpdate"]) -> None`:
  ...
  - Consume a [`BatchUpdate`](https://docs.vllm.ai/en/latest/api/vllm/v1/sample/logits_processor/interface/#vllm.v1.sample.logits_processor.interface.BatchUpdate "            BatchUpdate            dataclass   ") data structure representing persistent batch state changes at the beginning of the current engine step
  - Use the [`BatchUpdate`](https://docs.vllm.ai/en/latest/api/vllm/v1/sample/logits_processor/interface/#vllm.v1.sample.logits_processor.interface.BatchUpdate "            BatchUpdate            dataclass   ") members to update logits processor internal state
...
### How the vLLM engine builds the [`BatchUpdate`](https://docs.vllm.ai/en/latest/api/vllm/v1/sample/logits_processor/interface/#vllm.v1.sample.logits_processor.interface.BatchUpdate " BatchUpdate dataclass ") data structure[¶](#how-the-vllm-engine-builds-the-batchupdate-data-structure "Permanent link")
...
Logits processor `update_state()` implementations should assume the following model for how the model runner updates persistent batch state (expressed here in terms of the [`BatchUpdate`](https://docs.vllm.ai/en/latest/api/vllm/v1/sample/logits_processor/interface/#vllm.v1.sample.logits_processor.interface.BatchUpdate "            BatchUpdate            dataclass   ") abstraction):
...
Unlike built-in logits processors, custom logits processors may require configuration arguments that are not hard-coded into [`SamplingParams`](https://docs.vllm.ai/en/latest/api/vllm/sampling_params/#vllm.sampling_params.SamplingParams "            SamplingParams") or the vLLM server REST API. To solve this problem, custom logits processors may leverage vLLM [custom arguments](https://docs.vllm.ai/en/latest/features/custom_arguments/) support to receive configuration settings from the user (although you are also free to design a custom logits processor which utilizes the pre-existing fields in [`SamplingParams`](https://docs.vllm.ai/en/latest/api/vllm/sampling_params/#vllm.sampling_params.SamplingParams "            SamplingParams").)
...
fromvllm.v1.sample.logits_processorimport (BatchUpdate,
                                            LogitsProcessor,
                                            MoveDirectionality)
...
- Override `AdapterLogitsProcessor.new_req_logits_processor(self,params)` to create a new request-level logits processor instance from a [`SamplingParams`](https://docs.vllm.ai/en/latest/api/vllm/sampling_params/#vllm.sampling_params.SamplingParams "            SamplingParams") instance:
...
This method is supported in both offline and online vLLM usage scenarios. The custom logits processor's FQCN (in the form of `dotted.path.to.module:ClassName`) can be passed as an argument to the [`LLM`](https://docs.vllm.ai/en/latest/api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") and [`AsyncLLM`](https://docs.vllm.ai/en/latest/api/vllm/v1/engine/async_llm/#vllm.v1.engine.async_llm.AsyncLLM "            AsyncLLM") Python constructors, or as a CLI argument to `vllm serve` with the following syntax

```
vllmserve...--logits_processors<logitsprocessor1><logitsprocessor2>...
```
...
3. The object pointed to by the FQCN must be a subclass of [`LogitsProcessor`](https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/logits_processor/#vllm.model_executor.layers.logits_processor.LogitsProcessor "            LogitsProcessor")

See examples below:

Passing custom logits processor FQCN to [`LLM`](https://docs.vllm.ai/en/latest/api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") in Python
...
Passing custom logits processor FQCN to [`AsyncLLM`](https://docs.vllm.ai/en/latest/api/vllm/v1/engine/async_llm/#vllm.v1.engine.async_llm.AsyncLLM "            AsyncLLM") in Python
...
```
vllmservefacebook/opt-125m--logits_processorsyour.module.path:DummyLogitsProcessor
```
...
Once your package is installed, your custom logits processor will be loaded automatically whenever vLLM is initialized. You do *not* need to pass the custom logits processor to the [`LLM`](https://docs.vllm.ai/en/latest/api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") or [`AsyncLLM`](https://docs.vllm.ai/en/latest/api/vllm/v1/engine/async_llm/#vllm.v1.engine.async_llm.AsyncLLM "            AsyncLLM") constructors or to the vLLM server explicitly at initialization time if your logits processor is exposed as an entry point.
...
You can pass one or more custom logits processor class objects to the [`LLM`](https://docs.vllm.ai/en/latest/api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") and [`AsyncLLM`](https://docs.vllm.ai/en/latest/api/vllm/v1/engine/async_llm/#vllm.v1.engine.async_llm.AsyncLLM "            AsyncLLM") constructors. This option is very flexible, as the logits processor classes may either be (1) defined locally within the same Python source file where [`LLM`](https://docs.vllm.ai/en/latest/api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") or [`AsyncLLM`](https://docs.vllm.ai/en/latest/api/vllm/v1/engine/async_llm/#vllm.v1.engine.async_llm.AsyncLLM "            AsyncLLM") is instantiated, or (2) imported from a Python package.

Passing custom logits processor class object to [`LLM`](https://docs.vllm.ai/en/latest/api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") or [`AsyncLLM`](https://docs.vllm.ai/en/latest/api/vllm/v1/engine/async_llm/#vllm.v1.engine.async_llm.AsyncLLM "            AsyncLLM") in Python

```
# Import custom logits processor
fromsome.moduleimport DummyLogitsProcessor

# ...or...

# Define custom logits processor locally
fromvllm.v1.sample.logits_processorimport LogitsProcessor
...
Offline: configure custom logits processor for an [`LLM`](https://docs.vllm.ai/en/latest/api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") request
...
Offline: configure custom logits processor for an [`AsyncLLM`](https://docs.vllm.ai/en/latest/api/vllm/v1/engine/async_llm/#vllm.v1.engine.async_llm.AsyncLLM "            AsyncLLM") request
