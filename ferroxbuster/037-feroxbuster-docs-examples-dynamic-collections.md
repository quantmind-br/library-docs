...
url: https://epi052.github.io/feroxbuster-docs/examples/dynamic-collections
source: github_pages
fetched_at: 2026-02-06T10:55:16.634389217-03:00
rendered_js: true
word_count: 499
summary: This document explains how to use and configure feroxbuster's dynamic collection features to automatically discover backup files, extensions, and keywords during a scan.
tags:
    - feroxbuster
    - directory-discovery
    - security-tools
...
[Section titled “Dynamic Collection Settings”](#dynamic-collection-settings)
...
[Section titled “Collect Extensions”](#collect-extensions)

Using `--collect-extensions` means that any found url that has an extension will be added as an extension to try for all future requests.

There is a (large) [default list](https://github.com/epi052/feroxbuster/blob/main/src/lib.rs#L60) of extensions that are ignored by `--collect-extensions`. If `--dont-collect` is used, the default list isn’t used, and whatever values are passed to `--dont-collect` are used instead.

Using `--collect-words` means that each valid response’s html is parsed, and words considered **important** are added to the wordlist to be requested.
...
[Section titled “Default extensions”](#default-extensions)
...
[Section titled “Custom extension list”](#custom-extension-list)
...
[Section titled “Collect Extensions”](#collect-extensions-1)
...
`--collect-extensions` would effectively add `.php` to feroxbuster’s internal state, as though `.php` were passed via `--extensions|-x`. The result would be the following requests.
