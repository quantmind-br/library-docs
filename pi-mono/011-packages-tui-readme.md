---
title: README
url: https://github.com/badlogic/pi-mono/blob/main/packages/tui/README.md
source: git
fetched_at: 2026-05-03T09:32:20.624875867-03:00
rendered_js: false
word_count: 1165
summary: Lightweight TypeScript TUI framework with differential rendering and synchronized output for flicker-free interactive CLI applications.
tags:
    - cli
    - terminal-ui
    - typescript
...
- **Inline Images** — Renders images via Kitty or iTerm2 graphics protocols
...
Main container managing components and rendering.
...
const handle = tui.showOverlay(component, {
  ...
  maxHeight: "50%",       // Maximum height as percentage
...
tui.hideOverlay();
tui.hasOverlay();           // Check if any visible overlay is active
```
...
2. For position: absolute `row`/`col` > percentage > `anchor`
3. `margin` clamps final position to stay within terminal bounds
4. `visible` callback controls whether overlay renders
...
| Method | Description |
|--------|-------------|
| `render(width)` | Returns array of strings, one per line. **Each line must not exceed `width`** or the TUI will error. Use `truncateToWidth()` or manual wrapping. |
| `handleInput?(data)` | Called when component has focus and receives keyboard input. `data` contains raw terminal input (may include ANSI escape sequences). |
| `invalidate?()` | Clears cached render state. Components should re-render from scratch on next `render()` call. |

The TUI appends a full SGR reset and OSC 8 reset at the end of each rendered line. Styles do not carry across lines. If emitting multi-line styled text, reapply styles per line or use `wrapTextWithAnsi()`.

### Focusable Interface (IME Support)

Components displaying a text cursor that need IME support implement `Focusable`:
...
**Container components with embedded inputs:** Must implement `Focusable` and propagate focus state to child:
...
Without this propagation, typing with an IME shows the candidate window in the wrong position.
...
Container applying padding and background color to all children.
...
box.setBgFn((text) => chalk.bgBlue(text));  // Change dynamically
```
...
const editor = new Editor(tui, theme, options?);  // tui required for height-aware scrolling
editor.onSubmit = (text) => console.log(text);
editor.onChange = (text) => console.log("Changed:", text);
editor.disableSubmit = true; // Disable submit temporarily
editor.setAutocompleteProvider(provider);
editor.borderColor = (s) => chalk.blue(s); // Change dynamically
editor.setPaddingX(1); // Update padding dynamically
...
**Key Bindings:**
- `Enter` — Submit
- `Shift+Enter`, `Ctrl+Enter`, or `Alt+Enter` — New line (Alt+Enter most reliable)
- `Tab` — Autocomplete
- `Ctrl+K` — Delete to end of line
- `Ctrl+U` — Delete to start of line
- `Ctrl+W` or `Alt+Backspace` — Delete word backwards
...
- `Ctrl+A` / `Ctrl+E` — Line start/end
- `Ctrl+]` — Jump forward to character (awaits next keypress, moves to first occurrence)
...
- Padding support and render caching
...
Supported formats: PNG, JPEG, GIF, WebP. Dimensions parsed from image headers automatically.
...
Use `matchesKey()` with the `Key` helper for detecting keyboard input:
...
All updates wrapped in **synchronized output** (`\x1b[?2026h` ... `\x1b[?2026l`) for atomic, flicker-free rendering.

## Terminal Interface

Works with any object implementing the `Terminal` interface:
...
> [!warning]
> Each line returned by `render()` must not exceed the `width` parameter. The TUI errors if any line is wider than the terminal.
...
Both `visibleWidth()` and `truncateToWidth()` handle ANSI escape codes correctly:

```typescript
import chalk from "chalk";

const styled = chalk.red("Hello") + " " + chalk.blue("World");
const width = visibleWidth(styled); // 11 (not counting ANSI)
const truncated = truncateToWidth(styled, 8); // Red "Hello" + " W..." with proper reset
```

### Caching

For performance, cache rendered output and re-render only when necessary:
...
- [[009-packages-coding-agent-readme|Pi Coding Agent]] — Terminal coding harness

#cli #terminal-ui #typescript #rendering-engine
