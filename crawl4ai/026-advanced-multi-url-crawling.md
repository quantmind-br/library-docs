---
title: URL Crawling - Crawl4AI Documentation (v0.8.x)
url: https://docs.crawl4ai.com/advanced/multi-url-crawling/
source: sitemap
fetched_at: 2026-04-26T07:45:22.831042464-03:00
rendered_js: false
word_count: 1210
summary: This document explains how to use Crawl4AI dispatchers to manage concurrent web crawling, including configuration of rate limits, memory-based adaptive throttling, and real-time task monitoring.
tags:
    - web-crawling
    - concurrency
    - rate-limiting
...
```
fromcrawl4aiimport RateLimiter
...
```
fromcrawl4aiimport CrawlerMonitor, DisplayMode
...
```
fromcrawl4ai.async_dispatcherimport MemoryAdaptiveDispatcher
...
```
fromcrawl4ai.async_dispatcherimport SemaphoreDispatcher
...
```
async defcrawl_with_semaphore(urls):
    browser_config = BrowserConfig(headless=True, verbose=False)
...
```
fromcrawl4aiimport AsyncWebCrawler, CrawlerRunConfig, MatchMode
...
01. **Order Matters**: Configs are evaluated in order - put specific patterns before general ones
02. **Default Config Behavior**:
03. A config without `url_matcher` matches ALL URLs
04. Always include a default config as the last item if you want to handle all URLs
05. Without a default config, unmatched URLs will fail with "No matching configuration found"
06. **Test Your Patterns**: Use the config's `is_match()` method to test patterns:
...
08. Disable JS for static content
09. Skip screenshots for data APIs
