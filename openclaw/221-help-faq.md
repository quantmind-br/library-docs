...
source: sitemap
fetched_at: 2026-01-30T20:20:45.703049353-03:00
rendered_js: false
word_count: 14074
summary: Provides quick solutions and troubleshooting guidance for common setup and runtime issues with OpenClaw across various environments including local development, VPS deployments, and authentication scenarios.
tags:
    - troubleshooting
    - installation
    - configuration
...
Other useful CLI checks: `openclaw status --all`, `openclaw logs --follow`, `openclaw gateway status`, `openclaw health --verbose`. Quick debug loop: [First 60 seconds if something’s broken](#first-60-seconds-if-somethings-broken). Install docs: [Install](https://docs.openclaw.ai/install), [Installer flags](https://docs.openclaw.ai/install/installer), [Updating](https://docs.openclaw.ai/install/updating).
...
[https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md](https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md) Newest entries are at the top. If the top section is marked **Unreleased**, the next dated section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and **Fixes** (plus docs/other sections when needed).
...
Yes. OpenClaw fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard can run the OAuth flow for you. See [OAuth](https://docs.openclaw.ai/concepts/oauth), [Model providers](https://docs.openclaw.ai/concepts/model-providers), and [Wizard](https://docs.openclaw.ai/start/wizard).
...
Docs: [iMessage](https://docs.openclaw.ai/channels/imessage), [BlueBubbles](https://docs.openclaw.ai/channels/bluebubbles), [Mac remote mode](https://docs.openclaw.ai/platforms/mac/remote).
...
If you are on Windows, **WSL2 is the easiest VM style setup** and has the best tooling compatibility. See [Windows](https://docs.openclaw.ai/platforms/windows), [VPS hosting](https://docs.openclaw.ai/vps). If you are running macOS in a VM, see [macOS VM](https://docs.openclaw.ai/platforms/macos-vm).
...
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-openclaw-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS. **Option B - use a macOS node (no SSH).**  
...
Copilot tokens are read from `COPILOT_GITHUB_TOKEN` (also `GH_TOKEN` / `GITHUB_TOKEN`). See [/concepts/model-providers](https://docs.openclaw.ai/concepts/model-providers) and [/environment](https://docs.openclaw.ai/environment).
...
Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.openclaw.*`, `openclaw-gateway-<profile>.service`, `OpenClaw Gateway (<profile>)`). Full guide: [Multiple gateways](https://docs.openclaw.ai/gateway/multiple-gateways).
...
In the TUI, use `/status` to see the current state. If you expect replies in a chat channel, make sure delivery is enabled (`/deliver on`). Docs: [TUI](https://docs.openclaw.ai/tui), [Slash commands](https://docs.openclaw.ai/tools/slash-commands).
