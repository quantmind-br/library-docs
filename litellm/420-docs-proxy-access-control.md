...
### When to Use Each Key Type[​](#when-to-use-each-key-type "Direct link to When to Use Each Key Type")

Key TypeUse CaseSpend TrackingLifecycle**User-only**Personal API keys for individual developersTracked to the userDeleted when user is deleted**Team (Service Account)**Production apps, CI/CD pipelines, shared servicesTracked to the team onlyPersists even when team members leave**User + Team**User working within a team contextTracked to both user and teamDeleted when user is deleted
...
### Global Proxy Roles[​](#global-proxy-roles "Direct link to Global Proxy Roles")

Role NamePermissions`proxy_admin`Admin over the entire platform. Full control over all organizations, teams, and users`proxy_admin_viewer`Can login, view all keys, view all spend across the platform. **Cannot** create keys/delete keys/add new users`internal_user`Can login, view/create (when allowed by team-specific permissions)/delete their own keys, view their spend. **Cannot** add new users`internal_user_viewer`⚠️ **DEPRECATED** - Use team/org specific roles instead. Can login, view their own keys, view their own spend. **Cannot** create/delete keys, add new users

### Organization/Team Specific Roles[​](#organizationteam-specific-roles "Direct link to Organization/Team Specific Roles")

Role NamePermissions`org_admin`Admin over a specific organization. Can create teams and users within their organization ✨ **Premium Feature**`team_admin`Admin over a specific team. Can manage team members, update team member permissions, and create keys for their team. ✨ **Premium Feature**

## What Can Each Role Do?[​](#what-can-each-role-do "Direct link to What Can Each Role Do?")

Here's what each role can actually do. Think of it like levels of access.

* * *

## Global Proxy Roles[​](#global-proxy-roles-1 "Direct link to Global Proxy Roles")

These roles apply across the entire LiteLLM platform, regardless of organization or team boundaries.

### Proxy Admin - Full Access[​](#proxy-admin---full-access "Direct link to Proxy Admin - Full Access")
...
### Proxy Admin Viewer - Platform-Wide Read Access[​](#proxy-admin-viewer---platform-wide-read-access "Direct link to Proxy Admin Viewer - Platform-Wide Read Access")
...
### Internal User Viewer - Read-Only Access[​](#internal-user-viewer---read-only-access "Direct link to Internal User Viewer - Read-Only Access")
...
## Organization/Team Specific Roles[​](#organizationteam-specific-roles-1 "Direct link to Organization/Team Specific Roles")
...
### Org Admin - Organization Level Access[​](#org-admin---organization-level-access "Direct link to Org Admin - Organization Level Access")
...
### Team Admin - Team Level Access[​](#team-admin---team-level-access "Direct link to Team Admin - Team Level Access")
...
- Onboard a [team-BYOK](https://docs.litellm.ai/docs/proxy/team_model_add) model to LiteLLM (e.g. onboarding a team's finetuned model)
...
### Available Permissions[​](#available-permissions "Direct link to Available Permissions")

PermissionMethodDescription`/key/info`GETView information about virtual keys in the team`/key/health`GETCheck health status of virtual keys in the team`/key/list`GETList all virtual keys belonging to the team`/key/generate`POSTCreate new virtual keys for the team`/key/service-account/generate`POSTCreate service account keys (not tied to a specific user) for the team`/key/update`POSTModify existing virtual keys in the team`/key/delete`POSTDelete virtual keys belonging to the team`/key/regenerate`POSTRegenerate virtual keys in the team`/key/block`POSTBlock virtual keys in the team`/key/unblock`POSTUnblock virtual keys in the team
...
### Common Permission Scenarios[​](#common-permission-scenarios "Direct link to Common Permission Scenarios")
...
### How to Configure Team Member Permissions[​](#how-to-configure-team-member-permissions "Direct link to How to Configure Team Member Permissions")

#### View Current Permissions[​](#view-current-permissions "Direct link to View Current Permissions")
...
#### Update Team Member Permissions[​](#update-team-member-permissions "Direct link to Update Team Member Permissions")
...
### Who Can Configure These Permissions?[​](#who-can-configure-these-permissions "Direct link to Who Can Configure These Permissions?")
...
### Global Proxy Roles[​](#global-proxy-roles-2 "Direct link to Global Proxy Roles")

ActionProxy AdminProxy Admin ViewerInternal UserInternal User Viewer ⚠️ (Deprecated)Create organizations✅❌❌❌Create teams✅❌❌❌Manage all teams✅❌❌❌Create/delete any keys✅❌❌❌Create/delete own keys✅❌✅❌View all platform spend✅✅❌❌View own spend✅✅✅✅View all keys✅✅❌❌View own keys✅✅✅✅Add/remove users✅❌❌❌

> **Note:** The `internal_user_viewer` role is deprecated. Use team/org specific roles for better granular access control.

### Organization/Team Specific Roles[​](#organizationteam-specific-roles-2 "Direct link to Organization/Team Specific Roles")

ActionOrg AdminTeam AdminCreate teams (in their org)✅❌Manage teams in their org✅❌Manage their specific team✅✅Add/remove team members✅ (in their org)✅ (their team only)Update team budgets✅ (in their org)✅ (their team only)Create keys for team members✅ (in their org)✅ (their team only)View organization spend✅ (their org)❌View team spend✅ (in their org)✅ (their team)Create organizations❌❌View all platform spend❌❌

## Onboarding Organizations[​](#onboarding-organizations "Direct link to Onboarding Organizations")

✨ **This is a Premium Feature**

### 1. Creating a new Organization[​](#1-creating-a-new-organization "Direct link to 1. Creating a new Organization")
...
### 2. Adding an `org_admin` to an Organization[​](#2-adding-an-org_admin-to-an-organization "Direct link to 2-adding-an-org_admin-to-an-organization")

Create a user ([ishaan@berri.ai](mailto:ishaan@berri.ai)) as an `org_admin` for the `marketing_department` Organization (from [step 1](#1-creating-a-new-organization))
...
Now a user with user\_id = `ishaan@berri.ai` and role = `org_admin` has been created in the `marketing_department` Organization

Create a Virtual Key for user\_id = `ishaan@berri.ai`. The User can then use the Virtual key for their Organization Admin Operations
...
### 3. `Organization Admin` - Create a Team[​](#3-organization-admin---create-a-team "Direct link to 3-organization-admin---create-a-team")
...
### 4. `Organization Admin` - Add a Team Admin[​](#4-organization-admin---add-a-team-admin "Direct link to 4-organization-admin---add-a-team-admin")
...
### 5. `Team Admin` - Add Team Members[​](#5-team-admin---add-team-members "Direct link to 5-team-admin---add-team-members")
...
### 6. `Team Admin` - Update Team Settings[​](#6-team-admin---update-team-settings "Direct link to 6-team-admin---update-team-settings")
