...
source: sitemap
fetched_at: 2026-05-07T21:17:07.808324922-03:00
rendered_js: false
word_count: 471
summary: This document defines the configuration schema for distributed execution in a machine learning inference system, including parameters for pipeline, tensor, and expert parallelism.
tags:
...
"""Skip validation if the value is `None` when initialisation is delayed."""
    ...
    @classmethod
    def_validate_numa_bind_nodes(cls, value: list[int] | None) -> list[int] | None:
        if value is None:
            return None
        if not value:
    ...
    @classmethod
    def_validate_numa_bind_cpus(cls, value: list[str] | None) -> list[str] | None:
        if value is None:
            return None
        if not value:
    ...
    def_validate_parallel_config(self) -> Self:
    ...
    @property
    defworld_size_across_dp(self) -> int:
"""world_size_across_dp is TPxPPxDP, it is the size of the world
        including data parallelism."""
        return self.world_size * self.data_parallel_size

    @property
    defuse_ubatching(self) -> bool:
        return self.enable_dbo or self.ubatch_size > 1

    @property
    defnum_ubatches(self) -> int:
        return 2 if self.enable_dbo else self.ubatch_size

    @property
    deflocal_engines_only(self) -> bool:
"""
...
"""
    ...
    def_pick_stateless_dp_port(self) -> tuple[int, socket.socket | None]:
"""Return ``(port, listen_socket)`` for DP group init.
        ...
        fromvllm.distributed.utilsimport get_cached_tcp_store_client
    ...
    defstateless_init_dp_group(
        ...
        fromtorch.distributedimport DistNetworkError

        fromvllm.distributed.utilsimport (
    ...
    #
    @property
    defuse_sequence_parallel_moe(self) -> bool:
        return (
            self.all2all_backend
            in (
    ...
    @property
    defuse_batched_dp_moe(self) -> bool:
        return (
            self.all2all_backend
            in (
                "deepep_low_latency",
    ...
    @property
    defnode_rank_within_dp(self) -> int:
        return self.node_rank % self.nnodes_within_dp

    @property
    defnnodes_within_dp(self) -> int:
        if self.nnodes == 1:
            return 1
    ...
    @property
    deflocal_world_size(self) -> int:
        return self.world_size // self.nnodes_within_dp

    @staticmethod
    defhas_unfinished_dp(dp_group: ProcessGroup, has_unfinished: bool) -> bool:
...
"""Combined all-reduce for DP state synchronization.
    ...
    @staticmethod
    defsync_kv_cache_memory_size(dp_group: ProcessGroup, kv_cache_memory: int) -> int:
...
"""
    ...
    def__post_init__(self) -> None:
        ...
        if self.distributed_executor_backend is None and self.world_size_across_dp > 1:
            ...
            fromvllm.v1.executorimport ray_utils
            ...
            if current_platform.is_tpu() and envs.VLLM_XLA_USE_SPMD:
                ...
                if self.placement_group:
                    ...
                    fromrayimport is_initialized as ray_is_initialized

                    if ray_is_initialized():
                        fromray.utilimport get_current_placement_group
    ...
    @model_validator(mode="after")
    def_verify_args(self) -> Self:
        # Lazy import to avoid circular import
        fromvllm.v1.executorimport Executor
