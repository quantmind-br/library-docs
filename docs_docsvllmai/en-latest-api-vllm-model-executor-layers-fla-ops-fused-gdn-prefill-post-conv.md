---
title: fused_gdn_prefill_post_conv - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/fla/ops/fused_gdn_prefill_post_conv/
source: sitemap
fetched_at: 2026-05-07T21:24:18.665073719-03:00
rendered_js: false
word_count: 215
summary: This document describes a Triton-based fused kernel for the GDN prefill operation that combines split, L2 normalization, and gating into a single efficient process.
tags:
    - triton-kernel
    - cuda-optimization
    - deep-learning-ops
...
with a **single Triton kernel** that reads the conv'd mixed\_qkv output and writes directly to q/k/v/g/beta in the target contiguous layout.

## \_fused\_post\_conv\_kernel [¶](#vllm.model_executor.layers.fla.ops.fused_gdn_prefill_post_conv._fused_post_conv_kernel "Permanent link")
...
_fused_post_conv_kernel(
    ...
    K: constexpr,
    V: constexpr,
    APPLY_L2NORM: constexpr,
    L2NORM_EPS: constexpr,
    OUTPUT_G_EXP: constexpr,
    SOFTPLUS_THRESHOLD: constexpr,
    BLOCK_T: constexpr,
    BK: constexpr,
    BV: constexpr,
)
...
(ceil(L, BLOCK\_T), H + HV)

- program\_id(1) in [0, H): Q/K head processing + l2norm
- program\_id(1) in [H, H+HV): V head processing + gating

Source code in `vllm/model_executor/layers/fla/ops/fused_gdn_prefill_post_conv.py`

```
@triton.jit
def_fused_post_conv_kernel(
    # ---- inputs ----
...
## fused\_post\_conv\_prep [¶](#vllm.model_executor.layers.fla.ops.fused_gdn_prefill_post_conv.fused_post_conv_prep "Permanent link")
