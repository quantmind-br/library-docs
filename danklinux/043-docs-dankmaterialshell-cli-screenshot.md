...
source: sitemap
fetched_at: 2026-04-07T21:32:49.88224315-03:00
rendered_js: false
word_count: 155
summary: This document serves as a comprehensive guide and reference for the `dms screenshot` command, detailing its various capture modes, available output formats like PNG and JPEG, and numerous options to control saving location, clipboard interaction, and cursor inclusion.
tags:
    - screenshot
    - command-line
    - wayland
...
FlagShortDescription`--cursor`Include cursor in screenshot: on/off (default: off)`--stdout`Output to stdout (for piping)`--dir``-d`Output directory`--filename`Output filename (auto-generated if empty)`--format``-f`Output format: png, jpg, ppm (default: png)`--quality``-q`JPEG quality 1-100 (default: 90)`--output``-o`Output name for 'output' mode`--no-clipboard`Don't copy to clipboard`--no-file`Don't save to file`--no-notify`Don't show notification`--config``-c`Custom DMS config directory path`--help``-h`Show help
