...
url: https://doc.rust-lang.org/std/convert/trait.AsMut.html#tymethod.as_mut
source: crawler
fetched_at: 2026-05-06T21:23:05.310440173-03:00
rendered_js: false
word_count: 435
...
1.0.0 (const: [unstable](https://github.com/rust-lang/rust/issues/143773 "Tracking issue for const_convert")) · [Source](https://doc.rust-lang.org/src/core/convert/mod.rs.html#372)
...
Note that due to historic reasons, the above currently does not hold generally for all [mutably dereferenceable types](https://doc.rust-lang.org/std/ops/trait.DerefMut.html "trait std::ops::DerefMut"), e.g. `foo.as_mut()` will *not* work the same as `Box::new(foo).as_mut()`. Instead, many smart pointers provide an `as_mut` implementation which simply returns a reference to the [pointed-to value](https://doc.rust-lang.org/std/ops/trait.Deref.html#associatedtype.Target "associated type std::ops::Deref::Target") (but do not perform a cheap reference-to-reference conversion for that value). However, [`AsMut::as_mut`](https://doc.rust-lang.org/std/convert/trait.AsMut.html#tymethod.as_mut "method std::convert::AsMut::as_mut") should not be used for the sole purpose of mutable dereferencing; instead [‘`Deref` coercion’](https://doc.rust-lang.org/std/ops/trait.DerefMut.html#mutable-deref-coercion "trait std::ops::DerefMut") can be used:
...
Types which implement [`DerefMut`](https://doc.rust-lang.org/std/ops/trait.DerefMut.html "trait std::ops::DerefMut") should consider to add an implementation of `AsMut<T>` as follows:
...
1.0.0 · [Source](https://doc.rust-lang.org/src/core/convert/mod.rs.html#375)
