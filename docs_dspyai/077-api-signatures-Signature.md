---
title: Signature - DSPy
url: https://dspy.ai/api/signatures/Signature/
source: sitemap
fetched_at: 2026-01-23T08:02:45.542983346-03:00
rendered_js: false
word_count: 674
summary: This document provides an API reference for the programmatic methods used to manage and manipulate dspy.Signature classes, including adding, removing, and comparing fields.
tags:
    - dspy
    - signatures
    - api-reference
...
### Functions[¶](#dspy.Signature-functions "Permanent link")

#### `append(name, field, type_=None) -> type[Signature]` `classmethod` [¶](#dspy.Signature.append "Permanent link")
...
*required* `type_` `type | None`

Optional explicit type annotation. If `type_` is `None`, the effective type is resolved by `insert`.
...
````
@classmethod
defappend(cls, name, field, type_=None) -> type["Signature"]:
"""Insert a field at the end of the `inputs` or `outputs` section.
...
#### `delete(name) -> type[Signature]` `classmethod` [¶](#dspy.Signature.delete "Permanent link")
...
````
@classmethod
defdelete(cls, name) -> type["Signature"]:
"""Return a new Signature class without the given field.
...
#### `dump_state()` `classmethod` [¶](#dspy.Signature.dump_state "Permanent link")
...
#### `equals(other) -> bool` `classmethod` [¶](#dspy.Signature.equals "Permanent link")
...
#### `insert(index: int, name: str, field, type_: type | None = None) -> type[Signature]` `classmethod` [¶](#dspy.Signature.insert "Permanent link")
...
If `index` falls outside the valid range for the chosen section.
...
````
@classmethod
definsert(cls, index: int, name: str, field, type_: type | None = None) -> type["Signature"]:
"""Insert a field at a specific position among inputs or outputs.
...
#### `load_state(state)` `classmethod` [¶](#dspy.Signature.load_state "Permanent link")
...
#### `prepend(name, field, type_=None) -> type[Signature]` `classmethod` [¶](#dspy.Signature.prepend "Permanent link")
...
````
@classmethod
defprepend(cls, name, field, type_=None) -> type["Signature"]:
"""Insert a field at index 0 of the `inputs` or `outputs` section.
...
#### `with_instructions(instructions: str) -> type[Signature]` `classmethod` [¶](#dspy.Signature.with_instructions "Permanent link")
...
````
@classmethod
defwith_instructions(cls, instructions: str) -> type["Signature"]:
"""Return a new Signature class with identical fields and new instructions.
...
#### `with_updated_fields(name: str, type_: type | None = None, **kwargs: dict[str, Any]) -> type[Signature]` `classmethod` [¶](#dspy.Signature.with_updated_fields "Permanent link")

Create a new Signature class with the updated field information.

Returns a new Signature class with the field, name, updated with fields\[name].json\_schema\_extra\[key] = value.
...
```
@classmethod
defwith_updated_fields(cls, name: str, type_: type | None = None, **kwargs: dict[str, Any]) -> type["Signature"]:
"""Create a new Signature class with the updated field information.
