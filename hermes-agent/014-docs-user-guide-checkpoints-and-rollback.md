...
source: crawler
fetched_at: 2026-04-24T17:00:07.192112019-03:00
rendered_js: false
word_count: 657
summary: This document provides a comprehensive guide to Hermes Agent's automatic checkpointing system, explaining what triggers snapshots, how they are managed in a shadow git repository, and detailing commands for listing, previewing, and restoring project states.
tags:
    - agent-checkpointing
    - hermes-agent
    - git-safety-net
...
CommandDescription`/rollback`List all checkpoints with change stats`/rollback <N>`Restore to checkpoint N (also undoes last chat turn)`/rollback diff <N>`Preview diff between checkpoint N and current state`/rollback <N> <file>`Restore a single file from checkpoint N
