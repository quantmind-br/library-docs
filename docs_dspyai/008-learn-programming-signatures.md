...
source: sitemap
fetched_at: 2026-01-23T08:03:30.04566268-03:00
rendered_js: false
word_count: 687
summary: This document explains how to use DSPy Signatures to declaratively define the input and output behavior of language model modules using inline strings or class-based structures.
tags:
    - dspy
    - signatures
    - prompt-optimization
...
## Why should I use a DSPy Signature?[¶](#why-should-i-use-a-dspy-signature "Permanent link")
...
## **Inline** DSPy Signatures[¶](#inline-dspy-signatures "Permanent link")
...
### Example A: Sentiment Classification[¶](#example-a-sentiment-classification "Permanent link")
...
### Example B: Summarization[¶](#example-b-summarization "Permanent link")
...
## **Class-based** DSPy Signatures[¶](#class-based-dspy-signatures "Permanent link")
...
### Example C: Classification[¶](#example-c-classification "Permanent link")

```
fromtypingimport Literal

classEmotion(dspy.Signature):
"""Classify emotion."""
...
### Example D: A metric that evaluates faithfulness to citations[¶](#example-d-a-metric-that-evaluates-faithfulness-to-citations "Permanent link")

```
classCheckCitationFaithfulness(dspy.Signature):
...
### Example E: Multi-modal image classification[¶](#example-e-multi-modal-image-classification "Permanent link")

```
classDogPictureSignature(dspy.Signature):
...
## Type Resolution in Signatures[¶](#type-resolution-in-signatures "Permanent link")
...
### Working with Custom Types[¶](#working-with-custom-types "Permanent link")

```
# Simple custom type
classQueryResult(pydantic.BaseModel):
    text: str
    score: float
...
classMyContainer:
    classQuery(pydantic.BaseModel):
        text: str
    classScore(pydantic.BaseModel):
        score: float

signature = dspy.Signature("query: MyContainer.Query -> score: MyContainer.Score")
```

## Using signatures to build modules & compiling them[¶](#using-signatures-to-build-modules-compiling-them "Permanent link")
