---
title: Writing PHPUnit tests | Moodle Developer Resources
url: https://moodledev.io/docs/5.2/guides/testing
source: sitemap
fetched_at: 2026-02-17T15:47:14.444937-03:00
rendered_js: false
word_count: 2546
summary: This document outlines the framework for writing and structuring PHPUnit tests in Moodle, including naming conventions, testcase classes, and state management. It also explains how to use data generators to create mock users, courses, and activities for testing purposes.
tags:
    - moodle-development
    - phpunit
    - unit-testing
...
Moodle PHPUnit integration is designed to allow easy creation of new tests. At the start of each test the state is automatically reset to fresh new installation (unless explicitly told not to reset).

## Namespaces[​](#namespaces "Direct link to Namespaces")

All the files under `**/tests` directories are [subject to some simple rules](https://moodledev.io/general/development/policies/codingstyle#namespaces-within-tests) when using namespaces. They apply to test cases, fixtures, generators and, in general, any class within those directories. These are the same rules that are applied to the standard `**/classes` directories.

## Testcase classes[​](#testcase-classes "Direct link to Testcase classes")
...
Creating additional test case classes
...
Moodle versionPHPUnit versionLinksMoodle 5.0+PHPUnit 11[Documentation](https://phpunit.readthedocs.io/en/11.4/assertions.html)Moodle 4.4 - 4.5PHPUnit 9.6[Documentation](https://phpunit.readthedocs.io/en/9.6/assertions.html)Moodle 3.11 - 4.3PHPUnit 9.5[Documentation](https://phpunit.readthedocs.io/en/9.5/assertions.html)Moodle 3.10PHPUnit 8.5[Documentation](https://phpunit.readthedocs.io/en/8.5/assertions.html)Moodle 3.7 - 3.9PHPUnit 7.5[Documentation](https://phpunit.readthedocs.io/en/7.5/assertions.html)Moodle 3.4 - 3.6PHPUnit 6.5[Documentation](https://phpunit.de/manual/6.5/en/assertions.html)

## Writing new test files[​](#writing-new-test-files "Direct link to Writing new test files")
...
- *SHOULD* use the same namespace as the unit under test.

For example:

mod/myplugin/tests/sample\_test.php

```
namespacemod_myplugin;

classsample_testextends\advanced_testcase{
publicfunctiontest_adding(){
$this->assertEquals(2,1+2);
}
}
```

## Inclusion of Moodle library files[​](#inclusion-of-moodle-library-files "Direct link to Inclusion of Moodle library files")

Wherever possible, class autoloading *should* be used.

Where autoloading of classes is not possible:

- classes which are *required* to define the test class *may* be included in the file; but
- fixtures and code under test *should* be loaded in the `setUpBeforeClass()` method.

### Automatic state reset[​](#automatic-state-reset "Direct link to Automatic state reset")
...
If you modify data and *do not* call `$this->resetAfterTest()` then you will receive a warning such as:

> Warning: unexpected database modification, resetting DB state

Demonstration of database reset

```
namespacemod_myplugin;

classtest_somethingextends\advanced_testcase{
publicfunctiontest_deleting(){
global$DB;
...
publicfunctiontest_user_table_was_reset(){
global$DB;
...
Use of generators in data providers
...
Data providers **must not instantiate or create data**. They may only *define* it.

The data providers are called *before* any test has been run and the database will be reset before the test is actually run.

### Creating users[​](#creating-users "Direct link to Creating users")
...
### Creating course categories[​](#creating-course-categories "Direct link to Creating course categories")
...
### Creating courses[​](#creating-courses "Direct link to Creating courses")
...
### Creating activities[​](#creating-activities "Direct link to Creating activities")
...
### Creating cohorts[​](#creating-cohorts "Direct link to Creating cohorts")

```
$cohort=$this->getDataGenerator()->create_cohort();
```

### Simplified user enrolments[​](#simplified-user-enrolments "Direct link to Simplified user enrolments")
...
### Creating scales[​](#creating-scales "Direct link to Creating scales")
...
### Creating roles[​](#creating-roles "Direct link to Creating roles")
...
#### Adding users to groups[​](#adding-users-to-groups "Direct link to Adding users to groups")
...
#### Creating groupings[​](#creating-groupings "Direct link to Creating groupings")
...
#### Adding groups to groupings[​](#adding-groups-to-groupings "Direct link to Adding groups to groupings")
...
#### Creating repository instances[​](#creating-repository-instances "Direct link to Creating repository instances")

```
$this->getDataGenerator()->create_repository($type,$record,$options);
```

#### Creating repository types[​](#creating-repository-types "Direct link to Creating repository types")

```
$this->getDataGenerator()->create_repository_type($type,$record,$options);
```

### Creating grades[​](#creating-grades "Direct link to Creating grades")

#### Grade categories[​](#grade-categories "Direct link to Grade categories")
...
#### Grade items[​](#grade-items "Direct link to Grade items")
...
### Other types of plugin[​](#other-types-of-plugin "Direct link to Other types of plugin")
...
Alternatives to `\component_generator_base`
...
## Long tests[​](#long-tests "Direct link to Long tests")
...
## Large test data[​](#large-test-data "Direct link to Large test data")
...
## Testing sending of messages[​](#testing-sending-of-messages "Direct link to Testing sending of messages")
...
Fetching all messages sent for a component
...
Fetching all messages sent for a specific message type
...
## Testing sending of emails[​](#testing-sending-of-emails "Direct link to Testing sending of emails")
...
## Logstores[​](#logstores "Direct link to Logstores")
...
## Check your coverage[​](#check-your-coverage "Direct link to Check your coverage")
...
### Generating include and exclude configuration[​](#generating-include-and-exclude-configuration "Direct link to Generating include and exclude configuration")
...
All paths specified are relative to the component being tested. For example, when working with `mod_forum` your code will be in `mod/forum`, and its unit tests will be in `mod/forum/tests/`. The coverage file for this would be in `mod/forum/tests/coverage.php` and all paths specified would be relative to `mod/forum`
...
```
returnnewclassextendsphpunit_coverage_info{
...
### Defining covered code[​](#defining-covered-code "Direct link to Defining covered code")
...
### Guidance on defining Coverage[​](#guidance-on-defining-coverage "Direct link to Guidance on defining Coverage")
...
#### Using the `@covers ::methodName` syntax[​](#using-the-covers-methodname-syntax "Direct link to using-the-covers-methodname-syntax")
...
/**
     ...
     * @covers ::some_method
     */
publicfunctiontest_some_method():void{}
}
```

#### Qualification of classes[​](#qualification-of-classes "Direct link to Qualification of classes")
...
### Using the magic `::class` constant[​](#using-the-magic-class-constant "Direct link to using-the-magic-class-constant")
...
Using on classes which do not exist
...
### Keep use of `resetAfterTest` to a minimum[​](#keep-use-of-resetaftertest-to-a-minimum "Direct link to keep-use-of-resetaftertest-to-a-minimum")
...
### Be careful with shared setUp and instance variables[​](#be-careful-with-shared-setup-and-instance-variables "Direct link to Be careful with shared setUp and instance variables")
...
### Make use of the dataProvider functionality[​](#make-use-of-the-dataprovider-functionality "Direct link to Make use of the dataProvider functionality")
 ...
 * @param int $bar
 */
publicfunctiontest_foobar(int$foo,int$bar){
 ...
 */
publicfunctionfoobar_provider():\Generator{
yield'Same numbers'=>[
'foo'=>42,
'bar'=>42,
];
...
## Upgrading unit tests to work with Moodle 4.4 and up (PHPUnit 9.6)[​](#upgrading-unit-tests-to-work-with-moodle-44-and-up-phpunit-96 "Direct link to Upgrading unit tests to work with Moodle 4.4 and up (PHPUnit 9.6)")
