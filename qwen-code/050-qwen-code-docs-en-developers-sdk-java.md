---
title: Qwen Code Java SDK
url: https://qwenlm.github.io/qwen-code-docs/en/developers/sdk-java
source: github_pages
fetched_at: 2026-04-09T09:04:38.408832222-03:00
rendered_js: true
word_count: 1158
summary: This document provides an overview and implementation guide for the Qwen Code Java SDK, detailing its setup via Maven/Gradle, basic usage examples, architectural layers, and advanced callback mechanisms for handling various session events and assistant content.
tags:
    - java-sdk
    - qwen-code
    - api-integration
...
### Session Event Consumers and Assistant Content Consumers[](#session-event-consumers-and-assistant-content-consumers)

The SDK provides two key interfaces for handling events and content from the CLI:

#### SessionEventConsumers Interface[](#sessioneventconsumers-interface)
...
#### AssistantContentConsumers Interface[](#assistantcontentconsumers-interface)
...
#### Relationship Between the Interfaces[](#relationship-between-the-interfaces)
...
- `SessionEventSimpleConsumers` default timeout: 180 seconds (Timeout.TIMEOUT\_180\_SECONDS)
- `AssistantContentSimpleConsumers` default timeout: 60 seconds (Timeout.TIMEOUT\_60\_SECONDS)
...
### Session Control Features[](#session-control-features)
...
- **Thread Naming**: “qwen\_code\_cli-pool-{number}”
...
### Q: Do I need to install the Qwen CLI separately?[](#q-do-i-need-to-install-the-qwen-cli-separately)

A: yes, requires Qwen CLI 0.5.5 or higher.

### Q: What Java versions are supported?[](#q-what-java-versions-are-supported)

A: The SDK requires Java 1.8 or higher.

### Q: How do I handle long-running requests?[](#q-how-do-i-handle-long-running-requests)

A: The SDK includes timeout utilities. You can configure timeouts using the `Timeout` class in `TransportOptions`.

### Q: Why are some tools not executing?[](#q-why-are-some-tools-not-executing)
...
### Q: How do I resume a previous session?[](#q-how-do-i-resume-a-previous-session)

A: Use the `setResumeSessionId()` method in `TransportOptions` to resume a previous session.

### Q: Can I customize the environment for the CLI process?[](#q-can-i-customize-the-environment-for-the-cli-process)

A: Yes, use the `setEnv()` method in `TransportOptions` to pass environment variables to the CLI process.

## License[](#license)

Apache-2.0 - see [LICENSE](https://qwenlm.github.io/qwen-code-docs/en/developers/LICENSE/) for details.