---
title: Tools - DSPy
url: https://dspy.ai/learn/programming/tools/
source: sitemap
fetched_at: 2026-01-23T08:03:32.258755035-03:00
rendered_js: false
word_count: 677
summary: This document explains how to integrate external functions and APIs into DSPy workflows using automated reasoning with ReAct or manual tool management.
tags:
    - dspy
    - tool-use
    - react-agent
...
[](https://github.com/stanfordnlp/dspy/blob/main/docs/docs/learn/programming/tools.md "Edit this page")
...
## Approach 1: Using `dspy.ReAct` (Fully Managed)[¶](#approach-1-using-dspyreact-fully-managed "Permanent link")
...
### Basic Example[¶](#basic-example "Permanent link")
...
### ReAct Features[¶](#react-features "Permanent link")
...
### ReAct Parameters[¶](#react-parameters "Permanent link")
...
### Basic Setup[¶](#basic-setup "Permanent link")

```
importdspy

classToolSignature(dspy.Signature):
"""Signature for manual tool handling."""
    question: str = dspy.InputField()
    tools: list[dspy.Tool] = dspy.InputField()
    outputs: dspy.ToolCalls = dspy.OutputField()

defweather(city: str) -> str:
...
### Understanding `dspy.Tool`[¶](#understanding-dspytool "Permanent link")

The `dspy.Tool` class wraps regular Python functions to make them compatible with DSPy's tool system:

```
defmy_function(param1: str, param2: int = 5) -> str:
...
### Understanding `dspy.ToolCalls`[¶](#understanding-dspytoolcalls "Permanent link")
...
### Adapter Behavior[¶](#adapter-behavior "Permanent link")
...
### Using `acall` for Async Tools[¶](#using-acall-for-async-tools "Permanent link")
...
async defasync_weather(city: str) -> str:
...
### Running Async Tools in Sync Mode[¶](#running-async-tools-in-sync-mode "Permanent link")
...
### 1. Tool Function Design[¶](#1-tool-function-design "Permanent link")
...
```
defgood_tool(city: str, units: str = "celsius") -> str:
"""
...
### 2. Choosing Between ReAct and Manual Handling[¶](#2-choosing-between-react-and-manual-handling "Permanent link")
