---
title: Cross-site request forgery | Moodle Developer Resources
url: https://moodledev.io/general/development/policies/security/crosssite-request-forgery
source: sitemap
fetched_at: 2026-02-17T15:59:36.622897-03:00
rendered_js: false
word_count: 1244
summary: This document explains Cross-Site Request Forgery (CSRF) risks in Moodle and provides instructions on using session keys (sesskey) and proper HTTP methods to secure web applications.
tags:
    - moodle
    - security
    - csrf
...
### Session key (CSRF token)[​](#session-key "Direct link to Session key (CSRF token)")
...
### Use HTTP correctly[​](#use-http-correctly "Direct link to Use HTTP correctly")
...
Use the [Form API](https://moodledev.io/docs/5.2/apis/subsystems/form) whenever possible for handling HTML forms. This automatically checks the sesskey and request method for you.
...
```
$action=newmoodle_url('/admin/tool/do/something.php',['delete'=>$id,'sesskey'=>sesskey()]);
echohtml_writer::link($action,get_string('delete'));
```
...
## Ensure your code does not expose the sesskey inadvertently[​](#ensure-your-code-does-not-expose-the-sesskey-inadvertently "Direct link to Ensure your code does not expose the sesskey inadvertently")

There are various ways that the sesskey could be leaked, and if this happens then it opens the door to types of risks that would be otherwise be mitigated by the sesskey: [https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site\_Request\_Forgery\_Prevention\_Cheat\_Sheet.html](https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html)

There are broadly two ways it could be leaked, in the frontend and in the backend.

### Backend leaking[​](#backend-leaking "Direct link to Backend leaking")
...
### Frontend leaking[​](#frontend-leaking "Direct link to Frontend leaking")
...
### Guidelines for removing the sesskey from visible URLs[​](#guidelines-for-removing-the-sesskey-from-visible-urls "Direct link to Guidelines for removing the sesskey from visible URLs")
...
### Examples of sesskey fixed in core[​](#examples-of-sesskey-fixed-in-core "Direct link to Examples of sesskey fixed in core")
