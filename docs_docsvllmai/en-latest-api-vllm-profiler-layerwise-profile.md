---
title: layerwise_profile - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/profiler/layerwise_profile/
source: sitemap
fetched_at: 2026-05-07T21:34:46.17446452-03:00
rendered_js: false
word_count: 0
summary: This class provides functionality to process and display layer-wise profiling results for machine learning models, including tree-based statistics for CUDA and CPU execution time.
tags:
    - profiling
    - pytorch
    - dataclass
...
```
@dataclass
classLayerwiseProfileResults(profile):
    ...
    defprint_model_table(self, column_widths: dict[str, int] | None = None):
        _column_widths = dict(
    ...
    defprint_summary_table(self, column_widths: dict[str, int] | None = None):
        _column_widths = dict(
    ...
    defexport_model_stats_table_csv(self, filename: str):
        df = pd.DataFrame(
            [asdict(row) for _, row in self._flatten_stats_tree(self._model_stats_tree)]
        )
        df.to_csv(filename)

    defexport_summary_stats_table_csv(self, filename: str):
        df = pd.DataFrame(
            [
                asdict(row)
    ...
    defconvert_stats_to_dict(self) -> dict[str, Any]:
        return {
    ...
    @staticmethod
    def_indent_row_names_based_on_depth(
    ...
    def_build_correlation_map(self):
    ...
    def_get_kineto_gpu_event(self, node: _ModuleTreeNode):
    ...
    def_cumulative_cuda_time(self, node: _ModuleTreeNode):
        "Return cuda time in microseconds"

        def_cumulative_cuda_time_recursive(node: _ModuleTreeNode):
    ...
    def_convert_stats_tree_to_dict(
        self, tree: list[_StatsTreeNode[StatsEntryT]]
    ) -> list[dict[str, Any]]:
