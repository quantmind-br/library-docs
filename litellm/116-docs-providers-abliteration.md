...
PropertyDetailsDescriptionAbliteration provides an OpenAI-compatible `/chat/completions` endpoint.Provider Route on LiteLLM`abliteration/`Link to Provider Doc[Abliteration](https://abliteration.ai)Base URL`https://api.abliteration.ai/v1`Supported Operations[`/chat/completions`](#sample-usage)
...
## Direct API Usage (Bearer Token)[​](#direct-api-usage-bearer-token "Direct link to Direct API Usage (Bearer Token)")

Use the environment variable as a Bearer token against the OpenAI-compatible endpoint: `https://api.abliteration.ai/v1/chat/completions`.
