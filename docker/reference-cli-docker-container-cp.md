...
source: llms
fetched_at: 2026-01-24T14:34:54.181911892-03:00
rendered_js: false
word_count: 627
summary: This document provides a detailed reference for the docker cp command, which allows users to copy files and directories between a container and the local filesystem.
tags:
    - docker-cli
...
DescriptionCopy files/folders between a container and the local filesystemUsage`docker container cp [OPTIONS] CONTAINER:SRC_PATH DEST_PATH|- docker cp [OPTIONS] SRC_PATH|- CONTAINER:DEST_PATH`Aliases
...
OptionDefaultDescription`-a, --archive`Archive mode (copy all uid/gid information)`-L, --follow-link`Always follow symbol link in SRC\_PATH`-q, --quiet`Suppress progress output during copy. Progress output is automatically suppressed if no terminal is attached
