---
title: RLHF Async New APIs - vLLM
url: https://docs.vllm.ai/en/latest/examples/rl/rlhf_async_new_apis/
source: sitemap
fetched_at: 2026-05-07T21:13:44.763199183-03:00
rendered_js: false
word_count: 0
summary: This document provides an example of implementing asynchronous reinforcement learning using vLLM and Ray, focusing on synchronizing model weights between a training actor and an inference engine via NCCL.
tags:
    - vllm
    - ray
...
fromvllm.utils.network_utilsimport get_ip, get_open_port
...
classMyLLM(vllm.AsyncLLMEngine):
...
"""Generate a single request, setting the request pause flag once the
    ...
    async defpause_after_n_tokens(self):
...
"""Ray actor that wraps the training model on a dedicated GPU."""
    ...
    def__init__(self, model_name: str):
        fromvllm.model_executor.layers.batch_invariantimport (
    ...
    defget_master_address_and_port(self):
        return self.master_address, self.port

    defget_weight_metadata(self):
"""Return weight names, dtypes, and shapes for weight transfer."""
    ...
    definit_weight_transfer_group(self, world_size):
"""Initialize the NCCL process group for weight transfer."""
    ...
    defbroadcast_weights(self, packed: bool = True):
"""Broadcast weights to the inference engine."""
    ...
    defgenerate(self, token_ids: list[int], max_new_tokens: int) -> list[int]:
