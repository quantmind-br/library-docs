...
source: sitemap
fetched_at: 2026-05-11T05:49:25.665563045-03:00
rendered_js: false
word_count: 857
summary: This document explains the Prefill and Decoding (PD) Disaggregation architecture in SGLang, which decouples compute-intensive prefill and memory-intensive decode phases to improve inference performance and efficiency.
tags:
    - llm-inference
    - sglang
    - pd-disaggregation
...
VariableDescriptionDefault**`SGLANG_DISAGG_STAGING_BUFFER`**Enable GPU staging buffer for heterogeneous TP KV transfer`False`**`SGLANG_DISAGG_STAGING_BUFFER_SIZE_MB`**Prefill-side per-worker staging buffer size in MB`64`**`SGLANG_DISAGG_STAGING_POOL_SIZE_MB`**Decode-side ring buffer pool total size in MB`4096`
...
By default, NIXL uses the **UCX** backend for KV cache transfers. You can select a different NIXL plugin backend depending on your infrastructure using the environment variable `SGLANG_DISAGGREGATION_NIXL_BACKEND`. Example: `export SGLANG_DISAGGREGATION_NIXL_BACKEND=LIBFABRIC` **Available backends:** UCX (default), LIBFABRIC, or any installed NIXL plugin. Example usage:
