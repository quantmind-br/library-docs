...
url: https://moodledev.io/docs/5.2/apis/plugintypes/theme/layout
source: sitemap
fetched_at: 2026-02-17T15:44:59.676058-03:00
rendered_js: false
word_count: 1655
summary: This document explains how to define and implement page layouts in Moodle themes, covering configuration settings in config.php and the use of Mustache templates for layout files.
tags:
    - moodle-development
    - theme-development
...
Layouts are defined in [`config.php`](https://moodledev.io/docs/5.2/apis/plugintypes/theme#configphp) within `$THEME->layouts`.
...
You can define whatever regions you like. You just need to pick a name for each one. Most themes just use one or both of `side_pre` and `side_post`, which is like 'left side' and 'right side', except in right to left languages, when they are reversed. If you say in [`config.php`](https://moodledev.io/docs/5.2/apis/plugintypes/theme#configphp) that your the layout provides regions called `fred` and `barney`, then you must call `$OUTPUT->blocks_for_region('fred')` and `$OUTPUT->blocks_for_region('barney')` somewhere in the layout file.
...
In the [`config.php`](https://moodledev.io/docs/5.2/apis/plugintypes/theme#configphp) for a theme there is a list of **layouts** which map a page type to a specific PHP page in the layout folder for the theme.
...
[Read about mustache templates](https://moodledev.io/docs/5.2/guides/templates)
