---
title: Activity completion API | Moodle Developer Resources
url: https://moodledev.io/docs/4.1/apis/core/activitycompletion
source: sitemap
fetched_at: 2026-02-17T14:54:35.74489-03:00
rendered_js: false
word_count: 2016
summary: This document provides technical instructions for developers on how to implement and support the completion system in Moodle activities, covering view-based, grade-based, and custom completion rules.
tags:
    - moodle-development
    - activity-completion
    - plugin-development
...
/**
  ...
  * @return null|bool
  */
functionforum_supports(string$feature):bool{
switch($feature){
caseFEATURE_COMPLETION_TRACKS_VIEWS:
returntrue;
caseFEATURE_COMPLETION_HAS_RULES:
returntrue;
default:
returnnull;
}
...
## Completion on view[​](#completion-on-view "Direct link to Completion on view")
...
## Completion on grade[​](#completion-on-grade "Direct link to Completion on grade")
...
### How to implement[​](#how-to-implement-1 "Direct link to How to implement")

In your `[activityname]_supports` function, return true for `FEATURE_GRADE_HAS_GRADE`. No other action is necessary.

### Performance issues[​](#performance-issues-1 "Direct link to Performance issues")
...
## Custom completion rules[​](#custom-completion-rules "Direct link to Custom completion rules")
...
1. Return true for FEATURE\_COMPLETION\_HAS\_RULES in your activity's \_supports function.
2. Add database fields to your activity's main table to store the custom completion settings.
3. Add backup and restore code to back up these fields.
4. Add information about the completion settings to the activities cm\_info object.
5. Add controls to your activity's settings form so that users can select the custom rules, altering these database settings.
6. Add a function that checks the value of these rules (if set).
7. Add function returns descriptions for the completion states.
8. Add code so that whenever the value affecting a rule might change, you inform the completion system.

### Database fields for completion settings[​](#database-fields-for-completion-settings "Direct link to Database fields for completion settings")
...
### Backup and restore for completion fields[​](#backup-and-restore-for-completion-fields "Direct link to Backup and restore for completion fields")
...
```
$forum=newbackup_nested_element('forum',['id'],[
...
### Add information about the completion settings to the activities cm\_info object[​](#add-information-about-the-completion-settings-to-the-activities-cm_info-object "Direct link to Add information about the completion settings to the activities cm_info object")
 ...
 */
functionforum_get_coursemodule_info($coursemodule){
global$DB;
...
### Form changes for completion settings[​](#form-changes-for-completion-settings "Direct link to Form changes for completion settings")
 ...
 */
publicfunctionadd_completion_rules(){
...
- Note that this function must return the top-level element associated with the completion rule. (If there are multiple elements, you can return more than one.)
  ...
  - This is used so that your controls become disabled if automatic completion is not selected. Next, a function for checking whether the user selected this option:
 ...
 */
publicfunctioncompletion_rule_enabled($data){
...
### Completion state function[​](#completion-state-function "Direct link to Completion state function")
...
- **`COMPLETION_OR`** (not currently used) - if multiple conditions are selected, any one of them is good enough to complete the activity. Your function should return:
...
/**
  ...
  */
functionforum_get_completion_state($course,$cm,$userid,$type){
global$CFG,$DB;
...
### Add function returns descriptions for the completion states[​](#add-function-returns-descriptions-for-the-completion-states "Direct link to Add function returns descriptions for the completion states")
 ...
 */
functionmod_forum_get_completion_active_rule_descriptions($cm){
...
### Notifying the completion system[​](#notifying-the-completion-system "Direct link to Notifying the completion system")
...
### Completion Checks in Cron Tasks[​](#completion-checks-in-cron-tasks "Direct link to Completion Checks in Cron Tasks")

If you need to check completion as part of a cron task or another part of Moodle that does not already include the completion\_info class, you will need to include it.
...
### User Docs[​](#user-docs "Direct link to User Docs")
