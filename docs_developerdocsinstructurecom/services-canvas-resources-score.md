---
title: Score | Instructure Developer Documentation Portal
url: https://developerdocs.instructure.com/services/canvas/resources/score
source: sitemap
fetched_at: 2026-02-15T09:04:51.373984-03:00
rendered_js: false
word_count: 672
summary: This document outlines the Score API for the LTI Assignment and Grade Services, detailing how to submit student scores and submission metadata from external tools to Canvas LMS. It describes the Score object structure, the POST endpoint for creating results, and Canvas-specific extensions for submission handling.
tags:
    - lti
...
[Lti::Ims::ScoresController#createarrow-up-right](https://github.com/instructure/canvas-lms/blob/master/app/controllers/lti/ims/scores_controller.rb)
...
The lti\_user\_id or the Canvas user\_id. Returns a 422 if user not found in Canvas or is not a student.
...
Date and time when the score was modified in the tool. Should use ISO8601-formatted date with subsecond precision. Returns a 400 if the timestamp is earlier than the updated\_at time of the Result.
...
(EXTENSION field) flag to indicate that this is a new submission. Defaults to true unless submission\_type is none.
...
(EXTENSION field) permissible values are: none, basic\_lti\_launch, online\_text\_entry, external\_tool, online\_upload, or online\_url. Defaults to external\_tool. Ignored if content\_items are provided.

`https://canvas.instructure.com/lti/submission[submission_data]`

(EXTENSION field) submission data (URL or body text). Only used for submission\_types basic\_lti\_launch, online\_text\_entry, online\_url. Ignored if content\_items are provided.
