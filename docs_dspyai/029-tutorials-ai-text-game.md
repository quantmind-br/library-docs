---
title: Building a Creative Text-Based AI Game - DSPy
url: https://dspy.ai/tutorials/ai_text_game/
source: sitemap
fetched_at: 2026-01-23T08:03:37.018708069-03:00
rendered_js: false
word_count: 177
summary: This tutorial demonstrates how to build a dynamic, AI-powered text adventure game using DSPy's modular programming approach for narrative generation, character interaction, and action resolution.
tags:
    - dspy
    - text-adventure-game
    - ai-storytelling
...
[](https://github.com/stanfordnlp/dspy/blob/main/docs/docs/tutorials/ai_text_game/index.md "Edit this page")
...
```
pipinstalldspyrichtyper
```

## Step 1: Core Game Framework[¶](#step-1-core-game-framework "Permanent link")

```
importdspy
importjson
fromtypingimport Dict, List, Optional, Any
fromdataclassesimport dataclass, field
fromenumimport Enum
importrandom
fromrich.consoleimport Console
fromrich.panelimport Panel
...
classPlayer:
    ...
    defadd_item(self, item: str):
        self.inventory.append(item)
        console.print(f"[green]Added {item} to inventory![/green]")

    defremove_item(self, item: str) -> bool:
        if item in self.inventory:
            self.inventory.remove(item)
            return True
        return False

    defgain_experience(self, amount: int):
...
classGameContext:
    ...
    defadd_flag(self, flag: str, value: bool = True):
        self.game_flags[flag] = value

    defhas_flag(self, flag: str) -> bool:
...
classGameEngine:
    ...
    defsave_game(self, filename: str = "savegame.json"):
"""Save current game state."""
    ...
    defload_game(self, filename: str = "savegame.json") -> bool:
...
## Step 2: AI-Powered Story Generation[¶](#step-2-ai-powered-story-generation "Permanent link")

```
classStoryGenerator(dspy.Signature):
...
classDialogueGenerator(dspy.Signature):
...
classActionResolver(dspy.Signature):
...
"""Main AI module for game logic and narrative."""
    ...
    defgenerate_scene(self, player: Player, context: GameContext, recent_actions: str = "") -> Dict:
"""Generate current scene description and options."""
    ...
    defhandle_dialogue(self, npc_name: str, player_input: str, context: GameContext) -> Dict:
"""Handle conversation with NPCs."""
    ...
    defresolve_action(self, action: str, player: Player, context: GameContext) -> Dict:
"""Resolve player actions and determine outcomes."""
...
## Step 3: Game Interface and Interaction[¶](#step-3-game-interface-and-interaction "Permanent link")

```
defdisplay_game_header():
"""Display the game header."""
    header = Text("🏰 MYSTIC REALM ADVENTURE 🏰", style="bold magenta")
    console.print(Panel(header, style="bright_blue"))

defdisplay_player_status(player: Player):
...
defdisplay_location(context: GameContext, scene: Dict):
...
defshow_inventory(player: Player):
...
## Step 4: Main Game Loop[¶](#step-4-main-game-loop "Permanent link")

```
defcreate_new_character():
...
## Example Gameplay[¶](#example-gameplay "Permanent link")
