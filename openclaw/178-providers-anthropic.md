...
source: sitemap
fetched_at: 2026-01-30T20:25:51.190021751-03:00
rendered_js: false
word_count: 362
summary: This document explains how to authenticate with the Anthropic Claude API using either an API key or a setup-token within the OpenClaw framework, including setup instructions and troubleshooting steps.
tags:
    - anthropic
    - claude
    - api-key
...
OpenClaw does **not** override Anthropic’s default cache TTL unless you set it. This is **API-only**; subscription auth does not honor TTL settings. To set the TTL per model, use `cacheControlTtl` in the model `params`:
...
OpenClaw includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API requests; keep it if you override provider headers (see [/gateway/configuration](https://docs.openclaw.ai/gateway/configuration)).
