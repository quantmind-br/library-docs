---
title: protocol - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/entrypoints/openai/chat_completion/protocol/
source: sitemap
fetched_at: 2026-05-07T21:19:52.733090869-03:00
rendered_js: false
word_count: 30
summary: This document defines the ChatCompletionRequest data model, outlining the parameters and schema used for chat completion requests within the vLLM serving framework.
tags:
    - vllm
    - chat-completion
    - api-schema
...
```
classChatCompletionRequest(OpenAIBaseModel):
    ...
    @classmethod
    def_materialize_tool_calls_before(cls, data: Any) -> Any:
"""Eagerly convert tool_calls generators/iterators to lists.
    ...
    def_materialize_tool_calls_after(self) -> "ChatCompletionRequest":
...
"""CAUTION: Should only be set by ``ToolParser.adjust_request``."""
    ...
    @model_validator(mode="before")
    @classmethod
    defvalidate_response_format(cls, data):
    ...
    @model_validator(mode="before")
    @classmethod
    defvalidate_stream_options(cls, data):
    ...
    @model_validator(mode="before")
    @classmethod
    defcheck_structured_outputs_count(cls, data):
    ...
    @model_validator(mode="before")
    @classmethod
    defcheck_generation_prompt(cls, data):
    ...
    @model_validator(mode="before")
    @classmethod
    defcheck_system_message_content_type(cls, data):
