---
title: Deploy On Kubernetes - SGLang Documentation
url: https://docs.sglang.io/docs/references/multi_node_deployment/deploy_on_k8s
source: sitemap
fetched_at: 2026-05-11T05:48:18.768457822-03:00
rendered_js: false
word_count: 572
summary: This document provides instructions for deploying a distributed SGLang inference service on a Kubernetes cluster using LeaderWorkerSet (LWS) with RDMA RoCE networking.
tags:
    - sglang
    - kubernetes
    - lws
...
For the basic example documentation, refer to [Deploy Distributed Inference Service with SGLang and LWS on GPUs](https://github.com/kubernetes-sigs/lws/tree/main/docs/examples/sglang). However, that document only covers the basic NCCL socket mode. In this section, we’ll make some simple modifications to adapt the setup to the RDMA scenario.
...
This should resolve most NCCL-related issues. ***Notice: If you find that NCCL\_DEBUG=TRACE is not effective in the container environment, but the process is stuck or you encounter hard-to-diagnose issues, try switching to a different container image. Some images may not handle standard error output properly.***
