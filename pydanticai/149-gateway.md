...
source: sitemap
fetched_at: 2026-01-22T22:23:28.00145802-03:00
rendered_js: false
word_count: 936
summary: This document introduces the Pydantic AI Gateway, a unified interface for managing multiple LLM providers with features like centralized API key management, cost tracking, and observability. It provides instructions for account setup, provider configuration, and integration with various AI SDKs.
tags:
    - pydantic-ai
    - ai-gateway
    - llm-infrastructure
...
**Provider** **API Format** **Example Model** OpenAI `openai` `gateway/openai:gpt-5` Anthropic `anthropic` `gateway/anthropic:claude-sonnet-4-5` Google Vertex `google-vertex` `gateway/google-vertex:gemini-2.5-flash` Groq `groq` `gateway/groq:openai/gpt-oss-120b` AWS Bedrock `bedrock` `gateway/bedrock:amazon.nova-micro-v1:0`
...
frompydantic_ai.providers.gatewayimport gateway_provider
...
```
exportANTHROPIC_BASE_URL="https://gateway.pydantic.dev/proxy/anthropic"
exportANTHROPIC_AUTH_TOKEN="YOUR_PYDANTIC_AI_GATEWAY_API_KEY"
```
