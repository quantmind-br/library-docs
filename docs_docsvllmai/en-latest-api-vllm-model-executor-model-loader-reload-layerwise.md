---
title: layerwise - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/model_loader/reload/layerwise/
source: sitemap
fetched_at: 2026-05-07T21:28:43.225845958-03:00
rendered_js: false
word_count: 429
summary: This document describes internal utility functions for handling layerwise model weight loading, quantization, and reloading in the vLLM executor, specifically focusing on kernel tensor preservation and layer finalization.
tags:
    - vllm
    - model-loading
    - weight-quantization
...
## \_copy\_and\_restore\_kernel\_tensors [¶](#vllm.model_executor.model_loader.reload.layerwise._copy_and_restore_kernel_tensors "Permanent link")
...
```
def_copy_and_restore_kernel_tensors(layer: torch.nn.Module, info: LayerReloadingInfo):
...
## \_get\_original\_loader [¶](#vllm.model_executor.model_loader.reload.layerwise._get_original_loader "Permanent link")
...
```
def_get_original_loader(tensor: torch.Tensor) -> Callable:
"""Return the weight loader with any layerwise wrappers removed"""
...
## \_layerwise\_process [¶](#vllm.model_executor.model_loader.reload.layerwise._layerwise_process "Permanent link")
...
```
def_layerwise_process(layer: torch.nn.Module, info: LayerReloadingInfo):
"""
    Finalize layer loading after all weights have been buffered.
...
## \_reload\_attention\_scales [¶](#vllm.model_executor.model_loader.reload.layerwise._reload_attention_scales "Permanent link")
...
Load and process attention scale weights (k\_scale, v\_scale, etc.) during reload.

Assumes dtype/shapes of attention tensors do not change during processing, since we use .data.copy\_() to preserve kernel tensor references.

Source code in `vllm/model_executor/model_loader/reload/layerwise.py`

```
def_reload_attention_scales(layer: torch.nn.Module, info: LayerReloadingInfo) -> None:
...
## finalize\_layerwise\_processing [¶](#vllm.model_executor.model_loader.reload.layerwise.finalize_layerwise_processing "Permanent link")
...
## get\_layerwise\_info [¶](#vllm.model_executor.model_loader.reload.layerwise.get_layerwise_info "Permanent link")
...
```
defget_layerwise_info(layer: torch.nn.Module) -> LayerReloadingInfo:
"""
...
## initialize\_layerwise\_reload [¶](#vllm.model_executor.model_loader.reload.layerwise.initialize_layerwise_reload "Permanent link")
...
## initialize\_online\_processing [¶](#vllm.model_executor.model_loader.reload.layerwise.initialize_online_processing "Permanent link")
...
```
definitialize_online_processing(layer: torch.nn.Module):
"""
...
## make\_online\_process\_loader [¶](#vllm.model_executor.model_loader.reload.layerwise.make_online_process_loader "Permanent link")
...
```
defmake_online_process_loader(layer: torch.nn.Module, param_name: str) -> Callable:
"""Create a wrapped weight loader that defers processing."""
    info = get_layerwise_info(layer)
    ...
    defonline_process_loader(*args, **kwargs):
        if not info.can_load():
