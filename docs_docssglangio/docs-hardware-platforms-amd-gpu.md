---
title: AMD GPUs - SGLang Documentation
url: https://docs.sglang.io/docs/hardware-platforms/amd_gpu
source: sitemap
fetched_at: 2026-05-11T05:48:48.947131883-03:00
rendered_js: false
word_count: 585
summary: This document provides a comprehensive guide for configuring systems, installing SGLang, and deploying large language models on AMD GPU hardware.
tags:
    - sglang
    - amd-gpu
    - rocm
...
**NOTE:** We strongly recommend reading these docs and guides entirely to fully utilize your system. Below are a few key settings to confirm or enable for SGLang:
...
You can automate or verify this change using [this helpful script](https://github.com/ROCm/triton/blob/rocm_env/scripts/amd/env_check.sh). Again, please go through the entire documentation to confirm your system is using the recommended configuration.
...
The docker images are available on Docker Hub at [lmsysorg/sglang](https://hub.docker.com/r/lmsysorg/sglang/tags), built from [rocm.Dockerfile](https://github.com/sgl-project/sglang/tree/main/docker). The steps below show how to build and use an image.
...
3. Launch the server. **NOTE:** Replace `<secret>` below with your [huggingface hub token](https://huggingface.co/docs/hub/en/security-tokens).
   ...
   drun -p 30000:30000 \
       ...
       --port 30000
   ```
4. To verify the utility, you can run a benchmark in another terminal or refer to [other docs](https://docs.sglang.io/docs/basic_usage/openai_api_completions) to send requests to the engine.
...
The [Quantization documentation](https://docs.sglang.io/docs/advanced_features/quantization#platform-compatibility) has a full compatibility matrix. The short version: FP8, AWQ, MXFP4, W8A8, GPTQ, compressed-tensors, Quark, and **petit\_nvfp4** (NVFP4 on ROCm via [Petit](https://github.com/causalflow-ai/petit-kernel)) all work on AMD. Methods that depend on Marlin or NVIDIA-specific kernels (`awq_marlin`, `gptq_marlin`, `gguf`, `modelopt_fp8`, `modelopt_fp4`) do not. A few things to keep in mind:
