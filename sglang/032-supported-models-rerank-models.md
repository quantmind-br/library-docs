---
title: Rerank Models — SGLang
url: https://docs.sglang.io/supported_models/rerank_models.html
source: crawler
fetched_at: 2026-02-04T08:47:05.926831571-03:00
rendered_js: false
word_count: 606
summary: This document provides instructions for deploying and using various rerank models within the SGLang framework, covering cross-encoder, decoder-only, and multimodal architectures. It details server configuration commands, API request parameters, and response formats for optimizing document retrieval accuracy.
tags:
    - sglang
    - rerank-models
    - cross-encoder
...
## Rerank Models[#](#rerank-models "Link to this heading")
...
## Supported rerank models[#](#supported-rerank-models "Link to this heading")

## Cross-Encoder Rerank (embedding runner)[#](#cross-encoder-rerank-embedding-runner "Link to this heading")

### Launch Command[#](#launch-command "Link to this heading")

```
python3-msglang.launch_server\
--model-pathBAAI/bge-reranker-v2-m3\
--host0.0.0.0\
--disable-radix-cache\
...
### Example Client Request[#](#example-client-request "Link to this heading")
...
## Qwen3-Reranker (decoder-only yes/no rerank)[#](#qwen3-reranker-decoder-only-yes-no-rerank "Link to this heading")

### Launch Command[#](#id1 "Link to this heading")

```
python3-msglang.launch_server\
--model-pathQwen/Qwen3-Reranker-0.6B\
--trust-remote-code\
--disable-radix-cache\
--host0.0.0.0\
--port8001\
--chat-templateexamples/chat_template/qwen3_reranker.jinja
```
...
### Example Client Request (supports optional instruct, top\_n, and return\_documents)[#](#example-client-request-supports-optional-instruct-top-n-and-return-documents "Link to this heading")

```
curl-XPOSThttp://127.0.0.1:8001/v1/rerank\
...
## Qwen3-VL-Reranker (multimodal decoder-only rerank)[#](#qwen3-vl-reranker-multimodal-decoder-only-rerank "Link to this heading")
...
### Launch Command[#](#id2 "Link to this heading")

```
python3-msglang.launch_server\
--model-pathQwen/Qwen3-VL-Reranker-2B\
--trust-remote-code\
--disable-radix-cache\
--host0.0.0.0\
--port30000\
--chat-templateexamples/chat_template/qwen3_vl_reranker.jinja
```
...
### Text-Only Reranking (backward compatible)[#](#text-only-reranking-backward-compatible "Link to this heading")
...
### Image Reranking (text query, image/mixed documents)[#](#image-reranking-text-query-image-mixed-documents "Link to this heading")
...
### Multimodal Query Reranking (query with image)[#](#multimodal-query-reranking-query-with-image "Link to this heading")
...
### Request Parameters (Multimodal)[#](#request-parameters-multimodal "Link to this heading")
...
### Common Pitfalls[#](#id3 "Link to this heading")
