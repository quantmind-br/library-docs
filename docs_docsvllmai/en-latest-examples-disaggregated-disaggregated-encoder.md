...
source: sitemap
fetched_at: 2026-05-07T21:12:37.81624965-03:00
rendered_js: false
word_count: 230
summary: This document describes a Python proxy implementation that routes multimodal chat completion requests by fanning out image and audio processing to specialized encoder clusters before forwarding the final request to a decode server.
tags:
    - vllm
    - proxy
    - multimodal
...
defextract_mm_items(request_data: dict) -> list[dict]:
"""
...
async deffanout_encoder_primer(
    orig_request: dict,
    e_urls: list[str],
    req_id: str,
) -> None:
...
async defprocess_prefill_stage(
    req_data: dict,
    p_url: str,
    req_id: str,
) -> dict:
...
@app.middleware("http")
async deflog_requests(request: Request, call_next):
...
async defforward_non_stream(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Proxy error: {str(e)}") frome


async defforward_stream(
    req_data: dict, req_id: str, e_urls: list[str], p_url: str, d_url: str
) -> AsyncIterator[str]:
    try:
        # Step 1: Process through Encoder instance (if has MM input)
...
async defchat_completions(request: Request):
    try:
...
async def_profile_cmd(cmd: str, payload: dict, e_url: str, p_url: str, d_url: str):
"""
...
async defstart_profile(request: Request):
...
async defstop_profile(request: Request):
    body = await request.json()
    # TODO: handle multi urls properly
    e_url = random.choice(app.state.e_urls)
    p_url = random.choice(app.state.p_urls) if app.state.p_urls else None
