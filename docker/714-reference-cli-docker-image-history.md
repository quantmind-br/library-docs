...
source: llms
fetched_at: 2026-01-24T14:36:16.72117816-03:00
rendered_js: false
word_count: 395
summary: This document provides a reference for the `docker image history` command, which displays the layer history and creation details of a specific Docker image. It explains available options such as output formatting and platform selection to inspect image metadata and build steps.
tags:
    - docker-cli
    - image-management
    - docker-history
...
DescriptionShow the history of an imageUsage`docker image history [OPTIONS] IMAGE`Aliases
...
PlaceholderDescription`.ID`Image ID`.CreatedSince`Elapsed time since the image was created if `--human=true`, otherwise timestamp of when image was created`.CreatedAt`Timestamp of when image was created`.CreatedBy`Command that was used to create the image`.Size`Image disk size`.Comment`Comment for image
