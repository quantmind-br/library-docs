...
source: sitemap
fetched_at: 2026-01-23T08:03:19.143842813-03:00
rendered_js: false
word_count: 895
summary: This document describes the implementation of DSPy Assertions and Suggestions for enforcing computational constraints and enabling model self-refinement via backtracking. It explains the mechanics of dynamic signature modification and the differences between hard assertions and soft suggestions in LM pipelines.
tags:
    - dspy
    - assertions
    - backtracking
...
[](https://github.com/stanfordnlp/dspy/blob/main/docs/docs/learn/programming/7-assertions.md "Edit this page")
...
### dspy.Assert and dspy.Suggest API[¶](#dspyassert-and-dspysuggest-api "Permanent link")
...
## Use Case: Including Assertions in DSPy Programs[¶](#use-case-including-assertions-in-dspy-programs "Permanent link")
...
classSimplifiedBaleen(dspy.Module):
    def__init__(self, passages_per_hop=2, max_hops=2):
        super().__init__()
...
defvalidate_query_distinction_local(previous_queries, query):
...
```
classSimplifiedBaleenAssertions(dspy.Module):
    def__init__(self, passages_per_hop=2, max_hops=2):
        super().__init__()
        self.generate_query = [dspy.ChainOfThought(GenerateSearchQuery) for _ in range(max_hops)]
        self.retrieve = dspy.Retrieve(k=passages_per_hop)
...
```
fromdspy.primitives.assertionsimport assert_transform_module, backtrack_handler
...
## Assertion-Driven Optimizations[¶](#assertion-driven-optimizations "Permanent link")
