---
title: CLI Commands | goose
url: https://block.github.io/goose/docs/guides/goose-cli-commands
source: github_pages
fetched_at: 2026-01-22T22:13:51.679372663-03:00
rendered_js: true
word_count: 2354
summary: This guide provides a comprehensive overview of the goose command-line interface, detailing flag naming conventions and commands for session lifecycle management and configuration.
tags:
    - goose-cli
...
## Flag Naming Conventions[​](#flag-naming-conventions "Direct link to Flag Naming Conventions")
...
Starting with version 1.10.0, goose uses a SQLite database (`sessions.db`) instead of individual `.jsonl` files. Your existing sessions are automatically imported to the database. Legacy `.jsonl` files remain on disk but are no longer managed by goose.
...
- **`--session-id <session_id>`** : Specify a session by its ID (e.g., '20251108\_1')
...
#### session remove \[options][​](#session-remove-options "Direct link to session remove [options]")
...
#### session export \[options][​](#session-export-options "Direct link to session export [options]")
...
#### session diagnostics \[options][​](#session-diagnostics-options "Direct link to session diagnostics [options]")
...
### Task Execution[​](#task-execution "Direct link to Task Execution")

#### run \[options][​](#run-options "Direct link to run [options]")

Execute commands from an instruction file or stdin. Check out the [full guide](https://block.github.io/goose/docs/guides/running-tasks) for more info.
...
Used to evaluate system-configuration across a range of practical tasks. See the [detailed guide](https://block.github.io/goose/docs/tutorials/benchmarking) for more information.
...
Automate recipes by running them on a [schedule](https://block.github.io/goose/docs/guides/recipes/session-recipes#schedule-recipe).
...
This command is automatically invoked by ACP-compatible clients and is not typically run directly by users. The client manages the lifecycle of the `goose acp` process. See [Using goose in ACP Clients](https://block.github.io/goose/docs/guides/acp-clients) for details.
...
Start working on your last project or create a new one. For detailed usage examples and workflows, see [Managing Projects Guide](https://block.github.io/goose/docs/guides/managing-projects).
...
#### @goose / @g[​](#goose--g "Direct link to @goose / @g")

Ask goose questions directly from your shell prompt, with command history included in the context. These aliases are created when you set up [terminal integration](https://block.github.io/goose/docs/guides/terminal-integration).
...
## Interactive Session Features[​](#interactive-session-features "Direct link to Interactive Session Features")
...
- **`/mode <name>`** - Set the goose mode to use ('auto', 'approve', 'chat', 'smart\_approve')
...
You can also create [custom slash commands for running recipes](https://block.github.io/goose/docs/guides/context-engineering/slash-commands) in goose Desktop or the CLI.
...
- The theme setting is saved to the [configuration file](https://block.github.io/goose/docs/guides/config-files) as `GOOSE_CLI_THEME` and persists between sessions
- The saved configuration can be overridden for the session using the `GOOSE_CLI_THEME` [environment variable](https://block.github.io/goose/docs/guides/environment-variables#session-management)
...
## Navigation and Controls[​](#navigation-and-controls "Direct link to Navigation and Controls")
...
### Command History Search[​](#command-history-search "Direct link to Command History Search")

The `Ctrl+R` shortcut provides interactive search through your stored CLI [command history](https://block.github.io/goose/docs/guides/logs#command-history). This feature makes it easy to find and reuse recent commands without retyping them. When you type a search term, goose searches backwards through your history for matches.
