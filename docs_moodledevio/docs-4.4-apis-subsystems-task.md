...
url: https://moodledev.io/docs/4.4/apis/subsystems/task
source: sitemap
fetched_at: 2026-02-17T15:04:58.490429-03:00
rendered_js: false
word_count: 1077
summary: This document explains the Moodle Tasks API for background processing, detailing the implementation and management of scheduled and adhoc tasks along with error handling and security considerations.
tags:
    - moodle
    - tasks-api
    - scheduled-tasks
...
See the [scheduled tasks](https://moodledev.io/docs/4.4/apis/subsystems/task/scheduled) page for more information on creating scheduled tasks.
...
Historically many Moodle APIs have used static caches. Whilst many of these have been replaced by the [Moodle Universal Cache](https://moodledev.io/docs/4.4/apis/subsystems/muc), which can be cleared between runs, it is not possible to guarantee that this is always the case.
...
In the case of an adhoc task, you can call the [`set_userid`](https://moodledev.io/docs/4.4/apis/subsystems/task/adhoc#running-as-a-specific-user) function when queueing the task.
...
Since Moodle 3.5 it is safe to use the [Output API](https://moodledev.io/docs/4.4/apis/subsystems/output) in cron tasks. Prior to this there may be cases where the Output API has not been initialised.
