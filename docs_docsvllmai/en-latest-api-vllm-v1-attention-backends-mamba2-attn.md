---
title: mamba2_attn - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/attention/backends/mamba2_attn/
source: sitemap
fetched_at: 2026-05-07T21:39:14.565635844-03:00
rendered_js: false
word_count: 122
summary: This document describes a utility function that generates chunk-aligned, variable-length metadata tensors required for Mamba2 SSD attention kernels.
tags:
    - vllm
    - mamba2
    - attention-kernels
...
## vllm.v1.attention.backends.mamba2\_attn [¶](#vllm.v1.attention.backends.mamba2_attn "Permanent link")

Build chunk-aligned, variable-length metadata used by Mamba2 SSD kernels.

Given per-sequence cumulative token starts `query_start_loc` of shape \[B+1] and a physical `chunk_size`, returns three tensors on the same device: - cu\_chunk\_seqlens: (nchunks+1,) int32 exclusive prefix-sum of logical-chunk lengths (each logical chunk never crosses a sequence or physical-chunk boundary). - last\_chunk\_indices: (B,) int32 index of the last logical chunk for each sequence (=-1 for empty sequences). - seq\_idx\_chunks: (nchunks,) int32 sequence index for each logical chunk in order.
...
```
defcompute_varlen_chunk_metadata(
