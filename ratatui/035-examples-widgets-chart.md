...
source: crawler
fetched_at: 2026-02-01T21:12:50.587182144-03:00
rendered_js: false
word_count: 716
summary: This document demonstrates how to implement and configure various terminal-based data visualizations using the Ratatui Chart widget, including animated line charts, bar charts, and scatter plots.
tags:
    - ratatui
    - rust
    - terminal-ui
...
cdratatui
cargorun--example=chart--features=crossterm
```
...
impl SinSignal {
constfnnew(interval: f64, period: f64, scale: f64) ->Self {
Self {
x:0.0,
interval,
period,
...
impl App {
fnnew() ->Self {
letmutsignal1= SinSignal::new(0.2, 3.0, 18.0);
letmutsignal2= SinSignal::new(0.1, 2.0, 10.0);
letdata1=signal1.by_ref().take(200).collect::<Vec<(f64, f64)>>();
letdata2=signal2.by_ref().take(200).collect::<Vec<(f64, f64)>>();
Self {
signal1,
data1,
signal2,
...
fnrender_animated_chart(&self, frame:&mut Frame, area: Rect) {
letx_labels=vec![
Span::styled(
...
fnrender_barchart(frame:&mut Frame, bar_chart: Rect) {
letdataset= Dataset::default()
...
letchart= Chart::new(vec![dataset])
...
fnrender_line_chart(frame:&mut Frame, area: Rect) {
letdatasets=vec![Dataset::default()
...
fnrender_scatter(frame:&mut Frame, area: Rect) {
letdatasets=vec![
Dataset::default()
.name("Heavy")
.marker(Marker::Dot)
...
constHEAVY_PAYLOAD_DATA: [(f64, f64); 9] = [
(1965., 8200.),
(1967., 5400.),
(1981., 65400.),
(1989., 30800.),
...
constMEDIUM_PAYLOAD_DATA: [(f64, f64); 29] = [
(1963., 29500.),
(1964., 30600.),
(1965., 177_900.),
(1965., 21000.),
...
constSMALL_PAYLOAD_DATA: [(f64, f64); 23] = [
(1961., 118_500.),
(1962., 14900.),
(1975., 21400.),
(1980., 32800.),
