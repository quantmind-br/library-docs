---
title: Hyperparameter Tuning — SGLang
url: https://docs.sglang.io/advanced_features/hyperparameter_tuning.html
source: crawler
fetched_at: 2026-02-04T08:46:45.245441097-03:00
rendered_js: false
word_count: 759
summary: This document provides instructions on optimizing hyperparameter settings to achieve high throughput and avoid memory errors during offline batch inference with SGLang.
tags:
...
## Hyperparameter Tuning[#](#hyperparameter-tuning "Link to this heading")

## Achieving high throughput for offline batch inference[#](#achieving-high-throughput-for-offline-batch-inference "Link to this heading")
...
### Adjust the request submission speed to control `#queue-req`[#](#adjust-the-request-submission-speed-to-control-queue-req "Link to this heading")
...
### Achieve a high `token usage`[#](#achieve-a-high-token-usage "Link to this heading")
...
### Avoid out-of-memory errors by tuning `--chunked-prefill-size`, `--mem-fraction-static`, and `--max-running-requests`[#](#avoid-out-of-memory-errors-by-tuning-chunked-prefill-size-mem-fraction-static-and-max-running-requests "Link to this heading")
...
### Tune `--cuda-graph-max-bs`[#](#tune-cuda-graph-max-bs "Link to this heading")
...
### Tune `--dp-size` and `--tp-size`[#](#tune-dp-size-and-tp-size "Link to this heading")

Data parallelism is better for throughput. When there is enough GPU memory, always favor data parallelism for throughput. Refer to [SGLang Model Gateway (former Router)](https://docs.sglang.io/advanced_features/sgl_model_gateway.html) for a better data parallelism rather than using `dp_size` parameter.

### Try other options[#](#try-other-options "Link to this heading")
