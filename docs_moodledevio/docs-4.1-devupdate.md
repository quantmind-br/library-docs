---
title: Moodle 4.0 developer update | Moodle Developer Resources
url: https://moodledev.io/docs/4.1/devupdate
source: sitemap
fetched_at: 2026-02-17T14:57:06.146394-03:00
rendered_js: false
word_count: 5227
summary: This document outlines the architectural and API changes introduced in Moodle 4.0 to help developers update their plugins and themes for the new navigation hierarchy and user experience.
tags:
    - moodle-4-0
    - navigation-api
    - plugin-development
...
You can learn more about **Updating your Plugins for Moodle 4.0** from the [Update Plugins for Moodle 4.0](https://moodle.academy/course/view.php?id=66) course on Moodle Academy.

See [Question bank plugins](https://moodledev.io/docs/4.1/apis/plugintypes/qbank).

## Navigation changes[​](#navigation-changes "Direct link to Navigation changes")
...
### Primary navigation[​](#primary-navigation "Direct link to Primary navigation")
...
### Secondary navigation[​](#secondary-navigation "Direct link to Secondary navigation")
...
#### Adding items to the navigation[​](#adding-items-to-the-navigation "Direct link to Adding items to the navigation")
...
#### Changing the order of tabs[​](#changing-the-order-of-tabs "Direct link to Changing the order of tabs")
...
### Tertiary navigation[​](#tertiary-navigation "Direct link to Tertiary navigation")
...
### New API functions[​](#new-api-functions "Direct link to New API functions")

#### Page API[​](#page-api "Direct link to Page API")
...
#### Navigation library[​](#navigation-library "Direct link to Navigation library")
...
##### Theme updates[​](#theme-updates "Direct link to Theme updates")
...
- To leverage the activity\_header, it's data needs to be exported and included into the base template :
  
  - `headercontent` being the array element that contains the exported activity\_header data
...
- It is recommended to transition towards the secondary/tertiary navigation hierarchy to reduce user cognitive load and with a logical separation of components
  ...
  - Secondary navigation can be added to the templates by following the example [https://github.com/moodle/moodle/blob/main/theme/boost/templates/columns2.mustache#L64-L68](https://github.com/moodle/moodle/blob/main/theme/boost/templates/columns2.mustache#L64-L68) This leverages the secondary navigation class to generate it's content.
- Flat navigation classes have been marked for deprecation. Themes that leverage the flat\_navigation will need to make the following changes in their plugins in order to use it
...
- In order to reintroduce the settings cog in the templates, you can introduce the following:
  ...
  ```
  <divid="region-main-settings-menu"class="d-print-none {{#hasblocks}}has-blocks{{/hasblocks}}">
...
### Enabling the Component Library[​](#enabling-the-component-library "Direct link to Enabling the Component Library")

Component library pages are written in the markdown language. These pages need to be compiled to HTML pages before the Component Library is visible. To compile the pages the server running Moodle needs to have [nodeJS and Grunt tools](https://moodledev.io/general/development/tools/nodejs).
...
### Documenting new UI Components[​](#documenting-new-ui-components "Direct link to Documenting new UI Components")
...
## Theme changes[​](#theme-changes "Direct link to Theme changes")

### Edit switch[​](#edit-switch "Direct link to Edit switch")
...
### User initials as profile picture placeholder[​](#user-initials-as-profile-picture-placeholder "Direct link to User initials as profile picture placeholder")
...
### Removal of back to top link[​](#removal-of-back-to-top-link "Direct link to Removal of back to top link")
...
### Styling changes[​](#styling-changes "Direct link to Styling changes")
...
### New layout page[​](#new-layout-page "Direct link to New layout page")

Theme boost now uses the `drawers.php` layout for the course index and blocks.

## Question bank changes[​](#question-bank-changes "Direct link to Question bank changes")
...
### Question type plugins[​](#question-type-plugins "Direct link to Question type plugins")
...
## The course format system[​](#the-course-format-system "Direct link to The course format system")
...
### Mandatory renderer in course formats[​](#mandatory-renderer-in-course-formats "Direct link to Mandatory renderer in course formats")
...
### New format base class[​](#new-format-base-class "Direct link to New format base class")
...
### New course output classes and mustache files[​](#new-course-output-classes-and-mustache-files "Direct link to New course output classes and mustache files")
...
All course format output classes implements the new `named_templatable` interface, which allows the class to define its own template path using the `get_template_name` method. This new interface in combination with [mustache blocks](https://moodledev.io/docs/4.1/guides/templates#blocks) allows the format plugins to provide alternative templates to render the course.

Migrating 3.11 course formats doc

All the new output classes and a guide on how to migrate the current third-party plugins can be found in the [Migrating 3.11 formats](https://moodledev.io/docs/4.1/apis/plugintypes/format/migration) page.

### Course editor JavaScript modules and frontend components[​](#course-editor-javascript-modules-and-frontend-components "Direct link to Course editor JavaScript modules and frontend components")
...
## Behat changes[​](#behat-changes "Direct link to Behat changes")

### New steps[​](#new-steps "Direct link to New steps")
...
### Boost steps[​](#boost-steps "Direct link to Boost steps")
...
### Modified steps[​](#modified-steps "Direct link to Modified steps")
...
### Removed steps[​](#removed-steps "Direct link to Removed steps")
...
### Other things to consider[​](#other-things-to-consider "Direct link to Other things to consider")
...
```
AndI am on the "Test assignment name""assign activity" page logged in as teacher1
```

instead of:

```
WhenI log in as "teacher1"
AndI am on "Course" course homepage
AndI follow "Test assignment name"
```
...
### Core plugins review[​](#core-plugins-review "Direct link to Core plugins review")
...
### Core blocks cleanup[​](#core-blocks-cleanup "Direct link to Core blocks cleanup")

In the "Add a block" menu, the list of blocks was really long. A few changes have been done to reduce this list.

### Site admin presets plugin[​](#site-admin-presets-plugin "Direct link to Site admin presets plugin")
...
### JavaScript browser support changes[​](#javascript-browser-support-changes "Direct link to JavaScript browser support changes")
...
### The course index element[​](#the-course-index-element "Direct link to The course index element")
...
```
caseFEATURE_MOD_PURPOSE:returnMOD_PURPOSE_ASSESSMENT;
```

Available purposes are:

- MOD\_PURPOSE\_COMMUNICATION: e.g. chat, choice, feedback, survey
- MOD\_PURPOSE\_ASSESSMENT: e.g. assign, quiz, workshop
- MOD\_PURPOSE\_COLLABORATION: e.g. data, forum, glossary, wiki
- MOD\_PURPOSE\_CONTENT: e.g. book, folder, lesson, label, page
- MOD\_PURPOSE\_ADMINISTRATION: e.g. attendance
- MOD\_PURPOSE\_INTERFACE
...
## I'm a developer, what do I need to know?[​](#im-a-developer-what-do-i-need-to-know "Direct link to I'm a developer, what do I need to know?")
...
- Update my module to use the new activity\_header API. See [the activity header](#the-activity-header-class).
...
### Course format[​](#course-format "Direct link to Course format")

There have been a lot of changes made to the course format. Most in the process of moving the course format away from using the old rendering system and towards templates. We recommend the following sections be read carefully:
...
2. Consider moving the rendering of your content over to the [template system](https://moodledev.io/docs/4.1/guides/templates).

### Other plugins[​](#other-plugins "Direct link to Other plugins")

1. Are you adding settings? See the [secondary nav](#adding-items-to-the-navigation) for adding items to this navigation bar.
2. Have a look at [secondary](#secondary-navigation) and [tertiary](#tertiary-navigation) navigation changes in general if you have a plugin that has multiple pages to navigate around.
3. Do you have behat tests? Check the new [behat changes](#behat-changes).