...
source: llms
fetched_at: 2026-01-24T14:14:15.646895356-03:00
rendered_js: false
word_count: 179
summary: This document provides practical workflow examples for Docker Model Runner, including a sample local generative AI application and a GitHub Actions configuration for automated testing.
tags:
    - docker-model-runner
    - generative-ai
    - github-actions
...
```
name:Docker Model Runner Example Workflowpermissions:contents:readon:workflow_dispatch:inputs:test_model:description: 'Model to test with (default:ai/smollm2:360M-Q4_K_M)'required:falsetype:stringdefault:'ai/smollm2:360M-Q4_K_M'jobs:dmr-test:runs-on:ubuntu-latesttimeout-minutes:30steps:- name:Set up Dockeruses:docker/setup-docker-action@v4- name:Install docker-model-pluginrun:|          echo "Installing docker-model-plugin..."
          ...
          echo "Installation completed successfully"- name:Test docker model versionrun:|          echo "Testing docker model version command..."
          ...
          if [ $? -eq 0 ]; then
            ...
            exit 1
          fi- name:Pull the provided model and run itrun:|          MODEL="${{ github.event.inputs.test_model || 'ai/smollm2:360M-Q4_K_M' }}"
          ...
          timeout 60s sudo docker model run "$MODEL" "Give me a fact about whales." || {
            ...
            if [ $exit_code -eq 124 ]; then
              ...
              exit 1
               ...
               - name: Test model pull and runrun:|          MODEL="${{ github.event.inputs.test_model || 'ai/smollm2:360M-Q4_K_M' }}"
          echo "Testing with model: $MODEL"
          # Test model pull
          echo "Pulling model..."
          sudo docker model pull "$MODEL"
          ...
          }- name:Test API endpointrun:|          MODEL="${{ github.event.inputs.test_model || 'ai/smollm2:360M-Q4_K_M' }}"
          ...
          if [ $? -eq 0 ]; then
            ...
            exit 1
          fi- name:Test model cleanuprun:|          MODEL="${{ github.event.inputs.test_model || 'ai/smollm2:360M-Q4_K_M' }}"
          ...
          echo "✅ Model cleanup completed"- name:Report successif:success()run:|          echo "🎉 Docker Model Runner daily health check completed successfully!"
