---
title: Anti-Bot & Fallback - Crawl4AI Documentation (v0.8.x)
url: https://docs.crawl4ai.com/advanced/anti-bot-and-fallback/
source: sitemap
fetched_at: 2026-04-26T07:45:11.283728826-03:00
rendered_js: false
word_count: 757
summary: This document explains how to configure anti-bot detection and multi-stage retry strategies in Crawl4AI to bypass web blocking mechanisms like CAPTCHAs and 403 errors.
tags:
    - anti-bot
    - web-crawling
    - proxy-rotation
...
async defmy_scraping_api(url: str) -> str:
...
importaiohttp
fromcrawl4aiimport AsyncWebCrawler
fromcrawl4ai.async_configsimport BrowserConfig, CrawlerRunConfig, ProxyConfig

# Last-resort: fetch HTML via an external service
async defexternal_fetch(url: str) -> str:
    async with aiohttp.ClientSession() as session:
...
Round Attempt What runs 1 1 Datacenter proxy — blocked 1 2 Residential proxy — blocked 2 1 Datacenter proxy — blocked 2 2 Residential proxy — blocked 3 1 Datacenter proxy — blocked 3 2 Residential proxy — blocked - - `external_fetch(url)` called — returns HTML
