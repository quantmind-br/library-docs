...
> Mistral API conversion guide - parameter mapping, message handling, tool support, transcription, and streaming behavior
...
* **Parameter compatibility** - max\_completion\_tokens → max\_tokens conversion
...
**max\_completion\_tokens → max\_tokens:**
...
* `usage` - Token counts (prompt\_tokens, completion\_tokens)
* `finish_reason` - stop, tool\_calls, length
...
| `response_format`         | Format type  | response\_format | json, text, etc.        |
