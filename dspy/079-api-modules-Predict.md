---
title: Predict - DSPy
url: https://dspy.ai/api/modules/Predict/
source: sitemap
fetched_at: 2026-01-23T08:01:57.844870123-03:00
rendered_js: false
word_count: 926
summary: This document defines the dspy.Predict module, a fundamental component for mapping inputs to outputs using language models through signatures and configurable parameters.
tags:
    - dspy
    - llm-orchestration
    - python-api
...
Name Type Description Default `signature` `str | type[Signature]`

The input/output signature describing the task.

*required* `callbacks` `list[BaseCallback] | None`

Optional list of callbacks for instrumentation.

`None` `**config`

Default keyword arguments forwarded to the underlying language model. These values can be overridden for a single invocation by passing a `config` dictionary when calling the module. For example::
...
```
def__init__(self, signature: str | type[Signature], callbacks: list[BaseCallback] | None = None, **config):
...
### Functions[¶](#dspy.Predict-functions "Permanent link")

#### `__call__(*args, **kwargs)` [¶](#dspy.Predict.__call__ "Permanent link")
...
#### `acall(*args, **kwargs)` `async` [¶](#dspy.Predict.acall "Permanent link")

Source code in `dspy/predict/predict.py`

```
async defacall(self, *args, **kwargs):
    if args:
        raise ValueError(self._get_positional_args_error_message())
...
#### `aforward(**kwargs)` `async` [¶](#dspy.Predict.aforward "Permanent link")

Source code in `dspy/predict/predict.py`

```
async defaforward(self, **kwargs):
...
#### `deepcopy()` [¶](#dspy.Predict.deepcopy "Permanent link")
...
#### `dump_state(json_mode=True)` [¶](#dspy.Predict.dump_state "Permanent link")
...
#### `forward(**kwargs)` [¶](#dspy.Predict.forward "Permanent link")
...
#### `get_config()` [¶](#dspy.Predict.get_config "Permanent link")
...
#### `get_lm()` [¶](#dspy.Predict.get_lm "Permanent link")
...
#### `inspect_history(n: int = 1)` [¶](#dspy.Predict.inspect_history "Permanent link")
...
#### `load(path, allow_pickle=False)` [¶](#dspy.Predict.load "Permanent link")
...
#### `load_state(state: dict) -> Predict` [¶](#dspy.Predict.load_state "Permanent link")

Load the saved state of a `Predict` object.

Parameters:

Name Type Description Default `state` `dict`

The saved state of a `Predict` object.

*required*

Returns:

Type Description `Predict`
...
```
defload_state(self, state: dict) -> "Predict":
"""Load the saved state of a `Predict` object.
...
#### `map_named_predictors(func)` [¶](#dspy.Predict.map_named_predictors "Permanent link")
...
#### `named_parameters()` [¶](#dspy.Predict.named_parameters "Permanent link")
...
#### `named_predictors()` [¶](#dspy.Predict.named_predictors "Permanent link")
...
#### `named_sub_modules(type_=None, skip_compiled=False) -> Generator[tuple[str, BaseModule], None, None]` [¶](#dspy.Predict.named_sub_modules "Permanent link")
...
#### `parameters()` [¶](#dspy.Predict.parameters "Permanent link")
...
#### `predictors()` [¶](#dspy.Predict.predictors "Permanent link")
...
#### `reset()` [¶](#dspy.Predict.reset "Permanent link")
...
#### `reset_copy()` [¶](#dspy.Predict.reset_copy "Permanent link")
...
#### `save(path, save_program=False, modules_to_serialize=None)` [¶](#dspy.Predict.save "Permanent link")
...
#### `set_lm(lm)` [¶](#dspy.Predict.set_lm "Permanent link")
...
#### `update_config(**kwargs)` [¶](#dspy.Predict.update_config "Permanent link")

Source code in `dspy/predict/predict.py`

```
defupdate_config(self, **kwargs):
