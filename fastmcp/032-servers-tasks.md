---
title: Background Tasks - FastMCP
url: https://gofastmcp.com/servers/tasks
source: crawler
fetched_at: 2026-01-22T22:22:03.273195827-03:00
rendered_js: false
word_count: 768
summary: This document explains how to implement and configure background tasks in FastMCP, covering execution modes, backend options, and progress reporting for long-running operations.
tags:
    - fastmcp
    - background-tasks
    - mcp-protocol
...
ModeClient calls without taskClient calls with task`"forbidden"`Executes synchronouslyError: task not supported`"optional"`Executes synchronouslyExecutes as background task`"required"`Error: task requiredExecutes as background task
...
New in version `2.15.0` When clients poll for task status, the server tells them how frequently to check back. By default, FastMCP suggests a 5-second interval, but you can customize this per component:
...
Environment VariableDefaultDescription`FASTMCP_DOCKET_URL``memory://`Backend URL (`memory://` or `redis://host:port/db`)
