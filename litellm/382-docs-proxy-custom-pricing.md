...
- [**Provider Discounts**](https://docs.litellm.ai/docs/proxy/provider_discounts) - Apply percentage-based discounts to specific providers
- [**Provider Margins**](https://docs.litellm.ai/docs/proxy/provider_margins) - Add fees/margins to LLM costs for internal billing
- **Base Model Mapping** - Ensure accurate cost tracking for Azure deployments

By default, the response cost is accessible in the logging object via `kwargs["response_cost"]` on success (sync + async). [**Learn More**](https://docs.litellm.ai/docs/observability/custom_callback)
...
## Cost Per Second (e.g. Sagemaker)[​](#cost-per-second-eg-sagemaker "Direct link to Cost Per Second (e.g. Sagemaker)")
...
## Cost Per Token (e.g. Azure)[​](#cost-per-token-eg-azure "Direct link to Cost Per Token (e.g. Azure)")

#### Usage with LiteLLM Proxy Server[​](#usage-with-litellm-proxy-server-1 "Direct link to Usage with LiteLLM Proxy Server")
...
## Override Model Cost Map[​](#override-model-cost-map "Direct link to Override Model Cost Map")
...
### Additional Cost Keys[​](#additional-cost-keys "Direct link to Additional Cost Keys")
...
These keys evolve based on how new models handle multimodality. The latest version can be found at [https://github.com/BerriAI/litellm/blob/main/model\_prices\_and\_context\_window.json](https://github.com/BerriAI/litellm/blob/main/model_prices_and_context_window.json).

## Set 'base\_model' for Cost Tracking (e.g. Azure deployments)[​](#set-base_model-for-cost-tracking-eg-azure-deployments "Direct link to Set 'base_model' for Cost Tracking (e.g. Azure deployments)")
...
3. Check if 'input\_cost\_per\_token' and 'output\_cost\_per\_token' are top-level keys in the acompletion function.
