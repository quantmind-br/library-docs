...
url: https://ghostty.org/docs/vt/concepts/sequences
source: crawler
fetched_at: 2026-03-10T06:35:11.636492-03:00
rendered_js: true
word_count: 839
summary: This document explains terminal control sequences, which programs use to send commands like cursor movement and color changes to the terminal, detailing the various formats such as C0, CSI, OSC, DCS, and APC sequences.
tags:
    - terminal-control
    - escape-sequences
    - control-characters
...
Each type of control sequence has a different format and purpose. They are described below along with their specific syntax. The [reference](https://ghostty.org/docs/vt/reference) contains a full list of supported control sequences grouped by type.
...
C0AbbrFull nameFunction`0x07`[`BEL`](https://ghostty.org/docs/vt/control/bel)BellRaise the attention of the user.`0x08`[`BS`](https://ghostty.org/docs/vt/control/bs)BackspaceMove the cursor backward one position.`0x09`[`TAB`](https://ghostty.org/docs/vt/control/tab)TabMove the cursor right to the next tab stop.`0x0A`[`LF`](https://ghostty.org/docs/vt/control/lf)LinefeedMove the cursor down one line.`0x0D`[`CR`](https://ghostty.org/docs/vt/control/cr)Carriage ReturnMove the cursor to the leftmost column.
...
Not all sequences that begin with Escape are Escape sequences as defined here (they are more likely to be [C1 sequences](#c1-sequences)), but a true Escape sequence like `ESC D` are in the format below.
...
ESC codeC1AbbrFull nameFunction`ESC [``0x9b`CSIControl Sequence IntroducerBy far the most common. Uses integers to control the cursor, the screen, modes, cells, and their styles.`ESC ]``0x9d`OSCOperating System CommandOften used in modern terminal extensions to transmit string data.`ESC P``0x90`DCSDevice Control SequenceMost often used when querying the terminal's capabilities (XTGETTCAP). Rare otherwise.`ESC _``0x9f`APCApplication Program CommandUsed in very complex protocols like the Kitty Graphics Protocol that require arbitrary payloads.`ESC X``0x98`SOSStart of StringObsolete. Ignored by Ghostty.`ESC ^``0x9e`PMPrivate MessageObsolete. Ignored by Ghostty.
...
The format of an OSC sequence is below. By convention, OSC sequences are identified by an integer identifier followed by a semicolon (`;`), although this is never required by the standard. Certain obsolete OSC sequences use non-numeric identifiers before the semicolon, but Ghostty does not support these.
...
Ghostty supports APC sequences. Currently there is only one supported APC sequence: the Kitty Graphics Protocol sequence. This is documented [here](https://sw.kovidgoyal.net/kitty/graphics-protocol/).
