---
title: Course overview integration | Moodle Developer Resources
url: https://moodledev.io/docs/5.2/apis/plugintypes/mod/courseoverview
source: sitemap
fetched_at: 2026-02-17T15:44:35.599696-03:00
rendered_js: false
word_count: 1288
summary: This document explains how to implement and customize the course overview table for activity plugins by extending the activity overview base class. It provides technical details on defining overview items and overriding methods to display specific data like due dates and submission statuses.
tags:
    - moodle-development
    - activity-plugins
    - course-format
...
## Overview Class Structure[​](#overview-class-structure "Direct link to Overview Class Structure")
...
```
namespacemod_PLUGINNAME\courseformat;

usecore_courseformat\activityoverviewbase;

classoverviewextendsactivityoverviewbase{
// Implement methods here.
}
```

### The `overviewitem` class[​](#the-overviewitem-class "Direct link to the-overviewitem-class")
...
usecore_courseformat\activityoverviewbase;
usecore_courseformat\local\overview\overviewitem;
usecore\output\pix_icon;

classoverviewextendsactivityoverviewbase{
#[\Override]
publicfunctionget_extra_overview_items():array{
return[
...
/**
     ...
     */
privatefunctionget_extra_submitted_overview():?overviewitem{
...
## Fixed Overview Items[​](#fixed-overview-items "Direct link to Fixed Overview Items")
...
### Due Date Overview[​](#due-date-overview "Direct link to Due Date Overview")
...
classoverviewextendsactivityoverviewbase{
#[\Override]
publicfunctionget_due_date_overview():?overviewitem{
...
### Main Action Overview[​](#main-action-overview "Direct link to Main Action Overview")
...
usecore_courseformat\activityoverviewbase;
usecore_courseformat\local\overview\overviewitem;
usecore\output\action_link;
usecore\output\local\properties\button;
usecore\output\local\properties\text_align;

classoverviewextendsactivityoverviewbase{
#[\Override]
publicfunctionget_actions_overview():?overviewitem{
...
$content=newaction_link(
url:newurl('/mod/feedback/show_entries.php',['id'=>$this->cm->id]),
text:get_string(
'count_of_total',
'core',
...
### Custom grade overview items[​](#custom-grade-overview-items "Direct link to Custom grade overview items")
...
For activities with more than one grade item (for example, mod\_workshop), no grade will be shown by default because the system doesn't know the meaning of each grade item. For those cases, the plugin must implement `get_grade_item_names` to provide the generic name for each grade item. The overview report will show only the grade items with a generic name.

This is an example of a plugin with two grade items:

```
#[\Override]
protectedfunctionget_grade_item_names(array$items):array{
// Add some fallback in case some grade item is missing.
if(count($items)!=2){
returnparent::get_grade_item_names($items);
}
$names=[];
...
The `overview` class will be loaded using [dependency injection](https://moodledev.io/docs/5.2/apis/core/di). The constructor must accept a `cm_info` object to initialize the parent class, however, the constructor can also accept other dependencies that the plugin needs.
...
usecore_course\activityoverviewbase;
...
## Redirect the index.php page to the course overview[​](#redirect-the-indexphp-page-to-the-course-overview "Direct link to Redirect the index.php page to the course overview")
...
## Webservice and Moodle APP support[​](#webservice-and-moodle-app-support "Direct link to Webservice and Moodle APP support")
