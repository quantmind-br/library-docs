---
title: Advanced Tool Features - Pydantic AI
url: https://ai.pydantic.dev/tools-advanced/
source: sitemap
fetched_at: 2026-01-22T22:23:45.146348321-03:00
rendered_js: false
word_count: 1378
summary: This document explains advanced tool functionalities in Pydantic AI, focusing on multi-modal return types, the ToolReturn class for complex context management, and dynamic tool schema generation.
tags:
    - pydantic-ai
    - tool-calling
    - multi-modal
...
For scenarios where you need more control over both the tool's return value and the content sent to the model, you can use [`ToolReturn`](https://ai.pydantic.dev/api/messages/#pydantic_ai.messages.ToolReturn "ToolReturn            dataclass   "). This is particularly useful when you want to:
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) advanced\_tool\_return.py
...
- [`Tool`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.Tool "Tool            dataclass   ") dataclass

The `prepare` method, should be of type [`ToolPrepareFunc`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.ToolPrepareFunc "ToolPrepareFunc            module-attribute   "), a function which takes [`RunContext`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") and a pre-built [`ToolDefinition`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.ToolDefinition "ToolDefinition            dataclass   "), and should either return that `ToolDefinition` with or without modifying it, return a new `ToolDefinition`, or return `None` to indicate this tools should not be registered for that step.

Here's a simple `prepare` method that only includes the tool if the value of the dependency is `42`.

As with the previous example, we use [`TestModel`](https://ai.pydantic.dev/api/models/test/#pydantic_ai.models.test.TestModel "TestModel            dataclass   ") to demonstrate the behavior without calling a real model.
...
For the sake of variation, we create this tool using the [`Tool`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.Tool "Tool            dataclass   ") dataclass.
...
The `prepare_tools` function should be of type [`ToolsPrepareFunc`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.ToolsPrepareFunc "ToolsPrepareFunc            module-attribute   "), which takes the [`RunContext`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") and a list of [`ToolDefinition`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.ToolDefinition "ToolDefinition            dataclass   "), and returns a new list of tool definitions (or `None` to disable all tools for that step).
...
async deffast_tool() -> str:
"""This tool has its own timeout (5 seconds) that overrides the agent default."""
    await asyncio.sleep(1)
    return 'Done'
```

- **Agent-level timeout**: Set `tool_timeout` on the [`Agent`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.Agent "Agent            dataclass   ") to apply a default timeout to all tools.
- **Per-tool timeout**: Set `timeout` on individual tools via [`@agent.tool`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.Agent.tool "tool"), [`@agent.tool_plain`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.Agent.tool_plain "tool_plain"), or the [`Tool`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.Tool "Tool            dataclass   ") dataclass. This overrides the agent-level default.
...
When a model returns multiple tool calls in one response, Pydantic AI schedules them concurrently using `asyncio.create_task`. If a tool requires sequential/serial execution, you can pass the [`sequential`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.ToolDefinition.sequential "sequential            class-attribute       instance-attribute   ") flag when registering the tool, or wrap the agent run in the [`with agent.sequential_tool_calls()`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AbstractAgent.sequential_tool_calls "sequential_tool_calls            staticmethod   ") context manager.
...
You can cap tool executions within a run using [`UsageLimits(tool_calls_limit=...)`](https://ai.pydantic.dev/agents/#usage-limits). The counter increments only after a successful tool invocation. Output tools (used for [structured output](https://ai.pydantic.dev/output/)) are not counted in the `tool_calls` metric.

#### Output Tool Calls

When a model calls an [output tool](https://ai.pydantic.dev/output/#tool-output) in parallel with other tools, the agent's [`end_strategy`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.Agent.end_strategy "end_strategy            instance-attribute   ") parameter controls how these tool calls are executed. The `'exhaustive'` strategy ensures all tools are executed even after a final result is found, which is useful when tools have side effects (like logging, sending notifications, or updating metrics) that should always execute.
