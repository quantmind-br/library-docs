...
url: https://docs.mitmproxy.org/stable/howto/transparent/
source: crawler
fetched_at: 2026-01-28T16:21:40.077355881-03:00
rendered_js: false
word_count: 1438
summary: This document explains how to configure mitmproxy for transparent proxying on different operating systems, including Linux, OpenBSD, and macOS. It details the necessary network configuration steps, such as enabling IP forwarding and setting up firewall rules, to redirect traffic to mitmproxy without client-side adjustments.
tags:
...
[Edit on GitHub](https://github.com/mitmproxy/mitmproxy/blob/main/docs/src/content/howto/transparent.md)
...
The new [WireGuard](https://docs.mitmproxy.org/stable/concepts/modes/#wireguard) and [local capture](https://docs.mitmproxy.org/stable/concepts/modes/#local-capture) modes provides an alternative implementation for transparent proxying. It is much easier to set up, as it does not require setting up IP forwarding or modifying routing rules.

To set up transparent proxying, we need two new components. The first is a redirection mechanism that transparently reroutes a TCP connection destined for a server on the Internet to a listening proxy server. This usually takes the form of a firewall on the same host as the proxy server - [iptables](http://www.netfilter.org/) on Linux or [pf](https://en.wikipedia.org/wiki/PF_%28firewall%29) on OSX. When the proxy receives a redirected connection, it sees a vanilla HTTP request, without a host specification. This is where the second new component comes in - a host module that allows us to query the redirector for the original destination of the TCP connection.
...
If you want to persist this across reboots, you need to adjust your `/etc/sysctl.conf` or a newly created `/etc/sysctl.d/mitmproxy.conf` (see [here](https://superuser.com/a/625852)).
...
If you want to persist this across reboots, you can use the `iptables-persistent` package (see [here](http://www.microhowto.info/howto/make_the_configuration_of_iptables_persistent_on_debian.html)).
...
Set the test device up to use the host on which mitmproxy is running as the default gateway and [install the mitmproxy certificate authority on the test device](https://docs.mitmproxy.org/stable/concepts/certificates/).
...
OSX Lion integrated the [pf](https://en.wikipedia.org/wiki/PF_%28firewall%29) packet filter from the OpenBSD project, which mitmproxy uses to implement transparent mode on OSX. Note that this means we don’t support transparent mode for earlier versions of OSX.
...
This command enables the IP routing service. The `demand` option allows the service to manually be started. Alternatively, you can replace `demand` with `auto` to enable IP routing on startup.
...
This feature is currently unavailable in mitmproxy 7 and above ([#4914](https://github.com/mitmproxy/mitmproxy/discussions/4914)).
...
This mode does require root privileges though. There’s a wrapper in the examples directory called ‘mitmproxy\_shim.c’, which will enable you to use this mode with dropped privileges. It can be used as follows:
