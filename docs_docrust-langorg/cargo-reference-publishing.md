...
source: crawler
fetched_at: 2026-05-06T21:21:46.247559593-03:00
rendered_js: false
word_count: 1681
summary: This document provides a comprehensive guide on how to publish, manage, and update Rust packages (crates) on the crates.io registry using the Cargo command-line tool.
tags:
    - rust
    - cargo
    - crates-io
...
> **Note**: The [`cargo logout`](https://doc.rust-lang.org/cargo/commands/cargo-logout.html) command can be used to remove the token from `credentials.toml`. This can be useful if you no longer need it stored on the local machine.
...
Check out the [metadata you can specify](https://doc.rust-lang.org/cargo/reference/manifest.html) in `Cargo.toml` to ensure your crate can be discovered more easily! Before publishing, make sure you have filled out the following fields:
...
It would also be a good idea to include some [`keywords`](https://doc.rust-lang.org/cargo/reference/manifest.html#the-keywords-field) and [`categories`](https://doc.rust-lang.org/cargo/reference/manifest.html#the-categories-field), though they are not required.
...
The next step is to package up your crate and upload it to [crates.io](https://crates.io/). For this we’ll use the [`cargo publish`](https://doc.rust-lang.org/cargo/commands/cargo-publish.html) subcommand. This command performs the following steps:
...
It is recommended that you first run `cargo publish --dry-run` (or [`cargo package`](https://doc.rust-lang.org/cargo/commands/cargo-package.html) which is equivalent) to ensure there aren’t any warnings or errors before publishing. This will perform the first three steps listed above.
...
Cargo will automatically ignore files ignored by your version control system when packaging, but if you want to specify an extra set of files to ignore you can use the [`exclude` key](https://doc.rust-lang.org/cargo/reference/manifest.html#the-exclude-and-include-fields) in the manifest:
...
If you’d rather explicitly list the files to include, Cargo also supports an [`include` key](https://doc.rust-lang.org/cargo/reference/manifest.html#the-exclude-and-include-fields), which if set, overrides the `exclude` key:
...
When you are ready to publish, use the [`cargo publish`](https://doc.rust-lang.org/cargo/commands/cargo-publish.html) command to upload to [crates.io](https://crates.io/):
...
In order to release a new version, change [the `version` value](https://doc.rust-lang.org/cargo/reference/manifest.html#the-version-field) specified in your `Cargo.toml` manifest. Keep in mind [the SemVer rules](https://doc.rust-lang.org/cargo/reference/semver.html) which provide guidelines on what is a compatible change. Then run [`cargo publish`](https://doc.rust-lang.org/cargo/commands/cargo-publish.html) as described above to upload the new version.
