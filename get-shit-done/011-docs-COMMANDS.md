---
title: COMMANDS
url: https://github.com/gsd-build/get-shit-done/blob/main/docs/COMMANDS.md
source: git
fetched_at: 2026-04-16T16:20:04.615474974-03:00
rendered_js: false
word_count: 5354
summary: This document serves as a comprehensive command reference for the GSD workflow, detailing syntax, flags, and usage for project initialization, workspace management, and phase-based development tasks.
tags:
    - gsd-workflow
    - cli-commands
    - project-management
...
> Complete command syntax, flags, options, and examples. For feature details, see [[002-docs-FEATURES|Feature Reference]]. For workflow walkthroughs, see [[002-docs-USER-GUIDE|User Guide]].
...
**Use cases:**
- Multi-repo: work on subset of repos with isolated GSD state
- Feature isolation: `--repos .` creates a worktree of current repo
...
| Flag | Description |
|------|-------------|
| `--all` | Skip area selection — discuss all gray areas interactively |
| `--auto` | Auto-select recommended defaults for all questions |
| `--batch` | Group questions for batch intake instead of one-by-one |
| `--analyze` | Add trade-off analysis during discussion |
| `--power` | File-based bulk question answering from prepared answers file |
...
/gsd-discuss-phase 1 --all          # Discuss all gray areas without selection
...
| `--prd <file>` | Use PRD file instead of discuss-phase for context |
| `--reviews` | Replan with cross-AI review feedback from REVIEWS.md |
| `--validate` | Run state validation before planning begins |
| `--bounce` | Run external plan bounce validation (uses `workflow.plan_bounce_script`) |
...
Generate comprehensive project summary from milestone artifacts.
...
| `--reset-phase-numbers` | No | Restart new milestone at Phase 1 |
...
```bash
/gsd-insert-phase 3                 # Insert between phase 3 and 4 → 3.1
```
...
**Prerequisites:** `.planning/ROADMAP.md` exists
**Produces:** Dependency suggestion table; optionally updates `Depends on` fields

**Run this before `/gsd-manager`** when phases have empty `Depends on` fields.

**Detection methods:**
- File overlap — phases touching same files/domains
- Semantic dependencies — a phase consumes an API built by another
- Data flow — a phase reads output produced by another
...
Deep ecosystem research only (standalone).
...
| Flag | Description |
|------|-------------|
| `--forensic` | Append 6-check integrity audit (STATE consistency, orphaned handoffs, deferred scope drift, memory-flagged pending work, blocking todos, uncommitted code) |
...
- Recommends optimal next actions based on dependencies
- Dispatches work: discuss runs inline, plan/execute run as background agents
- Supports per-step passthrough flags via `manager.flags` config
...
Socratic ideation session — guide an idea through probing questions.
...
Safe git revert — roll back GSD phase or plan commits with dependency checks.
...
**Safety:** Checks dependent phases/plans before reverting; shows confirmation gate.

```bash
/gsd-undo --last 5                  # Pick from 5 most recent GSD commits
...
Ingest an external plan file into the GSD planning system with conflict detection.

| Flag | Required | Description |
|------|----------|-------------|
| `--from <filepath>` | **Yes** | Path to the external plan file |

**Process:** Detects conflicts → prompts for resolution → writes as GSD PLAN.md → validates via `gsd-plan-checker`

```bash
/gsd-import --from /tmp/team-plan.md  # Import and validate
```
...
Reverse migration from GSD-2 format (`.gsd/`) back to v1 `.planning/` format.

| Flag | Required | Description |
|------|----------|-------------|
| `--dry-run` | No | Preview what would be migrated |
| `--force` | No | Overwrite existing `.planning/` directory |
| `--path <dir>` | No | Specify GSD-2 root directory (defaults to current directory) |

**Flattening:** Milestone→Slice hierarchy flattened to sequential phase numbers.

**Produces:** `PROJECT.md`, `REQUIREMENTS.md`, `ROADMAP.md`, `STATE.md`, sequential phase directories.

**Safety:** Guards against overwriting existing `.planning/` without `--force`.

```bash
/gsd-from-gsd2                          # Migrate .gsd/ in current directory
/gsd-from-gsd2 --dry-run                # Preview migration
...
**Subcommands:**
- `/gsd-debug list` — List all active debug sessions
- `/gsd-debug status <slug>` — Print full summary of a session
- `/gsd-debug continue <slug>` — Resume a specific session
- `/gsd-debug [--diagnose] <description>` — Start new debug session

**TDD mode:** When `tdd_mode: true` in config, debug sessions require a failing test before any fix.
...
Generate developer behavioral profile from Claude Code session analysis.
...
```bash
/gsd-extract-learnings 3                    # Extract from phase 3
...
/gsd-workstreams progress           # Cross-workstream overview
/gsd-workstreams complete backend-api  # Archive completed
/gsd-workstreams resume backend-api    # Resume work
```
...
Rapid single-focus codebase assessment — lightweight alternative to `/gsd-map-codebase`.
...
**Prerequisites:** `intel.enabled: true` in `config.json`
...
AI framework selection wizard for integrating AI/LLM capabilities into a phase.

**Produces:** `{phase}-AI-SPEC.md` in the phase directory
**Spawns:** 3 parallel specialist agents: domain-researcher, framework-selector, ai-researcher, eval-planner

```bash
/gsd-ai-integration-phase              # Wizard for current phase
/gsd-ai-integration-phase 3           # Wizard for specific phase
```
...
**Produces:** `{phase}-EVAL-REVIEW.md` with findings, gaps, remediation guidance

```bash
/gsd-eval-review                       # Audit current phase
/gsd-eval-review 3                     # Audit specific phase
```
...
Review source files changed during a phase for bugs, security vulnerabilities, code quality.

| Argument | Required | Description |
|----------|----------|-------------|
| `N` | **Yes** | Phase number whose changes to review |
| `--depth=quick\|standard\|deep` | No | Review depth level (overrides `workflow.code_review_depth`) |
| `--files file1,file2,...` | No | Explicit comma-separated file list |
...
| `--auto` | No | Enable fix + re-review iteration loop (max 3 iterations) |

**Prerequisites:** Phase has a `{phase}-REVIEW.md` file
**Produces:** `{phase}-REVIEW-FIX.md` with applied fixes summary
**Spawns:** `gsd-code-fixer` agent

```bash
/gsd-code-review-fix 3                      # Fix Critical + Warning findings
/gsd-code-review-fix 3 --all               # Include Info findings
/gsd-code-review-fix 3 --auto              # Fix and re-review until clean
```
...
Autonomous audit-to-fix pipeline — runs audit, classifies findings, fixes auto-fixable issues.
...
```bash
/gsd-audit-fix                              # Run audit-uat, fix medium+ (max 5)
/gsd-audit-fix --severity high             # Only fix high-severity issues
/gsd-audit-fix --dry-run                   # Preview classification
/gsd-audit-fix --max 10 --severity all     # Fix up to 10 issues
```
...
**Produces:** Up to 9 documentation files
**Spawns:** `gsd-doc-writer` agents (one per doc type), then `gsd-doc-verifier` agents
...
**999.x numbering** keeps backlog items outside the active phase sequence.
...
Seeds solve context rot: instead of a one-liner in Deferred, a seed preserves the full WHY, WHEN to surface, and breadcrumbs.
...
Threads are lightweight cross-session knowledge stores. Lighter weight than `/gsd-pause-work`.
...
**Produces:** Validation report showing drift between STATE.md fields and filesystem reality
...
```bash
node gsd-tools.cjs state sync             # Reconstruct from disk
node gsd-tools.cjs state sync --verify   # Dry-run
```
...
Record state transition after plan-phase completes.
...
Optional git and session hooks gated behind `hooks.community: true` in config. All are no-ops unless explicitly enabled.
...
#gsd-workflow #cli-commands #project-management #workspace-automation #dev-ops #development-tools
