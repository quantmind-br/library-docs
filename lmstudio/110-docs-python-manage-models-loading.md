...
url: https://lmstudio.ai/docs/python/manage-models/loading
source: sitemap
fetched_at: 2026-04-07T21:31:26.660730204-03:00
rendered_js: false
word_count: 458
summary: This document details how to programmatically manage large AI models using LM Studio's SDK, covering methods for loading, accessing specific instances, managing namespaces, and safely unloading models from memory.
tags:
    - sdk-usage
    - model-management
    - llm-api
...
## Load a New Instance of a Model with `.load_new_instance()`[](#load-a-new-instance-of-a-model-with-loadnewinstance "Link to 'Load a New Instance of a Model with ,[object Object]'")
...
See [load-time configuration](https://lmstudio.ai/docs/python/llm-prediction/parameters) for more.

## Set an Auto Unload Timer (TTL)[](#set-an-auto-unload-timer-ttl "Link to 'Set an Auto Unload Timer (TTL)'")

You can specify a *time to live* for a model you load, which is the idle time (in seconds) after the last request until the model unloads. See [Idle TTL](https://lmstudio.ai/docs/app/api/ttl-and-auto-evict) for more on this.
