---
title: The Drag and Drop protocol
title: Dnd protocol
word_count: 3805
summary: This document defines a terminal escape code protocol for implementing drag-and-drop functionality, enabling terminal-based programs to interact with OS-level drag-and-drop events.
category: concept
optimized: true
...
When transferring binary data the payload is base64 <4648> encoded. The
...
must inform the terminal of its machine id <machine_id> using the escape code:

```
OSC _dnd_code ; t=a:x=1 ; machine id ST
```

Then, the client must first request the text/uri-list <2483> MIME
...
machines. See below <machine_id> for its semantics.
...
Terminals can examine the machine_id sent with the enable drag offers
...
This machine id is then hashed using a HMAC <2104> with :rfc:`SHA-256
