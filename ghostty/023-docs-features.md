...
source: crawler
fetched_at: 2026-03-10T06:35:09.275632-03:00
rendered_js: true
word_count: 745
summary: This document details the extensive features supported by the Ghostty terminal emulator, categorized for both end-users and application developers, highlighting its cross-platform capability and commitment to platform-native behavior.
tags:
    - terminal-emulator
    - feature-set
    - platform-native
...
One of the primary [design goals](https://ghostty.org/docs/about) of Ghostty is to look, feel, and behave like a purpose-built native application on each platform. Here is a list of some of the platform-native features that Ghostty has.

- **Quick Terminal**: Lightweight terminal that animates down below the menu bar for instant access without interrupting work.
- **AppleScript Automation**: Built-in AppleScript dictionary for scripting windows, tabs, terminals, layouts, and input events. See [AppleScript docs](https://ghostty.org/docs/features/applescript).
...
For terminal application developers, we believe Ghostty is one of the most modern and comprehensive terminal emulators available[1](#user-content-fn-1). Our goal with Ghostty is to be the most compatible for legacy applications while also providing the most modern features for new applications.
...
1. Xterm compatibility: xterm is the de facto standard for terminal emulation. If xterm does something, we should do the same thing[2](#user-content-fn-2).
2. Protocol origin compatibility: If the terminal that defined a protocol behaves a certain way, we should follow that behavior, even if it is not specified[3](#user-content-fn-3).
...
For a list of terminal developer features Ghostty supports, see the [reference](https://ghostty.org/docs/vt/reference) (but note at the time of writing this is still a work-in-progress).

1. If not, we want to know! Please [open a discussion](https://github.com/ghostty-org/ghostty/discussions). [↩](#user-content-fnref-1)
2. We may put some xterm behaviors behind a configuration flag if the behavior is questionable or not widely accepted. [↩](#user-content-fnref-2)
3. If a behavior differs between the specification and the terminal that defined the protocol, we open a discussion to determine the correct behavior. [↩](#user-content-fnref-3)

[Edit on GitHub](https://github.com/ghostty-org/website/edit/main/docs/features/index.mdx)

- [Feature Highlight](#feature-highlight)
- [Platform-Native Features](#platform-native-features)
- [macOS](#macos)
- [Terminal (VT) Features](#terminal-%28vt%29-features)