---
title: Meme Generation — Generate real meme images by picking a template and overlaying text with Pillow | Hermes Agent
url: https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/creative/creative-meme-generation
source: crawler
fetched_at: 2026-04-24T17:01:21.101168091-03:00
rendered_js: false
word_count: 575
summary: 'This document serves as a reference guide for a meme generation skill, detailing how it creates actual PNG meme images by selecting templates and overlaying user-provided text. It explains two modes of operation: classic template usage and custom AI image integration.'
tags:
    - meme-generation
    - image-creation
    - template-picking
...
SourceOptional — install with `hermes skills install official/creative/meme-generation`Path`optional-skills/creative/meme-generation`Version`2.0.0`AuthoradanaleycioLicenseMITTags`creative`, `memes`, `humor`, `images`Related skills[`ascii-art`](https://hermes-agent.nousresearch.com/docs/user-guide/skills/bundled/creative/creative-ascii-art), `generative-widgets`
...
IDNameFieldsBest for`this-is-fine`This is Finetop, bottomchaos, denial`drake`Drake Hotline Blingreject, approverejecting/preferring`distracted-boyfriend`Distracted Boyfrienddistraction, current, persontemptation, shifting priorities`two-buttons`Two Buttonsleft, right, personimpossible choice`expanding-brain`Expanding Brain4 levelsescalating irony`change-my-mind`Change My Mindstatementhot takes`woman-yelling-at-cat`Woman Yelling at Catwoman, catarguments`one-does-not-simply`One Does Not Simplytop, bottomdeceptively hard things`grus-plan`Gru's Planstep1-3, realizationplans that backfire`batman-slapping-robin`Batman Slapping Robinrobin, batmanshutting down bad ideas
