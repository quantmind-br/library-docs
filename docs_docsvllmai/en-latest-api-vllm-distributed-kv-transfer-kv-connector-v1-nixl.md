---
title: nixl - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/distributed/kv_transfer/kv_connector/v1/nixl/
source: sitemap
fetched_at: 2026-05-07T21:18:39.292460866-03:00
rendered_js: false
word_count: 154
summary: This class provides the implementation for NIXL worker-side operations, handling the computation of descriptor IDs for KV cache blocks and managing local memory splits for hybrid attention and SSM architectures.
tags:
    - nixl
    - kv-cache
...
```
classNixlConnectorWorker:
"""Implementation of Worker side methods"""

    def_compute_desc_ids(
        self,
        block_ids: BlockIds,
        dst_num_blocks: int,
...
"""Compute NIXL descriptor IDs for given block IDs."""
    ...
    def_build_local_splits_from_plan(
        self,
        plan: TPMapping,
...
"""Build split handle data for P_TP > D_TP scenario.
    ...
    def__init__(
        ...
        if self._has_mamba:
            ...
            fromvllm.model_executor.layers.mamba.mamba_utilsimport (
...
"""Do a NIXL handshake with a remote instance."""
    ...
    definitialize_host_xfer_buffer(self, kv_caches: dict[str, torch.Tensor]) -> None:
"""
    ...
    defset_host_xfer_buffer_ops(self, copy_operation: CopyBlocksOp):
...
"""Log transfer failure with structured context for easier debugging."""
    ...
    def_background_nixl_handshake(
        ...
        if fut is None:
            ...
            defdone_callback(f: Future[dict[int, str]], eid=remote_engine_id):
        ...
        defrequest_ready(f: Future[Any], entry=(req_id, meta)):
            try:
    ...
    defregister_cross_layers_kv_caches(self, kv_cache: torch.Tensor) -> None:
...
"""Build remote FA descriptors for all layers."""
    ...
    defregister_local_xfer_handler(
        self,
        block_size: int,
...
"""
    ...
    def_validate_remote_agent_handshake(
...
"""
    ...
    defsync_recved_kv_to_device(self, req_id: str, meta: ReqMeta):
"""copy recved kv from host buffer to device."""
    ...
    defsave_kv_to_host(self, metadata: NixlConnectorMetadata):
"""copy kv from device to host buffer."""
    ...
    defpost_process_device_kv_on_receive(
        self,
        block_size_ratio: int,
...
"""
    ...
    defpost_process_device_kv_on_receive_heterogeneous_attn(
...
"""
    ...
    defget_finished(self) -> tuple[set[str], set[str]]:
"""
    ...
    def_get_new_notifs(self) -> set[str]:
"""
    ...
    def_pop_done_transfers(self, transfers: dict[str, list[int]]) -> set[str]:
"""
    ...
    def_handle_failed_transfer(self, req_id: str, handle: int):
"""
    ...
    defstart_load_kv(self, metadata: NixlConnectorMetadata):
"""
    ...
    def_read_blocks_for_req(self, req_id: str, meta: ReqMeta):
...
"""
    ...
    def_logical_to_kernel_block_ids(self, block_ids: BlockIds) -> BlockIds:
"""
    ...
    def_logical_to_remote_kernel_block_ids(
...
"""Map logical block IDs to physical kernel block IDs on the remote.
    ...
    defget_backend_aware_kv_block_len(
