...
source: github_pages
fetched_at: 2026-02-15T21:17:16.452673-03:00
rendered_js: false
word_count: 852
summary: This document details the specialized component types available in SketchyBar, providing instructions for adding and configuring graphs, spaces, brackets, aliases, and sliders.
tags:
    - sketchybar
    - macos-customization
...
## Components -- Special Items with special properties[​](#components----special-items-with-special-properties "Direct link to heading")
...
### Data Graph -- Draws an arbitrary graph into the bar[​](#data-graph----draws-an-arbitrary-graph-into-the-bar "Direct link to heading")
...
&lt;property&gt;&lt;value&gt;defaultdescription`graph.color``<argb_hex>``0xffcccccc`Color of the graph line`graph.fill_color``<argb_hex>``0xffcccccc`Fill color of the graph`graph.line_width``<float>``0.5`Width of the line in points
...
```
sketchybar --set <graph name> background.color=0xff00ff00 background.height=20y_offset=2
```

### Space -- Associate mission control spaces with an item[​](#space----associate-mission-control-spaces-with-an-item "Direct link to heading")
...
### Item Bracket -- Group Items in e.g. colored sections[​](#item-bracket----group-items-in-eg-colored-sections "Direct link to heading")
...
### Item Alias -- Mirror items of the original macOS status bar into sketchybar[​](#item-alias----mirror-items-of-the-original-macos-status-bar-into-sketchybar "Direct link to heading")
...
This will put the default macOS menu bar item into sketchybar. If an application has multiple menu bar widgets the command can be overloaded by providing a *window\_owner* and a *window\_name*
...
### Slider -- A draggable progression indicator[​](#slider----a-draggable-progression-indicator "Direct link to heading")
...
&lt;property&gt;&lt;value&gt;defaultdescription`slider.width``<positive_integer>``100`Total width of the slider in points`slider.percentage``<positive_integer>``0`Progression of the slider in percent (0-100)`slider.highlight_color``<argb_hex>``0xff0000ff`Color that highlights the progression of the slider`slider.knob``<string>`Knob of the slider`slider.knob.<text_property>`The slider knob supports all `text` properties`slider.background.<background_property>`The slider supports all `background` properties
