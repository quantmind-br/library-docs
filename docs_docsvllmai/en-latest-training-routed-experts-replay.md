---
title: Routed Experts Replay - vLLM
url: https://docs.vllm.ai/en/latest/training/routed_experts_replay/
source: sitemap
fetched_at: 2026-05-07T21:15:22.059583314-03:00
rendered_js: false
word_count: 1647
summary: This document explains how to configure and retrieve routed expert information from Mixture of Experts (MoE) models in vLLM for use in reinforcement learning pipelines.
tags:
    - moe
    - vllm
...
[](https://github.com/vllm-project/vllm/edit/main/docs/training/routed_experts_replay.md "Edit this page")
...
### OpenAI API Server[¶](#openai-api-server "Permanent link")
...
### Python SDK (Offline Inference)[¶](#python-sdk-offline-inference "Permanent link")
...
### `CompletionOutput.routed_experts`[¶](#completionoutputrouted_experts "Permanent link")
...
### `RequestOutput.prompt_routed_experts`[¶](#requestoutputprompt_routed_experts "Permanent link")
...
### Why Separate Prompt and Generation Routing?[¶](#why-separate-prompt-and-generation-routing "Permanent link")

When a request has multiple completions (`n > 1`), each completion shares the same prompt but produces different generated text. Storing prompt routing once on the [`RequestOutput`](https://docs.vllm.ai/en/latest/api/vllm/outputs/#vllm.outputs.RequestOutput "            RequestOutput") (rather than duplicating it on every [`CompletionOutput`](https://docs.vllm.ai/en/latest/api/vllm/outputs/#vllm.outputs.CompletionOutput "            CompletionOutput            dataclass   ")) avoids redundant data. For RL training, the consumer typically needs:
...
### Device Cache[¶](#device-cache "Permanent link")
...
The `(L, N, K)` layout ensures that `buffer[layer_id]` gives a contiguous `(N, K)` view per layer. Each [`FusedMoE`](https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/fused_moe/layer/#vllm.model_executor.layers.fused_moe.layer.FusedMoE "            FusedMoE") layer gets a persistent reference to its slice via `module._routing_replay_out = buffer[layer_id]`.

**Dtype**: `int16` — sufficient for expert IDs (max ~512 experts in practice) and half the memory of `int32`.

### Host Cache[¶](#host-cache "Permanent link")
...
### Async D2H Pipeline[¶](#async-d2h-pipeline "Permanent link")
...
### CUDA Graph Compatibility[¶](#cuda-graph-compatibility "Permanent link")

CUDA graph compatibility requires two mechanisms:

1. **Persistent tensor attribute**: Each [`FusedMoE`](https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/fused_moe/layer/#vllm.model_executor.layers.fused_moe.layer.FusedMoE "            FusedMoE") layer stores a reference to its buffer slice as `module._routing_replay_out`. Because `torch.compile` captures module attributes by reference, graph replay always writes to the live buffer — not a stale snapshot.
...
### Multi-Node Support[¶](#multi-node-support "Permanent link")
...
### Routing Capture Path[¶](#routing-capture-path "Permanent link")
...
### MTP (Multi-Token Prediction) Handling[¶](#mtp-multi-token-prediction-handling "Permanent link")
...
## Design Decisions[¶](#design-decisions "Permanent link")

### Why Replace SharedMemory with Device Cache?[¶](#why-replace-sharedmemory-with-device-cache "Permanent link")
...
### Why `(L, N, K)` Layout Instead of `(N, L, K)`?[¶](#why-l-n-k-layout-instead-of-n-l-k "Permanent link")
...
### Why int16 Instead of int32?[¶](#why-int16-instead-of-int32 "Permanent link")
...
### Why Split Prompt and Generation Routing?[¶](#why-split-prompt-and-generation-routing "Permanent link")
...
With `n > 1` completions, all completions share the same prompt routing. Duplicating it per completion would waste memory proportional to `n * prompt_len * L * K`. Instead, `prompt_routed_experts` is stored once on [`RequestOutput`](https://docs.vllm.ai/en/latest/api/vllm/outputs/#vllm.outputs.RequestOutput "            RequestOutput") and shared.

### Why Async D2H Instead of Synchronous Copy?[¶](#why-async-d2h-instead-of-synchronous-copy "Permanent link")
...
Flag Description `--enable-return-routed-experts` Enable routing replay capture and return expert IDs in API responses.

## API Reference[¶](#api-reference "Permanent link")

### Completions (`/v1/completions`)[¶](#completions-v1completions "Permanent link")

**Response-level field:**

Field Type Description `prompt_routed_experts` `list[list[list[int]]]` or `null` Expert IDs for prompt tokens. Shape: `[prompt_len, num_moe_layers, top_k]`.

**Choice-level field:**

Field Type Description `routed_experts` `list[list[list[int]]]` or `null` Expert IDs for generated tokens. Shape: `[gen_len, num_moe_layers, top_k]`.

### Chat Completions (`/v1/chat/completions`)[¶](#chat-completions-v1chatcompletions "Permanent link")

Same fields as above on `ChatCompletionResponse` and `ChatCompletionResponseChoice`.

### Python SDK[¶](#python-sdk "Permanent link")

Object Field Type Description [`RequestOutput`](https://docs.vllm.ai/en/latest/api/vllm/outputs/#vllm.outputs.RequestOutput "            RequestOutput") `prompt_routed_experts` `np.ndarray` or `None` `[prompt_len, L, K]` i16 [`CompletionOutput`](https://docs.vllm.ai/en/latest/api/vllm/outputs/#vllm.outputs.CompletionOutput "            CompletionOutput            dataclass   ") `routed_experts` `np.ndarray` or `None` `[gen_len, L, K]` int16