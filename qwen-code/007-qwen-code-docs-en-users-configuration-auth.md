...
source: github_pages
fetched_at: 2026-04-09T09:04:11.660773929-03:00
rendered_js: true
word_count: 1202
summary: This document details the three authentication methods available for Qwen Code—OAuth, Alibaba Cloud Coding Plan, and generic API Key—and guides users on configuring their preferred setup via the settings file. It emphasizes how to connect to various AI model providers by defining model protocols in a central configuration file.
tags:
    - authentication
    - api-key
    - model-providers
...
## Option 1: Qwen OAuth (Free)[](#option-1-qwen-oauth-free)
...
## 💳 Option 2: Alibaba Cloud Coding Plan[](#-option-2-alibaba-cloud-coding-plan)
...
RegionConsole URLAliyun Bailian (aliyun.com)[bailian.console.aliyun.com](https://bailian.console.aliyun.com) Alibaba Cloud (alibabacloud.com)[bailian.console.alibabacloud.com](https://bailian.console.alibabacloud.com) 
...
### Alternative: configure via `settings.json`[](#alternative-configure-via-settingsjson)
...
## 🚀 Option 3: API Key (flexible)[](#-option-3-api-key-flexible)
...
### Recommended: One-file setup via `settings.json`[](#recommended-one-file-setup-via-settingsjson)
...
Protocol`modelProviders` keyEnvironment variablesProvidersOpenAI-compatible`openai``OPENAI_API_KEY`, `OPENAI_BASE_URL`, `OPENAI_MODEL`OpenAI, Azure OpenAI, OpenRouter, ModelScope, Alibaba Cloud, any OpenAI-compatible endpointAnthropic`anthropic``ANTHROPIC_API_KEY`, `ANTHROPIC_BASE_URL`, `ANTHROPIC_MODEL`Anthropic ClaudeGoogle GenAI`gemini``GEMINI_API_KEY`, `GEMINI_MODEL`Google Gemini

#### Step 1: Configure models and providers in `~/.qwen/settings.json`[](#step-1-configure-models-and-providers-in-qwensettingsjson)
...
FieldRequiredDescription`id`YesModel ID sent to the API (e.g. `gpt-4o`, `claude-sonnet-4-20250514`)`name`NoDisplay name in the `/model` picker (defaults to `id`)`envKey`YesEnvironment variable name for the API key (e.g. `OPENAI_API_KEY`)`baseUrl`NoAPI endpoint override (useful for proxies or custom endpoints)`generationConfig`NoFine-tune `timeout`, `maxRetries`, `samplingParams`, etc.
...
PrioritySourceOverride behavior1 (highest)CLI flags (`--openai-api-key`)Always wins2System env (`export`, inline)Overrides `.env` and `settings.json` → `env`3`.env` fileOnly sets if not in system env4 (lowest)`settings.json` → `env`Only sets if not in system env or `.env`

#### Step 3: Switch models with `/model`[](#step-3-switch-models-with-model)
