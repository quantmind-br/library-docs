---
title: Cheatsheet - DSPy
url: https://dspy.ai/cheatsheet/
source: sitemap
fetched_at: 2026-01-23T08:01:20.253894922-03:00
rendered_js: false
word_count: 474
summary: This document provides a collection of quick-reference code snippets and frequent usage patterns for DSPy modules, metrics, evaluation, and optimizers.
tags:
    - dspy
    - python-library
    - llm-orchestration
...
[](https://github.com/stanfordnlp/dspy/blob/main/docs/docs/cheatsheet.md "Edit this page")

This page will contain snippets for frequent usage patterns.

## DSPy Programs[¶](#dspy-programs "Permanent link")

### Forcing fresh LM outputs[¶](#forcing-fresh-lm-outputs "Permanent link")
...
### dspy.Signature[¶](#dspysignature "Permanent link")
...
### dspy.ChainOfThought[¶](#dspychainofthought "Permanent link")
...
### dspy.ProgramOfThought[¶](#dspyprogramofthought "Permanent link")
...
### dspy.ReAct[¶](#dspyreact "Permanent link")
...
### dspy.Retrieve[¶](#dspyretrieve "Permanent link")
...
### dspy.CodeAct[¶](#dspycodeact "Permanent link")

```
fromdspyimport CodeAct
...
### dspy.Parallel[¶](#dspyparallel "Permanent link")
...
## DSPy Metrics[¶](#dspy-metrics "Permanent link")

### Function as Metric[¶](#function-as-metric "Permanent link")

To create a custom metric you can create a function that returns either a number or a boolean value:

```
defparse_integer_answer(answer, only_first_line=True):
    try:
        if only_first_line:
...
# Metric Function
defgsm8k_metric(gold, pred, trace=None) -> int:
    return int(parse_integer_answer(str(gold.answer))) == int(parse_integer_answer(str(pred.answer)))
```

### LLM as Judge[¶](#llm-as-judge "Permanent link")

```
classFactJudge(dspy.Signature):
...
deffactuality_metric(example, pred):
    factual = judge(context=example.context, question=example.question, answer=pred.answer)
    return factual.factually_correct
```

## DSPy Evaluation[¶](#dspy-evaluation "Permanent link")

```
fromdspy.evaluateimport Evaluate
...
## DSPy Optimizers[¶](#dspy-optimizers "Permanent link")

### LabeledFewShot[¶](#labeledfewshot "Permanent link")

```
fromdspy.telepromptimport LabeledFewShot
...
### BootstrapFewShot[¶](#bootstrapfewshot "Permanent link")

```
fromdspy.telepromptimport BootstrapFewShot
...
#### Using another LM for compilation, specifying in teacher\_settings[¶](#using-another-lm-for-compilation-specifying-in-teacher_settings "Permanent link")
...
#### Compiling a compiled program - bootstrapping a bootstrapped program[¶](#compiling-a-compiled-program-bootstrapping-a-bootstrapped-program "Permanent link")
...
#### Saving/loading a compiled program[¶](#savingloading-a-compiled-program "Permanent link")
...
### BootstrapFewShotWithRandomSearch[¶](#bootstrapfewshotwithrandomsearch "Permanent link")

Detailed documentation on BootstrapFewShotWithRandomSearch can be found [here](https://dspy.ai/api/optimizers/BootstrapFewShot/).

```
fromdspy.telepromptimport BootstrapFewShotWithRandomSearch
...
### Ensemble[¶](#ensemble "Permanent link")

```
fromdspy.telepromptimport BootstrapFewShotWithRandomSearch
fromdspy.teleprompt.ensembleimport Ensemble
...
### BootstrapFinetune[¶](#bootstrapfinetune "Permanent link")

```
fromdspy.telepromptimport BootstrapFewShotWithRandomSearch, BootstrapFinetune
...
### COPRO[¶](#copro "Permanent link")

Detailed documentation on COPRO can be found [here](https://dspy.ai/api/optimizers/COPRO/).

```
fromdspy.telepromptimport COPRO
...
### MIPROv2[¶](#miprov2 "Permanent link")

Note: detailed documentation can be found [here](https://dspy.ai/api/optimizers/MIPROv2/). `MIPROv2` is the latest extension of `MIPRO` which includes updates such as (1) improvements to instruction proposal and (2) more efficient search with minibatching.

#### Optimizing with MIPROv2[¶](#optimizing-with-miprov2 "Permanent link")

This shows how to perform an easy out-of-the box run with `auto=light`, which configures many hyperparameters for you and performs a light optimization run. You can alternatively set `auto=medium` or `auto=heavy` to perform longer optimization runs. The more detailed `MIPROv2` documentation [here](https://dspy.ai/api/optimizers/MIPROv2/) also provides more information about how to set hyperparameters by hand.

```
# Import the optimizer
fromdspy.telepromptimport MIPROv2
...
#### Optimizing instructions only with MIPROv2 (0-Shot)[¶](#optimizing-instructions-only-with-miprov2-0-shot "Permanent link")
...
### KNNFewShot[¶](#knnfewshot "Permanent link")

```
fromsentence_transformersimport SentenceTransformer
fromdspyimport Embedder
fromdspy.telepromptimport KNNFewShot
fromdspyimport ChainOfThought
...
### BootstrapFewShotWithOptuna[¶](#bootstrapfewshotwithoptuna "Permanent link")

```
fromdspy.telepromptimport BootstrapFewShotWithOptuna
...
```
fromdspy.telepromptimport SIMBA
...
### dspy.Tool[¶](#dspytool "Permanent link")
...
### dspy.streamify[¶](#dspystreamify "Permanent link")
...
### dspy.asyncify[¶](#dspyasyncify "Permanent link")
...
### Track Usage[¶](#track-usage "Permanent link")
...
### dspy.configure\_cache[¶](#dspyconfigure_cache "Permanent link")
...
## DSPy `Refine` and `BestofN`[¶](#dspy-refine-and-bestofn "Permanent link")
