...
source: sitemap
fetched_at: 2026-01-30T20:30:51.296157225-03:00
rendered_js: false
word_count: 8695
summary: This document explains how to configure the OpenClaw gateway using JSON5 format, covering basic setup, strict validation, RPC methods for applying changes, and advanced features like config includes and partial updates.
tags:
    - configuration
    - json5
    - openclaw
...
See [Multi-Agent Sandbox & Tools](https://docs.openclaw.ai/multi-agent-sandbox-tools) for precedence and additional examples. Full access (no sandbox):
...
- `/reasoning stream` streams reasoning into the draft, then sends the final answer. Retry policy defaults and behavior are documented in [Retry policy](https://docs.openclaw.ai/concepts/retry).
...
- `allowlist`: reactions from `guilds.<id>.users` on all messages (empty list disables). Outbound text is chunked by `channels.discord.textChunkLimit` (default 2000). Set `channels.discord.chunkMode="newline"` to split on blank lines (paragraph boundaries) before length chunking. Discord clients can clip very tall messages, so `channels.discord.maxLinesPerMessage` (default 17) splits long multi-line replies even when under 2000 chars. Retry policy defaults and behavior are documented in [Retry policy](https://docs.openclaw.ai/concepts/retry).
...
Controls inbound/outbound prefixes and optional ack reactions. See [Messages](https://docs.openclaw.ai/concepts/messages) for queueing, sessions, and streaming context.
...
- `session.typingIntervalSeconds`: per-session override for the refresh interval. See [/concepts/typing-indicators](https://docs.openclaw.ai/concepts/typing-indicators) for behavior details.

`agents.defaults.model.primary` should be set as `provider/model` (e.g. `anthropic/claude-opus-4-5`). Aliases come from `agents.defaults.models.*.alias` (e.g. `Opus`). If you omit the provider, OpenClaw currently assumes `anthropic` as a temporary deprecation fallback. Z.AI models are available as `zai/<model>` (e.g. `zai/glm-4.7`) and require `ZAI_API_KEY` (or legacy `Z_AI_API_KEY`) in the environment. `agents.defaults.heartbeat` configures periodic heartbeat runs:
...
Optional **Docker sandboxing** for the embedded agent. Intended for non-main sessions so they cannot access your host system. Details: [Sandboxing](https://docs.openclaw.ai/gateway/sandboxing) Defaults (if enabled):
...
OpenCode Zen is a multi-model gateway with per-model endpoints. OpenClaw uses the built-in `opencode` provider from pi-ai; set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`) from [https://opencode.ai/auth](https://opencode.ai/auth). Notes:
...
Controls plugin discovery, allow/deny, and per-plugin config. Plugins are loaded from `~/.openclaw/extensions`, `<workspace>/.openclaw/extensions`, plus any `plugins.load.paths` entries. **Config changes require a gateway restart.** See [/plugin](https://docs.openclaw.ai/plugin) for full usage. Fields:
...
See [Gateway runbook](https://docs.openclaw.ai/gateway) for the derived port mapping (gateway/browser/canvas). See [Multiple gateways](https://docs.openclaw.ai/gateway/multiple-gateways) for browser/CDP port isolation details. Example:
...
*Next: [Agent Runtime](https://docs.openclaw.ai/concepts/agent)* 🦞