---
title: Paragraph
url: https://ratatui.rs/examples/widgets/paragraph/
source: crawler
fetched_at: 2026-02-01T21:12:54.600677613-03:00
rendered_js: false
word_count: 410
summary: This document provides a comprehensive code example demonstrating how to use the Ratatui Paragraph widget to display and style text. It illustrates features such as text alignment, line wrapping, scrolling, and the application of various visual styles and masked spans.
tags:
    - ratatui
    - rust-tui
    - paragraph-widget
...
cdratatui
cargorun--example=paragraph--features=crossterm
```
...
constTICK_RATE: Duration = Duration::from_millis(250);
...
self.handle_events()?;
ifself.last_tick.elapsed() >=Self::TICK_RATE {
self.on_tick();
...
lettimeout=Self::TICK_RATE.saturating_sub(self.last_tick.elapsed());
...
letareas= Layout::vertical([Constraint::Max(9); 4]).split(area);
...
fncreate_lines(area: Rect) -> Vec<Line<'static>> {
letshort_line="A long line to demonstrate line wrapping. ";
letlong_line=short_line.repeat(usize::from(area.width) /short_line.len() +4);
letmutstyled_spans=vec![];
forspanin [
"Styled".blue(),
"Spans".red().on_white(),
"Bold".bold(),
