---
title: utils - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/structured_output/utils/
source: sitemap
fetched_at: 2026-05-07T21:42:05.424861542-03:00
rendered_js: false
word_count: 275
summary: This document outlines utility functions for structured output in vLLM, including vocabulary management and the application of grammar bitmasks to model logits.
tags:
    - vllm
    - structured-output
    - logits
...
## OutlinesVocabulary [¶](#vllm.v1.structured_output.utils.OutlinesVocabulary "Permanent link")
...
## \_reduced\_vocabulary [¶](#vllm.v1.structured_output.utils._reduced_vocabulary "Permanent link")

Create a map from vocabulary tokens to lists of equivalent token ids.

Returns:

Type Description `dict[bytes, list[int]]`
...
```
def_reduced_vocabulary(tokenizer: TokenizerLike) -> dict[bytes, list[int]]:
"""Create a map from vocabulary tokens to lists of equivalent token ids.
    ...
    defconvert_token_to_string(token: str) -> str:
...
## apply\_grammar\_bitmask [¶](#vllm.v1.structured_output.utils.apply_grammar_bitmask "Permanent link")
...
*required* `input_batch` `InputBatch`

The input of model runner.

*required* `logits` `Tensor`
...
## convert\_lark\_to\_ebnf [¶](#vllm.v1.structured_output.utils.convert_lark_to_ebnf "Permanent link")
...
Name Type Description Default `grammar_str` `str`
...
```
defconvert_lark_to_ebnf(grammar_str: str) -> str:
"""
    Convert a Lark grammar string to EBNF format.
    ...
    defclean_line(line: str) -> str:
"""Remove comments and whitespace from line."""
        return re.sub(r"(#|//).*$", "", line).strip()

    defcheck_quotes(text: str, rule_name: str, line_num: int) -> None:
"""Validate quote matching in text."""
        if text.count("'") % 2 != 0 or text.count('"') % 2 != 0:
            raise ValueError(f"Mismatched quotes in {rule_name} on line {line_num}")

    defextract_references(text: str) -> set[str]:
"""Extract rule references from text."""
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        try:
            ...
            raise ValueError(f"Error on line {line_num}: {str(e)}") frome
...
## get\_outlines\_cache [¶](#vllm.v1.structured_output.utils.get_outlines_cache "Permanent link")
...
## get\_outlines\_cache\_path [¶](#vllm.v1.structured_output.utils.get_outlines_cache_path "Permanent link")
...
```
defget_outlines_cache_path() -> str:
"""Get the context object that contains previously-computed return values"""
...
## get\_outlines\_vocabulary [¶](#vllm.v1.structured_output.utils.get_outlines_vocabulary "Permanent link")
...
```
defget_outlines_vocabulary(tokenizer: TokenizerLike) -> oc.Vocabulary:
"""Get the `Vocabulary` object for a given tokenizer."""
...
## grammar\_is\_likely\_lark [¶](#vllm.v1.structured_output.utils.grammar_is_likely_lark "Permanent link")
...
```
defgrammar_is_likely_lark(grammar_str: str) -> bool:
"""
    Check if grammar appears to use Lark syntax.
