---
title: API Reference eSDK 3.211.126 | Spotify for Developers
url: https://developer.spotify.com/documentation/commercial-hardware/implementation/reference/3.211
source: crawler
fetched_at: 2026-02-27T23:41:28.374044-03:00
rendered_js: true
word_count: 15688
summary: This document provides a technical reference for the Spotify Embedded SDK header file, outlining the macros, data structures, enumerations, and callback functions necessary for integrating Spotify playback into embedded systems.
tags:
    - spotify-sdk
    - embedded-systems
    - api-reference
...
`#define SP_MAX_VIDEO_SURFACE 64`

Max length of the string representing the surface on which video is displayed.
...
### SP\_PLAYBACK\_RESTRICTION\_CONTENT\_RESTRICTED
...
`#define SP_PLAYBACK_RESTRICTION_CONTENT_RESTRICTED 8192`

The action is restricted for content reasons.
...
In some use cases it is preferred to not use the full bandwidth. This function asks eSDK to attempt to limit how fast it downloads a file. Please note that at the beginning of a download eSDK will first do a burst download and then try to obey the limit.

**Notes:**

- eSDK is not guaranteed to stay strictly below the limit but will not exceed it by much. There is also no guarantee that all available bandwidth will be used.
...
To register this callback, use the function [SpRegisterStreamCallbacks()](#spregisterstreamcallbacks). This callback will be called when there is a change affecting the playing position, such as skip, seek, track ends etc. It will also be called frequently at certain intervals when the Spotify service requires an update from the client. This is currently around when 30000 ms have been played, or during the playback of an ad.
...
[SpPlayUri](#spplayuri)Start local playback of the given Spotify URI. [SpPlayUriWithOptions](#spplayuriwithoptions)Start local playback of the given Spotify URI with additional options. [SpPlayContextUri](#spplaycontexturi)Start local playback of the given Spotify URI with an optional track UID. [SpPlayRecommendation](#spplayrecommendation)Start local playback of a Spotify recommendation. [SpQueueUri](#spqueueuri)Queue a URI. The track will be in queue only after kSpPlaybackNotifyQueuedTrackAccepted event is raised, but for UI purposes one can choose to monitor for kSpPlaybackNotifyMetadataChanged instead. [SpPlaybackBecomeActiveDevice](#spplaybackbecomeactivedevice)Become active without starting to play.
...
`SpError SpPlayRecommendation(int alias_index)`

Start local playback of a Spotify recommendation.

Using this call will 'pull' playback from any other Spotify Connect client active on the same account.
...
[SpRegisterTLSCallbacks](#spregistertlscallbacks)Register TLS-related callbacks.
