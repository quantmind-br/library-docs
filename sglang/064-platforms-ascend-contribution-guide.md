---
title: Contribution Guide — SGLang
url: https://docs.sglang.io/platforms/ascend_contribution_guide.html
source: crawler
fetched_at: 2026-02-04T08:48:16.643645589-03:00
rendered_js: false
word_count: 1278
summary: This document provides a comprehensive guide for contributing to the SGLang project, covering environment setup, testing procedures, coding standards, and the pull request process.
tags:
    - sglang
    - contribution-guide
    - development-setup
...
## Install SGLang from Source[#](#install-sglang-from-source "Link to this heading")

### Prepare Environment[#](#prepare-environment "Link to this heading")

Before contributing, please ensure that your environment is set up correctly. Follow the steps in the [Installation Guide](https://docs.sglang.io/platforms/ascend_npu.html) to install the necessary dependencies. we recommend [using docker](https://docs.sglang.io/platforms/ascend_npu.html#method-2-using-docker-image) to build the environment.

### Fork and clone the repository[#](#fork-and-clone-the-repository "Link to this heading")
...
```
gitclonehttps://github.com/<your_user_name>/sglang.git
# if you are using docker, the environment is already set up.
cdsglang
exportPYTHONPATH=$PWD/python:$PYTHONPATH
```

## Format code with pre-commit[#](#format-code-with-pre-commit "Link to this heading")
...
## Run and add unit tests[#](#run-and-add-unit-tests "Link to this heading")
...
## Write documentations[#](#write-documentations "Link to this heading")
...
## Test the accuracy[#](#test-the-accuracy "Link to this heading")
...
- [test\_moe\_eval\_accuracy\_large.py](https://github.com/sgl-project/sglang/blob/main/test/srt/test_moe_eval_accuracy_large.py)

## Benchmark the speed[#](#benchmark-the-speed "Link to this heading")

Refer to [Benchmark and Profiling](https://docs.sglang.io/developer_guide/benchmark_and_profiling.html).

## Requesting a review for merge[#](#requesting-a-review-for-merge "Link to this heading")

You can follow the pull request merge process described in [MAINTAINER.md](https://github.com/sgl-project/sglang/blob/main/.github/MAINTAINER.md). You will need to work with the Merge Oncall, Codeowner, and other reviewers to get their approvals. Then your PR can be merged.

## How to Trigger CI Tests[#](#how-to-trigger-ci-tests "Link to this heading")

We have a lot of open PRs but limited CI machines, so only top and trusted contributors have permission to trigger CI tests. Users with permission are listed in the [CI\_PERMISSIONS.json](https://github.com/sgl-project/sglang/blob/main/.github/CI_PERMISSIONS.json)
...
### CI rate limits[#](#ci-rate-limits "Link to this heading")
...
```
cool-down-minutes:
description:"Defaultcooldownperiodinminutes;0disablesratelimiting"
type:number
default:120
```

Users listed in [CI\_PERMISSIONS.json](https://github.com/sgl-project/sglang/blob/main/.github/CI_PERMISSIONS.json) may have a per-user cooldown interval. In practice, we use the minimum of the workflow’s default window and the user-specific interval.

## Code style guidance[#](#code-style-guidance "Link to this heading")
...
## How to update sgl-kernel[#](#how-to-update-sgl-kernel "Link to this heading")
...
## How to update sgl-kernel-npu[#](#how-to-update-sgl-kernel-npu "Link to this heading")
...
## Tips for newcomers[#](#tips-for-newcomers "Link to this heading")

If you want to contribute but don’t have a specific idea in mind, pick issues labeled [“good first issue” or “help wanted”](https://github.com/sgl-project/sglang/issues?q=is%3Aissue%20label%3A%22good%20first%20issue%22%2C%22help%20wanted%22). These tasks typically have lower complexity and provide an excellent introduction to the codebase. Also check out this [code walk-through](https://github.com/zhaochenyang20/Awesome-ML-SYS-Tutorial/tree/main/sglang/code-walk-through) for a deeper look into SGLang’s workflow.
