---
title: Repeat elements | Moodle Developer Resources
url: https://moodledev.io/docs/5.2/apis/subsystems/form/advanced/repeat-elements
source: sitemap
fetched_at: 2026-02-17T15:45:55.52491-03:00
rendered_js: false
word_count: 265
summary: This document explains how to use the Moodle Form API to create repeating groups of form elements using the repeat_elements method. It provides detailed parameter descriptions and configuration instructions for dynamically managing multiple sets of form fields.
tags:
    - moodle-form-api
    - repeat-elements
    - dynamic-forms
...
Most of the necessary information is in the phpdoc comment for the repeat\_elements() method:
...
- The list of elements you wish to repeat is set in the `$elementobjs` array, with any options passed into the `$options` array. A `{no}` placeholder can be placed into strings, such as the element label or default values, to represent the item number.
...
For other examples, have a look at the question type editing forms. They make extensive use of repeat\_elements().