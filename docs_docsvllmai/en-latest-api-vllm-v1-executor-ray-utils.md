---
title: ray_utils - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/executor/ray_utils/
source: sitemap
fetched_at: 2026-05-07T21:40:50.253562548-03:00
rendered_js: false
word_count: 621
summary: This document defines utility classes and methods for integrating vLLM with Ray, specifically focusing on managing worker execution, resource allocation, and asynchronous output handling.
tags:
    - ray-integration
...
## FutureWrapper [¶](#vllm.v1.executor.ray_utils.FutureWrapper "Permanent link")

Bases: `Future`

A wrapper around Ray output reference to meet the interface of .execute\_model(): The top level (core busy loop) expects .result() api to block and return a single output.
...
"""A wrapper around Ray output reference to meet the interface
    ...
    def__init__(self, ref_or_refs, aggregator: KVOutputAggregator | None = None):
        super().__init__()
...
## RayWorkerWrapper [¶](#vllm.v1.executor.ray_utils.RayWorkerWrapper "Permanent link")

Bases: `WorkerWrapperBase`

Ray wrapper for vllm.worker.Worker, allowing Worker to be lazily initialized after Ray sets CUDA\_VISIBLE\_DEVICES.

Source code in `vllm/v1/executor/ray_utils.py`

```
classRayWorkerWrapper(WorkerWrapperBase):
"""Ray wrapper for vllm.worker.Worker, allowing Worker to be
    ...
    defadjust_rank(self, rank_mapping: dict[int, int]) -> None:
"""
    ...
    defexecute_method(self, method: str | bytes, *args, **kwargs):
        try:
    ...
    defget_node_ip(self) -> str:
        return get_ip()

    defget_node_and_gpu_ids(self) -> tuple[str, list[int]]:
    ...
    defsetup_device_if_necessary(self):
    ...
    defoverride_env_vars(self, vars: dict[str, str]):
        os.environ.update(vars)

    def_is_intermediate_tensors(self, output) -> bool:
        return isinstance(output, IntermediateTensors)

    def_is_last_rank(self) -> bool:
        return get_pp_group().is_last_rank
```

### adjust\_rank [¶](#vllm.v1.executor.ray_utils.RayWorkerWrapper.adjust_rank "Permanent link")

Adjust the rpc\_rank based on the given mapping. It is only used during the initialization of the executor, to adjust the rpc\_rank of workers after we create all workers.
...
## \_verify\_bundles [¶](#vllm.v1.executor.ray_utils._verify_bundles "Permanent link")
...
There are 2 rules. - Warn if all tensor parallel workers cannot fit in a single node. - Fail if driver node is not included in a placement group (only when require\_gpu\_on\_driver is True).
...
## \_wait\_until\_pg\_ready [¶](#vllm.v1.executor.ray_utils._wait_until_pg_ready "Permanent link")
...
```
def_wait_until_pg_ready(current_placement_group: "PlacementGroup"):
"""Wait until a placement group is ready.
...
## assert\_ray\_available [¶](#vllm.v1.executor.ray_utils.assert_ray_available "Permanent link")
...
## build\_actor\_name [¶](#vllm.v1.executor.ray_utils.build_actor_name "Permanent link")
...
## detach\_zero\_copy\_from\_model\_runner\_output [¶](#vllm.v1.executor.ray_utils.detach_zero_copy_from_model_runner_output "Permanent link")
...
```
defdetach_zero_copy_from_model_runner_output(output: "ModelRunnerOutput") -> None:
"""Detach Ray SHM-channel zero-copy buffers from a ModelRunnerOutput in-place.
...
## get\_bundles\_for\_indices [¶](#vllm.v1.executor.ray_utils.get_bundles_for_indices "Permanent link")

Return GPU bundle indices paired with node IDs and node IPs for explicit bundle indices specified via VLLM\_RAY\_BUNDLE\_INDICES.

Source code in `vllm/v1/executor/ray_utils.py`

```
defget_bundles_for_indices(
    placement_group: "PlacementGroup",
...
## get\_bundles\_sorted\_by\_node [¶](#vllm.v1.executor.ray_utils.get_bundles_sorted_by_node "Permanent link")
...
## initialize\_ray\_cluster [¶](#vllm.v1.executor.ray_utils.initialize_ray_cluster "Permanent link")
...
## ray\_is\_available [¶](#vllm.v1.executor.ray_utils.ray_is_available "Permanent link")
