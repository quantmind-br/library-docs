...
url: https://doc.rust-lang.org/cargo/reference/manifest.html#the-exclude-and-include-fields
source: crawler
fetched_at: 2026-05-06T21:34:52.975335011-03:00
rendered_js: false
word_count: 3152
summary: This document provides a comprehensive reference for the structure and configuration fields of the Cargo.toml manifest file used in Rust projects.
tags:
    - rust
    - cargo
    - manifest
...
The `Cargo.toml` file for each package is called its *manifest*. It is written in the [TOML](https://toml.io/) format. It contains metadata that is needed to compile the package. Checkout the `cargo locate-project` section for more detail on how cargo finds the manifest file.
...
- [`[package]`](#the-package-section) — Defines a package.
  ...
  - [`default-run`](#the-default-run-field) — The default binary to run by [`cargo run`](https://doc.rust-lang.org/cargo/commands/cargo-run.html).
...
The only field required by Cargo is [`name`](#the-name-field). If publishing to a registry, the registry may require additional fields. See the notes below and [the publishing chapter](https://doc.rust-lang.org/cargo/reference/publishing.html) for requirements for publishing to [crates.io](https://crates.io/).
...
Note that [`cargo new`](https://doc.rust-lang.org/cargo/commands/cargo-new.html) and [`cargo init`](https://doc.rust-lang.org/cargo/commands/cargo-init.html) impose some additional restrictions on the package name, such as enforcing that it is a valid Rust identifier and not a keyword. [crates.io](https://crates.io/) imposes even more restrictions, such as:
...
The `version` field is formatted according to the [SemVer](https://semver.org) specification:
...
Cargo bakes in the concept of [Semantic Versioning](https://semver.org/), so versions are considered [compatible](https://doc.rust-lang.org/cargo/reference/semver.html) if their left-most non-zero major/minor/patch component is the same. See the [Resolver](https://doc.rust-lang.org/cargo/reference/resolver.html) chapter for more information on how Cargo uses versions to resolve dependencies.
...
The `edition` key is an optional key that affects which [Rust Edition](https://doc.rust-lang.org/edition-guide/index.html) your package is compiled with. Setting the `edition` key in `[package]` will affect all targets/crates in the package, including test suites, benchmarks, binaries, examples, etc.
...
Most manifests have the `edition` field filled in automatically by [`cargo new`](https://doc.rust-lang.org/cargo/commands/cargo-new.html) with the latest stable edition. By default `cargo new` creates a manifest with the 2024 edition currently.

If the `edition` field is not present in `Cargo.toml`, then the 2015 edition is assumed for backwards compatibility. Note that all manifests created with [`cargo new`](https://doc.rust-lang.org/cargo/commands/cargo-new.html) will not use this historical fallback because they will have `edition` explicitly specified to a newer value.

### [The `rust-version` field](#the-rust-version-field)

The `rust-version` field tells cargo what version of the Rust toolchain you support for your package. See [the Rust version chapter](https://doc.rust-lang.org/cargo/reference/rust-version.html) for more detail.
...
SPDX license expressions support AND and OR operators to combine multiple licenses.[1](#footnote-slash)
...
The `links` field specifies the name of a native library that is being linked to. More information can be found in the [`links`](https://doc.rust-lang.org/cargo/reference/build-scripts.html#the-links-manifest-key) section of the build script guide.
...
The `exclude` and `include` fields can be used to explicitly specify which files are included when packaging a project to be [published](https://doc.rust-lang.org/cargo/reference/publishing.html), and certain kinds of change tracking (described below). The patterns specified in the `exclude` field identify a set of files that are not included, and the patterns in `include` specify files that are explicitly included. You may run [`cargo package --list`](https://doc.rust-lang.org/cargo/commands/cargo-package.html) to verify which files will be included in the package.
...
- A minimized `Cargo.lock` is automatically included. See [`cargo package`](https://doc.rust-lang.org/cargo/commands/cargo-package.html) for more information.
...
Generally, these will only affect local development of the current package. Cargo only applies these to the current package and not to dependencies. As for dependents, Cargo suppresses lints from non-path dependencies with features like [`--cap-lints`](https://doc.rust-lang.org/rustc/lints/levels.html#capping-lints).

> **MSRV:** Respected as of 1.74

## [The `[hints]` section](#the-hints-section)
...
There are no stable hints at this time. See the [hint-mostly-unused documentation](https://doc.rust-lang.org/cargo/reference/unstable.html#profile-hint-mostly-unused-option) for information on an unstable hint.
...
See the [specifying dependencies page](https://doc.rust-lang.org/cargo/reference/specifying-dependencies.html) for information on the `[dependencies]`, `[dev-dependencies]`, `[build-dependencies]`, and target-specific `[target.*.dependencies]` sections.

## [The `[profile.*]` sections](#the-profile-sections)

The `[profile]` tables provide a way to customize compiler settings such as optimizations and debug settings. See [the Profiles chapter](https://doc.rust-lang.org/cargo/reference/profiles.html) for more detail.

* * *

1. Previously multiple licenses could be separated with a `/`, but that usage is deprecated. [↩](#fr-slash-1)