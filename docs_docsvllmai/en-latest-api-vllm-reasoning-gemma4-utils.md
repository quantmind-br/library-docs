---
title: gemma4_utils - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/reasoning/gemma4_utils/
source: sitemap
fetched_at: 2026-05-07T21:34:59.813307918-03:00
rendered_js: false
word_count: 332
summary: This document provides utilities for parsing and cleaning raw output strings from Gemma4 models, enabling the extraction of chain-of-thought reasoning and final answers.
tags:
    - gemma4
    - llm
    - offline-inference
...
## \_clean\_answer [¶](#vllm.reasoning.gemma4_utils._clean_answer "Permanent link")
...
## \_strip\_thought\_label [¶](#vllm.reasoning.gemma4_utils._strip_thought_label "Permanent link")
...
```
def_strip_thought_label(text: str) -> str:
"""Strip the spurious ``thought\\n`` label from the start of text.
...
## parse\_thinking\_output [¶](#vllm.reasoning.gemma4_utils.parse_thinking_output "Permanent link")
...
1. **Thinking enabled, tags present** — splits on `<|channel>`/ `<channel|>` to separate chain-of-thought from the answer and strips the `thought\n` role label.
2. **Thinking disabled, spurious label** — strips the bare `thought\n` prefix that some Gemma4 models emit even without thinking mode.
...
Decoded model output text (from `tokenizer.decode(...)`).
...
A dict with keys: - `"thinking"`: The chain-of-thought text, or `None` if no thinking delimiters were found. - `"answer"`: The final answer text.
...
```
defparse_thinking_output(text: str) -> dict[str, str | None]:
"""Parse decoded Gemma4 model output.
