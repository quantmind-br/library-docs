...
source: sitemap
fetched_at: 2026-04-26T08:35:55.648787608-03:00
rendered_js: false
word_count: 906
summary: This document provides instructions for installing the DankMaterialShell (DMS) across various Linux distributions including Arch, Fedora, CentOS, Debian, and Ubuntu.
tags:
    - linux-installation
    - dankmaterialshell
    - desktop-shell
...
For pre-built packages on Fedora, Debian, Ubuntu, and OpenSUSE, see [[039-docs-1.2-danklinux|DankLinux Repository]].
...
`dms` is available on the [AUR](https://aur.archlinux.org/). Use an AUR helper (e.g., `paru` or `yay`).

These packages ship the shell, widgets, and CLI. Pair with `niri`, `hyprland`, `sway`, `mangowc`, or `labwc` from official repositories.
...
Install the **stable release** or **latest development build** using `apt install dms` or `dms-git`.
...
> [!tip]
> Visit [[039-docs-1.2-danklinux|DankLinux Repository]] for OBS and PPA links. Add **niri** or **niri-git** for the best experience.
...
See [[008-docs-1.2-dankmaterialshell-nixos|NixOS Installation]].

## All Other Distributions

> [!warning]
> This guide does not cover compositor installation. You need a compatible Wayland compositor (niri, Hyprland, sway, dwl/MangoWC, etc.).
...
1. **Generate compositor config** (niri/Hyprland only): Run `dms setup` to create starter configuration with DMS keybinds and autostart. Other compositors (sway, MangoWC, labwc) require manual configuration.
2. Enable the systemd service (recommended) or add `dms run` to your compositor config
3. Configure keybinds — see [[075-docs-1.2-dankmaterialshell-keybinds-ipc|Keybinds & IPC]]
...
See [[007-docs-1.2-dankmaterialshell-managing|Managing Your Installation]] for service management, environment variables, and updates.

### Systemd Integration (Recommended)

> [!tip]
> If you used [[034-docs-1.2-dankinstall|dankinstall]], this is already configured. The installer runs `systemctl --user enable --now dms` during setup.
...
> [!tip]
> Using `add-wants` binds DMS to a specific compositor's service or session target, so it only runs where you want it. Using `systemctl --user enable dms` starts DMS in all desktop environments.
...
Hyprland does not initialize the systemd user session by default. Create a session target:
