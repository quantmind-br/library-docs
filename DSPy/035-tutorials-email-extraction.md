...
url: https://dspy.ai/tutorials/email_extraction/
source: sitemap
fetched_at: 2026-01-23T08:03:52.395697779-03:00
rendered_js: false
word_count: 330
summary: This tutorial explains how to build a structured email processing pipeline using DSPy for classification, entity extraction, and automated action generation.
tags:
    - dspy
    - email-processing
    - entity-extraction
...
[](https://github.com/stanfordnlp/dspy/blob/main/docs/docs/tutorials/email_extraction/index.md "Edit this page")
...
Recommended: Set up MLflow Tracing to understand what's happening under the hood. ### MLflow DSPy Integration [MLflow](https://mlflow.org/) is an LLMOps tool that natively integrates with DSPy and offer explainability and experiment tracking. In this tutorial, you can use MLflow to visualize prompts and optimization progress as traces to understand the DSPy's behavior better. You can set up MLflow easily by following the four steps below. !\[MLflow Trace](./mlflow-tracing-email-extraction.png) 1. Install MLflow

```
%pipinstallmlflow>=3.0.0
```

2\. Start MLflow UI in a separate terminal

```
mlflowui--port5000--backend-store-urisqlite:///mlruns.db
```
...
4\. Enabling tracing. To learn more about the integration, visit \[MLflow DSPy Documentation](https://mlflow.org/docs/latest/llms/dspy/index.html) as well.

## Step 1: Define Our Data Structures[¶](#step-1-define-our-data-structures "Permanent link")
...
classUrgencyLevel(str, Enum):
    LOW = "low"
    MEDIUM = "medium"
    HIGH = "high"
    CRITICAL = "critical"

classExtractedEntity(BaseModel):
    entity_type: str
    value: str
    confidence: float
```

## Step 2: Create DSPy Signatures[¶](#step-2-create-dspy-signatures "Permanent link")

Now let's define the signatures for our email processing pipeline:

```
classClassifyEmail(dspy.Signature):
...
classExtractEntities(dspy.Signature):
...
classGenerateActionItems(dspy.Signature):
...
classSummarizeEmail(dspy.Signature):
...
## Step 3: Build the Email Processing Module[¶](#step-3-build-the-email-processing-module "Permanent link")

Now let's create our main email processing module:

```
classEmailProcessor(dspy.Module):
"""A comprehensive email processing system using DSPy."""
    ...
    defforward(self, email_subject: str, email_body: str, sender: str = ""):
...
## Step 4: Running the Email Processing System[¶](#step-4-running-the-email-processing-system "Permanent link")

Let's create a simple function to test our email processing system:

```
importos
defrun_email_processing_demo():
...
## Expected Output[¶](#expected-output "Permanent link")
