...
### Step 1. Setup Proxy[​](#step-1-setup-proxy "Direct link to Step 1. Setup Proxy")
...
### Step 2. Create JWT with scopes[​](#step-2-create-jwt-with-scopes "Direct link to Step 2. Create JWT with scopes")
...
### Step 3. Test your JWT[​](#step-3-test-your-jwt "Direct link to Step 3. Test your JWT")
...
### Multiple OIDC providers[​](#multiple-oidc-providers "Direct link to Multiple OIDC providers")
...
### Kubernetes ServiceAccount Authentication[​](#kubernetes-serviceaccount-authentication "Direct link to Kubernetes ServiceAccount Authentication")
...
#### Step 1: Configure the OIDC Discovery URL[​](#step-1-configure-the-oidc-discovery-url "Direct link to Step 1: Configure the OIDC Discovery URL")
...
#### Step 2: Configure LiteLLM[​](#step-2-configure-litellm "Direct link to Step 2: Configure LiteLLM")
...
#### Step 3: Create ServiceAccount and Configure Pod[​](#step-3-create-serviceaccount-and-configure-pod "Direct link to Step 3: Create ServiceAccount and Configure Pod")
...
#### Step 4: Create Team for Namespace[​](#step-4-create-team-for-namespace "Direct link to Step 4: Create Team for Namespace")
...
#### Step 5: Use the Token[​](#step-5-use-the-token "Direct link to Step 5: Use the Token")
...
#### Example: ServiceAccount Token Structure[​](#example-serviceaccount-token-structure "Direct link to Example: ServiceAccount Token Structure")
...
#### Advanced: Map Namespace to Team Using Name Resolution[​](#advanced-map-namespace-to-team-using-name-resolution "Direct link to Advanced: Map Namespace to Team Using Name Resolution")
...
### Set Accepted JWT Scope Names[​](#set-accepted-jwt-scope-names "Direct link to Set Accepted JWT Scope Names")
...
### Tracking End-Users / Internal Users / Team / Org[​](#tracking-end-users--internal-users--team--org "Direct link to Tracking End-Users / Internal Users / Team / Org")
...
### Resolve by Name (Alias) Instead of ID[​](#resolve-by-name-alias-instead-of-id "Direct link to Resolve by Name (Alias) Instead of ID")
...
### JWT Scopes[​](#jwt-scopes "Direct link to JWT Scopes")
...
### Control model access with Teams[​](#control-model-access-with-teams "Direct link to Control model access with Teams")
...
- Validate if user id is in the DB (LiteLLM\_UserTable)
- Validate if any of the groups are in the DB (LiteLLM\_TeamTable)
...
### Custom JWT Validate[​](#custom-jwt-validate "Direct link to Custom JWT Validate")

Validate a JWT Token using custom logic, if you need an extra way to verify if tokens are valid for LiteLLM Proxy.

#### 1. Setup custom validate function[​](#1-setup-custom-validate-function "Direct link to 1. Setup custom validate function")

```
from typing import Literal

defmy_custom_validate(token:str)-> Literal[True]:
"""
...
if token["tenant_id"]notin allowed_tenants:
raise Exception("Invalid JWT token")
if token["claims"]notin allowed_claims:
raise Exception("Invalid JWT token")
returnTrue
```

#### 2. Setup config.yaml[​](#2-setup-configyaml "Direct link to 2. Setup config.yaml")
...
#### 3. Test the flow[​](#3-test-the-flow "Direct link to 3. Test the flow")
...
### Allowed Routes[​](#allowed-routes "Direct link to Allowed Routes")
...
### Allowing other provider routes for Teams[​](#allowing-other-provider-routes-for-teams "Direct link to Allowing other provider routes for Teams")
...
### Caching Public Keys[​](#caching-public-keys "Direct link to Caching Public Keys")
...
### Custom JWT Field[​](#custom-jwt-field "Direct link to Custom JWT Field")

Set a custom field in which the team\_id exists. By default, the 'client\_id' field is checked.
...
### Block Teams[​](#block-teams "Direct link to Block Teams")
...
### Upsert Users + Allowed Email Domains[​](#upsert-users--allowed-email-domains "Direct link to Upsert Users + Allowed Email Domains")
...
## OIDC UserInfo Endpoint[​](#oidc-userinfo-endpoint "Direct link to OIDC UserInfo Endpoint")
...
### Example: Azure AD[​](#example-azure-ad "Direct link to Example: Azure AD")
...
### Example: Keycloak[​](#example-keycloak "Direct link to Example: Keycloak")
...
## \[BETA] Control Access with OIDC Roles[​](#beta-control-access-with-oidc-roles "Direct link to [BETA] Control Access with OIDC Roles")
...
### Role Mapping Spec[​](#role-mapping-spec "Direct link to Role Mapping Spec")
...
### [Architecture Diagram (Control Model Access)](https://docs.litellm.ai/docs/proxy/jwt_auth_arch)[​](#architecture-diagram-control-model-access "Direct link to architecture-diagram-control-model-access")

## \[BETA] Control Model Access with Scopes[​](#beta-control-model-access-with-scopes "Direct link to [BETA] Control Model Access with Scopes")

Control which models a JWT can access. Set `enforce_scope_based_access: true` to enforce scope-based access control.

### 1. Setup config.yaml with scope mappings.[​](#1-setup-configyaml-with-scope-mappings "Direct link to 1. Setup config.yaml with scope mappings.")
...
#### Scope Mapping Spec[​](#scope-mapping-spec "Direct link to Scope Mapping Spec")
...
### 2. Create a JWT with the correct scopes.[​](#2-create-a-jwt-with-the-correct-scopes "Direct link to 2. Create a JWT with the correct scopes.")
...
### 3. Test the flow.[​](#3-test-the-flow-1 "Direct link to 3. Test the flow.")
...
## \[BETA] Sync User Roles and Teams with IDP[​](#beta-sync-user-roles-and-teams-with-idp "Direct link to [BETA] Sync User Roles and Teams with IDP")
...
#### 1. Configure JWT Role Mapping[​](#1-configure-jwt-role-mapping "Direct link to 1. Configure JWT Role Mapping")
...
#### 2. JWT Role Mapping Spec[​](#2-jwt-role-mapping-spec "Direct link to 2. JWT Role Mapping Spec")
...
#### 3. Example JWT Token[​](#3-example-jwt-token "Direct link to 3. Example JWT Token")
...
### Testing the Sync Feature[​](#testing-the-sync-feature "Direct link to Testing the Sync Feature")
...
## All JWT Params[​](#all-jwt-params "Direct link to All JWT Params")
