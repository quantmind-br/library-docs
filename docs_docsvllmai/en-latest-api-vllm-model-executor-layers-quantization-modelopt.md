---
title: modelopt - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/quantization/modelopt/
source: sitemap
fetched_at: 2026-05-07T21:27:23.296195172-03:00
rendered_js: false
word_count: 643
summary: This document defines the configuration and implementation classes for Model Optimizer FP8 quantization within the vLLM framework, supporting static weight and activation scaling for linear and MoE layers.
tags:
    - vllm
    - fp8-quantization
    - model-optimizer
...
## ModelOptFp8Config [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptFp8Config "Permanent link")
...
```
classModelOptFp8Config(ModelOptQuantConfigBase):
"""Config class for ModelOpt FP8."""
...
## ModelOptFp8LinearMethod [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptFp8LinearMethod "Permanent link")
...
Limitations: 1. Only support per-tensor quantization due to torch.\_scaled\_mm support. 2. Only support float8\_e4m3fn datatype Args: quant\_config: The ModelOpt quantization config.

Source code in `vllm/model_executor/layers/quantization/modelopt.py`

```
classModelOptFp8LinearMethod(LinearMethodBase):
"""Linear method for Model Optimizer static quantization.
    ...
    def__init__(self, quant_config: ModelOptFp8Config) -> None:
...
## ModelOptFp8MoEMethod [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptFp8MoEMethod "Permanent link")

Bases: `FusedMoEMethodBase`

MoE method for ModelOpt FP8. Supports loading FP8 checkpoints with static weight scale and activation scale. Args: quant\_config: The ModelOpt quantization config.

Source code in `vllm/model_executor/layers/quantization/modelopt.py`

```
classModelOptFp8MoEMethod(FusedMoEMethodBase):
...
## ModelOptFp8PbWoLinearMethod [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptFp8PbWoLinearMethod "Permanent link")

Bases: `LinearMethodBase`

Linear method for ModelOpt FP8\_PB\_WO checkpoints.

ModelOpt exports `weight_scale` as a 4D tensor: \[out\_blk, 1, in\_blk, 1] where block size is typically 128 for both dims.
...
```
classModelOptFp8PbWoLinearMethod(LinearMethodBase):
...
## ModelOptFp8PcPtLinearMethod [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptFp8PcPtLinearMethod "Permanent link")

Bases: `LinearMethodBase`

Linear method for ModelOpt FP8\_PER\_CHANNEL\_PER\_TOKEN checkpoints.

Expected checkpoint structure (per Linear): - weight: fp8-e4m3fn, shape \[out, in] - weight\_scale: fp32, shape \[out] (per-output-channel) - no input\_scale (activations are dynamically quantized per-token)

Source code in `vllm/model_executor/layers/quantization/modelopt.py`

```
classModelOptFp8PcPtLinearMethod(LinearMethodBase):
...
## ModelOptKVCacheMethod [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptKVCacheMethod "Permanent link")
...
```
classModelOptKVCacheMethod(BaseKVCacheMethod):
"""
    Supports loading kv-cache scaling factors from FP8 or NVFP4 checkpoints.
    """

    def__init__(self, quant_config: "ModelOptQuantConfigBase"):
        super().__init__(quant_config)
```

## ModelOptMixedPrecisionConfig [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptMixedPrecisionConfig "Permanent link")

Bases: `ModelOptQuantConfigBase`

Config class for ModelOpt MIXED\_PRECISION.
...
```
classModelOptMixedPrecisionConfig(ModelOptQuantConfigBase):
"""Config class for ModelOpt MIXED_PRECISION.
    ...
    def_resolve_quant_algo(self, prefix: str) -> str | None:
...
### \_resolve\_quant\_algo [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptMixedPrecisionConfig._resolve_quant_algo "Permanent link")

```
_resolve_quant_algo(prefix: str) -> str | None
```

Look up the quant\_algo for a vLLM-side layer prefix.

Tries three strategies in order: 1. Direct lookup in `quantized_layers`. 2. Packed/fused-layer lookup (unfuse via `packed_modules_mapping`). 3. Prefix-based lookup for FusedMoE (any child key starts with `prefix + "."`).

Returns the upper-cased quant\_algo string, or *None* if the prefix is not found.
...
### get\_quant\_method [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptMixedPrecisionConfig.get_quant_method "Permanent link")
...
## ModelOptMxFp8Config [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptMxFp8Config "Permanent link")
...
```
classModelOptMxFp8Config(ModelOptQuantConfigBase):
"""Config class for ModelOpt MXFP8."""
...
## ModelOptMxFp8FusedMoE [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptMxFp8FusedMoE "Permanent link")
...
```
classModelOptMxFp8FusedMoE(FusedMoEMethodBase):
"""FlashInfer TRTLLM MXFP8 block-scale MoE for ModelOpt checkpoints."""
    ...
    @staticmethod
    def_check_weight_dtypes(layer: torch.nn.Module) -> None:
"""Validate weight and scale dtypes before processing."""
    ...
    def_shuffle_weights_for_trtllm(self, layer: torch.nn.Module) -> None:
"""Shuffle weights and scales into FlashInfer TRTLLM MXFP8 layout."""
    ...
    defapply_monolithic(
        ...
        fromflashinfer.fused_moe.coreimport (
            ActivationType,
            Fp8QuantizationType,
        )
...
### \_check\_weight\_dtypes `staticmethod` [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptMxFp8FusedMoE._check_weight_dtypes "Permanent link")

```
_check_weight_dtypes(layer: Module) -> None
```
...
### \_shuffle\_weights\_for\_trtllm [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptMxFp8FusedMoE._shuffle_weights_for_trtllm "Permanent link")

```
_shuffle_weights_for_trtllm(layer: Module) -> None
```
...
## ModelOptMxFp8LinearMethod [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptMxFp8LinearMethod "Permanent link")
...
```
classModelOptMxFp8LinearMethod(LinearMethodBase):
"""Linear method for ModelOpt MXFP8 quantization."""

    def__init__(self, quant_config: ModelOptMxFp8Config) -> None:
        self.quant_config = quant_config
...
## ModelOptNvFp4Config [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptNvFp4Config "Permanent link")
...
```
classModelOptNvFp4Config(ModelOptQuantConfigBase):
"""Config class for ModelOpt FP4."""
...
## ModelOptNvFp4FusedMoE [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptNvFp4FusedMoE "Permanent link")

Bases: `FusedMoEMethodBase`

MoE Method for FP4 Quantization. Args: quant\_config: NVFP4 Quant Config

Source code in `vllm/model_executor/layers/quantization/modelopt.py`

```
classModelOptNvFp4FusedMoE(FusedMoEMethodBase):
"""
...
"""
    ...
    defprocess_weights_after_loading(self, layer: FusedMoE) -> None:
"""
...
### process\_weights\_after\_loading [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptNvFp4FusedMoE.process_weights_after_loading "Permanent link")
...
### uses\_weight\_scale\_2\_pattern [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptNvFp4FusedMoE.uses_weight_scale_2_pattern "Permanent link")

```
uses_weight_scale_2_pattern() -> bool
```

FP4 variants use 'weight\_scale\_2' pattern for per-tensor weight scales.
...
## ModelOptNvFp4LinearMethod [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptNvFp4LinearMethod "Permanent link")

Bases: `LinearMethodBase`

Linear method for Model Optimizer NVFP4. Supports loading NVFP4 checkpoints with the following structure:

input\_scale: torch.float32, scalar , weight: NVFP4(represented as byte) Shape: \[1, X, y/2] weight\_scale: FP8-E4M3, Shape: \[X, Y], aka per block scale, weight\_scale\_2: torch.float32, scalar, Args: quant\_config: The ModelOpt quantization config.

Source code in `vllm/model_executor/layers/quantization/modelopt.py`

```
classModelOptNvFp4LinearMethod(LinearMethodBase):
"""Linear method for Model Optimizer NVFP4.
    ...
    def__init__(self, quant_config: ModelOptNvFp4Config) -> None:
        self.quant_config = quant_config
...
## ModelOptQuantConfigBase [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptQuantConfigBase "Permanent link")
...
```
classModelOptQuantConfigBase(QuantizationConfig):
    ...
    defis_layer_excluded(self, prefix: str) -> bool:
"""
    ...
    @staticmethod
    def_extract_modelopt_quant_algo(
...
"""Extract upper-cased quant_algo from a modelopt config.
    ...
    @classmethod
    deffrom_config(cls, config: dict[str, Any]) -> "ModelOptQuantConfigBase":
...
_extract_modelopt_quant_algo(
    hf_quant_cfg: dict[str, Any] | None,
) -> str | None
```

Extract upper-cased quant\_algo from a modelopt config.

Returns the quant\_algo string (upper-cased), or None if the config is not a modelopt config.
...
### is\_layer\_excluded [¶](#vllm.model_executor.layers.quantization.modelopt.ModelOptQuantConfigBase.is_layer_excluded "Permanent link")
...
The ModelOpt exclude\_modules list is a list of wildcards.
