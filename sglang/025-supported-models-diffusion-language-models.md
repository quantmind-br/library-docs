---
title: Diffusion Language Models — SGLang
url: https://docs.sglang.io/supported_models/diffusion_language_models.html
source: crawler
fetched_at: 2026-02-04T08:47:02.478274919-03:00
rendered_js: false
word_count: 92
summary: This document provides instructions for launching and configuring diffusion language models for text generation using the sglang framework, including server setup and client-side integration.
tags:
    - diffusion-language-models
    - sglang
    - text-generation
    - llada
    - model-deployment
    - non-autoregressive
category: guide
---

## Contents

## Diffusion Language Models[#](#diffusion-language-models "Link to this heading")

Diffusion language models have shown promise for non-autoregressive text generation with parallel decoding capabilities. Unlike auto-regressive language models, different diffusion language models require different decoding strategies.

## Example Launch Command[#](#example-launch-command "Link to this heading")

```
python3-msglang.launch_server\
--model-pathinclusionAI/LLaDA2.0-mini\ # example HF/local path
--dllm-algorithmLowConfidence\
--dllm-algorithm-config./config.yaml\ # Optional. Uses the algorithm's default if not set.
--host0.0.0.0\
--port30000
```

## Example Configuration File[#](#example-configuration-file "Link to this heading")

```
# Confidence threshold for accepting predicted tokens
# - Higher values: More conservative, better quality but slower
# - Lower values: More aggressive, faster but potentially lower quality
# Range: 0.0 - 1.0
threshold:0.95

# Default: 32, for LLaDA2MoeModelLM
block_size:32
```

## Example Client Code Snippet[#](#example-client-code-snippet "Link to this heading")

Just like other supported models, diffusion language models can be used via the REST API or Python client.

Python client example for making a generation request to the launched server:

```
importsglangassgl

defmain():
    llm = sgl.Engine(model_path="inclusionAI/LLaDA2.0-mini",
                     dllm_algorithm="LowConfidence",
                     max_running_requests=1,
                     trust_remote_code=True)

    prompts = [
        "<role>SYSTEM</role>detailed thinking off<|role_end|><role>HUMAN</role> Write a brief introduction of the great wall <|role_end|><role>ASSISTANT</role>"
    ]

    sampling_params = {
        "temperature": 0,
        "max_new_tokens": 1024,
    }

    outputs = llm.generate(prompts, sampling_params)
    print(outputs)

if __name__ == '__main__':
    main()
```

Curl example for making a generation request to the launched server:

```
curl-XPOST"http://127.0.0.1:30000/generate"\
-H"Content-Type: application/json"\
-d'{
        "text": [
            "<role>SYSTEM</role>detailed thinking off<|role_end|><role>HUMAN</role> Write the number from 1 to 128 <|role_end|><role>ASSISTANT</role>",
            "<role>SYSTEM</role>detailed thinking off<|role_end|><role>HUMAN</role> Write a brief introduction of the great wall <|role_end|><role>ASSISTANT</role>"
        ],
        "stream": true,
        "sampling_params": {
            "temperature": 0,
            "max_new_tokens": 1024
        }
    }'
```

## Supported Models[#](#supported-models "Link to this heading")

Below the supported models are summarized in a table.