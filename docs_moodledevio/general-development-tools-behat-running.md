---
title: Running acceptance tests | Moodle Developer Resources
url: https://moodledev.io/general/development/tools/behat/running
source: sitemap
fetched_at: 2026-02-17T16:01:10.995878-03:00
rendered_js: false
word_count: 3702
summary: This document explains how to set up and run Behat automated functional tests within Moodle, covering system requirements, browser configuration, and Selenium integration.
tags:
    - behat
    - moodle-testing
    - automated-testing
...
1. Any recent OS with [supported version of Moodle](https://moodledev.io/general/releases) installed
2. A recent browser (Firefox and Chrome as standard, but [other browsers](https://moodledev.io/general/development/tools/behat/browsers) are possible)
...
#### Pre-configured browser profiles: moodle-browser-config[​](#pre-configured-browser-profiles-moodle-browser-config "Direct link to Pre-configured browser profiles: moodle-browser-config")

Available for [all supported versions of Moodle](https://moodledev.io/general/releases), the [moodle-browser-config](https://github.com/andrewnicols/moodle-browser-config) is a recommended inclusion for Behat. This configuration tooling provides a range of standard browser profiles for testing.
...
##### Provided profiles[​](#provided-profiles "Direct link to Provided profiles")
...
Profile nameDescriptionUses Selenium?Displays GUI?firefoxUse Firefox via SeleniumYesYesheadlessfirefoxUse Firefox via Selenium, without displaying the GUIYesNogeckodriverUse Firefox with Geckodriver directlyNoYesheadlessgeckodriverUse Firefox with Geckodriver directly, without displaying the GUINoNochromeUse Chrome via SeleniumYesYesheadlesschromeUse Chrome via Selenium, without displaying the GUIYesNochromedriverUse Chrome with Chromedriver directlyNoYesheadlesschromedriverUse Chrome with Chromedriver directly, without displaying the GUINoNoedgeUse Edge via SeleniumYesYesheadlessedgeUse Edge via Selenium, without displaying the GUIYesNoedgedriverUse Edge with Edgedriver directlyNoYesheadlessedgedriverUse Edge with Edgedriver directly, without displaying the GUINoNo

#### chromedriver-wrapper[​](#chromedriver-wrapper "Direct link to chromedriver-wrapper")
...
These notes assume that you have already installed a supported Java Runtime Environment, and the [moodle-browser-config](https://moodledev.io/general/development/tools/behat/running#pre-configured-browser-profiles-moodle-browser-config) tool.

### Setting up Selenium[​](#setting-up-selenium "Direct link to Setting up Selenium")
...
There appears to be an issue with the version 4 jars - particularly in 4.1.0 that generates a **"Could not open connection: Capability value must be set"** error which appears to be an issue between Selenium and the MInkExtension for JavaScript tests - see [https://githubmemory.com/repo/Behat/MinkExtension/activity](https://githubmemory.com/repo/Behat/MinkExtension/activity) for more information. The quick work around is to use the earlier version (3.141.59).
...
### Setting up your browsers[​](#setting-up-your-browsers "Direct link to Setting up your browsers")
...
You will need the [correct version of the chromedriver](https://chromedriver.chromium.org/downloads) as per the documentation. Alternatively you can make use of the [chromedriver-wrapper utility](https://moodledev.io/general/development/tools/behat/running#chromedriver-wrapper) noted in the Recommended extras sections.
...
### Setting up Moodle[​](#setting-up-moodle "Direct link to Setting up Moodle")
...
### Configure Behat for Moodle[​](#configure-behat-for-moodle "Direct link to Configure Behat for Moodle")
...
### Run Behat tests[​](#run-behat-tests "Direct link to Run Behat tests")
...
#### Running using a different browser[​](#running-using-a-different-browser "Direct link to Running using a different browser")
...
If you are using the `moodle-browser-config` utility, then you can use any profile listed in [moodle-browser-config](https://moodledev.io/general/development/tools/behat/running#pre-configured-browser-profiles-moodle-browser-config). Otherwise you can write your own browser profile configuration.

### Advanced testing[​](#advanced-testing "Direct link to Advanced testing")

#### Run tests without Selenium (chromedriver, geckodriver)[​](#run-tests-without-selenium-chromedriver-geckodriver "Direct link to Run tests without Selenium (chromedriver, geckodriver)")
...
#### Parallel runs[​](#parallel-runs "Direct link to Parallel runs")
...
##### Running Parallel tests[​](#running-parallel-tests "Direct link to Running Parallel tests")
...
##### Custom parameters for parallel runs[​](#custom-parameters-for-parallel-runs "Direct link to Custom parameters for parallel runs")

You can set following custom config options for parallel runs via $CFG-&gt;behat\_parallel\_run. It's an array of options where 1st array is for 1st run and so on.
...
#### Tests filters[​](#tests-filters "Direct link to Tests filters")
...
- `@javascript`: All the tests that runs in a browser using JavaScript; they require Selenium or the browser's own automation layer, as per [Run tests without Selenium](https://moodledev.io/general/development/tools/behat/running#run-tests-without-selenium-chromedriver-geckodriver) to be running, otherwise an exception will be thrown.
...
- `@_cross_browser`: All the tests that should run against multiple combinations of browsers + OS in a regular basis. The features that are sensitive to different combinations of OS and browsers should be tagged as @\_cross\_browser.
- `@componentname`: Moodle features uses the [Frankenstyle](https://moodledev.io/general/development/policies/codingstyle/frankenstyle) component name to tag the features according to the Moodle subsystem they belong to.
...
01. `progress`: Prints one character per step.
02. `pretty`: Prints the feature as is.
03. `junit`: Outputs the failures in JUnit compatible files.
04. `moodle_progress`: Prints Moodle branch information and dots for each step.
05. `moodle_list`: List all scenarios.
06. `moodle_stepcount`: List all features with total steps in each feature file. Used for parallel run.
07. `moodle_screenshot`: Take screenshot and core dump of each step. With following options you can dump either or both.
08. `--format-settings '{"formats": "image"}'`: will dump image only
09. `--format-settings '{"formats": "html"}'`: will dump html only.
10. `--format-settings '{"formats": "html,image"}'`: will dump both.
...
#### Rerun failed scenarios[​](#rerun-failed-scenarios "Direct link to Rerun failed scenarios")
...
#### Running behat with specified theme[​](#running-behat-with-specified-theme "Direct link to Running behat with specified theme")
...
#### Using Docker to start selenium server[​](#using-docker-to-start-selenium-server "Direct link to Using Docker to start selenium server")
...
#### Change config.php file[​](#change-configphp-file "Direct link to Change config.php file")
...
#### Manually configuring other browsers[​](#manually-configuring-other-browsers "Direct link to Manually configuring other browsers")
...
See [alternative browsers](https://moodledev.io/general/development/tools/behat/browsers) for more details.

## Troubleshooting[​](#troubleshooting "Direct link to Troubleshooting")

### Increasing timeouts[​](#increasing-timeouts "Direct link to Increasing timeouts")
...
### New step definitions or features are not executed[​](#new-step-definitions-or-features-are-not-executed "Direct link to New step definitions or features are not executed")
...
### Tests are failing[​](#tests-are-failing "Direct link to Tests are failing")

If you followed all the steps and you receive an unknown weird error probably your browser version is not compatible with the Selenium version you are running. Please refer to [Working combinations](https://moodledev.io/general/development/tools/behat/browsers#compatibility) to run the acceptance test.

### The tests are failing, and the error message is completely useless[​](#the-tests-are-failing-and-the-error-message-is-completely-useless "Direct link to The tests are failing, and the error message is completely useless")
...
### Errors during setup (before test are launched)[​](#errors-during-setup-before-test-are-launched "Direct link to Errors during setup (before test are launched)")

Typical errors are:

- Behat requirement not satisfied: [http://127.0.0.1/m/stable\_master](http://127.0.0.1/m/stable_master) is not available, ensure you specified correct url and that the server is set up and started.
...
### Selenium server is not running[​](#selenium-server-is-not-running "Direct link to Selenium server is not running")

#### Chrome specific[​](#chrome-specific "Direct link to Chrome specific")
...
### Quick setup and testing using moodle-docker[​](#quick-setup-and-testing-using-moodle-docker "Direct link to Quick setup and testing using moodle-docker")
...
- [Acceptance testing for the mobile app](https://moodledev.io/general/app/development/testing/acceptance-testing)
- Vagrant profile with Moodle and Behat preconfigured: [https://github.com/mackensen/moodle-hat](https://github.com/mackensen/moodle-hat)
- Docker containers for Moodle Developers and Behat: [https://github.com/moodlehq/moodle-docker](https://github.com/moodlehq/moodle-docker)
- Docker environment with Behat preconfigured : [https://github.com/tobiga/docker\_moodle\_environment](https://github.com/tobiga/docker_moodle_environment)