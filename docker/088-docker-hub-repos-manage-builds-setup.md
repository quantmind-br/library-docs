...
source: llms
fetched_at: 2026-01-24T14:21:49.938863339-03:00
rendered_js: false
word_count: 1603
summary: This document explains how to configure automated builds in Docker Hub to trigger image creation from source code pushes to GitHub or Bitbucket. It covers setting up build rules, specifying Dockerfile locations, and managing environment variables for the build process.
tags:
    - docker-hub
    - automated-builds
    - cicd
...
01. In [Docker Hub](https://hub.docker.com), go to **My Hub** &gt; **Repositories**, and select a repository to view its details.
02. Select the **Builds** tab.
03. Select either GitHub or Bitbucket to connect where the image's source code is stored.
...
04. Select the **source repository** to build the Docker images from.
...
05. Optional. Enable [autotests](https://docs.docker.com/docker-hub/repos/manage/builds/automated-testing/#enable-automated-tests-on-a-repository).
06. Review the default **Build Rules**.
...
07. Optional. Select the **plus** icon to add and [configure more build rules](#set-up-build-rules).
08. For each branch or tag, enable or disable the **Autobuild** toggle.
...
09. For each branch or tag, enable or disable the **Build Caching** toggle.
...
Action/PermissionReadWriteAdminOwnerview build detailsxxxxstart, cancel, retryxxxedit build settingsxxdelete buildx
