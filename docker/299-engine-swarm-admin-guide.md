...
source: llms
fetched_at: 2026-01-24T14:25:44.181923214-03:00
rendered_js: false
word_count: 2535
summary: This document explains the role and management of manager nodes in a Docker Swarm, focusing on maintaining quorum, fault tolerance, and best practices for cluster stability.
tags:
    - docker-swarm
    - manager-nodes
    - raft-consensus
...
Swarm SizeMajorityFault Tolerance110220**3**2**1**431**5**3**2**642**7**4**3**853**9**5**4**
...
Swarm manager nodesRepartition (on 3 Availability zones)31-1-152-2-173-2-293-3-3
...
You can monitor the health of manager nodes by querying the docker `nodes` API in JSON format through the `/nodes` HTTP endpoint. Refer to the [nodes API documentation](https://docs.docker.com/reference/api/engine/version/v1.25/#tag/Node) for more information.
