---
title: glmasr - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/models/glmasr/
source: sitemap
fetched_at: 2026-05-07T21:30:29.057502437-03:00
rendered_js: false
word_count: 829
summary: This document defines the architecture and implementation details for the GLM-ASR model components within vLLM, including dummy input generation, embedding schemas, and the optimized audio encoder.
tags:
    - vllm
    - glm-asr
    - audio-processing
...
## GlmAsrDummyInputsBuilder [¶](#vllm.model_executor.models.glmasr.GlmAsrDummyInputsBuilder "Permanent link")
...
```
classGlmAsrDummyInputsBuilder(BaseDummyInputsBuilder[GlmAsrProcessingInfo]):
"""
    Builder for dummy inputs used in profiling and testing.
...
## GlmAsrEmbeddingInputs [¶](#vllm.model_executor.models.glmasr.GlmAsrEmbeddingInputs "Permanent link")
...
```
classGlmAsrEmbeddingInputs(TensorSchema):
"""
    Dimensions:
        - bn: Batch size
        - naf: Number of audio features
...
## GlmAsrEncoder [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoder "Permanent link")
...
"""
    ...
    defforward(self, input_features: torch.Tensor) -> _GlmAsrEncoderOutput:
"""
...
"""Custom weight loading to handle q_proj/k_proj/v_proj -> qkv_proj mapping."""
        fromvllm.model_executor.model_loader.weight_utilsimport default_weight_loader
...
Name Type Description Default `input_lengths` `Tensor`

Input sequence lengths \[batch\_size]
...
### forward [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoder.forward "Permanent link")

Forward pass through the encoder.

Parameters:

Name Type Description Default `input_features` `Tensor`

\[batch\_size, num\_mel\_bins, seq\_len]

*required*

Returns:

Name Type Description `_GlmAsrEncoderOutput` `_GlmAsrEncoderOutput`

Object with .last\_hidden\_state attribute containing \[batch\_size, seq\_len', hidden\_size] where seq\_len' is the sequence length after convolutions
...
### load\_weights [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoder.load_weights "Permanent link")

Custom weight loading to handle q\_proj/k\_proj/v\_proj -&gt; qkv\_proj mapping.
...
## GlmAsrEncoderAttention [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoderAttention "Permanent link")
...
```
classGlmAsrEncoderAttention(nn.Module):
"""
    Optimized Multi-headed Grouped Query Attention for GLM-ASR encoder.
...
### forward [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoderAttention.forward "Permanent link")
...
*required* `rotary_pos_emb_cos` `Tensor`

\[seq\_len, rotary\_dim/2] - cosine of rotary embeddings

*required* `rotary_pos_emb_sin` `Tensor`

\[seq\_len, rotary\_dim/2] - sine of rotary embeddings
...
## GlmAsrEncoderLayer [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoderLayer "Permanent link")
...
```
classGlmAsrEncoderLayer(nn.Module):
"""
...
### forward [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoderLayer.forward "Permanent link")
...
## GlmAsrEncoderMLP [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoderMLP "Permanent link")
...
```
classGlmAsrEncoderMLP(nn.Module):
"""
...
## GlmAsrEncoderRotaryEmbedding [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoderRotaryEmbedding "Permanent link")

Bases: `Module`

Rotary Position Embedding for GLM-ASR encoder.

Computes rotary position embeddings on-demand for efficiency. Only caches inv\_freq as a buffer; cos/sin are computed during forward to avoid wasted computation during initialization and ensure correct device placement.

Source code in `vllm/model_executor/models/glmasr.py`

```
classGlmAsrEncoderRotaryEmbedding(nn.Module):
"""
    Rotary Position Embedding for GLM-ASR encoder.
    ...
    defforward(self, seq_len: int) -> torch.Tensor:
"""
...
### forward [¶](#vllm.model_executor.models.glmasr.GlmAsrEncoderRotaryEmbedding.forward "Permanent link")

Compute rotary position frequencies for given sequence length.

Parameters:

Name Type Description Default `seq_len` `int`

The sequence length to compute embeddings for.
...
Frequency tensor with shape \[seq\_len, dim/2]. Use .cos() and
...
## GlmAsrFeatureInputs [¶](#vllm.model_executor.models.glmasr.GlmAsrFeatureInputs "Permanent link")

Bases: `TensorSchema`

Dimensions

- num\_chunks: Number of audio chunks (flattened)
- nmb: Number of mel bins
- num\_audios: Number of original audio files

Source code in `vllm/model_executor/models/glmasr.py`

```
classGlmAsrFeatureInputs(TensorSchema):
"""
    Dimensions:
...
## GlmAsrForConditionalGeneration [¶](#vllm.model_executor.models.glmasr.GlmAsrForConditionalGeneration "Permanent link")
...
classGlmAsrForConditionalGeneration(
    ...
    def_parse_and_validate_audio_input(self, **kwargs: object) -> GlmAsrInputs | None:
    ...
    @classmethod
    def_get_audio_token(cls, model_config: ModelConfig) -> str:
...
### \_get\_audio\_token `classmethod` [¶](#vllm.model_executor.models.glmasr.GlmAsrForConditionalGeneration._get_audio_token "Permanent link")

Get the audio token from processor.

Similar to get\_placeholder\_str but returns single token.
...
### get\_generation\_prompt `classmethod` [¶](#vllm.model_executor.models.glmasr.GlmAsrForConditionalGeneration.get_generation_prompt "Permanent link")
...
## GlmAsrMultiModalDataParser [¶](#vllm.model_executor.models.glmasr.GlmAsrMultiModalDataParser "Permanent link")
...
```
classGlmAsrMultiModalDataParser(MultiModalDataParser):
"""
    Custom parser for GLM-ASR multimodal data.
...
## GlmAsrMultiModalProcessor [¶](#vllm.model_executor.models.glmasr.GlmAsrMultiModalProcessor "Permanent link")
...
"""
    ...
    def_get_prompt_updates(
        ...
        defget_replacement_glmasr(item_idx: int):
...
## GlmAsrMultiModalProjector [¶](#vllm.model_executor.models.glmasr.GlmAsrMultiModalProjector "Permanent link")
...
- Linear layer: intermediate\_size -&gt; hidden\_size * 2
- Activation function (e.g., GELU)
- Linear layer: hidden\_size * 2 -&gt; hidden\_size

Source code in `vllm/model_executor/models/glmasr.py`

```
classGlmAsrMultiModalProjector(nn.Module):
"""
    Projects audio encoder outputs to language model hidden space.
...
## GlmAsrProcessingInfo [¶](#vllm.model_executor.models.glmasr.GlmAsrProcessingInfo "Permanent link")
...
```
classGlmAsrProcessingInfo(BaseProcessingInfo):
"""
    Processing information provider for GLM-ASR model.
    ...
    defget_hf_config(self) -> GlmAsrConfig:
        return self.ctx.get_hf_config(GlmAsrConfig)

    defget_hf_processor(self, **kwargs: object) -> GlmAsrProcessor:
...
## \_GlmAsrEncoderOutput [¶](#vllm.model_executor.models.glmasr._GlmAsrEncoderOutput "Permanent link")
...
Name Type Description `last_hidden_state`

Final layer hidden states from the encoder. Shape: \[batch\_size, seq\_len, hidden\_size]

Source code in `vllm/model_executor/models/glmasr.py`

```
class_GlmAsrEncoderOutput:
"""
    Simple output container compatible with transformers' BaseModelOutput.
    ...
    def__init__(self, last_hidden_state: torch.Tensor):
        self.last_hidden_state = last_hidden_state
```

## \_glmasr\_field\_config [¶](#vllm.model_executor.models.glmasr._glmasr_field_config "Permanent link")

Configure multimodal field batching strategy for GLM-ASR.

Determines how to batch audio inputs based on whether chunking is used. When chunk\_counts is present, features are flattened across chunks; otherwise, they are batched normally.

Parameters:

Name Type Description Default `hf_inputs` `Mapping[str, Tensor]`

Dictionary of preprocessed inputs from HuggingFace processor.

*required*

Returns:

Type Description `dict[str, MultiModalFieldConfig]`
