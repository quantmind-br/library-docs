---
title: Automated code review | Moodle Developer Resources
url: https://moodledev.io/general/development/tools/cibot
source: sitemap
fetched_at: 2026-02-17T16:01:21.611473-03:00
rendered_js: false
word_count: 915
summary: This document explains the role and functionality of CiBot, Moodle's automated code checking system, and outlines the processes for code quality reviews during development.
tags:
    - moodle-development
    - cibot
...
## What is CiBot?[​](#what-is-cibot "Direct link to What is CiBot?")
...
## Will an issue be automatically rejected if CiBot checks fail?[​](#will-an-issue-be-automatically-rejected-if-cibot-checks-fail "Direct link to Will an issue be automatically rejected if CiBot checks fail?")
...
## Should coding style issues in existing code be fixed?[​](#should-coding-style-issues-in-existing-code-be-fixed "Direct link to Should coding style issues in existing code be fixed?")

In short, please only update lines relating to your own change.

There are some conventions that are not uniformly followed in the code base, many of these are conventions put in place after code was written. Our long term goal is for the entire codebase to follow the conventions, but in general, we don't want large-scale reformatting of existing code. See [Coding style#Policy about coding-style only fixes](https://moodledev.io/general/development/policies/codingstyle#policy-about-coding-style-only-fixes).

### Example "Should I fix coding style" situations[​](#example-should-i-fix-coding-style-situations 'Direct link to Example "Should I fix coding style" situations')
...
## Requesting CiBot checks an issue[​](#requesting-cibot-checks-an-issue "Direct link to Requesting CiBot checks an issue")
...
## Are additional CiBot checks possible?[​](#are-additional-cibot-checks-possible "Direct link to Are additional CiBot checks possible?")
...
## How do I report bugs in the CiBot report?[​](#how-do-i-report-bugs-in-the-cibot-report "Direct link to How do I report bugs in the CiBot report?")
...
## The git commit summary limit is too small[​](#the-git-commit-summary--limit-is-too-small "Direct link to The git commit summary limit is too small")
...
It is acknowledged that its often tricky to get a useful message in such a short space on the first line. However, the coding guidelines for git summary line length were established on the basis of [industry](https://github.com/blog/926-shiny-new-commit-styles) [best](http://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/tree/Documentation/SubmittingPatches?id=aad7fb916a10f1065ad23de0c80a4a04bcba8437#n594) [practice](http://stackoverflow.com/questions/2290016/git-commit-messages-50-72-formatting). As the [Coding style](https://moodledev.io/general/development/policies/codingstyle#git-commits) mentions, do not be afraid to go into much more detail in your commit body.

Try `git log --oneline --no-merges` if you want to see how other developers have tried to adapt to this situation.

## Branches based off integration.git[​](#branches-based-off-integrationgit "Direct link to Branches based off integration.git")
...
- Could not be branched from moodle.git with these conflicts resolved If this case applies:
...
## Why are issues held up by trivial issues reported which don't affect the functionality of the change?[​](#why-are-issues-held-up-by-trivial-issues-reported-which-dont-affect-the-functionality-of-the-change "Direct link to Why are issues held up by trivial issues reported which don't affect the functionality of the change?")
...
- [Coding style](https://moodledev.io/general/development/policies/codingstyle) and other links in the [coding guidelines category](https://docs.moodle.org/Category/Coding_guidelines)