---
title: triton_attention_helpers - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/v1/attention/ops/triton_attention_helpers/
source: sitemap
fetched_at: 2026-05-07T21:40:03.26895973-03:00
rendered_js: false
word_count: 674
summary: This document describes a collection of Triton JIT helper functions used for attention mechanism operations, including positional bias application, score clamping, masking, and tile-loop boundary calculations.
tags:
    - triton
    - attention-kernel
    - vllm
...
## vllm.v1.attention.ops.triton\_attention\_helpers [¶](#vllm.v1.attention.ops.triton_attention_helpers "Permanent link")
...
## apply\_alibi\_to\_score [¶](#vllm.v1.attention.ops.triton_attention_helpers.apply_alibi_to_score "Permanent link")
...
## apply\_softcap [¶](#vllm.v1.attention.ops.triton_attention_helpers.apply_softcap "Permanent link")
...
## cdiv\_fn [¶](#vllm.v1.attention.ops.triton_attention_helpers.cdiv_fn "Permanent link")
...
## compute\_kv\_seq\_mask [¶](#vllm.v1.attention.ops.triton_attention_helpers.compute_kv_seq_mask "Permanent link")
...
compute_kv_seq_mask(
    ...
    CHUNK_LOOKBACK: constexpr = -1,
    CHUNK_SIZE: constexpr = -1,
)
```
...
## compute\_tile\_loop\_bounds [¶](#vllm.v1.attention.ops.triton_attention_helpers.compute_tile_loop_bounds "Permanent link")
...
1. Longest prefix spanned by any query token in this q-block. Clamped to `seq_len` (causal) or extended to it when mm\_prefix is active (bidirectional ranges can reach past the causal prefix).
2. Sliding-window pruning: narrows `[tile_start, tile_end)` to only tiles that can contain an allowed key under SWA.
3. 3D scoping: when `IS_3D` is True, further narrows to the segment's slice via `(segm_idx * tiles_per_segment, (segm_idx + 1) * tiles_per_segment)`.

Source code in `vllm/v1/attention/ops/triton_attention_helpers.py`

```
@triton.jit
defcompute_tile_loop_bounds(
    context_len,
    seq_len,
    cur_batch_query_len,
    q_block_local_idx,
...
## find\_seq\_idx [¶](#vllm.v1.attention.ops.triton_attention_helpers.find_seq_idx "Permanent link")
...
## init\_softmax\_M [¶](#vllm.v1.attention.ops.triton_attention_helpers.init_softmax_M "Permanent link")
...
## load\_qq\_bias\_tile [¶](#vllm.v1.attention.ops.triton_attention_helpers.load_qq_bias_tile "Permanent link")
...
## resolve\_seq\_and\_query\_len [¶](#vllm.v1.attention.ops.triton_attention_helpers.resolve_seq_and_query_len "Permanent link")
...
## softmax\_step [¶](#vllm.v1.attention.ops.triton_attention_helpers.softmax_step "Permanent link")
...
## store\_segm\_reduce\_scalars [¶](#vllm.v1.attention.ops.triton_attention_helpers.store_segm_reduce_scalars "Permanent link")
