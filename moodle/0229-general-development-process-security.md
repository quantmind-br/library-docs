---
title: Security procedures | Moodle Developer Resources
url: https://moodledev.io/general/development/process/security
source: sitemap
fetched_at: 2026-02-17T16:00:25.251111-03:00
rendered_js: false
word_count: 646
summary: This document outlines Moodle's responsible disclosure policy and provides instructions for reporting security vulnerabilities through the official submission form or issue tracker.
tags:
    - moodle
    - security-policy
...
## Disclosure policy[​](#disclosure-policy "Direct link to Disclosure policy")
...
## How can I report a security issue?[​](#how-can-i-report-a--security-issue "Direct link to How can I report a security issue?")
...
## How we deal with a reported security issue[​](#how-we-deal-with-a-reported-security-issue "Direct link to How we deal with a reported security issue")

01. Issues submitted via the submission form are received by Bugcrowd's triage team, who perform initial triage on the report.
02. If the issue is confirmed valid and not a duplicate by the Bugcrowd team, the Moodle security team reviews the issue and evaluates its potential impact on all supported versions of Moodle. If the issue was submitted directly into Tracker rather than via the form, this will be the first step in the process.
03. Valid issues are then pushed to the [Moodle Tracker](https://moodle.atlassian.net) (restricted from public view).
04. The Moodle security team works with the issue reporter to resolve the problem, following the [Security issue development process](https://moodledev.io/general/development/process#security-issues) and keeping details of the problem and its solution hidden until a release is made.
05. New versions are created and tested.
06. Meanwhile Moodle requests [CVE identifiers](http://cve.mitre.org/) for the security issue.
07. New packages are created and made available on [download.moodle.org](https://download.moodle.org/).
08. Advisories are mailed to administrators of registered Moodle sites, giving a period of time when they can upgrade before the issue becomes public.
09. A public announcement is made about the security issue in the [Moodle security news forum](http://moodle.org/mod/forum/view.php?id=7128).
10. [Open Source Software Security](http://oss-security.openwall.org/wiki/) is notified about it.
...
- [Security issue development process](https://moodledev.io/general/development/process#security-issues)
- [Moodle Penetration Testing](https://moodledev.io/general/development/process/security/penetration-testing)
