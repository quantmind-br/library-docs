---
title: lamport_workspace - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/mamba/lamport_workspace/
source: sitemap
fetched_at: 2026-05-07T21:25:54.398800183-03:00
rendered_js: false
word_count: 374
summary: This module provides classes for managing cross-GPU communication workspaces in PyTorch, specifically handling CUDA IPC memory allocation and synchronization for Lamport-based all-reduce operations.
tags:
    - cuda
    - ipc
...
## IpcBuffer [¶](#vllm.model_executor.layers.mamba.lamport_workspace.IpcBuffer "Permanent link")
...
"""
    ...
    def__init__(self, rank: int, world_size: int, size: int, process_group=None):
        self.rank = rank
...
### serialize [¶](#vllm.model_executor.layers.mamba.lamport_workspace.IpcBuffer.serialize "Permanent link")
...
## LamportWorkspace [¶](#vllm.model_executor.layers.mamba.lamport_workspace.LamportWorkspace "Permanent link")

Self-contained workspace for Lamport-based cross-GPU AllReduce.

#### Parameters[¶](#vllm.model_executor.layers.mamba.lamport_workspace.LamportWorkspace--parameters "Permanent link")

rank : int Local rank (0-based). world\_size : int Total number of ranks in the TP group. comm\_size : int Size in bytes of *one* Lamport buffer slot. The total IPC allocation per rank is `3 * comm_size` (triple-buffering). Must be large enough to hold the per-slot data written by the kernel. Use `compute_comm_size_for_minimax()` for a safe default. process\_group : optional `torch.distributed` process group for IPC handle exchange. `None` uses the default group.

Source code in `vllm/model_executor/layers/mamba/lamport_workspace.py`

```
classLamportWorkspace:
"""
    Self-contained workspace for Lamport-based cross-GPU AllReduce.
    ...
    def__init__(self, rank: int, world_size: int, comm_size: int, process_group=None):
    ...
    @property
    defworkspace(self) -> torch.Tensor:
"""Device tensor (int64) that can be passed to the kernel
    ...
    @staticmethod
    defcompute_comm_size_for_minimax(
        max_tokens: int,
        world_size: int,
        fused_qk: bool = True,
    ) -> int:
...
### workspace `property` [¶](#vllm.model_executor.layers.mamba.lamport_workspace.LamportWorkspace.workspace "Permanent link")

Device tensor (int64) that can be passed to the kernel as `void** workspace`.

### compute\_comm\_size\_for\_minimax `staticmethod` [¶](#vllm.model_executor.layers.mamba.lamport_workspace.LamportWorkspace.compute_comm_size_for_minimax "Permanent link")
...
## \_check [¶](#vllm.model_executor.layers.mamba.lamport_workspace._check "Permanent link")
...
## \_lamport\_fill\_neg\_zero [¶](#vllm.model_executor.layers.mamba.lamport_workspace._lamport_fill_neg_zero "Permanent link")

```
_lamport_fill_neg_zero(device_ptr: int, size_bytes: int)
```
...
```
def_lamport_fill_neg_zero(device_ptr: int, size_bytes: int):
"""
...
## get\_allreduce\_workspace [¶](#vllm.model_executor.layers.mamba.lamport_workspace.get_allreduce_workspace "Permanent link")
...
Return a cached workspace tensor for the given (rank, world\_size) pair.
...
#### Parameters[¶](#vllm.model_executor.layers.mamba.lamport_workspace.get_allreduce_workspace--parameters "Permanent link")

rank, world\_size : int TP rank and TP size. comm\_size : int, optional Explicit slot size in bytes. If `None`, computed automatically from `max_tokens` and `world_size` (fused Q+K path). max\_tokens : int Maximum number of tokens per batch (used when `comm_size is None`). process\_group : optional `torch.distributed` process group.

Source code in `vllm/model_executor/layers/mamba/lamport_workspace.py`

```
defget_allreduce_workspace(
    rank: int,
    world_size: int,
    comm_size: int | None = None,
    max_tokens: int = 16384,
