---
title: Cache API | Moodle Developer Resources
url: https://moodledev.io/docs/4.4/apis/subsystems/muc
source: sitemap
fetched_at: 2026-02-17T15:04:40.757401-03:00
rendered_js: false
word_count: 4016
summary: This document explains the implementation and configuration of the Moodle Universal Cache (MUC) API for plugin developers. It details how to create cache definitions, manage data using the cache object, and configure advanced requirements for different cache modes.
tags:
    - moodle-muc
    - cache-api
    - php-development
...
### Creating a definition[​](#creating-a-definition "Direct link to Creating a definition")
...
mod/myplugin/lang/en/mod\_myplugin.php

```
$string['cachedef_somedata']='This is the description of the cache somedata';
```

### Getting a cache object[​](#getting-a-cache-object "Direct link to Getting a cache object")
...
### Using your cache object[​](#using-your-cache-object "Direct link to Using your cache object")
...
Easy enough. The key must be an `int` or a `string`. The value can be absolutely anything your want that is [serializable](https://www.php.net/manual/en/function.serialize.php). The result is true if the operation was a success, false otherwise.
...
## Ad-hoc Caches[​](#ad-hoc-caches "Direct link to Ad-hoc Caches")
...
## The definition[​](#the-definition "Direct link to The definition")
...
### Setting requirements[​](#setting-requirements "Direct link to Setting requirements")
...
- `simplekeys`: \[bool] Set to true if your cache will only use simple keys for its items.  
  ...
  It will be better for performance and possible only because we know the keys are safe.
...
### Cache modifiers[​](#cache-modifiers "Direct link to Cache modifiers")
...
- `maxsize`: \[int] If set this will be used as the maximum number of entries within the cache store for this definition.  
  It's important to note that cache stores don't actually have to acknowledge this setting or maintain it as a hard limit.
...
### Overriding a cache loader[​](#overriding-a-cache-loader "Direct link to Overriding a cache loader")
...
### Specifying a data source[​](#specifying-a-data-source "Direct link to Specifying a data source")
...
- `datasource`: \[string] A class to use as the data loader for this definition.  
  Any class used here must inherit the cache\_data\_source interface.
...
### Misc settings[​](#misc-settings "Direct link to Misc settings")
...
## Localized stores for distributed high performance caching[​](#localized-stores-for-distributed-high-performance-caching "Direct link to Localized stores for distributed high performance caching")
...
### When should you localize?[​](#when-should-you-localize "Direct link to When should you localize?")

Not all caches are good candidates to localize and some can have a detrimental effect if localized for the wrong reasons. ![When to localize a cache](https://moodledev.io/assets/images/When_to_localize_cache-f9a70c0ff95b3524ac076b51c5c6e258.png)

### Revision numbers as key suffix[​](#revision-numbers-as-key-suffix "Direct link to Revision numbers as key suffix")
...
### Revision numbers as value suffix instead of key suffix[​](#revision-numbers-as-value-suffix-instead-of-key-suffix "Direct link to Revision numbers as value suffix instead of key suffix")
...
### Using time as a key suffix[​](#using-time-as-a-key-suffix "Direct link to Using time as a key suffix")
...
It may look like Moodle theme-related caching uses this strategy, but actually if you look at [the code for theme\_get\_next\_revision](https://github.com/moodle/moodle/blob/df0e58adb140f90712bcd3229ad936d3b4bc15d9/lib/outputlib.php#L88), you will see that it is actually guaranteeing to generate a unique new integer which mitigates the clashing mentioned above. It is just making that integer close to current time-stamp, to make it more self-documenting.

### Content hashes as keys[​](#content-hashes-as-keys "Direct link to Content hashes as keys")
...
### Primary and Final caches[​](#primary-and-final-caches "Direct link to Primary and Final caches")
...
### Beware fast churning keys[​](#beware-fast-churning-keys "Direct link to Beware fast churning keys")
...
### Time-To-Live for distributed caches[​](#time-to-live-for-distributed-caches "Direct link to Time-To-Live for distributed caches")
