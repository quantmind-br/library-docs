---
title: Deferred Tools - Pydantic AI
url: https://ai.pydantic.dev/deferred-tools/
source: sitemap
fetched_at: 2026-01-22T22:23:21.272622632-03:00
rendered_js: false
word_count: 1428
summary: This document explains how to implement deferred tools in Pydantic AI for scenarios requiring human approval or external execution before completing a tool call. It details the workflow of using DeferredToolRequests and DeferredToolResults to persist agent state across multiple runs.
tags:
    - pydantic-ai
    - deferred-tools
    - human-in-the-loop
...
When the model calls a tool that requires approval, the agent run will end with a [`DeferredToolRequests`](https://ai.pydantic.dev/api/output/#pydantic_ai.output.DeferredToolRequests "DeferredToolRequests            dataclass   ") output object with an `approvals` list holding [`ToolCallPart`s](https://ai.pydantic.dev/api/messages/#pydantic_ai.messages.ToolCallPart "ToolCallPart            dataclass   ") containing the tool name, validated arguments, and a unique tool call ID.

Once you've gathered the user's approvals or denials, you can build a [`DeferredToolResults`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.DeferredToolResults "DeferredToolResults            dataclass   ") object with an `approvals` dictionary that maps each tool call ID to a boolean, a [`ToolApproved`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.ToolApproved "ToolApproved            dataclass   ") object (with optional `override_args`), or a [`ToolDenied`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.ToolDenied "ToolDenied            dataclass   ") object (with an optional custom `message` to provide to the model). You can also provide a `metadata` dictionary on `DeferredToolResults` that maps each tool call ID to a dictionary of metadata that will be available in the tool's [`RunContext.tool_call_metadata`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext.tool_call_metadata "tool_call_metadata            class-attribute       instance-attribute   ") attribute. This `DeferredToolResults` object can then be provided to one of the agent run methods as `deferred_tool_results`, alongside the original run's [message history](https://ai.pydantic.dev/message-history/).
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) tool\_requires\_approval.py
...
If whether a tool call should be executed externally depends on the tool call arguments, the agent [run context](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext "RunContext            dataclass   ") (e.g. [dependencies](https://ai.pydantic.dev/dependencies/) or message history), or how long the task is expected to take, you can define a tool function and conditionally raise the [`CallDeferred`](https://ai.pydantic.dev/api/exceptions/#pydantic_ai.exceptions.CallDeferred "CallDeferred") exception. Before raising the exception, the tool function would typically schedule some background task and pass along the [`RunContext.tool_call_id`](https://ai.pydantic.dev/api/tools/#pydantic_ai.tools.RunContext.tool_call_id "tool_call_id            class-attribute       instance-attribute   ") so that the result can be matched to the deferred tool call later.
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) external\_tool.py
...
async defcalculate_answer_task(task_id: str, question: str) -> TaskResult:
