---
title: silu_mul_fp8 - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/kernels/helion/ops/silu_mul_fp8/
source: sitemap
fetched_at: 2026-05-07T21:22:15.315526087-03:00
rendered_js: false
word_count: 66
summary: This document outlines the selection strategy for choosing the optimal pre-tuned configuration for FP8 SiLU and multiplication operations based on input tensor dimensions.
tags:
    - fp8
    - kernel-optimization
    - config-selection
...
1. Find the closest intermediate\_size among available configs (exact match preferred).
2. Among the num\_tokens values tuned for that intermediate\_size, pick the smallest num\_tokens &gt;= the input's num\_tokens. If the input is larger than all available num\_tokens, fall back to the largest.

Config keys must be "default" or follow the format "intermediate\_{int}*numtokens*".

Source code in `vllm/kernels/helion/ops/silu_mul_fp8.py`

```
defpick_silu_mul_fp8_config(
