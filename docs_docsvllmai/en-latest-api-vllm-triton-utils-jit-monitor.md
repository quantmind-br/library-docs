---
title: jit_monitor - vLLM
url: https://docs.vllm.ai/en/latest/api/vllm/triton_utils/jit_monitor/
source: sitemap
fetched_at: 2026-05-07T21:38:25.073276287-03:00
rendered_js: false
word_count: 220
summary: This module provides utilities to monitor and log unexpected Triton kernel JIT compilation and autotuning events that occur during inference, helping to identify sources of latency spikes.
tags:
    - vllm
    - triton
    - jit-compilation
...
## vllm.triton\_utils.jit\_monitor [¶](#vllm.triton_utils.jit_monitor "Permanent link")
...
## \_setup\_triton\_autotuning\_print [¶](#vllm.triton_utils.jit_monitor._setup_triton_autotuning_print "Permanent link")

```
_setup_triton_autotuning_print() -> None
```
...
```
def_setup_triton_autotuning_print() -> None:
"""Enable ``TRITON_PRINT_AUTOTUNING`` unless the user opted out."""
    if not HAS_TRITON:
        return
    fromtritonimport knobs  # type: ignore[import-untyped]
...
## \_setup\_triton\_jit\_hook [¶](#vllm.triton_utils.jit_monitor._setup_triton_jit_hook "Permanent link")

```
_setup_triton_jit_hook() -> None
```
...
```
def_setup_triton_jit_hook() -> None:
"""Register a ``jit_post_compile_hook`` that warns on compilation."""
    if not HAS_TRITON:
        return
    fromtritonimport knobs  # type: ignore[import-untyped]
...
## activate [¶](#vllm.triton_utils.jit_monitor.activate "Permanent link")
...
## is\_active [¶](#vllm.triton_utils.jit_monitor.is_active "Permanent link")
