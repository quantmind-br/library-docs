...
> Explains what each GitHub workflow does, when it runs, and whether it blocks merges.

For event-by-event delivery behavior across PR, merge, push, and release, see [[../../.github/workflows/master-branch-flow|master-branch-flow]].
...
- `.github/workflows/pr-labeler.yml` (`PR Labeler`)
  ...
  - Additional behavior: provider-related keywords in provider/config/onboard/integration changes are promoted to `provider:*` labels (e.g. `provider:kimi`, `provider:deepseek`)
  - Additional behavior: hierarchical de-duplication keeps only the most specific scope labels (e.g. `tool:composio` suppresses `tool:core` and `tool`)
...
1. `CI Required Gate` failing: start with [[../../.github/workflows/ci-run.yml|ci-run]]
2. Docker failures on PRs: inspect [[../../.github/workflows/pub-docker-img.yml|pub-docker-img]] `pr-smoke` job
3. Release failures (tag/manual/scheduled): inspect [[../../.github/workflows/pub-release.yml|pub-release]] and the `prepare` job outputs
4. Homebrew formula publish failures: inspect [[../../.github/workflows/pub-homebrew-core.yml|pub-homebrew-core]] summary output and bot token/fork variables
5. Security failures: inspect [[../../.github/workflows/sec-audit.yml|sec-audit]] and `deny.toml`
6. Workflow syntax/lint failures: inspect [[../../.github/workflows/workflow-sanity.yml|workflow-sanity]]
7. PR intake failures: inspect [[../../.github/workflows/pr-intake-checks.yml|pr-intake-checks]] sticky comment and run logs
8. Label policy parity failures: inspect [[../../.github/workflows/pr-label-policy-check.yml|pr-label-policy-check]]
9. Docs failures in CI: inspect `docs-quality` job logs in [[../../.github/workflows/ci-run.yml|ci-run]]
...
- Follow [[157-contributing-release-process|release-process]] for verify-before-publish release cadence and tag discipline
- Keep merge-blocking rust quality policy aligned across [[../../.github/workflows/ci-run.yml|ci-run]], `dev/ci.sh`, and `.githooks/pre-push` (`./scripts/ci/rust_quality_gate.sh` + `./scripts/ci/rust_strict_delta_gate.sh`)
...
- Run full strict lint audits regularly via `./scripts/ci/rust_quality_gate.sh --strict` (e.g. through `./dev/ci.sh lint-strict`) and track cleanup in focused PRs
...
- Keep Actions source policy restricted to approved allowlist patterns (see [[156-contributing-actions-source-policy|actions-source-policy]])
...
#ci-cd #github-actions #workflow-automation #repository-management #devops #rust-tooling
