...
source: llms
fetched_at: 2026-01-24T14:28:20.180880134-03:00
rendered_js: false
word_count: 642
summary: This document provides a quickstart guide for setting up and using Docker Offload to build and run containers in the cloud using local Docker Desktop tools. It covers prerequisites, how to enable and verify the service, and instructions for managing active sessions through the CLI and Dashboard.
tags:
    - docker-offload
    - docker-desktop
    - cloud-containers
...
- You must have [Docker Desktop](https://docs.docker.com/desktop/) installed. Docker Offload works with Docker Desktop version 4.50 or later.
...
- You must have committed usage available or on-demand usage enabled for your organization. This is set up by your organization owner. For more details, see [Docker Offload usage and billing](https://docs.docker.com/offload/usage/).
...
2. Open a terminal and run the following command to start Docker Offload:
   ...
   > To learn more about the Docker Offload CLI commands, see the [Docker Offload CLI reference](https://docs.docker.com/reference/cli/docker/offload/).
...
When Docker Offload is started and you have started session (for example, you've ran a container), then you can see current session duration estimate in the Docker Desktop Dashboard footer next to the hourglass icon ( ![Offload session duration](https://docs.docker.com/offload/images/hourglass-icon.png) ).
